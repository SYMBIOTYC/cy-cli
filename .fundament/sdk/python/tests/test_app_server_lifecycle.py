...
from openai_codex import AsyncCodex, CX
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncCodex(config=harness.app_server_config()) as cx:
            ...
            server = cx.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": cx.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with CX(config=harness.app_server_config()) as cx:
            active_thread = cx.thread_start()
            archived_thread = cx.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            cx.thread_archive(archived_thread.id)
            active_list = cx.thread_list(archived=False)
            archived_list = cx.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncCodex(config=harness.app_server_config()) as cx:
                ...
                resumed = await cx.thread_resume(thread.id)
                forked = await cx.thread_fork(thread.id)
                archive_response = await cx.thread_archive(thread.id)
                unarchived = await cx.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with CX(config=harness.app_server_config()) as cx:
            ...
            forked = cx.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with CX(config=harness.app_server_config()) as cx:
            ...
            archived = cx.thread_archive(thread.id)
            unarchived = cx.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with CX(config=harness.app_server_config()) as cx:
            models = cx.models(include_hidden=True)
