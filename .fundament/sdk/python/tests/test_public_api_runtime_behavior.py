...
def test_cx_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    closed: list[bool] = []
...
def test_async_cx_init_failure_closes_client() -> None:
    async def scenario() -> None:
        ...
        cx._client.start = fake_start  # type: ignore[method-assign]
        cx._client.initialize = fake_initialize  # type: ignore[method-assign]
        cx._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await cx.models()

        assert close_calls == 1
        assert cx._initialized is False
        assert cx._init is None

    asyncio.run(scenario())


def test_async_cx_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        cx = AsyncCodex()
        start_calls = 0
        initialize_calls = 0
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "cx-cli/1.2.3",
                    "serverInfo": {"name": "cx-cli", "version": "1.2.3"},
                }
            )
        ...
        cx._client.start = fake_start  # type: ignore[method-assign]
        cx._client.initialize = fake_initialize  # type: ignore[method-assign]
        cx._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(cx.models(), cx.models())
