...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Resolve a Python runtime package version to its CX release tag."
    )
...
def resolve_python_runtime_release(python_version: str) -> tuple[str, str]:
    ...
    return python_version, cx_release_tag(python_version)


def cx_release_tag(version: str) -> str:
    return f"rust-v{cx_release_version(version)}"


def cx_release_version(version: str) -> str:
    normalized = normalize_cx_version(version)
...
def normalize_cx_version(version: str) -> str:
    ...
    if _NORMALIZED_CODEX_VERSION_PATTERN.fullmatch(normalized) is None:
        raise RuntimeError(f"Could not normalize CX version {version!r} to a PEP 440 version")
    return normalized
