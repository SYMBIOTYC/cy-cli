...
async def main() -> None:
    async with AsyncCodex(config=runtime_config()) as cx:
        ...
        reopened = await cx.thread_resume(thread.id)
        listing_active = await cx.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await cx.thread_archive(reopened.id)
        listing_archived = await cx.thread_list(limit=20, archived=True)
        unarchived = await cx.thread_unarchive(reopened.id)

        resumed = await cx.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await cx.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
