...
with CX(config=runtime_config()) as cx:
    ...
    reopened = cx.thread_resume(thread.id)
    listing_active = cx.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = cx.thread_archive(reopened.id)
    listing_archived = cx.thread_list(limit=20, archived=True)
    unarchived = cx.thread_unarchive(reopened.id)

    resumed = cx.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = cx.thread_fork(unarchived.id, model="gpt-5.4")
