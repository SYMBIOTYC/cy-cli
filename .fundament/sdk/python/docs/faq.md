...
`openai-cx` publishes stable releases. Install the latest one with
`pip install openai-cx`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding CX CLI release. Each SDK release
...
If your app is not already async, stay with `CX`.
...
When `sandbox=` is omitted, CX uses its configured default. A turn
...
`CX()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-cx-cli-bin` dependency is missing
- local `cx_bin` override points to a missing file
- a custom local CX executable does not support the SDK operation being used
