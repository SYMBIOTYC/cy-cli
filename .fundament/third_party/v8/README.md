...
`RUSTY_V8_ARCHIVE`/`RUSTY_V8_SRC_BINDING_PATH` with CX release assets. Bazel
...
1. Bump the `v8` crate version and refresh `cx-rs/Cargo.lock`.
...
by CX's Bazel Windows GNU toolchain.
...
downloaded `RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/cx` release
...
the exact resolved `v8` crate version in `cx-rs/Cargo.lock`.
