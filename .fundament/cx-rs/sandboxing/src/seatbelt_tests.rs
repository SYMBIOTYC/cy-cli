...
use cx_network_proxy::NetworkProxyConstraints;
use cx_network_proxy::NetworkProxyState;
use cx_network_proxy::build_config_state;
...
use cx_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/cx-unreadable");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.cx".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/cx-readable");
    let unreadable = absolute_path("/tmp/cx-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
            path: root.into(),
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/cx-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/cx-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/cx-proxy-use";
    let explicit_socket = "/tmp/cx-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_cx_subpaths() {
    ...
    // write to .cx/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.cx carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_cx_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .cx/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_cx_canonical.join("config.toml");
    let output = Command::new(MACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    // Verify that writing a file to the folder containing .git and .cx is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_cx_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_codex = repo_root.join(".cx");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .cx/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".cx")).expect("create .cx");
    fs::write(
        vulnerable_root.join(".cx").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .cx/config.toml");
    ...
    let dot_cx_canonical = vulnerable_root_canonical.join(".cx");
