...
pub use cx_windows_sandbox::WindowsSandboxProxySettingsMode;
...
pub fn system_bwrap_warning(
    _permission_profile: &cx_protocol::models::PermissionProfile,
) -> Option<String> {
    None
}

impl From<SandboxTransformError> for CxErr {
    fn from(err: SandboxTransformError) -> Self {
        match err {
            ...
            SandboxTransformError::MissingLinuxSandboxExecutable => {
                CxErr::LandlockSandboxExecutableNotProvided
            }
            SandboxTransformError::EnvironmentNetworkProxy(message) => {
                CxErr::UnsupportedOperation(message)
            }
            #[cfg(target_os = "macos")]
            ...
            SandboxTransformError::Wsl1UnsupportedForBubblewrap => {
                CxErr::UnsupportedOperation(crate::bwrap::WSL1_BWRAP_WARNING.to_string())
            }
            #[cfg(not(target_os = "macos"))]
            SandboxTransformError::SeatbeltUnavailable => CxErr::UnsupportedOperation(
