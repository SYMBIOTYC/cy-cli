...
pub struct SandboxTransformRequest<'a> {
    ...
    pub cx_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: cx_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing cx-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let cx_home = cx_utils_home_dir::find_cx_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_cx_home(request, cx_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_cx_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        cx_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    cx_home: &Path,
    proxy_settings_mode: cx_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = cx_windows_sandbox::resolve_exe_for_launch(source.as_path(), cx_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        cx_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
