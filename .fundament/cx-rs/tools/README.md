...
`cx-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `cx-core`.
...
compatibility-sensitive orchestration in `cx-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `cx-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `cx-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `cx-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `cx-core`.
3. Leave compatibility-sensitive adapters in `cx-core` while downstream
...
`cx-core`, that is a sign to revisit the extraction boundary before adding
more here.
