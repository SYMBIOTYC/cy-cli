...
`cx-execve-wrapper` executable.

`cx-execve-wrapper` receives the arguments to an intercepted `execve(2)` call and delegates the
...
- `Run`: `cx-execve-wrapper` should invoke `execve(2)` on itself to run the original command
...
- `Escalate`: forward the file descriptors of the current process so the command can be run
  ...
  back to `cx-execve-wrapper`.
- `Deny`: the server has declared the proposed command to be forbidden, so
  `cx-execve-wrapper` prints an error to `stderr` and exits with `1`.
...
`cx-zsh-vX.Y.Z` tag is pushed. When the zsh commit or patch changes, publish
