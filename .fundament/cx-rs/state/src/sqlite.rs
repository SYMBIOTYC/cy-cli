...
impl RuntimeDbSpec {
    fn path(self, cx_home: &Path) -> PathBuf {
        cx_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all CX SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable CX SQLite database, creating it if necessary.
    ...
    /// Open an existing CX SQLite database without creating or modifying it.
