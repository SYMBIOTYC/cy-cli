...
//!
//! The history is stored at `~/.cx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.cx`.
...
impl HistoryConfig {
    pub fn new(cx_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            cx_home: cx_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.cx_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.cx/history.jsonl` and ensure the parent directory exists.
