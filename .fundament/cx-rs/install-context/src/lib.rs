...
const PACKAGE_METADATA_FILENAME: &str = "cx-package.json";
const PATH_DIRNAME: &str = "cx-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "cx-resources";
...
pub struct CodexPackageLayout {
    ...
    /// Directory containing the CX entrypoint executable.
...
/// Version metadata recorded in a bundled CX runtime package.
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.cx/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `cx-resources/`, and `cx-path/`.
    ...
    /// A CX binary launched through the npm-managed `cx.js` shim.
    Npm,
    /// A CX binary launched through the bun-managed `cx.js` shim.
    Bun,
    /// A CX binary launched through the pnpm-managed `cx.js` shim.
    Pnpm,
    /// A CX binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled CX binaries, custom
    /// internal launchers, and tests that execute CX from an arbitrary path.
    Other,
}
...
impl InstallContext {
    ...
    fn from_exe_with_cx_home(
        ...
        let method = if let Some(method) = method_override {
            ...
            install_method_from_exe(exe_path, cx_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("CX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("CX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("CX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under cx-resources
...
fn standalone_install_method(
    ...
    let canonical_cx_home = canonical_absolute_path(cx_home?)?;
    ...
    let releases_root = canonical_cx_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "cx-test-helper";
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "cx.exe" } else { "cx" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = cx_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "cx.exe" } else { "cx" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_cx_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*cx_home*/ Some(cx_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
...
"#,
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_cx_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "cx.exe" } else { "cx" });
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-cx");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new("/opt/cx/bin/cx"))),
            PathBuf::from("/opt/cx/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/cx")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_cx_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/cx")),
            /*method_override*/ None,
            /*cx_home*/ None,
        );
        assert_eq!(
