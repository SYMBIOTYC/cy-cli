...
/// Remote endpoint used by the oi-compatible model manager.
///
...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use CX backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_cx_backend);
...
/// oi-compatible model manager backed by bundled models, cache, and `/models`.
#[derive(Debug)]
...
impl OpenAiModelsManager {
    /// Construct an oi-compatible remote model manager.
    pub fn new(
        cx_home: PathBuf,
        endpoint_client: Arc<dyn ModelsEndpointClient>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Self {
        let cache_path = cx_home.join(MODEL_CACHE_FILE);
    ...
    /// Construct an oi-compatible model manager with caching disabled.
    ...
    /// Constructs an oi-compatible model manager with a caller-provided cache.
    ///
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_cx_backend().await || self.endpoint_client.has_command_auth()
    }
    ...
    async fn apply_remote_models(&self, models: Vec<ModelInfo>) {
        ...
        // non-hidden model and the user is using gt auth.
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    // retry for namespaced slugs like `custom/gpt-5.3-cx`.
