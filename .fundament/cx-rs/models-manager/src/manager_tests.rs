...
use cx_login::AuthCredentialsStoreMode;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> cx_login::ExternalAuthFuture<'_, CodexAuth> {
    ...
    ) -> cx_login::ExternalAuthFuture<'_, CodexAuth> {
        Box::pin(async { Ok(CodexAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_cx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_cx_backend })
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(cx_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(cx_home: &Path, f: F)
where
    F: FnOnce(&mut ModelsCacheEntry),
{
    let client_version = crate::client_version_to_whole();
    let cache = FileModelsCache::new(cx_home.join(MODEL_CACHE_FILE), DEFAULT_MODEL_CACHE_TTL);
    let mut entry = cache
        .load(&client_version)
        .await
...
async fn file_cache_implements_models_cache_contract() {
    ...
    let cache = FileModelsCache::new(
        cx_home.path().join(MODEL_CACHE_FILE),
...
c2ln",
    ...
    std::fs::create_dir_all(cx_home).expect("cx home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(cx_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-cx/gpt-image".to_string();
...
#[tokio::test]
async fn refresh_available_models_uses_remote_only_catalog_for_gt_auth() {
    let remote_models = vec![remote_model(
        "gt-visible-source-of-truth",
        "gt Visible",
        /*priority*/ 0,
    )];
    let cx_home = tempdir().expect("temp dir");
...
#[tokio::test]
async fn refresh_available_models_uses_cached_remote_only_catalog_for_gt_auth() {
    let remote_models = vec![remote_model(
        "gt-cached-source-of-truth",
        "gt Cached",
        /*priority*/ 0,
    )];
    let cx_home = tempdir().expect("temp dir");
    ...
    let fetch_manager =
        openai_manager_for_tests(cx_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(cx_home.path().to_path_buf(), cache_endpoint.clone());
...
#[tokio::test]
async fn get_model_info_uses_fallback_for_bundled_models_when_gt_remote_is_authoritative() {
    let remote_models = vec![remote_model(
        "gt-authoritative-model-info",
        "gt Model Info",
        /*priority*/ 0,
    )];
    let cx_home = tempdir().expect("temp dir");
...
#[tokio::test]
async fn refresh_available_models_preserves_bundled_catalog_for_empty_gt_remote() {
    let cx_home = tempdir().expect("temp dir");
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(cx_home.path(), |cache| {
...
#[tokio::test]
async fn refresh_available_models_skips_network_without_gt_auth() {
    ...
    manager
        ...
        .await
        .expect("refresh should no-op without gt auth");
    let cached_remote = manager.get_remote_models().await;
    assert!(
        !cached_remote
            .iter()
            .any(|candidate| candidate.slug == dynamic_slug),
        "remote refresh should be skipped without gt auth"
    );
    assert_eq!(
        endpoint.fetch_count(),
        0,
...
impl TestAuthAwareModelsEndpoint {
    ...
    async fn uses_cx_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => auth_manager
                .auth()
                .await
                .as_ref()
                .is_some_and(CodexAuth::uses_cx_backend),
            None => false,
        }
    }
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_cx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_cx_backend(self))
    }
...
#[tokio::test]
async fn refresh_available_models_skips_network_when_external_api_key_overrides_gt_auth() {
...
#[tokio::test]
async fn refresh_available_models_uses_cached_gt_when_external_api_key_is_unresolved() {
    ...
    manager
        .refresh_available_models(RefreshStrategy::Online, &DEFAULT_HTTP_CLIENT_FACTORY)
        .await
        .expect("refresh should fall back to cached gt auth");
    ...
    assert!(
        manager
            ...
            .iter()
            .any(|candidate| candidate.slug == dynamic_slug),
        "remote refresh should include models fetched with cached gt auth"
    );
    assert_eq!(
        endpoint.fetch_count(),
        1,
        "endpoint should fetch models when unresolved external API key falls back to gt auth"
    );
}

#[tokio::test]
async fn refresh_available_models_fetches_with_gt_auth_tokens() {
    let dynamic_slug = "dynamic-model-only-for-test-gt-auth-tokens";
    let cx_home = tempdir().expect("temp dir");
    let endpoint = TestModelsEndpoint::new(vec![vec![remote_model(
        dynamic_slug,
        "gt Auth Tokens",
    ...
    let auth = gt_auth_tokens_for_tests(cx_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        cx_home.path().to_path_buf(),
        endpoint.clone(),
    ...
    manager
        .refresh_available_models(RefreshStrategy::Online, &DEFAULT_HTTP_CLIENT_FACTORY)
        .await
        .expect("refresh should fetch with gt auth tokens");
    ...
    assert!(
        manager
            ...
            .iter()
            .any(|candidate| candidate.slug == dynamic_slug),
        "remote refresh should include models fetched with gt auth tokens"
    );
    assert_eq!(
        endpoint.fetch_count(),
        1,
        "endpoint should fetch models with gt auth tokens"
    );
}
...
async fn static_manager_reads_latest_auth_mode() {
    let auth_manager =
        AuthManager::from_auth_for_testing(CodexAuth::create_dummy_gt_auth_for_testing());
    let gt_only_model = {
        let mut model = remote_model("gt-only", "gt Only", /*priority*/ 0);
    ...
    let manager = StaticModelsManager::new(
        Some(Arc::clone(&auth_manager)),
        ModelsResponse {
            models: vec![gt_only_model, api_model],
        },
    );

    let gt_models = manager
