...
fn test_config(cx_home: &Path) -> RolloutConfig {
    RolloutConfig {
        cx_home: cx_home.to_path_buf(),
        sqlite: cx_state::SqliteConfig::new_for_testing(cx_home.abs()),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        cx_state::BackfillStatus::Complete
    );
...
-> std::io::Result<()> {
    ...
    let valid_created_at = chrono::Utc
        ...
        .single()
        .expect("valid datetime");
    let mut valid_builder = cx_state::ThreadMetadataBuilder::new(
        valid_thread_id,
        valid_path.clone(),
        valid_created_at,
        SessionSource::Cli,
...
fn fill_missing_thread_item_metadata_preserves_identity_and_prefers_state_git_fields() {
    ...
    let state_item = ThreadItem {
        ...
        project_id: None,
        section: Some(cx_state::ThreadSection {
