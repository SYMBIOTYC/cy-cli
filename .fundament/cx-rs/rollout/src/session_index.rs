...
pub async fn append_thread_name(
    ...
    append_session_index_entry(cx_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(cx_home);
...
pub async fn find_thread_meta_candidates_by_name_str(
    ...
    while let Some(thread_id) = rx.recv().await {
        ...
        if let Some(path) =
            super::list::find_thread_path_by_id_str(cx_home, &thread_id.to_string(), state_db_ctx)
                .await?
...
fn session_index_path(cx_home: &Path) -> PathBuf {
    cx_home.join(SESSION_INDEX_FILE)
}
