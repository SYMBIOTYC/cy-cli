...
//! existing rollout path. They must not do that at the same time for one CX home, so they share
...
/// Try to exclude rollout compression and migration for one CX home.
pub fn try_acquire_rollout_maintenance_lock(
    cx_home: &Path,
) -> io::Result<Option<RolloutMaintenanceGuard>> {
    let directory = cx_home.join(".tmp");
