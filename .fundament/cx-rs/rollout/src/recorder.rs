//! Persist CX session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use cx_protocol::protocol::GitInfo as ProtocolGitInfo;
use cx_protocol::protocol::HistoryPosition;
use cx_protocol::protocol::MultiAgentVersion;
use cx_protocol::protocol::SessionContextWindow;
...
///
/// ```ignore
/// $ jq -C . ~/.cx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.cx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided CX home directory.
    #[allow(clippy::too_many_arguments)]
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = cx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(cx_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(cx_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_new_rollout_path(
    ...
    // Resolve ~/.cx/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.cx_home().to_path_buf();
...
impl From<cx_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: cx_state::ThreadsPage) -> Self {
