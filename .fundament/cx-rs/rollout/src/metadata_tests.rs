...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let cx_home = dir.path().to_path_buf();
    ...
    let runtime = cx_state::StateRuntime::init(
        cx_state::SqliteConfig::new_for_testing(cx_home.as_path().abs()),
        "test-provider".to_string(),
    )
    .await
    .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(cx_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), cx_home.as_path(), "test-provider").await;
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = cx_home.join("sessions");
