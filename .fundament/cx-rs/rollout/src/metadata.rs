...
use cx_state::BackfillState;
use cx_state::BackfillStats;
use cx_state::BackfillStatus;
use cx_state::DB_ERROR_METRIC;
use cx_state::DB_METRIC_BACKFILL;
use cx_state::DB_METRIC_BACKFILL_DURATION_MS;
use cx_state::ExtractionOutcome;
use cx_state::ThreadMetadataBuilder;
use cx_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    ...
    backfill_sessions_with_lease(runtime, cx_home, default_provider, BACKFILL_LEASE_SECONDS).await;
}
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = cx_otel::global();
    ...
    let sessions_root = cx_home.join(SESSIONS_SUBDIR);
    let archived_root = cx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(cx_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(cx_home: &Path, path: &Path) -> String {
    path.strip_prefix(cx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
