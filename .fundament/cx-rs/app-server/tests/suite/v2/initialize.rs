...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_cx_home = AbsolutePathBuf::try_from(cx_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        cx_home: response_cx_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("cx_vscode/"));
    assert_eq!(response_cx_home, expected_cx_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "cx_app_server_daemon".to_string(),
            title: Some("CX App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("cx_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_cx_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let cx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri())
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "cx-backend".to_string(),
            title: Some("CX Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    assert!(user_agent.starts_with("cx_originator_via_env_var/"));
    assert_eq!(response_cx_home, expected_cx_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let cx_home = TempDir::new()?;
    let notify_file = cx_home.path().join("notify.json");
    let notify_capture = cargo_bin("cx-app-server-test-notify-capture")?;
