...
async fn thread_start_routes_project_exec_policy_warning_to_requester() -> Result<()> {
    ...
    let rules_dir = project.path().join(".cx/rules");
    std::fs::create_dir_all(&rules_dir)?;
    let rules_path = rules_dir.join("broken.rules");
    std::fs::write(&rules_path, "prefix_rule(")?;
    set_project_trust_level(cx_home.path(), project.path(), TrustLevel::Trusted)?;
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let cx_home = TempDir::new()?;
    let token_file = cx_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(cx_home.path(), "ws://0.0.0.0:0", &auth_args).await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let cx_home = TempDir::new()?;
    let shared_secret_file = cx_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "cx-enroller".to_string(),
        "--ws-audience".to_string(),
        "cx-app-server".to_string(),
        "--ws-max-clock-skew-seconds".to_string(),
        "1".to_string(),
    ];

    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(cx_home.path(), "ws://127.0.0.1:0", &auth_args).await?;
...
pub(super) async fn spawn_websocket_server(cx_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(cx_home, "ws://127.0.0.1:0", &[]).await
}
