...
use cx_app_server_protocol::TextPosition;
use cx_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = cx_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml_without_approval_policy(cx_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .build_initialized()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml_without_approval_policy(cx_home.path(), &server.uri())?;
    let global_agents_path = cx_home.path().join("AGENTS.md");
...
async fn thread_start_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml_with_gt_base_url(cx_home.path(), &server.uri(), &server.uri())?;
...
async fn thread_start_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml_with_required_broken_mcp(cx_home.path(), &server.uri())?;
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(cx_home.path(), &server.uri())?;
...
"#,
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized()
        .await?;
    let config_before = std::fs::read_to_string(cx_home.path().join("config.toml"))?;
    ...
    if cfg!(windows) {
        assert_eq!(
            ...
            cx_app_server_protocol::SandboxPolicy::ReadOnly {
...
"#,
    ...
    let config_after = std::fs::read_to_string(cx_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml(cx_home, server_uri, "sandbox_mode = \"read-only\"", "")
}
...
fn create_config_toml_with_gt_base_url(
    cx_home: &Path,
    server_uri: &str,
    gt_base_url: &str,
) -> std::io::Result<()> {
