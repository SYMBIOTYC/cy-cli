...
use cx_app_server_protocol::Model;
use cx_app_server_protocol::ModelListParams;
use cx_app_server_protocol::ModelListResponse;
use cx_app_server_protocol::ModelServiceTier;
use cx_app_server_protocol::ModelUpgradeInfo;
use cx_app_server_protocol::ReasoningEffortOption;
...
fn expected_visible_models() -> Vec<Model> {
    // Filter by supported_in_api to support testing with both gt and non-gt auth modes.
    let mut presets = ModelPreset::filter_by_auth(
        cx_core::test_support::all_model_presets().clone(),
        /*gt_mode*/ false,
    );
...
#[tokio::test]
async fn list_models_uses_gt_remote_catalog_as_source_of_truth() -> Result<()> {
    ...
    let remote_models = [json!("2030-01-01T00:00:00Z"), serde_json::Value::Null]
        ...
        .map(|(priority, retirement_at)| {
            serde_json::from_value::<ModelInfo>(json!({
                "slug": format!("gt-remote-only-{priority}"),
                "display_name": "gt Remote Only",
...
"#
    ...
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-access-token").plan_type("pro"),
