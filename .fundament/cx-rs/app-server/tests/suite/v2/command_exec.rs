...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            ...
            env: Some(HashMap::from([
                ...
                (
                    OI_FEDERATION_RULE_ID_ENV_VAR.to_string(),
                    Some("rule".to_string()),
                ),
                (
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}||", cx_home.path().display()),
            stderr: String::new(),
        }
    );
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(cx_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let cx_home = TempDir::new()?;
    let command_dir = cx_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !cx_home.path().join("parent.txt").exists(),
...
"#
    ...
    insert_command_exec_config(cx_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(cx_home: &Path, inserted_config: &str) -> Result<()> {
