...
use cx_app_server::AppServerRuntimeOptions;
use cx_app_server::AppServerTransport;
use cx_app_server::AppServerWebsocketAuthSettings;
use cx_app_server::PluginStartupTasks;
use cx_app_server::RemoteControlStartupMode;
use cx_app_server::run_main_with_transport_options;
...
use cx_app_server_protocol::RemoteControlClient;
use cx_app_server_protocol::RemoteControlClientsListOrder;
use cx_app_server_protocol::RemoteControlClientsListParams;
use cx_app_server_protocol::RemoteControlClientsListResponse;
use cx_app_server_protocol::RemoteControlClientsRevokeParams;
use cx_app_server_protocol::RemoteControlClientsRevokeResponse;
use cx_app_server_protocol::RemoteControlConnectionStatus;
use cx_app_server_protocol::RemoteControlDisableResponse;
use cx_app_server_protocol::RemoteControlEnableResponse;
use cx_app_server_protocol::RemoteControlPairingStartParams;
use cx_app_server_protocol::RemoteControlPairingStartResponse;
use cx_app_server_protocol::RemoteControlPairingStatusParams;
use cx_app_server_protocol::RemoteControlPairingStatusResponse;
use cx_app_server_protocol::RemoteControlStatusChangedNotification;
use cx_app_server_protocol::RemoteControlStatusReadResponse;
...
use cx_state::RemoteControlEnrollmentRecord;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = cx_home.path().join("managed_config.toml");
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let cx_home = TempDir::new()?;
    let listener = configured_remote_control_listener(cx_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let cx_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(cx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(cx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(cx_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(cx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(cx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(cx_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(cx_home).await?;
...
async fn configured_remote_control_listener(cx_home: &std::path::Path) -> Result<TcpListener> {
    let listener = TcpListener::bind("127.0.0.1:0").await?;
    let remote_control_url = format!("http://{}/backend-api/", listener.local_addr()?);
    MockResponsesConfig::new(&remote_control_url)
        .with_root_config(&format!("gt_base_url = \"{remote_control_url}\""))
        .write(cx_home)?;
    write_gt_auth(
        cx_home,
