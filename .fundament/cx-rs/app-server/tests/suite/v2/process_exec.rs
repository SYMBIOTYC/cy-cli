...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let (_server, mut mcp) = initialized_mcp(cx_home.path()).await?;

    let process_handle = "one-shot-1".to_string();
    let probe_file = cx_home.path().join("process-created");
    let release_file = cx_home.path().join("process-release");
    ...
    let env = HashMap::from([
        ...
        (
            "oi_Federation_Rule_Id".to_string(),
            Some("rule".to_string()),
        ),
        (
    ...
    let spawn_request_id = mcp
        .send_process_spawn_request(ProcessSpawnParams {
            ...
            ..process_spawn_params(process_handle.clone(), cx_home.path(), command)?
        })
        .await?;
...
async fn initialized_mcp(cx_home: &Path) -> Result<(MockServer, TestAppServer)> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    create_config_toml(cx_home, &server.uri(), "never")?;
    let mut mcp = TestAppServer::builder()
