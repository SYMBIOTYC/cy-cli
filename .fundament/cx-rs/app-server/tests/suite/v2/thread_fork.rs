...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(cx_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(cx_home.path(), filename_ts, &conversation_id);
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    let source_path = rollout_path(cx_home.path(), "2025-01-05T12-00-00", &source_thread_id);
    ...
    assert!(matches!(
        ...
        [
            ...
            &response_item.item,
            cx_protocol::models::ResponseItem::Message { role, .. }
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_cx_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let cx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(cx_home.path())?;
