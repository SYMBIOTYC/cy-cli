...
const RATE_LIMIT_RESET_REQUEST_TIMEOUT_ENV_VAR: &str =
    "CX_TEST_RATE_LIMIT_RESET_REQUEST_TIMEOUT_MS";
...
#[tokio::test]
async fn consume_rate_limit_reset_credit_requires_gt_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mut mcp = initialized_app_server(cx_home.path()).await?;
    ...
    assert_eq!(
        ...
        "cx account authentication required for rate limit reset credits"
    );
    ...
    assert_eq!(
        consume_error.error.message,
        "gt authentication required for rate limit reset credits"
    );
    Ok(())
}
...
async fn consume_account_rate_limit_reset_credit_maps_backend_outcomes() -> Result<()> {
    let (cx_home, server) = gt_test_context().await?;
    let cases = [
        (
            "request-reset",
            "reset",
    ...
    for (idempotency_key, backend_code, _, windows_reset) in cases {
        Mock::given(method("POST"))
            .and(path("/api/cx/rate-limit-reset-credits/consume"))
...
async fn consume_account_rate_limit_reset_credit_rejects_empty_idempotency_key() -> Result<()> {
    let (cx_home, _server) = gt_test_context().await?;
    let mut mcp = initialized_app_server(cx_home.path()).await?;
...
async fn gt_test_context() -> Result<(TempDir, MockServer)> {
    let cx_home = TempDir::new()?;
    write_gt_auth(
        cx_home.path(),
...
async fn initialized_app_server(cx_home: &Path) -> Result<TestAppServer> {
