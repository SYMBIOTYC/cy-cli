...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "cx@example.com"])?;
    run_git(root, &["config", "user.name", "CX Tests"])?;
...
fn disable_plugin_startup_tasks(cx_home: &Path) -> Result<()> {
...
fn marketplace_install_root(cx_home: &Path) -> std::path::PathBuf {
    cx_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(cx_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&cx_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(cx_home.path())?;
    ...
    let debug_root = expected_installed_root(cx_home.path(), "debug")?;
    let tools_root = expected_installed_root(cx_home.path(), "tools")?;
    ...
    for (root, expected_revision) in [
        ...
        let metadata: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(
            root.as_path().join(".cx-marketplace-install.json"),
        )?)?;
        assert_eq!(
...
-> Result<()> {
    ...
    let plugin_root = plugin_source.path().join(".cx-plugin");
    ...
    run_git(cx_home.path(), &["init", "--quiet"])?;
    let source_path = source.path().display().to_string();
    let missing_path = cx_home.path().join("missing").display().to_string();
    ...
    for untrusted_source in [&source_path, &plugin_url] {
        ...
        assert!(run_git(cx_home.path(), &["ls-remote", untrusted_source, "HEAD"]).is_err());
    }
    ...
    let plugin_root = cx_home
        .path()
        .join("plugins/cache/trusted/toolkit/1.0.0/.cx-plugin/plugin.json");
    ...
    std::fs::write(
        plugin_source.path().join(".cx-plugin/plugin.json"),
    ...
    assert!(
        cx_home
            .path()
            .join("plugins/cache/trusted/toolkit/1.1.0/.cx-plugin/plugin.json")
            .is_file(),
...
async fn marketplace_upgrade_named_marketplace_only() -> Result<()> {
    ...
    assert!(
        !marketplace_install_root(cx_home.path())
            .join("debug")
            .exists()
    );
    Ok(())
