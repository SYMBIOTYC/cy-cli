...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    MockResponsesConfig::new(&responses_server.uri())
        ...
        .write(cx_home.path())?;
    let executor_config: cx_config::types::McpServerConfig = serde_json::from_value(json!({
    ...
    let oauth_credentials_path = cx_home.path().join(".credentials.json");
    std::fs::write(
        &oauth_credentials_path,
        serde_json::to_vec(&json!({"host": host_oauth_credential.clone()}))?,
    )?;
    let cx_bin = toml::Value::String(
        cx_utils_cargo_bin::cargo_bin("cx")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".cx-plugin"))?;
    std::fs::write(
        plugin.path().join(".cx-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(registration_request["client_name"], json!("CX"));
    assert_eq!(
