...
use cx_app_server_protocol::ChatgptAuthTokensRefreshReason;
use cx_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use cx_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
const LOGIN_ISSUER_ENV_VAR: &str = "CX_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "CX_APP_SERVER_DEV_OPEN_APP_URL";
...
fn create_config_toml(cx_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let forced_workspace_line = if let Some(ws) = params.forced_workspace_id {
        format!("forced_gt_workspace_id = \"{ws}\"\n")
        ...
        let workspaces = workspaces
            ...
            .collect::<Vec<_>>()
            .join(", ");
        format!("forced_gt_workspace_id = [{workspaces}]\n")
    } else {
        String::new()
    };
    ...
    let gt_base_url_line = params
        .gt_base_url
        .map(|url| format!("gt_base_url = \"{url}\"\n"))
        .unwrap_or_default();
...
fn read_config_toml(cx_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        cx_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(cx_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(
        !cx_home.path().join("auth.json").exists(),
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    std::fs::write(cx_home.path().join("config.toml"), "invalid = [")?;
    ...
    assert_eq!(load_file_auth(cx_home.path())?, None);
...
async fn startup_enforces_local_auth_requirements_before_cloud_fetch() -> Result<()> {
    ...
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
            gt_base_url: Some(format!("{}/backend-api", mock_server.uri())),
            ..Default::default()
        },
    )?;
    std::fs::write(
    ...
    assert!(
        mock_server
            ...
            .is_empty(),
        "disallowed gt auth must not fetch cloud requirements"
    );
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    write_models_cache(cx_home.path())?;

    let access_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
            .email("embedded@example.com")
            .plan_type("pro")
            .gt_account_id(WORKSPACE_ID_EMBEDDED),
    )?;
    ...
    let set_id = mcp
        .send_gt_auth_tokens_login_request(
            access_token,
...
async fn account_read_refresh_token_is_noop_in_external_mode() -> Result<()> {
    ...
    assert!(
        ...
        "external mode should not emit account/gtAuthTokens/refresh for refreshToken=true"
    );
...
async fn respond_to_refresh_request(
    ...
    let ServerRequest::ChatgptAuthTokensRefresh { request_id, params } = refresh_req else {
        bail!("expected account/gtAuthTokens/refresh request, got {refresh_req:?}");
    };
    assert_eq!(params.reason, ChatgptAuthTokensRefreshReason::Unauthorized);
    let response = ChatgptAuthTokensRefreshResponse {
        access_token: access_token.to_string(),
        gt_account_id: gt_account_id.to_string(),
        gt_plan_type: gt_plan_type.map(str::to_string),
    };
...
#[tokio::test]
// 401 response triggers account/gtAuthTokens/refresh and retries with new tokens.
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let initial_access_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
            ...
            .plan_type("pro")
            .gt_account_id(WORKSPACE_ID_INITIAL),
    )?;
    let refreshed_access_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
            .email("refreshed@example.com")
            .plan_type("pro")
            .gt_account_id(WORKSPACE_ID_REFRESHED),
    )?;
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(cx_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: cx_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(cx_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![cx_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: cx_app_server_protocol::TurnStartResponse =
...
async fn external_auth_refresh_mismatched_workspace_fails_turn() -> Result<()> {
    ...
    mcp.send_response(
        ...
        serde_json::to_value(ChatgptAuthTokensRefreshResponse {
            ...
            gt_account_id: WORKSPACE_ID_DISALLOWED.to_string(),
...
async fn external_auth_refresh_invalid_access_token_fails_turn() -> Result<()> {
    ...
    mcp.send_response(
        ...
        serde_json::to_value(ChatgptAuthTokensRefreshResponse {
            ...
            gt_account_id: WORKSPACE_ID_INITIAL.to_string(),
            gt_plan_type: Some("pro".to_string()),
        })?,
    )
    .await?;
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let mut expected_config = read_config_toml(cx_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(read_config_toml(cx_home.path())?, expected_config);
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(cx_home.path())?;
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let expected_config = read_config_toml(cx_home.path())?;
    ...
    assert_eq!(load_file_auth(cx_home.path())?, expected_auth);
    assert_eq!(read_config_toml(cx_home.path())?, expected_config);
...
async fn logout_aws_managed_bedrock_errors_without_changing_auth_or_config() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), aws_managed_bedrock_config())?;
    login_with_api_key(
        cx_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
#[tokio::test]
async fn login_account_amazon_bedrock_rejected_when_forced_gt() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
            forced_method: Some("gt".to_string()),
            ..Default::default()
        },
    )?;
...
#[tokio::test]
async fn login_account_amazon_bedrock_rejected_with_external_gt_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
    let access_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
    ...
    assert_eq!(
        error.error.message,
        "External auth is active. Use account/login/start (gtAuthTokens) to update it or account/logout to clear it."
    );
    assert_eq!(load_file_auth(cx_home.path())?, None);
    Ok(())
}
...
#[tokio::test]
async fn login_account_api_key_rejected_when_forced_gt() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
...
#[tokio::test]
async fn login_account_gt_rejected_when_forced_api() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
    ...
    let request_id = mcp.send_login_account_gt_request().await?;
    let err: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(request_id)),
    )
...
#[tokio::test]
async fn login_account_gt_device_code_returns_error_when_disabled() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mock_server = MockServer::start().await;
    create_config_toml(
        cx_home.path(),
    ...
    let request_id = mcp.send_login_account_gt_device_code_request().await?;
    let err: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(request_id)),
    )
...
#[tokio::test]
async fn login_account_gt_device_code_succeeds_and_notifies() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mock_server = MockServer::start().await;
    create_config_toml(
        cx_home.path(),
    ...
    let id_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
            ...
            .plan_type("pro")
            .gt_account_id(WORKSPACE_ID_DEVICE),
    )?;
    ...
    assert_eq!(verification_url, format!("{issuer}/cx/device"));
...
#[tokio::test]
async fn login_account_gt_device_code_failure_notifies_without_account_update() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mock_server = MockServer::start().await;
    create_config_toml(
        cx_home.path(),
...
#[tokio::test]
async fn login_account_gt_device_code_can_be_cancelled() -> Result<()> {
    let cx_home = TempDir::new()?;
    let mock_server = MockServer::start().await;
    create_config_toml(
        cx_home.path(),
...
#[serial(login_port)]
async fn login_account_gt_start_can_be_cancelled() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
...
// Serialize tests that launch the login server since it binds to a fixed port.
#[serial(login_port)]
async fn login_account_gt_uses_debug_oauth_overrides() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
...
// Serialize tests that launch the login server since it binds to a fixed port.
#[serial(login_port)]
async fn login_account_gt_redirects_to_hosted_success_page() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
    let mock_server = MockServer::start().await;
    let id_token = encode_id_token(
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            (
                ...
                Some("http://localhost:3000/cx/open-app"),
            ),
        ])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    assert_eq!(
        ...
        "http://localhost:3000/cx/open-app?source=login&app_brand=gt"
    );
...
// Serialize tests that launch the login server since it binds to a fixed port.
#[serial(login_port)]
async fn set_auth_token_cancels_active_gt_login() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(cx_home.path(), CreateConfigTomlParams::default())?;
    ...
    // Set an external auth token instead of completing the gt login flow.
...
// Serialize tests that launch the login server since it binds to a fixed port.
#[serial(login_port)]
async fn login_account_gt_includes_forced_workspace_query_param() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
...
// Serialize tests that launch the login server since it binds to a fixed port.
#[serial(login_port)]
async fn login_account_gt_includes_forced_workspace_allowlist_query_param() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
...
#[tokio::test]
async fn get_account_with_gt() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
    ...
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("access-gt")
...
#[tokio::test]
async fn get_account_with_gt_plan_variants_returns_plan_type(
    plan_type: &str,
...
#[tokio::test]
async fn get_account_with_gt_without_email() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
    ...
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("access-gt").plan_type("pro"),
        AuthCredentialsStoreMode::File,
    )?;
...
#[tokio::test]
async fn get_account_omits_gt_after_permanent_refresh_failure() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
...
#[tokio::test]
async fn get_account_with_gt_missing_plan_claim_returns_unknown() -> Result<()> {
    let cx_home = TempDir::new()?;
    create_config_toml(
        cx_home.path(),
        CreateConfigTomlParams {
    ...
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("access-gt").email("user@example.com"),
        AuthCredentialsStoreMode::File,
    )?;
