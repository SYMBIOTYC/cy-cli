...
async fn account_switch_reloads_telemetry_collectors_and_preserves_trace_context() -> Result<()> {
    ...
    write_otel_config(cx_home.path(), &initial_endpoint)?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("initial-access-token")
            .account_id(INITIAL_ACCOUNT_ID)
            .gt_account_id(INITIAL_ACCOUNT_ID)
            .gt_user_id("initial-user")
            .plan_type("pro")
            .email(INITIAL_EMAIL),
    ...
    let mut app_server = TestAppServer::builder()
        ...
        .with_json_logging("cx_app_server::otel_reloader=info")
    ...
    write_otel_config(cx_home.path(), &next_endpoint)?;
    let access_token = encode_id_token(
        &ChatGptIdTokenClaims::new()
            .email(NEXT_EMAIL)
            .plan_type("pro")
            .gt_account_id(NEXT_ACCOUNT_ID)
            .gt_user_id("next-user"),
    )?;
    ...
    timeout(
        TEST_TIMEOUT,
        app_server.wait_for_json_log_event("cx.app_server.otel_reloaded"),
    )
    .await??;
    app_server
        .start_thread(ThreadStartParams::default())
    ...
    assert!(
        next_metrics.contains("cx.thread.started"),
...
fn write_otel_config(cx_home: &Path, collector_endpoint: &str) -> Result<()> {
    std::fs::write(
