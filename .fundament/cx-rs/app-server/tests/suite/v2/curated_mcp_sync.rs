...
impl CuratedMcpSyncFixture {
    async fn set_up() -> Result<Self> {
        ...
        let cx_home = fixture_root.path().join("cx-home");
        ...
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.email", "cx-tests@openai.com"],
        )?;
        run_git(
            &real_git,
            &curated_repo,
        ...
        run_git(&real_git, &cx_home, &["init", "-b", "main"])?;
        run_git(
            &real_git,
            &cx_home,
...
fn write_plugin(
    ...
    std::fs::create_dir_all(root.join(".cx-plugin"))?;
    std::fs::write(
        root.join(".cx-plugin/plugin.json"),
