...
use cx_protocol::mcp_approval_meta as approval_meta;
...
const TOOL_NAMESPACE: &str = "mcp__cx_apps__calendar";
...
async fn openai_form_capability_follows_the_turn_starting_connection() -> Result<()> {
    ...
    write_config_toml(cx_home.path(), &responses_server.uri(), &apps_server_url)?;
    write_gt_auth(
        cx_home.path(),
    ...
    let (mut process, bind_addr) = spawn_websocket_server(cx_home.path()).await?;
        ...
        match scenario {
            ...
            ElicitationScenario::OpenAiForm => {
                ...
                let result = match result {
                    ...
                    result => {
                        return Err(rmcp::ErrorData::internal_error(
                            format!("unexpected oi form response: {result:?}"),
                            None,
                        ));
                    }
                };
...
async fn start_apps_server(scenario: ElicitationScenario) -> Result<(String, JoinHandle<()>)> {
    let state = Arc::new(AppsServerState {
        expected_bearer: "Bearer gt-token".to_string(),
