...
use cx_app_server_protocol::PluginSearchResponse;
use cx_app_server_protocol::PluginSearchScope;
...
async fn plugin_search_stitches_local_results_into_the_first_remote_page() -> Result<()> {
    ...
    write_remote_plugin_search_config(cx_home.path(), &server, /*remote_plugin*/ true)?;
    write_gt_search_auth(cx_home.path())?;
...
async fn plugin_search_returns_local_matches_for_api_key_auth() -> Result<()> {
    for remote_plugin_enabled in [false, true] {
        let cx_home = TempDir::new()?;
        let repo_root = TempDir::new()?;
        let curated_root = cx_home.path().join(".tmp/plugins");
        let bundled_alpha_root = cx_home
            .path()
        ...
        write_remote_plugin_search_config(cx_home.path(), &server, remote_plugin_enabled)?;
        login_with_api_key(
            cx_home.path(),
            "sk-test-key",
            AuthCredentialsStoreMode::File,
...
-> Result<()> {
    ...
    write_installed_plugin(cx_home.path(), "openai-curated", "calendar")?;
    write_installed_plugin(cx_home.path(), "personal-tools", "local-planner")?;
    write_installed_plugin(cx_home.path(), "personal-tools", "calendar-local-only")?;
    std::fs::write(
        cx_home.path().join("config.toml"),
        format!(
            r#"gt_base_url = "{}/backend-api/"
...
fn write_local_marketplace(
    ...
    for plugin in plugins {
        let plugin_manifest = root.join("plugins").join(plugin.name).join(".cx-plugin");
...
fn write_gt_search_auth(cx_home: &std::path::Path) -> Result<()> {
    write_gt_auth(
        cx_home,
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
