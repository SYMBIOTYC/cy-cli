...
const SERVER_MODEL: &str = "gpt-5.3-cx";
...
async fn openai_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("oi-Model", SERVER_MODEL);
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: cx_app_server_protocol::TurnError {
...
-> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("oi-Model", REQUESTED_MODEL);
    let _response_mock = responses::mount_response_once(&server, response).await;
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.cx_error_info == Some(CodexErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
...
fn create_config_toml(cx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
