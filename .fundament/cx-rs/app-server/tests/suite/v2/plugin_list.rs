...
use cx_app_server_protocol::HookTrustStatus;
...
use cx_app_server_protocol::PluginAuthPolicy;
use cx_app_server_protocol::PluginDisabledReason;
use cx_app_server_protocol::PluginInstallParams;
use cx_app_server_protocol::PluginInstallPolicy;
use cx_app_server_protocol::PluginInstallPolicySource;
...
fn write_plugins_enabled_config(cx_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(cx_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = cx_home.path().to_string_lossy().into_owned();
...
async fn plugin_rpcs_reject_repository_spoofing_openai_curated() -> Result<()> {
    ...
    std::fs::create_dir_all(repository.path().join("attacker/.cx-plugin"))?;
    std::fs::write(
        repository.path().join("attacker/.cx-plugin/plugin.json"),
        r#"{"name":"attacker"}"#,
    )?;
    let marketplace_path =
    ...
    write_plugins_enabled_config(cx_home.path())?;
    let original_config = std::fs::read(cx_home.path().join("config.toml"))?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
    ...
    assert!(!cx_home.path().join("plugins/cache").exists());
    assert_eq!(
        std::fs::read(cx_home.path().join("config.toml"))?,
        original_config
    );
    Ok(())
}
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&cx_home, "openai-api-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(cx_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&cx_home, "openai-curated", "linear")?;
    write_installed_plugin(&cx_home, "openai-curated", "calendar")?;
    std::fs::write(
        cx_home.path().join("config.toml"),
        format!(
            r#"gt_base_url = "{}/backend-api/"
...
async fn plugin_installed_prefers_api_curated_conflicts_after_switching_to_api_auth() -> Result<()>
{
    let cx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_openai_api_curated_marketplace(cx_home.path(), &["linear"])?;
    write_installed_plugin(&cx_home, "openai-api-curated", "linear")?;
    let config = format!(
        r#"gt_base_url = "{}/backend-api/"
...
"#,
    ...
    std::fs::write(cx_home.path().join("config.toml"), &config)?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
    ...
    // Keep the gt remote snapshot cached while changing auth to exercise endpoint-level
    // filtering even when the account-change cache refresh cannot run.
    std::fs::write(cx_home.path().join("config.toml"), "invalid config")?;
    let request_id = app_server
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.cx-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.cx-plugin/plugin.json"),
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(cx_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let cx_home = TempDir::new()?;
    std::fs::create_dir_all(cx_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(cx_home.path())?;
    std::fs::write(
        cx_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "cx-curated",
  "plugins": [
    {
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.cx-plugin/plugin.json");
...
"#,
    ...
    let plugin_cache = cx_home
        .path()
        .join("plugins/cache/sample-marketplace/sample-plugin");
    let installed_manifest = plugin_cache.join("1.1.0/.cx-plugin/plugin.json");
    assert!(
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&cx_home, "cx-curated", "enabled-plugin")?;
    write_installed_plugin(&cx_home, "cx-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "cx-curated",
  "interface": {
    "displayName": "gt Official"
  },
  "plugins": [
    {
      "name": "enabled-plugin",
...
[plugins."enabled-plugin@cx-curated"]
enabled = true

[plugins."disabled-plugin@cx-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "cx-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@cx-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@cx-curated");
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let cx_home = TempDir::new()?;
    std::fs::create_dir_all(cx_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&cx_home, "cx-curated", "shared-plugin")?;
    std::fs::write(
        cx_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "cx-curated",
...
[plugins."shared-plugin@cx-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".cx"))?;
    std::fs::write(
        workspace_enabled.path().join(".cx/config.toml"),
        r#"[plugins."shared-plugin@cx-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@cx-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@cx-curated");
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = cx_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".cx-plugin"))?;
    std::fs::write(
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&installed_path.join(".cx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".cx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&cx_home, "openai-curated-remote", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&cx_home, "openai-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = cx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = cx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = cx_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".cx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".cx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
    ...
    write_installed_plugin_with_version(&cx_home, "openai-curated-remote", "linear", "1.2.3")?;
  ...
  "pagination": {
    ...
    Mock::given(method("GET"))
        ...
        .and(header("gt-account-id", "account-123"))
    ...
    assert_eq!(
        ...
        cx_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(cx_home.path().join("cache/remote_plugin_catalog"))?
...
async fn plugin_list_honors_global_remote_catalog_cache_ttl() -> Result<()> {
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(cx_home.path(), &[refreshed_remote_plugin_id])
        .await?;
    sleep(Duration::from_millis(100)).await;
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
...
async fn app_server_startup_skips_disabled_remote_plugin_catalog_scopes() -> Result<()> {
    ...
    write_remote_plugin_catalog_config(cx_home.path(), &base_url)?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
    ...
    write_remote_plugins_disabled_config_with_base_url(cx_home.path(), &base_url)?;
    server.reset().await;
    mount_remote_plugin_list(&server, "GLOBAL", &global_body).await;
    mount_remote_plugin_list(&server, "USER", &user_body).await;
    mount_remote_plugin_list(&server, "WORKSPACE", &workspace_body).await;
...
-> Result<()> {
    ...
    assert!(
        err.error
            .message
            .contains("list oi Curated remote plugin catalog")
    );
    Ok(())
}
...
"#,
    ...
    write_openai_curated_marketplace(cx_home.path(), &["gt-plugin"])?;
    write_openai_api_curated_marketplace(cx_home.path(), &["api-plugin"])?;
    ...
    assert!(!cx_home.path().join("auth.json").exists());
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-api-curated")
...
"#,
    ...
    let gt_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-curated")
        .expect("expected gt curated marketplace");
    assert_eq!(gt_curated_marketplace.plugins.len(), 1);
    assert_eq!(
        gt_curated_marketplace.plugins[0].id,
        "gt-plugin@openai-curated"
    );
    assert!(
        response
            .marketplaces
...
"#,
    ...
    write_remote_plugin_test_auth(cx_home.path())?;
    ...
    wait_for_path_exists(
        &cx_home
            .path()
            .join("plugins/cache/workspace-directory/no-hooks/1.2.3/.cx-plugin/plugin.json"),
    )
    .await?;
...
async fn plugin_installed_hook_trust_write_failure_stays_untrusted() -> Result<()> {
    ...
    symlink(&config_target, cx_home.path().join("config.toml"))?;
    write_remote_plugin_test_auth(cx_home.path())?;
...
async fn plugin_list_fetches_shared_with_me_kind() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some("https://gt.example/plugins/share/share-key-1")
    );
    assert_eq!(share_context.share_principals, None);
    assert_eq!(
        marketplace.plugins[1].id,
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    let remote_marketplace = response
        ...
        .into_iter()
        .find(|marketplace| marketplace.name == "openai-curated-remote")
        .expect("expected gt remote marketplace");
    ...
    assert_eq!(
        plugin.availability,
        cx_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
#[tokio::test]
async fn plugin_list_omits_featured_plugin_ids_without_gt_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(cx_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_api_curated_marketplace(cx_home.path(), &["linear", "gmail"])?;
...
async fn plugin_list_uses_warmed_featured_plugin_ids_cache_on_first_request() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_plugin_sync_config(cx_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(cx_home.path(), &["linear", "gmail"])?;
    write_remote_plugin_test_auth(cx_home.path())?;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(cx_home)?;
...
fn cached_remote_catalog_plugin_ids(cx_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = cx_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn workspace_remote_plugin_page_body(
  ...
  "plugins": [
    {{
      ...
      "share_url": "https://gt.example/plugins/share/share-key-1",
      "installation_policy": "AVAILABLE",
      "authentication_policy": "ON_USE",
      "status": "ENABLED",
      "creator_name": "Gavin",
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(cx_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(cx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        cx_home.join("config.toml"),
        format!(
            r#"
gt_base_url = "{base_url}"
...
fn write_remote_plugin_test_auth(cx_home: &std::path::Path) -> Result<()> {
    write_gt_auth(
        cx_home,
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = cx_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.cx-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(cx_home.join(".tmp"))?;
    std::fs::write(
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(cx_home.join(".tmp"))?;
    std::fs::write(
        cx_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}
