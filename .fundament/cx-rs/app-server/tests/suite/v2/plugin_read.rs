...
use cx_app_server_protocol::AppMetadata;
...
use cx_app_server_protocol::ScheduledTaskSchedule;
use cx_app_server_protocol::ScheduledTaskSummary;
use cx_app_server_protocol::ScheduledTaskWeekday;
use cx_app_server_protocol::SkillInterface;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                cx_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_read_includes_share_url_for_admin_disabled_remote_plugin() -> Result<()> {
  ...
  "name": "example-plugin",
  "scope": "GLOBAL",
  "share_url": "https://gt.example/plugins/share/example-plugin",
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        Some("https://gt.example/plugins/share/example-plugin")
    );
    assert_eq!(
...
async fn plugin_read_rejects_invalid_remote_plugin_name() -> Result<()> {
    let cx_home = TempDir::new()?;
    write_remote_plugin_catalog_config(cx_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
...
async fn plugin_read_returns_canonical_openai_curated_marketplace_name() -> Result<()> {
    let cx_home = TempDir::new()?;
    let repo_root = cx_home.path().join(".tmp/plugins");
    write_plugin_marketplace(&repo_root, "openai-curated", "demo-plugin", "./demo-plugin")?;
    std::fs::create_dir_all(repo_root.join("demo-plugin/.cx-plugin"))?;
    std::fs::write(
        repo_root.join("demo-plugin/.cx-plugin/plugin.json"),
        r#"{
  "name": "demo-plugin",
  "description": "oi curated plugin"
}"#,
    )?;
    std::fs::write(
        cx_home.path().join("config.toml"),
...
"#,
    ...
    write_installed_plugin(&cx_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(cx_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&cx_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "cx-curated",
        "demo-plugin",
...
async fn plugin_read_agent_plugin_excludes_nested_skills() -> Result<()> {
    ...
    assert_eq!(
        ...
        vec![cx_app_server_protocol::SkillSummary {
...
async fn plugin_read_returns_plugin_details_with_bundle_contents() -> Result<()> {
    ...
    std::fs::create_dir_all(plugin_root.join("skills/gt-only"))?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "cx-curated",
...
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/gt-only/SKILL.md"),
        r#"---
name: gt-only
description: Visible only for gt
---
...
"#,
    ...
    std::fs::write(
        plugin_root.join("skills/gt-only/agents/openai.yaml"),
        r#"policy:
  products:
    - CHATGPT
"#,
...
[plugins."demo-plugin@cx-curated"]
enabled = true

[hooks.state."demo-plugin@cx-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&cx_home, "cx-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "cx-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@cx-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            cx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@cx-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            cx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@cx-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            cx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@cx-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    write_connectors_config(cx_home.path(), &server_url)?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    write_connectors_config(cx_home.path(), &server.uri())?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
...
fn write_plugins_enabled_config(cx_home: &TempDir) -> Result<()> {
    std::fs::write(
        cx_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn batch_apps(
    ...
    let bearer_ok = headers
        ...
        .is_some_and(|value| value == "Bearer gt-token");
...
fn write_connectors_config(cx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
