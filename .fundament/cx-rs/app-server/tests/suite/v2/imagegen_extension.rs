...
use cx_app_server_protocol::ImageGenerationFailure;
...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(cx_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
    write_gt_auth(
        cx_home.path(),
    ...
    let turn_id = start_image_generation_turn(
        &mut mcp,
        ThreadStartParams {
            service_name: Some("gt_cca".to_string()),
            ..Default::default()
        },
    )
    .await?;
    ...
    let image_request = server
        ...
        .into_iter()
        .find(|request| request.url.path() == "/api/cx/images/generations")
...
async fn standalone_image_generation_failure_emits_terminal_item() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/cx/images/generations"))
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(429)
                .insert_header("x-cx-active-limit", "image_gen")
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let (edit_request, _) = run_image_edit_test(|cx_home| {
        let image_path = cx_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let cx_home = TempDir::new()?;
    let (arguments, input) = input(cx_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    let image_request = requests
        .iter()
        .find(|request| request.url.path() == "/api/cx/images/edits")
...
fn create_config_toml(
    ...
    let mut config = MockResponsesConfig::new(server_uri)
        ...
        .with_provider_name("oi")
        .with_provider_base_url(&format!("{server_uri}/api/cx"))
        .with_root_config(&format!("gt_base_url = \"{server_uri}\""))
