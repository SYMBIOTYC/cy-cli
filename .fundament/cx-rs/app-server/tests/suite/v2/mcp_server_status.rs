...
-> Result<()> {
    ...
    let client_id = format!("https://chatgpt.com/oauth/cx/{callback_id}/client.json");
...
-> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let cx_home = TempDir::new()?;
    let barrier_file = cx_home.path().join("allow-initialize");
    let pid_file = cx_home.path().join("mcp.pid");
