...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(cx_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".cx-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                cx_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
            install_attempt_id: None,
...
async fn plugin_install_tracks_analytics_when_marketplace_file_cannot_be_read() -> Result<()> {
    ...
    let marketplace_path = cx_home.path().join("marketplace-dir");
...
#[tokio::test]
async fn plugin_install_skips_mcp_oauth_for_gt_dual_surface_plugin() -> Result<()> {
    ...
    let cx_home = TempDir::new()?;
    write_connectors_config(cx_home.path(), &apps_server_url)?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
...
"#,
    ...
    let persisted_config = std::fs::read_to_string(cx_home.path().join("config.toml"))?;
...
async fn plugin_install_skips_mcp_oauth_for_unowned_environment() -> Result<()> {
    ...
    std::fs::write(
        cx_home.path().join("environments.toml"),
        format!(
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/cx/ps/mcp", mcp_service);
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/cx/analytics-events/events"))
...
fn configure_remote_plugin_test(cx_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(cx_home, &format!("{}/backend-api/", server.uri()))?;
    write_gt_auth(
        cx_home,
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
