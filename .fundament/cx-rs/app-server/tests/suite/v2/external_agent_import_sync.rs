...
const NATIVE_USER: &str = "native CX message";
const NATIVE_ASSISTANT: &str = "native CX answer";
...
impl ImportFixture {
    async fn new() -> Result<Self> {
        ...
        let app_server = start_app_server(cx_home.path()).await?;
        Ok(Self {
            _cx_home: cx_home,
            project_root,
            session_path,
    ...
    async fn restart(&mut self) -> Result<()> {
        ...
        self.app_server = start_app_server(self._cx_home.path()).await?;
        Ok(())
    }
...
async fn start_app_server(cx_home: &Path) -> Result<TestAppServer> {
    let home = cx_home.display().to_string();
...
async fn cold_diverged_target_is_deferred_without_checkpoint() -> Result<()> {
    ...
    let mut fixture = ImportFixture::new().await?;
    MockResponsesConfig::new(&model.uri()).write(fixture._cx_home.path())?;
