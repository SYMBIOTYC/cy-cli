...
async fn assert_standalone_web_search_round_trips_output(
    ...
    let search_path = match provider {
        WebSearchProvider::ChatGpt => "/api/cx/alpha/search",
    ...
    let config = match provider {
        WebSearchProvider::ChatGpt => config
            ...
            .with_provider_base_url(&format!("{}/api/cx", server.uri()))
    ...
    let expected_authorization = match provider {
        WebSearchProvider::ChatGpt => "Bearer access-gt",
    ...
    let turn_metadata_header = search_request
        ...
        .context("standalone search should include x-cx-turn-metadata")?
        .to_str()
        .context("x-cx-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-cx-turn-metadata should be valid JSON")?;
