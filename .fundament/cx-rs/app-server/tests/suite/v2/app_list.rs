...
use cx_app_server_protocol::AppBranding;
...
use cx_app_server_protocol::AppReview;
use cx_app_server_protocol::AppScreenshot;
...
async fn list_apps_returns_empty_with_api_key_auth() -> Result<()> {
    ...
    save_auth(
        ...
        &AuthDotJson {
            ...
            cy_api_key: Some("test-api-key".to_string()),
            tokens: None,
            last_refresh: None,
            agent_identity: None,
...
#[tokio::test]
async fn list_apps_uses_external_gt_auth() -> Result<()> {
...
#[tokio::test]
async fn list_apps_includes_plugin_apps_for_gt_auth() -> Result<()> {
    let (server_url, server_handle) =
        start_apps_server_with_delays(Vec::new(), Vec::new(), Duration::ZERO, Duration::ZERO)
            .await?;

    let cx_home = TempDir::new()?;
    write_connectors_and_plugins_config(cx_home.path(), &server_url)?;
    write_plugin_app_fixture(cx_home.path(), "sample", "connector_sample")?;
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
            .gt_user_id("user-plugin-apps")
            .gt_account_id("account-123"),
        AuthCredentialsStoreMode::File,
    )?;
...
async fn list_apps_waits_for_accessible_data_before_emitting_directory_updates() -> Result<()> {
    ...
    write_gt_auth(
        ...
        ChatGptAuthFixture::new("gt-token")
            .account_id("account-123")
            .gt_user_id("user-directory-first")
            .gt_account_id("account-123"),
        AuthCredentialsStoreMode::File,
    )?;
...
async fn list_apps_force_refetch_preserves_previous_cache_on_failure() -> Result<()> {
    ...
    write_gt_auth(
        cx_home.path(),
        ChatGptAuthFixture::new("gt-token-invalid")
            .account_id("account-123")
            .gt_user_id("user-123")
            .gt_account_id("account-123"),
        AuthCredentialsStoreMode::File,
...
fn write_connectors_and_plugins_config(cx_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = cx_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(cx_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {
