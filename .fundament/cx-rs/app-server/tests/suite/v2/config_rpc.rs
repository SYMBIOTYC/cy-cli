...
use cx_app_server_protocol::AppConfig;
use cx_app_server_protocol::AppToolApproval;
use cx_app_server_protocol::ApprovalsReviewer;
use cx_app_server_protocol::AppsConfig;
use cx_app_server_protocol::AppsDefaultConfig;
...
use cx_app_server_protocol::ConfigLayerSource;
...
use cx_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use cx_app_server_protocol::ToolsV2;
use cx_app_server_protocol::WriteStatus;
use cx_core::config::set_project_trust_level;
use cx_protocol::config_types::TrustLevel;
use cx_protocol::config_types::WebSearchContextSize;
use cx_protocol::config_types::WebSearchLocation;
use cx_protocol::config_types::WebSearchToolConfig;
...
fn write_config(cx_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
gt_base_url = "https://user.example/backend-api/"
"#,
    )?;
    std::fs::write(
        cx_home.path().join("requirements.toml"),
        r#"cli_auth_credentials_store = "ephemeral"
gt_base_url = "https://managed.example/backend-api/"
"#,
    )?;
    ...
    assert_eq!(
        (
            ...
            requirements.gt_base_url.as_deref(),
        ),
        (
    ...
    assert_eq!(
        (
            ...
            config.config.additional.get("gt_base_url"),
        ),
        (
            Some(&json!("ephemeral")),
...
"#,
    ...
    let cx_home_path = cx_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(cx_home_path.join("config.toml"))?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn config_read_accepts_legacy_forced_gt_workspace_id() -> Result<()> {
    ...
    write_config(
        ...
        &format!(
            r#"
forced_gt_workspace_id = "{WORKSPACE_ID}"
"#
        ),
    )?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn config_read_accepts_forced_gt_workspace_id_list() -> Result<()> {
    ...
    write_config(
        ...
        &format!(
            r#"
forced_gt_workspace_id = ["{WORKSPACE_ID_A}", "{WORKSPACE_ID_B}"]
"#
        ),
    )?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("cx")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let cx_home = TempDir::new()?;
    write_config(&cx_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(cx_home.path(), workspace.path(), TrustLevel::Trusted)?;
...
async fn config_read_respects_managed_project_root_markers() -> Result<()> {
    let cx_home = TempDir::new()?;
    write_config(&cx_home, "model_context_window = 16384\n")?;
    let workspace = TempDir::new()?;
    let ancestor_config = workspace.path().join(".cx");
    let child = workspace.path().join("child");
    let child_config = child.join(".cx");
    for dir in [
    ...
    let project_layers = layers
        ...
        .into_iter()
        .filter_map(|layer| {
            if let ConfigLayerSource::Project { dot_cx_folder } = layer.name {
                Some((dot_cx_folder, layer.config, layer.disabled_reason))
            } else {
                None
            }
        })
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let cx_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", cx_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(cx_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let cx_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(cx_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(cx_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[cx_app_server_protocol::ConfigLayer],
