...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_cx_home(fixture.cx_home.path())
    ...
    let mut exec_server =
        spawn_exec_server(fixture.cx_home.path(), &fixture.exec_server_url).await?;
    ...
    exec_server = spawn_exec_server(fixture.cx_home.path(), &fixture.exec_server_url).await?;
    add_environment(&mut app_server, &fixture.exec_server_url).await?;
    wait_for_selected_mcp_server(&mut app_server, &thread_id).await?;
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = cx_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".cx-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__cx_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(cx_home: &std::path::Path, url: &str) -> Result<Child> {
