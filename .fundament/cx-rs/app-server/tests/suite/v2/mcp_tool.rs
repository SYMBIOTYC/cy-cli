...
use axum::Router;
use cx_app_server_protocol::CapabilityRootLocation;
...
use cx_protocol::protocol::ThreadSource as CoreThreadSource;
...
-> Result<()> {
    mcp_server_tool_call_round_trips_elicitation_for_thread(ElicitationThread::Start {
        params: ThreadStartParams {
            ...
            sandbox: Some(cx_app_server_protocol::SandboxMode::DangerFullAccess),
            thread_source: Some(cx_app_server_protocol::ThreadSource::User),
            ..Default::default()
        },
        session_source: "vscode",
...
{
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(cx_app_server_protocol::ThreadSource::Feature(
...
async fn mcp_server_tool_call_declines_full_access_elicitation_for_subagent_thread() -> Result<()> {
    assert_full_access_form_elicitation_is_declined(FullAccessElicitationCase {
        thread_source: Some(cx_app_server_protocol::ThreadSource::Subagent),
        client_advertises_standard_form_input: true,
    })
    .await
}
...
struct FullAccessElicitationCase {
    thread_source: Option<cx_app_server_protocol::ThreadSource>,
...
async fn assert_full_access_form_elicitation_is_declined(
    ...
    assert_eq!(
        approval_policy,
        cx_app_server_protocol::AskForApproval::Never
    );
    assert_eq!(
        sandbox,
...
async fn mcp_server_tool_call_round_trips_elicitation_for_thread(
    ...
    let thread = match elicitation_thread {
        ...
        ElicitationThread::Resume { source, params } => {
            ...
            assert_eq!(
                thread.source,
                cx_app_server_protocol::SessionSource::from(source)
            );
            assert_eq!(
                approval_policy,
                cx_app_server_protocol::AskForApproval::Never
