...
use cx_app_server_protocol::QueuedSubmission;
use cx_app_server_protocol::RequestId;
use cx_app_server_protocol::ServerRequest;
use cx_app_server_protocol::ThreadItem;
use cx_app_server_protocol::ThreadQueueAddParams;
use cx_app_server_protocol::ThreadQueueAddResponse;
use cx_app_server_protocol::ThreadQueueChangedNotification;
use cx_app_server_protocol::ThreadQueueDeleteParams;
use cx_app_server_protocol::ThreadQueueDeleteResponse;
use cx_app_server_protocol::ThreadQueueListParams;
use cx_app_server_protocol::ThreadQueueListResponse;
use cx_app_server_protocol::ThreadQueueReorderParams;
use cx_app_server_protocol::ThreadQueueReorderResponse;
use cx_app_server_protocol::ThreadQueueStartParams;
use cx_app_server_protocol::ThreadQueueStartResponse;
use cx_app_server_protocol::ThreadQueueUpdateParams;
use cx_app_server_protocol::ThreadQueueUpdateResponse;
...
async fn queue_requires_experimental_handshake() -> Result<()> {
    let (mut app, cx_home, _server) = queue_app(Vec::new()).await?;
...
async fn queue_crud_preserves_identity_order_and_notifications() -> Result<()> {
    ...
    let (mut app, _cx_home, _server) = queue_app(responses).await?;
    let thread_id = app
...
async fn queue_list_returns_ordered_pages_and_lightweight_notifications() -> Result<()> {
    ...
    let (mut app, _cx_home, _server) = queue_app(vec![blocked_turn_response()?]).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn idle_queue_dispatch_preserves_client_id() -> Result<()> {
    ...
    let (mut app, _cx_home, server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
    ...
    let metadata_header = request
        .headers
        .get("x-cx-turn-metadata")
        .context("queued model request is missing its x-cx-turn-metadata header")?
        .to_str()
...
async fn cold_thread_resume_dispatches_a_persisted_queued_submission() -> Result<()> {
    ...
    let (mut first, cx_home, _server) = queue_app(responses).await?;
    let thread_id = first
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn interrupt_preserves_queue_and_queue_start_can_resume_a_non_head_item() -> Result<()> {
    ...
    let (mut app, cx_home, _server) = queue_app(responses).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn queue_start_while_active_returns_busy_and_preserves_the_queue() -> Result<()> {
    ...
    .await;
    let (mut app, _cx_home, _server) = queue_app_with_server(server).await?;
    let thread_id = app
        .start_thread(ThreadStartParams::default())
        .await?
        .thread
...
async fn queue_app_with_server(server: MockServer) -> Result<(TestAppServer, TempDir, MockServer)> {
    ...
    config.write(cx_home.path())?;
    let app = TestAppServer::builder()
        .with_cx_home(cx_home.path())
        .without_managed_config()
        .build_initialized()
        .await?;
    Ok((app, cx_home, server))
}
