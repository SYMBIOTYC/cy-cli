...
use cx_app_server_protocol::SkillScope;
use cx_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_local_curated_plugin_with_skill(
    ...
    let plugin_root = cx_home.join(format!(
...
async fn skills_list_uses_each_cwds_bundled_skills_configuration() -> Result<()> {
    ...
    for (cwd, enabled) in [(disabled_cwd.path(), false), (enabled_cwd.path(), true)] {
        ...
        std::fs::create_dir_all(cwd.join(".cx"))?;
        std::fs::write(
            cwd.join(".cx/config.toml"),
            format!("[skills.bundled]\nenabled = {enabled}\n"),
        )?;
        set_project_trust_level(cx_home.path(), cwd, TrustLevel::Trusted)?;
    }
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(cx_home.path(), "openai-curated")?;
    std::fs::write(
        cx_home.path().join("config.toml"),
        format!(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(cx_home.path())?)?;
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    let cx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_skill(&cx_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".cx/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(cx_home.path().join(&relative_cwd))?;
...
async fn skills_list_preserves_requested_cwd_order() -> Result<()> {
    ...
    write_skill(&cx_home, "shared-skill")?;
    write_cached_local_curated_plugin_with_skill(cx_home.path(), "openai-api-curated")?;
    std::fs::write(
        cx_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".cx/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
