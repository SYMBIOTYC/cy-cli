...
#[tokio::test]
async fn get_account_rate_limits_requires_gt_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
...
async fn get_account_rate_limits_returns_snapshot(
    ...
    write_gt_base_url(cx_home.path(), &server_url)?;
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/cx/rate-limit-reset-credits"))
        .and(header("authorization", "Bearer gt-token"))
        .and(header("gt-account-id", "account-123"))
...
async fn get_account_rate_limits_preserves_count_when_reset_credit_details_fail() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_gt_base_url(cx_home.path(), &server.uri())?;
...
#[tokio::test]
async fn send_add_credits_nudge_email_requires_gt_auth() -> Result<()> {
    let cx_home = TempDir::new()?;
...
async fn send_add_credits_nudge_email_posts_expected_body() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/cx/accounts/send_add_credits_nudge_email"))
        .and(header("authorization", "Bearer gt-token"))
        .and(header("gt-account-id", "account-123"))
...
fn write_gt_base_url(cx_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = cx_home.join("config.toml");
    std::fs::write(config_toml, format!("gt_base_url = \"{base_url}\"\n"))
}
