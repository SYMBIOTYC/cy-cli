...
use cx_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut cx_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = cx_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("cx-app-server"),
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, cx_home: &Path) -> Result<()> {
    ...
    let body = timeout(read_timeout, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/cx/analytics-events/events"
            }) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "cx_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("cx_thread_initialized event should be present"))
}
...
fn write_curated_metrics_plugin(cx_home: &Path) -> Result<PathBuf> {
    let plugin_id = PluginId::parse(METRICS_PLUGIN_ID)?;
    let plugin_root = PluginStore::new(cx_home.to_path_buf()).plugin_root(
        &plugin_id,
...
"#,
    ...
    let curated_repo = cx_home.join(".tmp/plugins");
...
async fn assert_plugin_measurement_analytics(runtime: PluginMetricsRuntime) -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "trusted plugin metrics fixture uses a local CX home cache"
    );
    skip_if_wine_exec!(Ok(()), "plugin metrics fixture is Unix-only");

    let cx_home = TempDir::new()?;
    let script_path = write_curated_metrics_plugin(cx_home.path())?.canonicalize()?;
...
"#,
    ...
    mount_analytics_capture(&analytics_server, cx_home.path()).await?;
    ...
    if remote {
        assert_eq!(
            ...
            cx_exec_server::REMOTE_ENVIRONMENT_ID
        );
    }
    ...
    for measurement_name in ["findings", "files_scanned"] {
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "cx_plugin_measurement_event"
    ...
    let command_event =
        wait_for_matching_analytics_event(&analytics_server, Duration::from_secs(10), |event| {
            event["event_type"] == "cx_command_execution_event"
                && event["event_params"]["item_id"] == call_id
        })
        .await?;
    assert_eq!(
