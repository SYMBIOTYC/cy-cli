...
use cx_core::find_archived_thread_path_by_id_str;
...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(cx_home.path(), &thread.id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
    ...
    // Verify file moved.
    let archived_directory = cx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(cx_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = cx_home
        .path()
    ...
    wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "cx_thread_archive_event"
    ...
    for request in requests {
        if request.url.path() != "/cx/analytics-events/events" {
            continue;
        }
        ...
        for event in events
            .iter()
            .filter(|event| event["event_type"] == "cx_thread_archive_event")
        {
            for (header, expected) in [
                ("authorization", "Bearer gt-token"),
                ("gt-account-id", "account-123"),
            ] {
                assert_eq!(
                    request
                        .headers
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(cx_home.path(), &parent_id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
