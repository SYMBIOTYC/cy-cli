...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(cx_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(cx_home);
        command.env("CX_HOME", cx_home);
        #[cfg(target_os = "linux")]
        command.env(
            CX_LINUX_SANDBOX_EXE_ENV_VAR,
