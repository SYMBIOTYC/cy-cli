...
/// Builder for writing a fake gt auth.json in tests.
#[derive(Debug, Clone)]
...
impl ChatGptAuthFixture {
    ...
    pub fn gt_user_id(mut self, gt_user_id: impl Into<String>) -> Self {
        self.claims.gt_user_id = Some(gt_user_id.into());
        self
    }

    pub fn gt_account_id(mut self, gt_account_id: impl Into<String>) -> Self {
        self.claims.gt_account_id = Some(gt_account_id.into());
        self
    }
...
pub struct ChatGptIdTokenClaims {
    ...
    pub gt_user_id: Option<String>,
    pub gt_account_id: Option<String>,
}
...
impl ChatGptIdTokenClaims {
    ...
    pub fn gt_user_id(mut self, gt_user_id: impl Into<String>) -> Self {
        self.gt_user_id = Some(gt_user_id.into());
        self
    }

    pub fn gt_account_id(mut self, gt_account_id: impl Into<String>) -> Self {
        self.gt_account_id = Some(gt_account_id.into());
        self
    }
}
...
pub fn encode_id_token(claims: &ChatGptIdTokenClaims) -> Result<String> {
    ...
    if let Some(plan_type) = &claims.plan_type {
        auth_payload.insert("gt_plan_type".to_string(), json!(plan_type));
    }
    if let Some(gt_user_id) = &claims.gt_user_id {
        auth_payload.insert("gt_user_id".to_string(), json!(gt_user_id));
    }
    if let Some(gt_account_id) = &claims.gt_account_id {
        auth_payload.insert("gt_account_id".to_string(), json!(gt_account_id));
    }
...
pub fn write_gt_auth(
    cx_home: &Path,
    fixture: ChatGptAuthFixture,
    cli_auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> Result<()> {
    let id_token_raw = encode_id_token(&fixture.claims)?;
    let id_token = parse_gt_jwt_claims(&id_token_raw).context("parse id token")?;
