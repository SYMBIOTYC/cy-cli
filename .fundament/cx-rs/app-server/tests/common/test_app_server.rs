...
use cx_app_server_protocol::FsCopyParams;
use cx_app_server_protocol::FsCreateDirectoryParams;
use cx_app_server_protocol::FsGetMetadataParams;
use cx_app_server_protocol::FsReadDirectoryParams;
use cx_app_server_protocol::FsReadFileParams;
use cx_app_server_protocol::FsRemoveParams;
use cx_app_server_protocol::FsUnwatchParams;
use cx_app_server_protocol::FsWatchParams;
...
use cx_app_server_protocol::PluginSearchParams;
...
use cx_app_server_protocol::ProjectImportParams;
use cx_app_server_protocol::ProjectListParams;
use cx_app_server_protocol::ProjectReadParams;
use cx_app_server_protocol::RemoteControlClientsListParams;
use cx_app_server_protocol::RemoteControlClientsRevokeParams;
use cx_app_server_protocol::RemoteControlPairingStartParams;
use cx_app_server_protocol::RemoteControlPairingStatusParams;
...
impl TestAppServer {
    ...
    async fn new_with_program_env_and_args(
        ...
        let mut process = loop {
            ...
            if !process
                ...
                || retries == 2
            {
                break process.context("cx-mcp-server proc should start")?;
            }
            retries += 1;
    ...
    /// Send an `account/login/start` JSON-RPC request with gt auth tokens.
    pub async fn send_gt_auth_tokens_login_request(
        &mut self,
        access_token: String,
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: cx_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
    ...
    /// Send an `account/login/start` JSON-RPC request for gt login.
    pub async fn send_login_account_gt_request(&mut self) -> anyhow::Result<i64> {
        let params = serde_json::json!({
            "type": "gt"
        });
        self.send_login_account_request(params).await
    ...
    /// Send an `account/login/start` JSON-RPC request for gt device code login.
    pub async fn send_login_account_gt_device_code_request(&mut self) -> anyhow::Result<i64> {
        let params = serde_json::json!({
            "type": "gtDeviceCode"
        });
        self.send_login_account_request(params).await
...
impl TestAppServerBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (cx_home, owned_cx_home) = match cx_home {
        ...
        let attribution_settings_server = if cx_home.join("auth.json").is_file() {
            let config_path = cx_home.join("config.toml");
            let config = std::fs::read_to_string(&config_path)?;
            if config
                .lines()
                .any(|line| line.trim_start().starts_with("gt_base_url"))
            {
                None
            } else {
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = cx_home.join("environments.toml");
                ensure!(
        ...
        let mut program = match program {
            ...
            None => cx_utils_cargo_bin::cargo_bin("cx-app-server")
                .context("should find binary for cx-app-server")?,
        };
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `cx-app-server` child process.
        //
