...
mod tests {
    ...
    use cx_config::ThreadConfigContext;
    use cx_config::ThreadConfigLoadError;
    use cx_config::ThreadConfigLoadErrorCode;
    ...
    async fn strict_refresh_reports_thread_planning_failures() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(cx_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_config_preserves_thread_mcp_overrides() -> anyhow::Result<()> {
        ...
        let thread_config = initial_config_manager
            ...
            .await?;
        let thread = thread_manager
            .start_thread(cx_core::StartThreadOptions::new(thread_config))
            .await?
            .thread;
        std::fs::write(
            temp_dir.path().join(cx_config::CONFIG_TOML_FILE),
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                cx_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: cx_analytics::AnalyticsEventsClient::disabled(),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(cx_goal_extension::GoalService::new()),
                        environment_manager: Arc::clone(&environment_manager),
                        executor_skill_provider: Arc::clone(&executor_skill_provider),
                        git_attribution_base_url: good_config.gt_base_url.clone(),
                        http_client_factory: good_config.http_client_factory(),
                        queue_service: None,
                    },
                ),
                Arc::new(CodexHomeUserInstructionsProvider::new(
                    good_config.cx_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                cx_core::local_agent_graph_store_from_state_db(Some(&state_db)),
        ...
        thread_manager
            .start_thread(cx_core::StartThreadOptions::new(good_config))
            .await?;
        thread_manager
            .start_thread(cx_core::StartThreadOptions::new(bad_config))
            .await?;
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> cx_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
