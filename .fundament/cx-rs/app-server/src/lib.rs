...
use cx_code_mode::GrpcCodeModeSessionProvider;
use cx_code_mode::WebSocketCodeModeSessionProvider;
use cx_config::LoaderOverrides;
use cx_config::NoopThreadConfigLoader;
use cx_core::config::Config;
use cx_core::config::UnsupportedUntrustedApprovalPolicyError;
...
use cx_analytics::AppServerRpcTransport;
...
use cx_app_server_protocol::TextPosition as AppTextPosition;
use cx_app_server_protocol::TextRange as AppTextRange;
use cx_config::ConfigLayerSource;
use cx_config::ConfigLoadError;
use cx_config::TextRange as CoreTextRange;
use cx_core::ExecPolicyError;
...
use cx_rollout::state_db as rollout_state_db;
use cx_state::log_db;
use tokio::sync::mpsc;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "CX rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "cx-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "CX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.all_layers_low_to_high() {
        let ConfigLayerSource::Project { dot_cx_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_cx_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let otel = cx_core::otel_init::build_provider(
        &config,
    ...
    cx_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    cx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&cx_home)?;
    ...
    let (remote_control_accept_handle, remote_control_handle) = start_remote_control(
        RemoteControlStartConfig {
            remote_control_url: config.gt_base_url.clone(),
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !cx_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "CX local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = cx_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged CX local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("cx-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
