...
use cx_thread_store::LocalQueueStore;
use cx_thread_store::QueueStore;
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = cx_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let executor_skill_provider: Arc<dyn cx_skills_extension::SkillProvider> = Arc::new(
            cx_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ...
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                cx_core::build_models_manager(config.as_ref(), auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        git_attribution_base_url: config.gt_base_url.clone(),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let cx_request = deserialize_client_request(request);
                let result = match cx_request {
                    Ok(cx_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = cx_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = cx_request.experimental_reason()
        ...
        let serialization_scope = cx_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: cx_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match cx_request {
