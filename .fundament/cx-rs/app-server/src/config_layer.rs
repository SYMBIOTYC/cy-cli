use cx_app_server_protocol::ConfigLayer as ApiConfigLayer;
use cx_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use cx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use cx_config::ConfigLayer;
use cx_config::ConfigLayerMetadata;
use cx_config::ConfigLayerSource;

/// Converts a config-layer source owned by `cx-config` into the app-server wire type owned by
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_cx_folder } => {
            ApiConfigLayerSource::Project { dot_cx_folder }
        }
...
/// Converts config-layer metadata owned by `cx-config` into the app-server wire type owned by
/// `cx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `cx-config` into the app-server wire type owned by
/// `cx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
