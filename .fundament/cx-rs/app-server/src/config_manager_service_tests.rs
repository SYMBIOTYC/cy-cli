...
use cx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
...
async fn write_value_preserves_comments_and_order() -> Result<()> {
    ...
    let original = r#"# CX user configuration
model = "gpt-5.2"
...
"#;
    ...
    let expected = r#"# CX user configuration
model = "gpt-5.2"
approval_policy = "on-request"
...
async fn psp_feature_configures_first_party_routing() -> Result<()> {
    ...
    assert!(config.features.enabled(cx_features::Feature::Psp));
    assert_eq!(
        config.http_client_factory(),
        HttpClientFactory::new(OutboundProxyPolicy::ReqwestDefault)
            .with_gt_cookies([HeaderValue::from_static("oai-chat-psp=true")])
    );
    assert_eq!(
        config
            .config_layer_stack
...
async fn batch_write_rejects_legacy_profile_selector() -> Result<()> {
    ...
    let error = service
        .batch_write(ConfigBatchWriteParams {
            edits: vec![
                cx_app_server_protocol::ConfigEdit {
...
async fn load_default_config_preserves_managed_requirements_and_selected_user_config_path() {
    ...
    std::fs::write(
        ...
        "allowed_login_methods = [\"api\"]\nallowed_gt_workspaces = [\"managed-workspace\"]\n",
    )
    .unwrap();
    let selected_path = tmp.path().join("work.config.toml");
    ...
    assert_eq!(
        config
            ...
            .requirements()
            .managed_auth_policy(),
        cx_config::ManagedAuthPolicy {
            allowed_login_methods: Some(vec![cx_protocol::config_types::ForcedLoginMethod::Api]),
            allowed_gt_workspaces: Some(vec!["managed-workspace".to_string()]),
        }
    );
}
...
async fn managed_auth_policy_survives_unusable_requirements_file_changes() -> Result<()> {
    ...
    for refreshed in [
        ...
        assert_eq!(refreshed.forced_gt_workspace_id, None);
    }
    assert!(
        auth_manager.is_login_method_allowed(cx_protocol::config_types::ForcedLoginMethod::Api)
    );
    assert!(
        !auth_manager
            .is_login_method_allowed(cx_protocol::config_types::ForcedLoginMethod::Chatgpt)
    );
    assert_eq!(
        auth_manager.effective_gt_workspaces(),
...
async fn reserved_builtin_provider_override_rejected() {
    ...
    let error = service
        .write_value(ConfigValueWriteParams {
            ...
            value: serde_json::json!("oi Override"),
            merge_strategy: MergeStrategy::Replace,
            expected_version: None,
        })
        .await
