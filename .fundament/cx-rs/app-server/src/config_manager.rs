...
use cx_cloud_config::cloud_config_bundle_loader;
...
use cx_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective CX configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        ...
        let mut config = ConfigBuilder::default()
            ...
            .fallback_cwd(Some(self.cx_home.clone()))
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let merged_cli_overrides = cli_overrides
            ...
            .collect::<Vec<_>>();
        let mut config = cx_core::config::ConfigBuilder::default()
            .cx_home(self.cx_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.cx_self_exe = self.arg0_paths.cx_self_exe.clone();
        config.cx_linux_sandbox_exe = self.arg0_paths.cx_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(cx_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(cx_home: PathBuf) -> Self {
        Self::new_for_tests(
            cx_home,
            Vec::new(),
