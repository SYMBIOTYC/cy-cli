use cx_backend_client::ApiKeyTurnCost;
use cx_backend_client::ApiKeyTurnCostStatus;
...
impl WorkerRuntime {
    ...
    async fn probe_backend(&self) -> BackendAvailability {
            ...
            .to_api_provider(Some(AuthMode::ApiKey))
        {
            ...
            Err(error) => {
                warn!("failed to resolve oi API-key provider headers: {error}");
    ...
    async fn poll_api_key_entries(&mut self, turn_ids: &[String], auth: &CodexAuth) {
        ...
        .await
        {
            Ok(Ok(costs)) => costs,
            Ok(Err(error)) => {
                warn!("failed to query oi API-key turn costs: {error}");
                self.retry_entries(turn_ids);
                return;
            }
            Err(_) => {
                warn!("timed out querying oi API-key turn costs");
                self.retry_entries(turn_ids);
                return;
            }
        };
