...
use cx_connectors::ConnectorRuntimeTool;
...
use cx_mcp::host_owned_cx_apps_enabled;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "cx.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "cx.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "cx.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "cx.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                .cx_apps_tools_cache()
                .current_snapshot(config.cx_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    ...
                    let cx_apps_auth_manager =
                        host_owned_cx_apps_enabled(&mcp_config, auth.as_ref())
                    ...
                    let runtime = McpRuntime::new(McpRuntimeInput {
                        ...
                        cx_apps_tools_cache_key: cache_key.clone(),
