...
//! the configured CX executable as a subprocess, accepts only valid JSON from
//! `cx doctor --json`, derives a small set of Sentry tags, and otherwise
...
use cx_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `cx-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `cx --cd <workspace> doctor --json` and returns a best-effort
/// feedback attachment.
///
/// Failure to spawn CX, finish before the timeout, or parse JSON means the
...
pub(crate) async fn doctor_feedback_report(
    ...
    let mut command = doctor_command(&executable, workspace, config.cx_home.as_path());
...
fn doctor_command(executable: &Path, cwd: &Path, cx_home: &Path) -> Command {
...
mod tests {
    ...
    fn doctor_command_keeps_missing_workspace_out_of_process_cwd() {
        let cx_home = tempdir().expect("cx home");
        let workspace = cx_home.path().join("deleted-workspace");

        let command = doctor_command(Path::new("cx"), &workspace, cx_home.path());
        let command = command.as_std();

        assert_eq!(command.get_current_dir(), Some(cx_home.path()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            [("CODEX_HOME".as_ref(), Some(cx_home.path().as_os_str()))]
        );
    }
