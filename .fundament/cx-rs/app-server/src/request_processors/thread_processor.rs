...
use cx_app_server_protocol::SelectedCapabilityRoot;
use cx_app_server_protocol::ThreadRevertParams;
use cx_app_server_protocol::ThreadRevertResponse;
use cx_app_server_protocol::ThreadRevertedNotification;
use cx_app_server_protocol::ThreadSection;
use cx_app_server_protocol::ThreadSectionAppearance;
use cx_app_server_protocol::ThreadSectionMoveParams;
use cx_app_server_protocol::ThreadSectionMoveResponse;
use cx_extension_api::ExtensionDataInit;
use cx_extension_api::ThreadIdleCause;
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: cx_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                cx_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = cx_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            cx_home: self.config.cx_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(cx_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<cx_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<cx_app_server_protocol::ThreadStartSource>,
        thread_source: Option<cx_protocol::protocol::ThreadSource>,
            ...
            && effective_permissions_trust_project
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                cx_core::config::set_project_trust_level(
                    &listener_task_context.cx_home,
        ...
        if let Ok(Some(err)) =
            cx_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let new_thread = listener_task_context
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(cx_app_server_protocol::ThreadStartSource::Startup)
                {
                    cx_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    cx_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<cx_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<cx_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            cx_linux_sandbox_exe: self.arg0_paths.cx_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = cx_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.cx_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.cx_home.display()
                ))
            })?;
    ...
    async fn thread_revert_response(
        ...
        let revert_result = self
            .thread_store
            .revert_thread(cx_thread_store::RevertThreadParams {
                thread_id,
                before_turn_id,
            })
            .await
    ...
    async fn reload_paginated_thread(
        ...
        let thread_state = self.thread_state_manager.thread_state(thread_id).await;
        self.ensure_listener_task_running(thread_id, Arc::clone(&cx_thread), thread_state)
            .await?;
        let mut thread = self
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                cx_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<cx_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = cx_thread.legacy_instruction_sources().await;
                let SessionConfiguredEvent { rollout_path, .. } = session_configured;
                ...
                thread.thread_source = cx_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = cx_thread.config_snapshot().await;
                let (turns_backwards_cursor, items_backwards_cursor) =
                    if matches!(config_snapshot.history_mode, ThreadHistoryMode::Paginated) {
    ...
    async fn read_stored_thread_for_resume(
            ...
            && matches!(stored_thread.history_mode, ThreadHistoryMode::Paginated)
        {
            ...
            if let Some(current_path) = current_thread.rollout_path.as_ref()
                && !path_utils::paths_match_after_normalization(
                    cx_rollout::plain_rollout_path(requested_path).as_path(),
                    cx_rollout::plain_rollout_path(current_path).as_path(),
                )
            {
                return Err(invalid_request(format!(
        ...
        if stored_thread.archived_at.is_some() {
            ...
            return Err(invalid_request(format!(
                "session {thread_id} is archived. Run `cx unarchive {thread_id}` to unarchive it first."
            )));
        }
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(cx_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    cx_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    cx_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => cx_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(cx_thread_store::PrepareForkParams {
...
fn deserialize_stored_thread_item(
    item: cx_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        cx_error_info: error.cx_error_info,
...
) -> (Thread, Option<cx_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<cx_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(cx_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match cx_core::parse_turn_item(&item.item) {
                Some(cx_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<cx_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
