...
use cx_analytics::PluginInstallSource;
use cx_app_server_protocol::PluginAvailability;
use cx_app_server_protocol::PluginInstallPolicy;
use cx_app_server_protocol::PluginSharePrincipalRole;
use cx_app_server_protocol::PluginShareTargetRole;
use cx_config::types::McpServerConfig;
use cx_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use cx_core_plugins::PluginListBackgroundTaskOptions;
use cx_core_plugins::is_openai_curated_marketplace_name;
use cx_core_plugins::loader::load_configured_plugin_mcp_servers;
use cx_core_plugins::manifest::is_agent_plugin_manifest;
use cx_core_plugins::remote::REMOTE_CREATED_BY_ME_MARKETPLACE_NAME;
use cx_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use cx_core_plugins::remote::REMOTE_WORKSPACE_MARKETPLACE_NAME;
use cx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_MARKETPLACE_NAME;
use cx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_PRIVATE_MARKETPLACE_NAME;
use cx_core_plugins::remote::REMOTE_WORKSPACE_SHARED_WITH_ME_UNLISTED_MARKETPLACE_NAME;
use cx_core_plugins::remote::RemoteAppTemplateUnavailableReason;
use cx_core_plugins::remote::RemotePluginCatalogCacheMode;
use cx_core_plugins::remote::RemotePluginScope;
use cx_core_plugins::remote::is_valid_remote_plugin_id;
use cx_core_plugins::remote::validate_remote_plugin_id;
use cx_core_plugins::remote_bundle::RemotePluginBundleInstallError;
use cx_mcp::McpOAuthLoginSupport;
use cx_mcp::McpRuntimeContext;
use cx_mcp::oauth_login_support;
use cx_mcp::should_retry_without_scopes;
use cx_plugin::PluginId;
use cx_plugin::PluginTelemetryMetadata;
use cx_protocol::auth::AuthMode as DomainAuthMode;
use cx_rmcp_client::McpOAuthClientRegistration;
use cx_rmcp_client::OAuthDiscoveryTimeout;
use cx_rmcp_client::StreamableHttpRedirectMode;
use cx_rmcp_client::perform_oauth_login_silent;
...
pub(crate) struct PluginRequestProcessor {
    ...
    on_effective_plugins_changed:
        Arc<dyn Fn(cx_core_plugins::EffectivePluginsChange) + Send + Sync>,
}

fn plugin_skills_to_info(
    skills: &[cx_skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                cx_app_server_protocol::SkillInterface {
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: cx_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
) -> cx_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            cx_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            cx_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            cx_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> cx_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Listed => {
            cx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Listed
        }
        PluginShareUpdateDiscoverability::Unlisted => {
            cx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            cx_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> cx_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            cx_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            cx_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: cx_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        cx_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        cx_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        cx_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
fn plugin_share_principal_from_remote(
    principal: cx_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    pub(crate) fn new(
        ...
        on_effective_plugins_changed: Arc<
            dyn Fn(cx_core_plugins::EffectivePluginsChange) + Send + Sync,
        >,
    ) -> Self {
        Self {
            auth_manager,
    ...
    async fn plugin_list_response(
        ...
        let use_remote_global_catalog =
            include_global_remote && auth_mode.is_some_and(DomainAuthMode::uses_cx_backend);
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<cx_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
                    ...
                    outcome
                        .errors
                        .into_iter()
                        .map(|err| cx_app_server_protocol::MarketplaceLoadErrorInfo {
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            match cx_core_plugins::remote::fetch_openai_curated_remote_collection_marketplace(
            ...
            .await
            {
                ...
                Err(err) if explicit_marketplace_kinds => {
                    return Err(remote_plugin_catalog_error_to_jsonrpc(
                        err,
                        "list oi Curated remote plugin catalog",
                    ));
                }
        ...
        if !remote_sources.is_empty() {
            match cx_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
                /*catalog_cache_root*/ Some(config.cx_home.as_path()),
    ...
    async fn plugin_installed_response(
        ...
        let use_remote_global_catalog = config.features.enabled(Feature::RemotePlugin)
            && auth_mode.is_some_and(DomainAuthMode::uses_cx_backend);
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<cx_core_plugins::PluginsManager>,
        config: &Config,
        plugins_input: &cx_core_plugins::PluginsConfigInput,
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        let remote_plugin_service_config = remote_plugin_service_config(&config);
                        match cx_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| cx_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = cx_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
    ...
    async fn plugin_skill_read_response(
        ...
        let auth = self.auth_manager.auth().await;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let remote_skill_detail = cx_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let access_policy = cx_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = cx_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.cx_home.as_path(),
            &plugin_path,
        ...
        cx_core_plugins::remote::invalidate_cached_remote_plugin_catalog_scopes(
            config.cx_home.as_path(),
            &remote_plugin_service_config,
            auth.as_ref(),
    ...
    async fn plugin_share_update_targets_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = cx_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let (config, auth) = self.load_plugin_share_config_and_auth().await?;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let data = cx_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.cx_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let result = cx_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.cx_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        cx_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.cx_home.as_path(),
            &remote_plugin_id,
    ...
    async fn remote_plugin_install_response(
        ...
        let _remote_plugin_cache_mutation =
            cx_core_plugins::remote::mark_remote_plugin_cache_mutation_in_flight(
                config.cx_home.as_path(),
                &actual_remote_marketplace_name,
                &remote_plugin_name,
            );
        let validated_bundle = cx_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &remote_plugin_id,
            &actual_remote_marketplace_name,
            &remote_plugin_name,
        ...
        let result = cx_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
            &remote_plugin_service_config,
        ...
        let install_result = if let Some(install_attempt_id) = install_attempt_id.as_deref() {
            cx_core_plugins::remote::install_remote_plugin_with_install_attempt_id(
                &remote_plugin_service_config,
                auth.as_ref(),
                &actual_remote_marketplace_name,
                &remote_plugin_id,
            ...
            cx_core_plugins::remote::install_remote_plugin(
                &remote_plugin_service_config,
                auth.as_ref(),
                &actual_remote_marketplace_name,
                &remote_plugin_id,
        ...
        let is_gt_auth = auth.as_ref().is_some_and(CodexAuth::is_gt_auth);
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        auth: Option<&CodexAuth>,
        plugin_id: &str,
        plugin_app_declarations: &[cx_plugin::AppDeclaration],
    ) -> Vec<AppSummary> {
        if plugin_app_declarations.is_empty()
            || !config
                .features
                .apps_enabled_for_auth(auth.is_some_and(CodexAuth::is_gt_auth))
        {
            return Vec::new();
        }
        ...
        let (accessible_connectors, cx_apps_ready) = match accessible_connectors_result {
            Ok(status) => (status.connectors, status.cx_apps_ready),
            Err(err) => {
                warn!(
                    plugin = plugin_id,
        ...
        if !cx_apps_ready {
            warn!(
                plugin = plugin_id,
                "cx_apps MCP not ready after plugin install; skipping appsNeedingAuth check"
            );
            return Vec::new();
        }
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let auth = self.auth_manager.auth().await;
        let remote_plugin_service_config = remote_plugin_service_config(&config);
        let uninstall_target = cx_core_plugins::remote::resolve_remote_plugin_uninstall_target(
            &remote_plugin_service_config,
            auth.as_ref(),
            &plugin_id,
        )
        ...
        let uninstall_result = cx_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.cx_home.to_path_buf(),
            uninstall_target,
...
async fn load_plugin_app_summaries(
    config: &Config,
    auth: Option<&CodexAuth>,
    plugin_apps: &[cx_plugin::AppConnectorId],
    ...
    if let Some(auth) = auth.filter(|auth| {
        config
            .features
            .apps_enabled_for_auth(auth.uses_cx_backend())
    }) {
        metadata_by_id.extend(
            cx_connectors::ConnectorMetadataStore::new(
                config.gt_base_url.clone(),
                auth.get_account_id(),
                auth.get_gt_user_id(),
    ...
    app_ids
        ...
        .map(|app_id| {
            ...
            AppSummary {
                install_url: Some(cx_connectors::metadata::connector_install_url(
                    &name, &app_id,
                )),
                id: app_id,
                name,
...
fn plugin_app_category_by_id_from_value(value: &serde_json::Value) -> HashMap<String, String> {
    cx_core_plugins::loader::plugin_app_declarations_from_value(value)
        .into_iter()
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: cx_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: cx_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
