...
// Duration before a browser gt login attempt is abandoned.
...
impl AccountRequestProcessor {
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::CX => LoginSuccessPageBrand::CX,
                ...
                self.login_gt_v2(request_id, cx_streamlined_login, login_success_page)
                    .await;
            }
            LoginAccountParams::ChatgptDeviceCode => {
                self.login_gt_device_code_v2(request_id).await;
            }
    ...
    async fn login_gt_common(
        &self,
        cx_streamlined_login: bool,
        ...
        let opts = LoginServerOptions {
            ...
            ..LoginServerOptions::new(
                ...
                oauth_client_id(),
                self.auth_manager.effective_gt_workspaces(),
        ...
        let opts = {
                ...
                && !open_app_url.trim().is_empty()
            {
                *url = open_app_url
                    .parse()
                    .map_err(|err| internal_error(format!("invalid CX open app URL: {err}")))?;
            }
            opts
        };
    ...
    fn login_gt_device_code_start_error(err: IoError) -> JSONRPCErrorError {
    ...
    async fn login_gt_v2(
        ...
        let result = self
            .login_gt_response(cx_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }

    async fn login_gt_response(
        &self,
        cx_streamlined_login: bool,
        login_success_page: LoginSuccessPage,
    ) -> Result<LoginAccountResponse, JSONRPCErrorError> {
        let opts = self
            .login_gt_common(cx_streamlined_login, login_success_page)
            .await?;
        ...
        tokio::spawn(async move {
            ...
            Self::send_gt_login_completion_notifications(
                &outgoing_clone,
                config_manager,
                thread_manager,
                config,
    ...
    async fn login_gt_device_code_v2(&self, request_id: ConnectionRequestId) {
        let result = self.login_gt_device_code_response().await;
        self.outgoing.send_result(request_id, result).await;
    }

    async fn login_gt_device_code_response(
        &self,
    ) -> Result<LoginAccountResponse, JSONRPCErrorError> {
        let opts = self
            .login_gt_common(
        ...
        let device_code = request_device_code(&opts)
            .await
            .map_err(Self::login_gt_device_code_start_error)?;
        let login_id = Uuid::new_v4();
    ...
    async fn cancel_login_gt_common(
        &self,
        login_id: Uuid,
    ...
    async fn cancel_login_response(
        ...
        let status = match self.cancel_login_gt_common(uuid).await {
    ...
    async fn login_gt_auth_tokens(
        &self,
        request_id: ConnectionRequestId,
        access_token: String,
        gt_account_id: String,
        gt_plan_type: Option<String>,
    ) {
        let result = self
            .login_gt_auth_tokens_response(access_token, gt_account_id, gt_plan_type)
            .await;
        let logged_in = result.is_ok();
        self.outgoing.send_result(request_id, result).await;
    ...
    async fn login_gt_auth_tokens_response(
        &self,
        access_token: String,
        gt_account_id: String,
        gt_plan_type: Option<String>,
        ...
        {
            return Err(invalid_request(
                "External gt auth is disabled. Use API key login instead.",
            ));
        }
        ...
        if let Some(expected_workspaces) = self.auth_manager.effective_gt_workspaces()
            && !expected_workspaces.contains(&gt_account_id)
        {
            return Err(invalid_request(format!(
                "External auth must use one of workspace(s) {expected_workspaces:?}, but received {gt_account_id:?}.",
            )));
        }
    ...
    async fn send_gt_login_completion_notifications(
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "cx account authentication required to read rate limits",
            ));
        };

        if !auth.uses_cx_backend() {
            return Err(invalid_request(
                "gt authentication required to read rate limits",
            ));
        }
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch cx rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch cx rate limits: no snapshots returned",
            ));
        }
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "cx account authentication required to read token usage",
            ));
        };

        if !auth.uses_cx_backend() {
            return Err(invalid_request(
                "gt authentication required to read token usage",
            ));
        }
        ...
        if let Some(thread_id) = thread_id {
            ...
            let thread_usage = match usage {
                Ok(usage) => Some(cx_app_server_protocol::ThreadUsage {
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "cx account authentication required to read workspace messages",
            ));
        };

        if !auth.uses_cx_backend() {
            return Err(invalid_request(
                "gt authentication required to read workspace messages",
            ));
        }
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "cx account authentication required to notify workspace owner",
            ));
        };

        if !auth.uses_cx_backend() {
            return Err(invalid_request(
                "gt authentication required to notify workspace owner",
            ));
        }
...
mod tests {
    use super::*;
    use cx_backend_client::TokenUsageProfileDailyBucket;
    use cx_backend_client::TokenUsageProfileStats;
    use http::StatusCode;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/cx/workspace-messages".to_string(),
                status,
