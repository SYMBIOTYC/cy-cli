...
use cx_app_server_protocol::AutoReviewRequirements;
use cx_app_server_protocol::BrowserUseRequirements;
use cx_app_server_protocol::CliAuthCredentialsStoreMode;
use cx_app_server_protocol::ClientResponsePayload;
use cx_app_server_protocol::ComputerUseRequirements;
use cx_app_server_protocol::ConfigBatchWriteParams;
use cx_app_server_protocol::ConfigReadParams;
use cx_app_server_protocol::ConfigReadResponse;
use cx_app_server_protocol::ConfigRequirements;
use cx_app_server_protocol::ConfigRequirementsReadResponse;
use cx_app_server_protocol::ConfigValueWriteParams;
use cx_app_server_protocol::ConfigWriteErrorCode;
use cx_app_server_protocol::ConfigWriteResponse;
use cx_app_server_protocol::ConfiguredHookHandler;
use cx_app_server_protocol::ConfiguredHookMatcherGroup;
use cx_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use cx_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use cx_app_server_protocol::FeedbackRequirements;
use cx_app_server_protocol::JSONRPCErrorError;
use cx_app_server_protocol::ManagedHooksRequirements;
use cx_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use cx_app_server_protocol::ModelsRequirements;
use cx_app_server_protocol::NetworkDomainPermission;
use cx_app_server_protocol::NetworkRequirements;
use cx_app_server_protocol::NetworkUnixSocketPermission;
...
use cx_config::HookEventsToml;
use cx_config::HookHandlerConfig as CoreHookHandlerConfig;
use cx_config::ManagedHooksRequirementsToml;
use cx_config::MatcherGroup as CoreMatcherGroup;
use cx_config::ResidencyRequirement as CoreResidencyRequirement;
use cx_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use cx_core::ThreadManager;
use cx_features::canonical_feature_for_key;
use cx_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<cx_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = cx_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        cli_auth_credentials_store: requirements.cli_auth_credentials_store.map(
            |mode| match mode {
                cx_config::types::AuthCredentialsStoreMode::File => {
                    CliAuthCredentialsStoreMode::File
                }
                cx_config::types::AuthCredentialsStoreMode::Keyring => {
                    CliAuthCredentialsStoreMode::Keyring
                }
                cx_config::types::AuthCredentialsStoreMode::Auto => {
                    CliAuthCredentialsStoreMode::Auto
                }
                cx_config::types::AuthCredentialsStoreMode::Ephemeral => {
                    CliAuthCredentialsStoreMode::Ephemeral
                }
            },
        ),
        gt_base_url: requirements.gt_base_url,
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(cx_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(cx_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            cx_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            cx_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: cx_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: cx_config::BrowserUseRequirementsToml,
...
) -> cx_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => cx_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: cx_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(cx_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(cx_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(cx_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: cx_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        cx_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        cx_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: cx_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        cx_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        cx_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use cx_app_server_protocol::FeedbackRequirements;
    use cx_app_server_protocol::WindowsSandboxSetupMode;
    use cx_config::AutoReviewRequirementsToml;
    use cx_config::ComputerUseRequirementsToml;
    use cx_config::ConfigRequirementsToml;
    use cx_config::ModelsRequirementsToml;
    use cx_config::NewThreadModelDefaultsToml;
    use cx_config::WindowsRequirementsToml;
