    ...
    async fn upload_feedback_response(
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.cx_home)
...
fn tool_cache_feedback_attachments(
    ...
    let tools_cache_path = connector_runtime_cache_path(cx_home, auth);
...
#[cfg(target_os = "windows")]
fn windows_sandbox_log_attachment(cx_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path = cx_windows_sandbox::current_log_file_path_for_cx_home(cx_home);
    sandbox_log_path
        .is_file()
...
fn windows_sandbox_log_attachment(_cx_home: &Path) -> Option<FeedbackAttachmentPath> {
    None
}
...
mod tests {
        ...
        let mut lines = vec![RolloutLine {
            ...
            item: RolloutItem::SessionMeta(SessionMetaLine {
                meta: cx_protocol::protocol::SessionMeta {
                    ...
                    base_instructions: Some(cx_protocol::models::BaseInstructions {
        ...
        lines.extend(turns.iter().map(|(turn_id, model, effort)| {
            RolloutLine {
                ...
                item: RolloutItem::TurnContext(TurnContextItem {
                    ...
                    current_date: None,
                    timezone: None,
                    approval_policy: cx_protocol::protocol::AskForApproval::Never,
                    approvals_reviewer: None,
                    sandbox_policy: cx_protocol::protocol::SandboxPolicy::new_read_only_policy(),
    ...
    fn tool_cache_feedback_attachments_include_existing_active_cache_files() {
        let cx_home = tempfile::tempdir().expect("create tempdir");
        let auth = CodexAuth::create_dummy_gt_auth_for_testing();
        let tools_cache_path = connector_runtime_cache_path(cx_home.path(), Some(&auth));
    ...
    fn tool_cache_feedback_attachments_include_directory_cache_without_account_id() {
        let cx_home = tempfile::tempdir().expect("create tempdir");
        let auth = CodexAuth::Headers(cx_login::AuthHeaders::new(HeaderMap::new()));
        let directory_cache_context = ConnectorDirectoryCacheContext::new(
            cx_home.path().to_path_buf(),
            ConnectorDirectoryCacheKey::new(
                "https://cy.symbiotyc.workers.dev/v1".to_string(),
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let cx_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = cx_windows_sandbox::sandbox_dir(cx_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            cx_windows_sandbox::current_log_file_path_for_cx_home(cx_home.path());
        std::fs::write(&sandbox_log_path, "sandbox log").expect("write sandbox log");

        let attachment = windows_sandbox_log_attachment(cx_home.path())
            .map(|attachment| (attachment.path, attachment.attachment_filename_override));
