...
use cx_agent_extension::AgentRun;
...
use cx_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use cx_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
use cx_protocol::protocol::MultiAgentVersion;
use cx_protocol::protocol::SessionSource;
use cx_protocol::protocol::SubAgentSource;
use cx_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<cx_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = cx_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input && started {
            ...
            if config_snapshot.is_primary_environment_configured() {
                cx_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<cx_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        Ok(cx_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != cx_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match reason {
                    ...
                    NotSubmittedReason::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            cx_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            cx_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            cx_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.cx_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(
