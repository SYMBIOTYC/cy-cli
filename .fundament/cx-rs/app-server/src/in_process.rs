...
//!
//! # Relationship to `cx-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `cx-app-server-client`,
...
use cx_app_server_protocol::AgentMessageDelivery;
...
use cx_app_server_protocol::Result;
...
pub use cx_rollout::StateDbHandle;
pub use cx_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `cx-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_cx_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.cx_home).await?;
    let auth_manager =
        AuthManager::shared_from_config(args.config.as_ref(), args.enable_cx_api_key_env)
            .await
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.cx_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(cx_home.path()).await);
        let state_db = cx_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "cx-in-process-test".to_string(),
                    title: None,
        ...
        client._test_cx_home = Some(cx_home);
        client
    }
