...
use cx_app_server_protocol::Config as ApiConfig;
...
use cx_app_server_protocol::OverriddenMetadata;
use cx_app_server_protocol::WriteStatus;
use cx_config::CONFIG_TOML_FILE;
use cx_config::ConfigLayerEntry;
use cx_config::ConfigLayerMetadata;
use cx_config::ConfigLayerSource;
use cx_config::ConfigLayerStack;
use cx_config::ConfigRequirementsToml;
use cx_config::ShellEnvironmentPolicyFilterRepresentation;
use cx_config::config_toml::ConfigToml;
use cx_config::merge_toml_values;
use cx_config::shell_environment_filter_entry;
use cx_config::validate_shell_environment_policy_filter_config;
use cx_core::config::deserialize_config_toml_with_base;
use cx_core::config::edit::ConfigEdit;
use cx_core::config::edit::ConfigEditsBuilder;
use cx_core::config::validate_feature_requirements_for_config_toml;
use cx_core::path_utils;
use cx_core::path_utils::SymlinkWritePaths;
use cx_core::path_utils::resolve_symlink_write_paths;
use cx_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.cx_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .cx/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_cx_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_cx_folder.display(),
        ),
