...
use cx_extension_api::AgentSpawnFuture;
use cx_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn cx_skills_extension::SkillProvider>,
    ...
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = CxErr> + 'static,
{
    ...
    if let Some(queue_service) = queue_service {
        cx_queue_extension::install(&mut builder, queue_service);
    }
    if let Some(state_db) = state_db {
        cx_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            cx_otel::global(),
            ...
            |config: &Config| GoalExtensionConfig {
                enabled: config.features.enabled(cx_features::Feature::Goals),
    ...
    cx_guardian_v2::install(
        &mut builder,
        guardian_agent_spawner,
        auth_manager.clone(),
        thread_manager,
    );
    cx_memories_extension::install(&mut builder, cx_otel::global());
    cx_mcp_extension::install(&mut builder);
    cx_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    cx_web_search_extension::install(&mut builder, auth_manager.clone());
    cx_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
    ...
    let skill_providers = cx_skills_extension::SkillProviders::new()
        ...
        .with_orchestrator_provider(Arc::new(
            cx_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
#[cfg(test)]
mod tests {
    use cx_protocol::protocol::ThreadGoal as CoreThreadGoal;
