...
use cx_app_server_protocol::ChatgptAuthTokensRefreshParams;
...
use cx_login::auth::ExternalAuthRefreshReason;
...
impl ExternalAuthBridge {
    ...
    async fn refresh(&self, context: ExternalAuthRefreshContext) -> std::io::Result<CodexAuth> {
        ...
        let auth = CodexAuth::from_external_gt_tokens(
            ...
            response.gt_account_id.as_str(),
            response.gt_plan_type.as_deref(),
        )?;
        *self
            .auth
            .write()
