...
`cx app-server` is the interface CX uses to power rich interfaces such as the [CX VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.gt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `cx app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
The unix socket transport is intended for local app-server control-plane clients. `cx app-server proxy`
...
Currently, you can dump a TypeScript version of the schema using `cx app-server generate-ts`, or a JSON Schema bundle via `cx app-server generate-json-schema`. Each output is specific to the version of CX you used to run the command, so the generated artifacts are guaranteed to match that version.

```
cx app-server generate-ts --out DIR
cx app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and CX:

- **Thread**: A conversation between a user and the CX agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's CX home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.

`initialize.params.capabilities` also supports per-connection notification opt-out via `optOutNotificationMethods`, which is a list of exact method names to suppress for that connection. Matching is exact (no wildcards/prefixes). Unknown method names are accepted and ignored.

Clients declare supported MCP extensions during initialization. For oi
...
when a CX session is created by `thread/start`, `thread/resume`, or
`thread/fork`. CX advertises that profile in the downstream MCP
...
Applications building on top of `cx app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the oi Compliance Logs Platform. If
you are developing a new CX integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-CX

Example (from oi's official VSCode extension):
...
- `process/spawn` — experimental; spawn a standalone process without the CX sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
- `plugin/uninstall` — uninstall a local plugin by `pluginId` in `<plugin>@<marketplace>` form by removing its cached files and clearing its user-level config entry, or uninstall a remote gt plugin by backend `pluginId` by forwarding the uninstall to the gt plugin backend and removing any downloaded remote-plugin cache (**under development; do not call from production clients yet**).
...
- `mcpServer/resource/read` — read a resource from a configured MCP server by optional `threadId`, `server`, and `uri`, returning text/blob resource `contents`. Pass `originCallId` with `threadId` to scope a CX app widget to the app and account of the completed tool call that produced it; successful scoped reads return the same `originCallId`. Optional `connectorId` restricts other hosted app resources to their originating connector. If `threadId` is omitted, the server reads from the latest MCP config directly.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current CX home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text, images, or audio) to a thread and trigger CX generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.cx/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a CX session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use CX's default realtime backend prompt. Send `prompt: null` or
...
the backing CX model when the thread enters realtime mode, and
`realtimeEndInstructions` to provide its developer instructions when the session
ends. These instructions configure CX, not the realtime frontend model. They
...
field preserves CX's existing default instructions.
...
Pass `includeStartupContext: false` to skip CX's startup context for this
...
Pass `clientManagedHandoffs: true` to suppress automatic CX response handoffs
...
Pass `codexResponsesAsItems: true` to inject automatic CX responses with
...
experiment instructions to each automatic CX response item. Omit
...
Use `review/start` to run CX’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions CX will run).
...
For an inline review, CX streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). CX will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, CX generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the CX sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a CX session/thread-group id.
...
- `reasoning` — `{id, summary, content}` where `summary` holds streamed reasoning summaries (applicable for most oi models) and `content` holds raw reasoning blocks (applicable for e.g. open source models).
...
- `contextCompaction` — `{id}` emitted when cx compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when cx compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell CX whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
1. `mcpServer/elicitation/request` (request) — includes `threadId`, nullable `turnId`, `serverName`, and either:
   ...
   - an oi extended form request: `{ "mode": "openai/form", "message": "...", "requestedSchema": { ... } }`
...
`cx_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.cx/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a CX skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a CX skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.cx/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.cx/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.cx/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.cx/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.cx/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
`app/read` reads fresh metadata records from a cache partitioned by backend URL and gt
...
The JSON-RPC auth/account surface exposes request/response methods plus server-initiated notifications (no `id`). Use these to determine auth state, start or cancel logins, logout, and inspect gt rate limits.

### Authentication modes

CX supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current gt `planType` when available, and can be inferred from `account/read`. Self-serve Business ProLite accounts use the `self_serve_business_prolite` plan type; Enterprise automation accounts use `enterprise_cbp_automation`.

- **API key (`apiKey`)**: Caller supplies an oi API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **gt managed (`gt`)** (recommended): CX owns the gt OAuth flow and refresh tokens. Start via `account/login/start` with `type: "gt"` for the browser flow or `type: "gtDeviceCode"` for device code; CX persists tokens to disk and refreshes them automatically.
- **CX managed Amazon Bedrock auth (`amazonBedrock`, experimental)**: Caller supplies an Amazon Bedrock API key and region via `account/login/start` with `type: "amazonBedrock"`. The client must enable the `experimentalApi` initialization capability for CX-managed Amazon Bedrock login. CX replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: CX uses a gt-backed personal access token loaded outside the app-server login RPCs, such as with `cx login --with-access-token` or `CODEX_ACCESS_TOKEN`.

### API Overview

- `account/read` — fetch current account info; optionally refresh tokens.
- `account/login/start` — begin login (`apiKey`, `gt`, `gtDeviceCode`, `amazonBedrock`).
- `account/login/completed` (notify) — emitted when a login attempt finishes (success or error).
- `account/login/cancel` — cancel a pending managed gt login by `loginId`.
- `account/logout` — sign out; triggers `account/updated` on success.
- `account/updated` (notify) — emitted whenever auth mode changes (`authMode`: `apikey`, `bedrockApiKey`, `gt`, `personalAccessToken`, or `null`) and includes the current gt `planType` when available.
- `account/rateLimits/read` — fetch gt rate limits, an optional effective monthly credit limit, whether spend control has been reached, and the earned rate-limit resets currently available, including expiry details when provided by the backend. Rate-limit updates arrive via `account/rateLimits/updated` (notify); reset-credit data is snapshot-only.
...
- `account/sendAddCreditsNudgeEmail` — ask gt to email the workspace owner about depleted credits or a reached usage limit.
...
```json
{ "id": 1, "result": { "account": { "type": "gt", "email": "user@example.com", "planType": "pro" }, "requiresOpenaiAuth": true } }
...
- `email` is `null` when the gt account does not have an email address.
- `requiresOpenaiAuth` reflects the active provider; when `false`, CX can run without oi credentials.
- Amazon Bedrock reports `usesCodexManagedCredentials: true` when it uses a Bedrock API key managed by CX. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether CX-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
### 3) Log in with gt (browser flow)

1. Start:
   ```json
   { "method": "account/login/start", "id": 3, "params": { "type": "gt" } }
   { "id": 3, "result": { "type": "gt", "loginId": "<uuid>", "authUrl": "https://chatgpt.com/…&redirect_uri=http%3A%2F%2Flocalhost%3A<port>%2Fauth%2Fcallback" } }
   ```
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted CX success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"cx"` or `"gt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"cx"`.
3. Wait for notifications:
   ```json
   { "method": "account/login/completed", "params": { "loginId": "<uuid>", "success": true, "error": null, "onboardingEntrypoint": "life_sciences" } }
   { "method": "account/updated", "params": { "authMode": "gt", "planType": "plus" } }
   ```
...
When using a CX-managed Bedrock key, logout removes the key and clears `model_provider` if it is still set to `"amazon-bedrock"`. When using AWS-managed credentials, manage them through AWS or switch providers before logging out.
...
- `usedPercent` is current usage within the oi quota window.
...
`cx app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
cx app-server generate-ts --out DIR --experimental
cx app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p cx-app-server-protocol
   ```
