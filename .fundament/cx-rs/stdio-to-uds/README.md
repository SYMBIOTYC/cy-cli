...
```
cx --config mcp_servers.example={command="cx-stdio-to-uds",args=["/tmp/mcp.sock"]}
```
...
As a workaround, this crate uses `cx-uds`, which provides a cross-platform async UDS API backed by https://crates.io/crates/uds_windows on Windows.
