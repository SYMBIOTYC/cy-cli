...
/// Set the user-configured syntax theme override and cx home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `cx_home` in
...
pub(crate) fn set_theme_override(
    ...
    if !override_set_ok || !cx_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, cx_home: Option<&Path>) -> Option<String> {
    let name = name?;
...
fn custom_theme_path(name: &str, cx_home: &Path) -> PathBuf {
    cx_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{cx_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, cx_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, cx_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, cx_home: Option<&Path>) -> Theme {
...
pub(crate) fn resolve_theme_by_name(name: &str, cx_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// found in `{cx_home}/themes/`.
pub(crate) fn list_available_themes(cx_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*cx_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*cx_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*cx_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*cx_home*/ None).is_none());
    }
