...
async fn state_runtime(config: &Config) -> std::io::Result<Arc<cx_state::StateRuntime>> {
    let runtime = cx_state::StateRuntime::init(
        cx_state::SqliteConfig::new_for_testing(config.cx_home.as_path().abs()),
...
async fn upsert_thread(
    ...
    metadata: cx_state::ThreadMetadata,
) -> std::io::Result<()> {
    runtime
...
fn thread_metadata(
    ...
    builder.cwd = config.cx_home.join("project").to_path_buf();
...
fn write_rollout(
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            cwd: config.cx_home.join("project").to_path_buf(),
...
async fn recovers_legacy_index_name_without_requiring_sidecar_rewrite() -> color_eyre::Result<()> {
    ...
    let index_path = config.cx_home.join("session_index.jsonl");
...
async fn prefers_most_recent_duplicate_legacy_name() -> color_eyre::Result<()> {
    ...
    cx_rollout::append_thread_name(config.cx_home.as_path(), recent_id, "same-name").await?;
    cx_rollout::append_thread_name(config.cx_home.as_path(), older_id, "same-name").await?;
    for (path, timestamp) in [
