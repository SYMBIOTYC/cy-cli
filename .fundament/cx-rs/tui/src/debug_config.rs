...
mod tests {
    ...
    use cx_config::ConstraintError;
    use cx_config::FeatureRequirementsToml;
    use cx_config::FilesystemConstraints;
    use cx_config::HookEventsToml;
    use cx_config::HookHandlerConfig;
    use cx_config::LoaderOverrides;
    use cx_config::ManagedHooksRequirementsToml;
    use cx_config::MatcherGroup;
    ...
    use cx_config::NetworkUnixSocketPermissionToml;
    use cx_config::NetworkUnixSocketPermissionsToml;
    use cx_config::RequirementSource;
    use cx_config::ResidencyRequirement;
    use cx_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\cx\\config.toml")
        } else {
            absolute_path("/etc/cx/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.cx")
        } else {
            absolute_path("/repo/.cx")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\oi\\CX\\requirements.toml")
        } else {
            absolute_path("/etc/cx/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.cx\\state")
        } else {
            absolute_path("/home/alice/.cx/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.cx\\logs")
        } else {
            absolute_path("/home/alice/.cx/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.cx\\models.json")
        } else {
            absolute_path("/home/alice/.cx/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.cx\\config.toml")
        } else {
            absolute_path("/home/alice/.cx/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/cx.sock=allow}} (source: {requirements_source})"
        )));
    }
