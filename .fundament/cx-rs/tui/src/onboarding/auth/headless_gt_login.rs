...
pub(super) fn start_headless_gt_login(widget: &mut AuthModeWidget) {
...
pub(super) fn render_device_code_login(
        ...
        (&state.verification_url, &state.user_code)
    {
        ...
        lines.push("".into());
        lines.push(
            "  Continue only if you started this login in CX. If a website or another person gave you this code, cancel."
                .dim()
                .into(),
        );
        lines.push("".into());
