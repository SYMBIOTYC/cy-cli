...
//!
//! This module owns the auth-step state machine (gt login/device-code/API
...
fn onboarding_request_id() -> cx_app_server_protocol::RequestId {
    cx_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<cx_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn is_gt_login_allowed(&self) -> bool {
        self.auth_config
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with gt to use CX as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
        ...
        let gt_description = if !self.is_gt_login_allowed() {
            "gt login is disabled"
        } else {
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with gt to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_gt_success_message(&self, area: Rect, buf: &mut Buffer) {
        ...
        let lines = vec![
            ...
            "".into(),
            "  Decide how much autonomy you want to grant CX".into(),
            docs_line,
            "".into(),
            "  CX can make mistakes".into(),
            HyperlinkLine::new(
            ...
            "".into(),
            "  Powered by your gt account".into(),
            preferences_line,
            "".into(),
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  CX will use usage-based billing with your API key.".into(),
        ];
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        let mut intro_lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "Use your own oi API key for usage-based billing".bold(),
            ]),
            "".into(),
        ...
        if state.prepopulated_from_env {
            intro_lines.push("  Detected CY_API_KEY environment variable.".into());
            intro_lines.push(
    ...
    fn start_api_key_entry(&mut self) {
        ...
        self.set_error(/*message*/ None);
        let prefill_from_env = read_cy_api_key_from_env();
        let mut guard = self.sign_in_state.write().unwrap();
        match &mut *guard {
    ...
    fn handle_existing_gt_login(&mut self) -> bool {
        if matches!(
            self.login_status,
            LoginStatus::AuthMode(auth_mode) if auth_mode.has_gt_account()
        ) {
            *self.sign_in_state.write().unwrap() = SignInState::ChatGptSuccess;
            self.request_frame.schedule_frame();
            true
    ...
    /// Kicks off the gt auth flow and keeps the UI state consistent with the attempt.
    fn start_gt_login(&mut self) {
        // If we're already authenticated with gt, don't start a new login –
        // just proceed to the success message flow.
        if self.handle_existing_gt_login() {
            return;
        }
    ...
    fn start_device_code_login(&mut self) {
        ...
        self.set_error(/*message*/ None);
        headless_gt_login::start_headless_gt_login(self);
    }
...
impl WidgetRef for AuthModeWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        match &*sign_in_state {
            ...
            SignInState::ChatGptDeviceCode(state) => {
                headless_gt_login::render_device_code_login(self, area, buf, state);
            }
            SignInState::ChatGptSuccessMessage => {
                self.render_gt_success_message(area, buf);
            }
            SignInState::ChatGptSuccess => {
                self.render_gt_success(area, buf);
            }
            SignInState::ApiKeyEntry(state) => {
...
mod tests {
    ...
    const PRODUCTION_LENGTH_AUTH_URL: &str = concat!(
        ...
        "cx_cli_simplified_flow=true&",
        "state=8cHjQ4nVx2Yp7Lm9Rk3Wf6Ta1Bs5Du0Ei4Go7Nz2PqM&",
        "originator=cx_cli_rs"
    );

    async fn widget_forced_gt() -> (AuthModeWidget, TempDir) {
        let cx_home = TempDir::new().unwrap();
        let cx_home_path = cx_home.path().to_path_buf();
        let config = ConfigBuilder::default()
            .cx_home(cx_home_path.clone())
            .build()
            .await
            .unwrap();
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                cx_app_server_client::EnvironmentManager::default_for_tests(),
            ),
    ...
    #[tokio::test]
    async fn api_key_flow_disabled_when_gt_forced() {
        let (mut widget, _tmp) = widget_forced_gt().await;
    ...
    #[tokio::test]
    async fn saving_api_key_is_blocked_when_gt_forced() {
        let (mut widget, _tmp) = widget_forced_gt().await;
    ...
    #[tokio::test]
    async fn existing_non_oauth_gt_login_counts_as_signed_in() {
        for auth_mode in [AuthMode::ChatgptAuthTokens, AuthMode::PersonalAccessToken] {
            ...
            let handled = widget.handle_existing_gt_login();
    ...
    async fn cancel_active_attempt_resets_browser_login_state() {
        let (widget, _tmp) = widget_forced_gt().await;
    ...
    fn continue_in_browser_preserves_long_link_and_footer_at_narrow_width() {
        ...
        let (widget, _tmp) = runtime.block_on(widget_forced_gt());
    ...
    #[test]
    fn gt_success_message_renders_osc8_hyperlinks() {
        let runtime = tokio::runtime::Runtime::new().unwrap();
        let (widget, _tmp) = runtime.block_on(widget_forced_gt());
        ...
        ✓ Signed in with your gt account
          ...
          Decide how much autonomy you want to grant CX
          For more details see the CX docs

          CX can make mistakes
          Review the code it writes and commands it runs

          Powered by your gt account
