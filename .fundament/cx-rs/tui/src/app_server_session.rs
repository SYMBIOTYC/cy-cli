...
use cx_app_server_protocol::ConfigRequirementsReadResponse;
use cx_app_server_protocol::ConfigWriteResponse;
use cx_app_server_protocol::ExternalAgentConfigDetectParams;
use cx_app_server_protocol::ExternalAgentConfigDetectResponse;
use cx_app_server_protocol::ExternalAgentConfigImportParams;
use cx_app_server_protocol::ExternalAgentConfigImportResponse;
use cx_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use cx_app_server_protocol::ThreadStartSource;
...
use cx_protocol::approvals::GuardianAssessmentEvent;
...
use cx_protocol::openai_models::ModelAvailabilityNux;
use cx_protocol::openai_models::ModelPreset;
use cx_protocol::openai_models::ModelServiceTier;
use cx_protocol::openai_models::ModelUpgrade;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<cx_protocol::account::PlanType>,
    /// Whether the configured model provider needs oi-style auth. Combined
    /// with `has_gt_account` to decide if a startup rate-limit prefetch
    /// should be fired.
    pub(crate) requires_openai_auth: bool,
    pub(crate) default_model: String,
    pub(crate) feedback_audience: FeedbackAudience,
    pub(crate) has_gt_account: bool,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: cx_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<cx_protocol::openai_models::ReasoningEffort>,
        summary: Option<cx_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<cx_protocol::config_types::CollaborationMode>,
        personality: Option<cx_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<cx_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<cx_app_server_protocol::ApprovalsReviewer> {
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(cx_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(cx_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(cx_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
pub(crate) fn turn_permissions_overrides(
    ...
    Option<cx_app_server_protocol::SandboxPolicy>,
    Option<String>,
)> {
...
fn display_permission_profile_from_thread_response(
    sandbox: &cx_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
    match thread_params_mode {
        ThreadParamsMode::Embedded
            if matches!(
                ...
                sandbox,
                cx_app_server_protocol::SandboxPolicy::DangerFullAccess
            ) =>
        {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<cx_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        cx_message_history::HistoryConfig::new(config.cx_home.clone(), &config.history);
    let (log_id, entry_count) = cx_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .cx_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    async fn bootstrap_reuses_prefetched_account_without_another_account_read() -> Result<()> {
        ...
        let config = build_config(&cx_home).await;
        ...
        assert_eq!(
            (
                ...
                bootstrap.has_gt_account,
            ),
            (
                Some("teammate@openai.com"),
                Some(TelemetryAuthMode::Chatgpt),
                Some(cx_protocol::account::PlanType::Plus),
                FeedbackAudience::OpenAiEmployee,
                true,
            )
        );
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(cx_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn api_model_with_upgrade_retirement_at(retirement_at: Option<i64>) -> ApiModel {
        ApiModel {
            ...
            upgrade_info: Some(cx_app_server_protocol::ModelUpgradeInfo {
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("cx"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("cx".to_string(), rate_limit_snapshot("cx")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(cx_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    fn legacy_turn_permissions_preserve_workspace_write() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(cx_app_server_protocol::SandboxPolicy::WorkspaceWrite {
    ...
    async fn persisted_resume_does_not_forward_implicit_service_tier() -> Result<()> {
        let cx_home = tempfile::tempdir().expect("tempdir");
        let mut config = build_config(&cx_home).await;
    ...
    async fn side_fork_uses_one_request_for_long_paginated_history() -> Result<()> {
        ...
        let source_path =
            app_test_support::rollout_path(cx_home.path(), filename_ts, source_id.as_str());
    ...
    async fn side_fork_excludes_turns_without_clearing_regular_ephemeral_fork() -> Result<()> {
        ...
        assert!(matches!(
            ...
            [cx_app_server_protocol::ThreadItem::UserMessage { content, .. }]
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: cx_app_server_protocol::Thread {
                ...
                source: cx_app_server_protocol::SessionSource::Cli,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        cx_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![cx_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        cx_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: cx_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: cx_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .cx_home(temp_dir.path().join("embedded-cx-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        cx_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        cx_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            cx_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(cx_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(cx_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let business_prolite = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(cx_protocol::account::PlanType::SelfServeBusinessProLite),
        );
        assert!(matches!(
            business_prolite,
            Some(StatusAccountDisplay::ChatGpt {
