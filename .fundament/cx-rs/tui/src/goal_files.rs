...
const GOAL_FILE_PREFIX: &str = "Read the CX goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, cx_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, cx_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let cx_home =
        cx_home.context("App server did not report $CX_HOME; cannot materialize goal files")?;
    let path = cx_home
        .join(GOAL_ATTACHMENT_DIR)
