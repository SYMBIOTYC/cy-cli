...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "CX just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like CX to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
...
mod tests {
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                ...
                /*model_link*/ None,
                Some(
                    "Upgrade to gpt-5.2-cx for the latest and greatest agentic coding model."
                        .to_string(),
                ),
                /*migration_markdown*/ None,
                "gpt-5.1-cx-max".to_string(),
                Some("CX-optimized flagship for deep and fast reasoning.".to_string()),
                /*can_opt_out*/ true,
            ),
        );
    ...
    fn prompt_snapshot_gpt5_family() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5",
                "gpt-5.1",
                Some("https://www.cx.com/models/gpt-5.1".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1".to_string(),
    ...
    fn prompt_snapshot_gpt5_codex() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-cx",
                "gpt-5.1-cx-max",
                Some("https://www.cx.com/models/gpt-5.1-cx-max".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-cx-max".to_string(),
                Some("CX-optimized flagship for deep and fast reasoning.".to_string()),
    ...
    #[test]
    fn prompt_snapshot_gpt5_cx_mini() {
        let backend = VT100Backend::new(/*width*/ 60, /*height*/ 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-cx-mini",
                "gpt-5.1-cx-mini",
                Some("https://www.cx.com/models/gpt-5.1-cx-mini".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-cx-mini".to_string(),
                Some("Optimized for cx. Cheaper, faster, but less capable.".to_string()),
                /*can_opt_out*/ false,
            ),
        );
        {
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_cx_mini", terminal.backend());
    }
    ...
    fn escape_key_accepts_prompt() {
        let mut screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-old",
                "gpt-new",
                Some("https://www.cx.com/models/gpt-new".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-new".to_string(),
