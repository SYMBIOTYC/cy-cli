...
const MCP_CODEX_APPS_SERVER_NAME: &str = "cx_apps";
const MCP_TOOL_CODEX_APPS_META_KEY: &str = "_cx_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &cx_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let cx_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
        ...
        if server_name == MCP_CODEX_APPS_SERVER_NAME {
            let url = validate_external_url(url, /*require_gt_host*/ true)?;
            return Self::from_cx_apps_auth_url_parts(
                thread_id,
                server_name,
                request_id,
                meta.as_ref(),
        ...
        let url = validate_external_url(url, /*require_gt_host*/ false)?;
    ...
    fn from_cx_apps_auth_url_parts(
        thread_id: ThreadId,
        server_name: &str,
        request_id: AppServerRequestId,
...
fn validate_external_url(url: &str, require_gt_host: bool) -> Option<Url> {
    ...
    if require_gt_host && !is_allowed_gt_auth_host(parsed.host_str()?) {
        return None;
    }
    Some(parsed)
}

fn is_allowed_gt_auth_host(host: &str) -> bool {
    let host = host.to_ascii_lowercase();
    host == "chatgpt.com"
        || host == "gt-staging.com"
        || host.ends_with(".chatgpt.com")
        || host.ends_with(".gt-staging.com")
}
...
impl AppLinkView {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let is_cx_apps_auth = is_auth_suggestion
            && self
                .elicitation_target
                .as_ref()
        ...
        if is_auth_suggestion {
            for line in wrap(
                if is_cx_apps_auth {
                    "Sign in to the app on gt in the browser window that just opened."
                } else {
            ...
            for line in wrap(
                "Complete app setup on gt in the browser window that just opened.",
                usable_width,
            ) {
                lines.push(Line::from(line.into_owned()));
            }
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> cx_app_server_protocol::McpServerElicitationRequest {
        cx_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            message: "Reconnect Google Calendar on gt.".to_string(),
            url: url.to_string(),
            elicitation_id: "cx_apps_auth_call_123".to_string(),
        }
    }

    #[test]
    fn cx_apps_auth_url_elicitation_builds_auth_app_link_params() {
    ...
    #[test]
    fn non_cx_apps_url_elicitation_builds_generic_app_link_params() {
        let target = generic_url_target();
        let request = cx_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    #[test]
    fn cx_apps_auth_url_elicitation_rejects_untrusted_urls() {
        let target = suggestion_target();
        for url in [
    ...
    fn installed_app_has_toggle_action() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on gt", "Disable app", "Back"]
        );
    }
    ...
    fn toggle_action_sends_set_app_enabled_and_updates_label() {
        ...
        assert_eq!(
            view.action_labels(),
            vec!["Manage on gt", "Enable app", "Back"]
        );
    }
    ...
    fn auth_suggestion_with_reason_snapshot() {
        ...
        let view = AppLinkView::new(
            AppLinkViewParams {
                ...
                is_installed: true,
                is_enabled: true,
                suggest_reason: Some("Reconnect Google Calendar on gt.".to_string()),
                suggestion_type: Some(AppLinkSuggestionType::Auth),
                elicitation_target: Some(suggestion_target()),
            },
            tx,
