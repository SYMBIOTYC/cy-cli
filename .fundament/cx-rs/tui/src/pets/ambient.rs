//! Ambient terminal rendering for the CX companion.
//!
...
impl AmbientPet {
    ...
    /// CX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_cx_home(
            ...
            /*cx_home*/ Some(cx_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = cx_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_cx_app_vocabulary() {
