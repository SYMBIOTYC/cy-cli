...
//! managed CX_HOME cache, while custom pets remain entirely user-owned data
//! under `$CX_HOME/pets/<pet-id>/pet.json` or legacy avatar directories.
//!
...
pub(crate) const DEFAULT_PET_ID: &str = "cx";
...
async fn ensure_builtin_pack_for_pet(
    ...
    if let Some(pet) = catalog::builtin_pet(pet_id) {
        asset_pack::ensure_builtin_pet(cx_home, pet, http_client).await?;
    }
    Ok(())
}
