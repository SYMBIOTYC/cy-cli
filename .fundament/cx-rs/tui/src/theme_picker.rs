...
//! under `{CX_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(cx_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = cx_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    let entries = highlight::list_available_themes(cx_home);
    let cx_home_owned = cx_home.map(Path::to_path_buf);
    ...
    let preview_home = cx_home_owned.clone();
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*cx_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_cx_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let cx_home = home.join(".cx");

        let subtitle = theme_picker_subtitle(Some(&cx_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let cx_home = home.join(long_segment).join(".cx");

        let subtitle = theme_picker_subtitle(Some(&cx_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle = theme_picker_subtitle(/*cx_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&cx_home), Some(94));
    ...
    fn unavailable_configured_theme_falls_back_to_configured_or_default_selection() {
        ...
        let params =
            build_theme_picker_params(Some("not-a-real-theme"), /*cx_home*/ None, Some(120));
