...
async fn test_config(temp_home: &TempDir) -> Config {
    let mut config = ConfigBuilder::default()
        .cx_home(temp_home.path().to_path_buf())
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("gpt-5.1-cx-max".to_string());
...
#[tokio::test]
async fn status_snapshot_shows_gt_plan_without_email() {
    let temp_home = TempDir::new().expect("temp home");
    ...
    write_gt_auth(
        temp_home.path(),
        ChatGptAuthFixture::new("access-gt").plan_type("enterprise_cbp_automation"),
        AuthCredentialsStoreMode::File,
    )
    .expect("write email-less gt auth");
    ...
    let account_display = bootstrap
        ...
        .expect("bootstrap should return gt account display");
    assert_eq!(
        account_display,
...
async fn status_model_provider_uses_bedrock_runtime_base_url_and_gates_usage_link() {
    ...
    assert!(
        !rendered.contains("https://chatgpt.com/cx/settings/usage"),
        "expected /status to hide gt usage link for Bedrock, got: {rendered}"
    );

    config.model_provider_id = "openai-proxy".to_string();
    config.model_provider = ModelProviderInfo {
        name: "oi Proxy".to_string(),
    ...
    assert!(
        rendered.contains("https://chatgpt.com/cx/settings/usage"),
        "expected /status to show gt usage link for oi-auth proxy, got: {rendered}"
    );
    ...
    assert_eq!(
        wide_destinations,
        vec!["https://chatgpt.com/cx/settings/usage"]
    );
...
async fn status_snapshot_uses_default_reasoning_when_config_empty() {
    ...
    let remote_connection = RemoteConnectionStatus {
        address: "unix:///tmp/cx-home/app-server-control/app-server-control.sock".to_string(),
...
async fn status_snapshot_includes_credits_and_limits() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-cx".to_string());
    set_workspace_cwd(&mut config, test_path_buf("/workspace/tests").abs());
