...
use chrono::Local;
use chrono::Utc;
use cx_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use cx_app_server_protocol::RateLimitSnapshot;
use cx_app_server_protocol::RateLimitWindow;
use cx_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use cx_protocol::num_format::format_with_separators;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `cx` or `cx_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "cx".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("cx");
        ...
        let combine_non_cx_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_cx_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_cx_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_cx_single_limit_renders_combined_row() {
        let now = Local::now();
        let cx = RateLimitSnapshotDisplay {
        ...
        let rows = match compose_rate_limit_data_many(&[cx, other], now) {
    ...
    #[test]
    fn non_cx_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "cx-other".to_string(),
            captured_at: now,
