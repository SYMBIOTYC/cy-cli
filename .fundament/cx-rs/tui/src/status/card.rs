...
use cx_utils_sandbox_summary::summarize_permission_profile;
use ratatui::prelude::*;
...
const CHATGPT_USAGE_URL: &str = "https://chatgpt.com/cx/settings/usage";
...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        let show_gt_usage_link = config.model_provider.requires_openai_auth;
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            StatusAccountDisplay::ChatGpt { email, plan } => match (email, plan) {
                ...
                (None, None) => "gt".to_string(),
            },
            StatusAccountDisplay::ApiKey => {
                "API key configured (run cx login to use gt)".to_string()
            }
        });
        ...
        // The gt usage page only applies to providers backed by oi auth;
        // providers like Bedrock manage limits and billing elsewhere.
        if self.show_gt_usage_link {
        ...
        lines.push(Line::from(Vec::<Span<'static>>::new()));
        // Hide token usage only for gt subscribers
