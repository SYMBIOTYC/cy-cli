...
mod tests {
    ...
    async fn test_config(cx_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let cx_home = TempDir::new().expect("temp cx home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = cx_home.path().join("global.md");
        let config = test_config(&cx_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let cx_home = TempDir::new().expect("temp cx home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = cx_home.path().join("override.md");
        let config = test_config(&cx_home, &cwd).await;
