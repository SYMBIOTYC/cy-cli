...
impl PendingAppServerRequests {
    fn canonical_thread_id(thread_id: &str) -> String {
        cx_protocol::ThreadId::from_string(thread_id)
...
mod tests {
    ...
    use cx_app_server_protocol::McpElicitationObjectType;
    use cx_app_server_protocol::McpElicitationSchema;
    ...
    use cx_app_server_protocol::RequestId as AppServerRequestId;
    use cx_app_server_protocol::ServerRequest;
    use cx_app_server_protocol::ToolRequestUserInputAnswer;
    ...
    fn colliding_approvals_resolve_only_on_their_own_thread() {
        let approvals = [
            ...
            (
                ...
                Op::RequestPermissionsResponse {
                    id: "shared-id".to_string(),
                    response: cx_protocol::request_permissions::RequestPermissionsResponse {
                        permissions: RequestPermissionProfile::default(),
                        scope: cx_protocol::request_permissions::PermissionGrantScope::Turn,
        ...
        for (method, op) in approvals {
            let mut pending = PendingAppServerRequests::default();
            let thread_ids = [
                cx_protocol::ThreadId::new().to_string(),
                cx_protocol::ThreadId::new().to_string(),
            ];
    ...
    fn rejects_permissions_with_paths_that_cannot_be_localized() {
        ...
        let permissions = cx_app_server_protocol::RequestPermissionProfile {
            network: None,
    ...
    fn resolves_permissions_and_user_input_through_app_server_request_id() {
            ...
            .take_resolution(
                ...
                &Op::RequestPermissionsResponse {
                    ...
                    response: cx_protocol::request_permissions::RequestPermissionsResponse {
                        ...
                        scope: cx_protocol::request_permissions::PermissionGrantScope::Session,
                        strict_auto_review: false,
                    },
                },
            )
        ...
        assert_eq!(
            ...
            PermissionsRequestApprovalResponse {
                permissions: cx_app_server_protocol::GrantedPermissionProfile {
                    ...
                    file_system: Some(AdditionalFileSystemPermissions {
                        ...
                        glob_scan_max_depth: None,
                        entries: Some(vec![
                            cx_app_server_protocol::FileSystemSandboxEntry {
                                path: cx_app_server_protocol::FileSystemPath::Path {
                                    path: absolute_path(read_path).into(),
                                },
                                access: cx_app_server_protocol::FileSystemAccessMode::Read,
                            },
                            cx_app_server_protocol::FileSystemSandboxEntry {
                                path: cx_app_server_protocol::FileSystemPath::Path {
                                    path: absolute_path(write_path).into(),
                                },
                                access: cx_app_server_protocol::FileSystemAccessMode::Write,
                            },
                        ]),
                    }),
                },
    ...
    #[test]
    fn does_not_mark_gt_auth_refresh_as_unsupported() {
        let mut pending = PendingAppServerRequests::default();

        assert_eq!(
            pending.note_server_request(&ServerRequest::ChatgptAuthTokensRefresh {
                request_id: AppServerRequestId::Integer(100),
                params: cx_app_server_protocol::ChatgptAuthTokensRefreshParams {
                    reason: cx_app_server_protocol::ChatgptAuthTokensRefreshReason::Unauthorized,
    ...
    fn resolve_notification_returns_resolved_exec_request() {
        let mut pending = PendingAppServerRequests::default();
        let thread_id = cx_protocol::ThreadId::new().to_string();
        assert_eq!(
