...
use cx_protocol::items::ExitedReviewModeItem;
...
async fn start_recording_app_server_with_history(
    ...
    let embedded = crate::start_embedded_app_server(
        cx_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        cx_config::LoaderOverrides::default(),
        /*strict_config*/ false,
        cx_config::CloudConfigBundleLoader::default(),
    ...
    .await?;
    let cx_home = config.cx_home.display().to_string();
...
async fn make_history_test_app() -> Result<(App, tempfile::TempDir)> {
    ...
    app.config.sqlite = SqliteConfig::new_for_testing(cx_home.path().abs());
    Ok((app, cx_home))
}

#[tokio::test]
async fn older_pagination_reconciles_review_prompts_across_page_boundaries() -> Result<()> {
    let (mut app, cx_home) = make_history_test_app().await?;
...
async fn transcript_home_loads_every_older_history_page() -> Result<()> {
    ...
    let export_path = cx_home.path().join("complete-export.md");
    app.chat_widget
...
async fn remote_legacy_history_start_negotiates_once_for_resume_and_fork() -> Result<()> {
    let (app, _cx_home) = make_history_test_app().await?;
...
async fn cold_paginated_subagent_transcript_excludes_inherited_parent_history() -> Result<()> {
    let (app, cx_home) = make_history_test_app().await?;
    let parent_thread_id = create_history_rollout(
        &app.config,
        ThreadHistoryMode::Paginated,
...
async fn changing_directory_preserves_project_trust_permissions_history_and_hooks() -> Result<()> {
    use cx_protocol::config_types::TrustLevel as T;
    use serde_json::json;
    use std::fs;
    ...
    let [current, trusted, unknown, untrusted, mismatch, failed] =
        names.map(|name| cx_home.path().join(name));
    fs::create_dir_all(&current)?;
    for directory in [&trusted, &unknown, &untrusted, &mismatch, &failed] {
        fs::create_dir_all(directory.join(".cx"))?;
        fs::write(directory.join(".cx/config.toml"), "")?;
    }
    let contents = "developer_instructions = \"destination policy\"\nmodel_reasoning_effort = \"high\"\napproval_policy = \"on-request\"\n[tui]\ntheme = \"dracula\"\n[tui.keymap.global]\nopen_transcript = \"f12\"";
    fs::write(trusted.join(".cx/config.toml"), contents)?;
    ...
    fs::write(trusted.join(".cx/hooks.json"), hooks)?;
    let contents = "default_permissions = \"dev\"\n[permissions.dev.filesystem]\n\":root\" = \"write\"\n[tui.keymap.global]\nopen_transcript = \"ctrl-l\"";
    fs::write(mismatch.join(".cx/config.toml"), contents)?;
    let requirements = cx_home.path().join("requirements.toml");
    ...
    for dir in [&trusted, &untrusted, &mismatch, &failed] {
        ...
        crate::legacy_core::config::set_project_trust_level(cx_home.path(), dir, trust)
    ...
    let meta = cx_rollout::read_session_meta_line(&forked_rollout).await?;
    ...
    let source = cx_utils_path_uri::PathUri::from_abs_path(&agents.abs());
