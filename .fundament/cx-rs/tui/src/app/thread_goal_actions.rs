...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `cx` to start a saved session, or `cx resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let cx_home = app_server.cx_home_path(&self.config.cx_home);
        match goal_files::objective_text_for_edit(app_server, cx_home.as_ref(), &goal.objective)
            .await
        {
    ...
    pub(super) async fn set_thread_goal_draft(
        ...
        let (objective, output_dir) =
            match goal_files::materialize_goal_draft(app_server, cx_home.as_ref(), draft).await {
