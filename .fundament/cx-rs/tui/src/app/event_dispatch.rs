    ...
    pub(super) async fn handle_event(
        ...
        match event {
                    ...
                    if self.chat_widget.thread_id() == Some(thread_id)
                        ...
                        && let Err(error) = app_server
                            .thread_goal_set(
                                thread_id,
                                /*objective*/ None,
                                Some(cx_app_server_protocol::ThreadGoalStatus::Paused),
                                /*token_budget*/ None,
                            )
                            .await
                    {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt {
                ...
                self.session_telemetry.counter(
                    "cx.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "cx.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let cx_home = self.config.cx_home.clone();
                    let tx = self.app_event_tx.clone();

                    // If the elevated setup already ran on this machine, don't prompt for
                    // elevation again - just flip the config to use the elevated path.
                    if crate::windows_sandbox::sandbox_setup_is_complete(cx_home.as_path()) {
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "cx.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                        ...
                        Ok(_) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "CY can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.cx_home.clone();
            ...
            AppEvent::AgentsDaemonStarted { result } => match result {
                Ok(()) => self.chat_widget.add_info_message(
                    "Background server started. Run `cy agents` in another terminal; this session remains unchanged."
                        .to_string(),
                    /*hint*/ None,
                ),
                Err(error) => self
