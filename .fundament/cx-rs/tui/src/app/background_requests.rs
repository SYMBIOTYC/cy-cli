...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<cx_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<cx_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<cx_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: cx_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    if err.contains("api key auth is not supported") {
        "Sign in with gt auth; API key auth cannot load remote plugin catalogs."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this CX session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![cx_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, cx_protocol::mcp::Tool>,
    HashMap<String, Vec<cx_protocol::mcp::Resource>>,
    HashMap<String, Vec<cx_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                cx_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
                cx_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                cx_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                cx_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                cx_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    async fn fetch_thread_usage_uses_app_server_auth_after_persisted_account_changes() {
        ...
        app.config.gt_base_url = server.uri();
        app.config.cli_auth_credentials_store_mode = AuthCredentialsStoreMode::File;
        write_gt_auth(
            app.config.cx_home.as_path(),
            ChatGptAuthFixture::new("gt-token").account_id("account-123"),
            AuthCredentialsStoreMode::File,
        )
        .expect("write gt authentication");
    ...
    fn plugin_remote_section_error_message_adds_concrete_next_steps() {
        let cases = [
            (
                "Workspace",
                "gt authentication required for remote plugin catalog",
                "Sign in to gt, then try loading this section again.",
            ),
            (
                "oi Curated",
