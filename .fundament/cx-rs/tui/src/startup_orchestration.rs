...
pub(super) async fn run_main_inner(
    ...
    // `oss` model provider.
    let overrides_cli = cx_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let cx_home = match find_cx_home() {
        Ok(cx_home) => cx_home.to_path_buf(),
        Err(err) => {
    ...
    let initial_screen = if cli.resume_picker || cli.fork_picker || cli.agents_overview {
        ...
        && startup_preflight::should_delay_startup_composer_for_first_login(
            &cx_home,
            cx_config::loader::system_config_toml_file(),
            || cx_config::loader::has_local_managed_configuration(&cx_home),
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        startup_draft
            .run_until(maybe_probe_default_daemon_socket(&cx_home))
            .await?
    } else {
        None
    };
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            startup_draft
                .run_until(EnvironmentManager::prepare_from_cx_home(&cx_home))
                .await?
        } else {
            startup_draft
    ...
    remove_legacy_tui_log_file(config.cx_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        cx_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(cx_otel::OtelProvider::metrics) {
        let _ = cx_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            cx_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = cx_state::install_process_db_telemetry(telemetry);
    }
    ...
    let feedback = cx_feedback::CodexFeedback::new();
