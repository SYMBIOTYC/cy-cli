//! Shared implementation for `cx archive`, `cx delete`, and `cx unarchive`.
//!
...
async fn run_session_archive_action_with_app_server(
    ...
    let resolved = resolve_session_target(app_server, cx_home, action, target).await?;
...
pub(super) async fn lookup_session_by_exact_name(
    ...
    let source_kind_filters = if match_policy == SessionNameMatch::FirstIncludingNonInteractive {
        // An empty filter includes Atlas/gt sessions; explicit kinds additionally include exec.
        vec![
...
pub(super) async fn start_app_server_for_session_command(
    ...
    let workload_identity_selected = cx_login::is_workload_identity_selected();
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(cx_home.as_path()).await
    } else {
        None
    };
    ...
    if cli.oss {
        return Err(eyre!("OSS providers were removed; CY is the only provider"));
    }
