...
#[cfg(test)]
use cx_app_server_protocol::McpAuthStatus;
...
use cx_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use cx_app_server_protocol::SkillErrorInfo;
...
use cx_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use cx_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        cx_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn resume_hint_for_resumable_thread(
    ...
    cx_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_gt_account: self.chat_widget.has_gt_account(),
            has_cx_backend_auth: self.chat_widget.has_cx_backend_auth(),
