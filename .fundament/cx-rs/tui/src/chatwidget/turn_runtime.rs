...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        cx_error_info: &AppServerCodexErrorInfo,
    ) -> bool {
        matches!(
            cx_error_info,
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.cx_rate_limit_reached_type.map(|kind| {
        ...
        self.cx_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        cx_error_info: Option<AppServerCodexErrorInfo>,
    ) {
        if cx_error_info == Some(AppServerCodexErrorInfo::MisalignmentPolicyViolation) {
            self.on_misalignment_policy_violation();
        } else if cx_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = cx_error_info
            .as_ref()
