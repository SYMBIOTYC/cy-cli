...
impl ChatWidget {
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_cx_limit = limit_id.eq_ignore_ascii_case("cx");
            if is_cx_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.cx_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_cx_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.cx_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.cx_spend_control_reached == Some(true)
                || matches!(
                    self.cx_rate_limit_reached_type,
                    Some(
            ...
            if is_cx_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_cx_limit && !has_workspace_credits;
            ...
            self.cx_rate_limit_reached_type = None;
            self.cx_spend_control_reached = None;
        }
    ...
    pub(super) fn should_prefetch_rate_limits(&self) -> bool {
        self.config.model_provider.requires_openai_auth && self.has_gt_account
    }
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using cx. Request increase?",
            ),
        };
