...
impl ChatWidget {
    ...
    /// Clears the terminal title CX most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_cx_folder } => {
                    dot_cx_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::CodexVersion => Some(CX_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("cx".to_string()),
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("cx".to_string()),
...
fn five_hour_status_window(
    ...
    find_primary_cx_window(snapshot, "5h")
...
fn weekly_status_window(
    ...
    find_cx_window(snapshot, "weekly")
