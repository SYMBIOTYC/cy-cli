...
const APPS_HELP_ARTICLE_URL: &str = "https://help.openai.com/en/articles/11487775-apps-in-gt";
...
const OPENAI_CURATED_LOADING_DESCRIPTION: &str =
    "This updates when oi Curated plugins finish loading.";
...
impl MarketplaceProduct {
    ...
    fn label(self) -> Option<&'static str> {
        match self {
            Self::OpenAiCurated => Some("oi Curated"),
...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from CX.".dim(),
        ));
    ...
    pub(super) fn plugins_popup_params(
        ...
        let (curated_empty_name, curated_empty_description) =
            if curated_loading && !curated_has_entries {
                (
                    "Loading oi Curated plugins...",
                    OPENAI_CURATED_LOADING_DESCRIPTION,
                )
            } else if let Some(section_error) = by_openai_section_error
                && !curated_has_entries
            {
                ("oi Curated unavailable", section_error.message.as_str())
            } else {
                (
                    "No oi Curated plugins available",
                    "No oi Curated plugins available.",
                )
            };
        ...
        tabs.push(SelectionTab {
            ...
            label: "oi Curated".to_string(),
            header: plugins_header(
                "oi Curated marketplace.".to_string(),
                format!("Installed {curated_installed} of {curated_total} oi Curated plugins."),
            ),
            items: curated_items,
        });
                ...
                if let Some((location, plugin_name)) = plugin_detail_request {
                    vec![Box::new(move |tx| {
                        ...
                        tx.send(AppEvent::FetchPluginDetail {
                            cwd: cwd.clone(),
                            params: cx_app_server_protocol::PluginReadParams {
                                marketplace_path,
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(cx_app_server_protocol::HookEventName, usize)>::new();
