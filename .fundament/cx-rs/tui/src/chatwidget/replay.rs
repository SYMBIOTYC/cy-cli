            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: cx_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            cx_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        cx_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: cx_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            } => self.on_command_execution_started(item),
            item @ ThreadItem::CommandExecution {
                source: ExecCommandSource::Agent | ExecCommandSource::UnifiedExecStartup,
                status:
                    cx_app_server_protocol::CommandExecutionStatus::Completed
                    | cx_app_server_protocol::CommandExecutionStatus::Failed,
                ..
            } if from_replay => {
                if matches!(
                    &item,
                    ThreadItem::CommandExecution {
                        status: cx_app_server_protocol::CommandExecutionStatus::Failed,
                        ..
                    }
                ) {
            ...
            item @ ThreadItem::McpToolCall {
                status: cx_app_server_protocol::McpToolCallStatus::InProgress,
                ..
            ...
            ThreadItem::WebSearch(item) => {
                ...
                self.on_web_search_end(
                    ...
                    item.query,
                    item.action
                        .unwrap_or(cx_app_server_protocol::WebSearchAction::Other),
                );
            }
