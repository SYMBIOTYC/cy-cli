...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![cx_utils_path_uri::PathUri::from_abs_path(
...
async fn goal_slash_command_emits_attached_images() {
    ...
    let local_image = chat.config.cx_home.join("goal-local.png");
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(cx_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(cx_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
#[tokio::test]
async fn signed_out_usage_command_reports_gt_login_requirement() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert_chatwidget_snapshot!(
        "signed_out_usage_command_reports_gt_login_requirement",
        rendered
    );
...
#[tokio::test]
async fn signed_out_usage_command_with_args_reports_gt_login_requirement() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let rendered = cells
        ...
        .join("\n");
    assert!(
        rendered.contains("Sign in with gt to use /usage."),
        "expected gt login requirement, got: {rendered:?}"
    );
...
#[tokio::test]
async fn usage_command_runs_with_backend_auth_without_gt_account_flag() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
async fn usage_command_runs_with_backend_auth_from_widget_init() {
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::RefreshTokenActivity { .. }));
    assert!(!chat.has_gt_account());
    assert!(chat.has_cx_backend_auth());
}
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask CX to do anything")
    );
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(cx_config::types::KeybindingsSpec::One(
        cx_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            cx_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: cx_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: cx_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/cx-test-rollout.jsonl");
