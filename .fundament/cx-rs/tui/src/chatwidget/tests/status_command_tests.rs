...
use cx_app_server_protocol::ThreadUsageBreakdownGroup;
use cx_utils_path_uri::PathUri;

#[tokio::test]
async fn status_command_renders_immediately_and_refreshes_rate_limits_for_gt_auth() {
...
async fn status_command_renders_immediately_without_rate_limit_refresh() {
    ...
    assert!(
        ...
        "non-gt sessions should not request a rate-limit refresh for /status"
    );
}
