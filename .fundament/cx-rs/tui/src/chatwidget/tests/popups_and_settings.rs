...
use cx_app_server_protocol::HookErrorInfo;
use cx_app_server_protocol::HooksListEntry;
use cx_app_server_protocol::HooksListResponse;
use cx_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugins_popup_truncates_long_descriptions_in_list_rows() {
    ...
    insta::assert_snapshot!(
        verbose_row,
        @"  [-] Verbose Plugin  Available · oi Curated · This description…"
    );
    assert!(
        !popup
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (cx_app_server_protocol::HookEventName::PreToolUse, 1),
            (cx_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn plugin_detail_popup_distinguishes_admin_installed_from_enabled() {
    ...
    insta::assert_snapshot!(
        popup
            ...
            .trim(),
        @"Figma · Enabled by Admin · gt Marketplace"
    );
}
...
async fn plugin_detail_popup_shows_local_share_context_as_read_only_snapshot() {
    ...
    let summary = PluginSummary {
        share_context: Some(PluginShareContext {
            ...
            share_url: Some("https://chatgpt.com/cx/plugins/share/docs".to_string()),
...
async fn plugin_detail_popup_shows_admin_disabled_status_snapshot() {
    ...
    insta::assert_snapshot!(
        status_row,
        @"  Admin Blocked · Disabled by admin · gt Marketplace"
    );
    assert!(
...
async fn plugins_popup_remote_section_fallback_states_when_remote_plugin_disabled_snapshot() {
    ...
    let curated_loading_popup = select_tab_containing(&mut chat, "Loading oi Curated plugins...");
    ...
    chat.on_plugin_remote_sections_loaded(
        ...
        vec![crate::app_event::PluginRemoteSectionError {
            ...
            message: "Sign in to gt to load workspace plugins.".to_string(),
        }],
    );
    ...
    let remote_curated_empty_popup =
        select_tab_containing(&mut remote_chat, "No oi Curated plugins available");
    ...
    insta::assert_snapshot!(
        ...
        .join("\n\n"),
        @r###"
        oi Curated marketplace.
        Loading oi Curated plugins...  This updates when oi Curated plugins finish loading.
        ...
        Workspace unavailable  Sign in to gt to load workspace plugins.

        oi Curated marketplace.
        No oi Curated plugins available  No oi Curated plugins available.
        "###
    );
}
...
async fn plugins_popup_openai_curated_tab_omits_marketplace_in_rows() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert!(
        popup.contains("oi Curated marketplace."),
        "expected oi Curated tab header, got:\n{popup}"
    );
    assert!(
        popup.contains("Calendar") && !popup.contains("Repo Plugin"),
        "expected oi Curated tab to show only official marketplace plugins, got:\n{popup}"
    );
    assert!(
        !popup.contains("gt Marketplace ·"),
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = cx_config::types::TuiKeymap::default();
    config.list.accept = Some(cx_config::types::KeybindingsSpec::One(
        cx_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn auto_model_advertising_advanced_effort_opens_reasoning_picker() {
    ...
    preset.id = "cx-auto-test".to_string();
    preset.model = "cx-auto-test".to_string();
    preset.display_name = "cx-auto-test".to_string();
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &cx_feedback::FeedbackDiagnostics::new(vec![cx_feedback::FeedbackDiagnostic {
