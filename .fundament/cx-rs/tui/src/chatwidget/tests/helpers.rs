...
pub(super) async fn test_config() -> Config {
    ...
    let cx_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_cx_home(cx_home.clone(), Vec::new())
            .await
            .expect("config");
    config.cx_home = cx_home.abs();
    config.sqlite = cx_state::SqliteConfig::new_for_testing(cx_home.as_path().abs());
    config.log_dir = cx_home.join("log");
...
pub(crate) fn set_gt_auth(chat: &mut ChatWidget) {
...
fn token_usage_breakdown(usage: TokenUsage) -> cx_app_server_protocol::TokenUsageBreakdown {
    cx_app_server_protocol::TokenUsageBreakdown {
        ...
        Some(info) => {
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    cx_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: cx_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    cx_error_info: Option<CodexErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn handle_agent_message_delta(chat: &mut ChatWidget, delta: impl Into<String>) {
    chat.handle_server_notification(
        ServerNotification::AgentMessageDelta(
            cx_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = cx_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            cx_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_curated_marketplace(
    ...
    PluginMarketplaceEntry {
        ...
        path: Some(plugins_test_absolute_path("marketplaces/gt")),
        interface: Some(MarketplaceInterface {
            display_name: Some("gt Marketplace".to_string()),
        }),
        plugins,
    }
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(cx_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        marketplace_name: "gt Marketplace".to_string(),
        marketplace_path: Some(plugins_test_absolute_path("marketplaces/gt")),
        summary,
        share_url: None,
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    cx_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                cx_app_server_protocol::HookOutputEntry {
                    kind: cx_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: cx_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        cx_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        cx_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        cx_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        cx_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        cx_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        cx_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        cx_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        cx_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        cx_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        cx_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        cx_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
