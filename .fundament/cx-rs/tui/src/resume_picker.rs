...
use cx_utils_path as path_utils;
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.cx_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/cx-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/cx")),
            git_branch: Some(String::from("cx/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      cx/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/cx", " main");
        assert_metadata_order(&created[0], "⌁ tmp/cx", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/cx"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/cx", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/cx.etraut-animations-false-improvements/cx-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/cx.owner-extremely-long-worktree-name-that-needs-truncating/cx-rs";
        ...
        assert!(footer.contains("⌁ ~/code/cx."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/cx.owner-worktree/cx-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let cx_home_file = tmp.path().join("cx-home-file");
        std::fs::write(&cx_home_file, "not a directory").expect("write cx home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![cx_app_server_protocol::Turn {
