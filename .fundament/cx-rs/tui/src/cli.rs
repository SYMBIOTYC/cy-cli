...
use clap::Parser;
use cx_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `cx resume` subcommand.
    // These are not exposed as user flags on the base `cx` command.
    #[clap(skip)]
    ...
    /// top-level `cx resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `cx fork` subcommand.
    // These are not exposed as user flags on the base `cx` command.
    #[clap(skip)]
    ...
    /// top-level `cx fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
