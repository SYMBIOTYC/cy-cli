...
pub(super) fn should_delay_startup_composer_for_first_login(
    ...
    }) || environment_variable(OI_FEDERATION_RULE_ID_ENV_VAR).is_some()
        || environment_variable(OI_IDENTITY_TOKEN_FILE_ENV_VAR).is_some()
    {
        return false;
    }
    ...
    match std::fs::metadata(cx_home) {
        ...
        Err(error)
            if error.kind() == io::ErrorKind::NotFound
                && matches!(cx_home.try_exists(), Ok(false)) => {}
        Err(_) => return false,
    }
