...
const LIGHT_BG_ACCENT_RGB: (u8, u8, u8) = (138, 106, 120);
...
pub(crate) fn accent_style_for(terminal_bg: Option<(u8, u8, u8)>) -> Style {
    if terminal_bg.is_some_and(is_light) {
        ...
        Style::default().fg(rgb_color((138, 106, 120))).bold()
    }
}
...
mod tests {
    ...
    #[test]
    fn accent_style_uses_muted_rose_on_dark_or_unknown_backgrounds() {
        let expected = Style::default().fg(rgb_color((138, 106, 120))).bold();
