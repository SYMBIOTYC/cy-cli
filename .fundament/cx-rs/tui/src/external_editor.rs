...
pub(crate) async fn run_editor(
    ...
    let default_cx_home = dirs::home_dir().map(|home| home.join(".cx"));
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    let project_cx_home = cwd.join(".cx");
    let mut candidate_homes = vec![cx_home];
    if let Some(default_cx_home) = default_cx_home.as_deref() {
        candidate_homes.push(default_cx_home);
    }
    #[cfg(any(target_os = "macos", target_os = "linux"))]
    candidate_homes.push(&project_cx_home);
