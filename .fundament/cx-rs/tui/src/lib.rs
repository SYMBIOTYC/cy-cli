...
// The standalone `cx-tui` binary prints a short help message before the
...
use cx_app_server_client::InProcessClientStartArgs;
use cx_app_server_client::RemoteAppServerClient;
use cx_app_server_client::RemoteAppServerConnectArgs;
pub use cx_app_server_client::RemoteAppServerEndpoint;
use cx_app_server_protocol::Account as AppServerAccount;
...
use cx_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use cx_config::format_config_error_with_source;
...
use cx_login::default_client::set_default_client_residency_requirement;
...
pub(crate) use cx_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "cx-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: cx_feedback::CodexFeedback,
...
impl AppServerTarget {
    ...
    fn auth_config_for_cloud_loader(&self, mut auth_config: AuthConfig) -> AuthConfig {
        if self.uses_remote_workspace() {
            ...
            auth_config.forced_gt_workspace_id = None;
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = cx_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(cx_home: &Path) {
    ...
    let _ = std::fs::remove_file(cx_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let cx_home = find_cx_home().wrap_err("failed to resolve CODEX_HOME")?;
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(cx_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = cx_app_server_client::app_server_control_socket_path(cx_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_cx_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&cx_state::StateRuntime>,
...
async fn cloud_config_bundle_for_app_server_target(
    ...
    cloud_config_bundle_loader_for_storage(
        app_server_target
            .auth_config_for_cloud_loader(bootstrap_auth_config(cx_home, bootstrap_config)?),
        /*enable_cx_api_key_env*/ false,
    )
    .await
}
...
async fn run_ratatui_app(
         ...
         terminal_restore_guard: &mut TerminalRestoreGuard| {
            ...
            Ok(AppExitInfo {
                ...
                exit_reason: ExitReason::Fatal(format!(
                    "No saved session found with ID {id_str}. Run `cx {action}` without an ID to choose from existing sessions."
                )),
            })
        };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_cx_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.cx_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, cx_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
...
fn should_show_login_screen(login_status: LoginStatus, config: &Config) -> bool {
    // Only show the login screen for providers that actually require oi auth
    // (oi or equivalents). For OSS/other providers, skip login entirely.
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let session_meta = cx_protocol::protocol::SessionMeta {
            ...
            source: cx_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(cx_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_cx_home(cx_home.path()).await?;
            ...
            let cloud_config_bundle =
                cx_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: cx_app_server_client::app_server_control_socket_path(&cx_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://cx.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("cx.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("cx.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let cx_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(cx_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(cx_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("cx.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            cx_app_server_client::AppServerClient::InProcess(
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = cx_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            cx_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let cx_home = temp_dir.path().to_path_buf();
