...
const ANNOUNCEMENT_TIP_URL: &str =
    "https://raw.githubusercontent.com/SYMBIOTYC/cy-cli/main/announcement_tip.toml";
...
/// Pick a random tooltip to show to the user when starting CX.
...
pub(crate) mod announcement {
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // CX currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
    ...
    pub(crate) fn parse_announcement_tip_toml(
        ...
        for raw in announcements {
            ...
            if tip.version_matches(CX_CLI_VERSION)
...
mod tests {
    ...
    fn desktop_app_tooltip_uses_supported_platform_launcher() {
        ...
        if linux_app_tooltip(LinuxDesktopSession::current()).is_some() {
            ...
            insta::assert_snapshot!(tooltip, @"Run `cx app` to open the Desktop app (it installs on macOS if needed).");
    ...
    fn linux_desktop_app_tooltip_requires_graphical_native_session() {
        ...
        insta::assert_snapshot!(tooltip, @"Try the **Desktop app** on Linux: install it from https://learn.chatgpt.com/docs/linux/linux-app and run 'gt'.");
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for CX TUI.
...
[[announcements]]
content = "Welcome to CY! Check out the new onboarding flow."
