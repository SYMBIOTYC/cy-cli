...
async fn lookup_legacy_index_target(
    ...
    let candidates = cx_rollout::find_thread_meta_candidates_by_name_str(
    ...
    for (path, session_meta) in candidates {
        if !path.starts_with(config.cx_home.join(cx_rollout::SESSIONS_SUBDIR)) {
            continue;
        }
        ...
        loop {
            if let Some(thread) = self.pending.next() {
                ...
                let session_meta = if app_server.uses_remote_workspace() {
                    ...
                    let Some(path) = cx_rollout::existing_rollout_path(path).await else {
                        continue;
                    };
                    let expected_root = self.cx_home.join(match self.collection {
                        SessionCollection::Active => cx_rollout::SESSIONS_SUBDIR,
                        SessionCollection::Archived => cx_rollout::ARCHIVED_SESSIONS_SUBDIR,
                    });
