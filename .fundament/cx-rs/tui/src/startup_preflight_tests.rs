...
fn startup_delays_composer_for_homes_without_authentication_state() -> std::io::Result<()> {
    ...
    let cx_home = temporary_directory.path().join("cx-home");
    ...
    for disabled_credential in [cx_login::CY_API_KEY_ENV_VAR] {
        assert!(should_delay_startup_composer_for_first_login(
            &cx_home,
            Ok(system_config_path.clone()),
            || Ok(false),
    ...
    for existing_state in ["history.jsonl", "log", "sessions"] {
        let state_path = cx_home.join(existing_state);
    ...
    let daemon_directory = cx_home.join("app-server-control");
    ...
    let additional_temporary_state = cx_home.join("tmp").join("other");
...
fn startup_keeps_composer_for_workload_identity_markers() -> std::io::Result<()> {
    ...
    for marker in [
        cx_protocol::shell_environment::OI_FEDERATION_RULE_ID_ENV_VAR,
        cx_protocol::shell_environment::OI_IDENTITY_TOKEN_FILE_ENV_VAR,
    ] {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(!should_delay_startup_composer_for_first_login(
            ...
            || panic!("workload identity should not probe managed configuration"),
            |name| {
                (name == cx_protocol::shell_environment::OI_IDENTITY_TOKEN_FILE_ENV_VAR)
