...
fn live_001_commit_on_overflow() {
    ...
    let mut term = cx_tui::Terminal::with_options(backend).expect("terminal should be constructed");
    let area = Rect::new(
    ...
    let mut rb = cx_tui::RowBuilder::new(/*target_width*/ 20);
    ...
    cx_tui::insert_history_lines(&mut term, lines).expect("Failed to insert history lines in test");
