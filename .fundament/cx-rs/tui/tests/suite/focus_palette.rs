...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(cx_home.path(), &repo_root)?;

    let mut terminal = PtyCodex::start(&repo_root, cx_home)?;
...
impl PtyCodex {
    fn start(repo_root: &Path, cx_home: TempDir) -> Result<Self> {
        let mut master_fd = -1;
        let mut slave_fd = -1;
        ...
        let cx = cx_utils_cargo_bin::cargo_bin("cx")
            .or_else(|_| cx_utils_cargo_bin::cargo_bin("cx-tui"))?;
        let child = Command::new(cx)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            .arg("-c")
            ...
            .stderr(slave)
            .spawn()
            .context("start CX in focus-test pseudo-terminal")?;
    ...
    fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "CX exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "CX did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
