...
#[ignore = "requires tmux and a locally built cx binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let session_name = format!("cx-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("CODEX_HOME={}", cx_home.path().display()))
    ...
    let baseline_capture =
        wait_for_capture_contains(&cx_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&cx_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("cx-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&cx_pane)?;
    let restored_row =
...
async fn tmux_scrolled_composer_resize_preserves_visible_draft_text() -> Result<()> {
    ...
    let session_name = format!("cx-resize-scrolled-composer-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    let baseline =
        wait_for_capture_contains(&cx_pane, &final_line, Duration::from_secs(/*secs*/ 15))?;
        ...
        [("narrowed", "28", "9", 2), ("restored", "44", "14", 6)]
    {
        ...
        let capture = capture_pane(&cx_pane)?;
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("cx-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn cx_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = cx_utils_cargo_bin::cargo_bin("cx") {
        return Ok(path);
    }

    let fallback = repo_root.join("cx-rs/target/debug/cx");
    anyhow::ensure!(
        fallback.is_file(),
        "cx binary is unavailable; run `cargo build -p cx-cli` first"
    );
    Ok(fallback)
}

fn write_config(cx_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(cx_home.join("config.toml"), config)?;
    Ok(())
}
