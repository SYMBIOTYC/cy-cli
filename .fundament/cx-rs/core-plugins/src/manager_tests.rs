...
use cx_config::McpServerConfig;
use cx_config::McpServerOAuthConfig;
use cx_config::McpServerToolConfig;
use cx_config::RequirementSource;
use cx_config::RequirementsLayerEntry;
use cx_config::SkillConfigRules;
use cx_config::compose_requirements;
use cx_config::types::McpServerTransportConfig;
use cx_login::CodexAuth;
use cx_login::test_support::auth_manager_with_agent_identity;
use cx_model_provider::AMAZON_BEDROCK_PROVIDER_ID;
use cx_plugin::AppDeclaration;
...
use cx_utils_plugins::SkillDiscoveryMode;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file =
        AbsolutePathBuf::try_from(cx_home.join(CONFIG_TOML_FILE)).expect("absolute config path");
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(cx_home, user_config, requirements),
        String::new(),
...
#[test]
fn curated_repo_sync_stays_deferred_for_remote_gt_catalog() {
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let cx_home = TempDir::new().expect("create CX home");
    write_plugin(
        &cx_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = test_plugins_manager(cx_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let cx_home = TempDir::new().expect("create CX home");
    let marketplace_root = cx_home.path().join("marketplace");
...
"#,
    ...
    let err = test_plugins_manager(cx_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let cx_home = TempDir::new().expect("create CX home");
    let repo_root = cx_home.path().join("repo");
...
fn write_auth_projection_plugin(cx_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(cx_home, name, name);
    }
}

fn write_auth_projection_app(cx_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = cx_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(cx_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&cx_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(cx_home, cx_home).await
}
...
#[tokio::test]
async fn plugin_auth_projection_hides_apps_without_gt_auth() {
    let cx_home = TempDir::new().unwrap();
    write_auth_projection_plugin(cx_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(cx_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(cx_home.path()).await;
...
#[tokio::test]
async fn plugin_auth_projection_hides_matching_mcp_with_gt_apps_route() {
    let cx_home = TempDir::new().unwrap();
    write_auth_projection_plugin(cx_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(cx_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(cx_home.path()).await;
...
#[tokio::test]
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_gt_apps_route() {
    let cx_home = TempDir::new().unwrap();
    write_auth_projection_plugin(cx_home.path(), "sample", /*include_app*/ false);
    write_auth_projection_app(cx_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(cx_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(cx_home.path()).await;
    let manager = test_plugins_manager_with_options(
        cx_home.path().to_path_buf(),
...
"#,
    ...
    let config = load_config(cx_home.path(), cx_home.path()).await;
    let manager = test_plugins_manager_with_options(
        cx_home.path().to_path_buf(),
        Some(Product::CX),
        Some(AuthMode::Chatgpt),
...
async fn plugin_auth_projection_reprojects_cached_plugins_when_auth_changes() {
    ...
    let gt_outcome = manager.plugins_for_config(&config).await;
    assert_eq!(
        sorted_effective_mcp_server_names(&gt_outcome),
        vec!["docs".to_string()]
    );
    assert_eq!(
        gt_outcome.effective_apps(),
        vec![AppConnectorId("connector_sample".to_string())]
    );
    assert_eq!(
        gt_outcome.capability_summaries(),
        &[
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".cx-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "cx-test@example.com"]);
    run_git(repo, &["config", "user.name", "CX Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&cx_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(cx_home, cx_home).await;
    test_plugins_manager_with_options(cx_home.to_path_buf(), Some(Product::CX), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(cx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(cx_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        installed_at: None,
        enabled: true,
        install_policy: cx_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: cx_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: cx_app_server_protocol::PluginAvailability::Available,
...
fn write_cached_plugin(cx_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &cx_home
            .join("plugins/cache")
...
async fn remote_installed_plugin_preserves_configured_mcp_server_policy() {
    ...
    write_cached_plugin(cx_home.path(), "openai-curated-remote", "linear");
    write_file(
        &plugin_root.join(".mcp.json"),
        r#"{
  "mcpServers": {
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let cx_home = TempDir::new().unwrap();
    write_cached_plugin(cx_home.path(), "test", "sample");
    let manager = test_plugins_manager(cx_home.path().to_path_buf());
...
"#,
    ...
    write_cached_plugin(cx_home.path(), "openai-curated", "linear");
    write_cached_plugin(cx_home.path(), "openai-curated", "calendar");
    write_cached_plugin(cx_home.path(), "openai-api-curated", "linear");
    write_cached_plugin(cx_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(cx_home.path(), "openai-curated-remote", "remote-only");
...
#[tokio::test]
async fn api_curated_plugin_does_not_suppress_remote_curated_conflict_for_gt() {
    let cx_home = TempDir::new().unwrap();
    write_file(
        &cx_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
#[tokio::test]
async fn non_gt_auth_rejects_cached_remote_curated_plugins_after_auth_switch() {
    let cx_home = TempDir::new().unwrap();
    write_file(
        &cx_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    let mut config = load_config(cx_home.path(), cx_home.path()).await;
    let auth_manager = test_auth_manager(/*auth_mode*/ None);
    set_test_auth_mode(&auth_manager, Some(AuthMode::Chatgpt)).await;
    let manager = test_plugins_manager_with_auth_manager(
        cx_home.path().to_path_buf(),
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = cx_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = cx_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(cx_app_server_protocol::PluginInterface {
    ...
    assert_eq!(marketplaces[0].display_name, "oi Curated Remote");
    ...
    assert_eq!(
        ...
        cx_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        cx_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    let cx_home = TempDir::new().unwrap();
    let source_root = cx_home.path().join("source/sample");

    write_file(
        &source_root.join(".cx-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".cx-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(cx_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".cx-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".cx-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".cx-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".cx-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let cx_home = TempDir::new().unwrap();
    let plugin_a_root = cx_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = cx_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".cx-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".cx-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, cx_home.path(), Some(AuthMode::Chatgpt)).await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(cx_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        skill_discovery_mode: SkillDiscoveryMode::Recursive,
        disabled_skill_paths: HashSet::new(),
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![cx_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![cx_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(cx_home.path(), cx_home.path()).await;
    let outcome = test_plugins_manager(cx_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = cx_home
        ...
        .join("test/sample/local");
    write_plugin(
        cx_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = cx_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let cx_home = TempDir::new().expect("create CX home");
    let marketplace_root = cx_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = config_layer_stack_with_requirements(cx_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = test_plugins_manager(cx_home.path().to_path_buf());
    ...
    assert!(!cx_home.path().join(CONFIG_TOML_FILE).exists());
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".cx-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".cx-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".cx-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".cx-plugin/plugin.json").is_file());
}
...
#[tokio::test]
async fn read_plugin_for_config_filters_mcp_servers_for_cx_backend_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.cx-plugin/plugin.json"),
...
"#,
    ...
    let gt_outcome = test_plugins_manager_with_options(
        tmp.path().to_path_buf(),
        Some(Product::CX),
        Some(AuthMode::Chatgpt),
    )
    ...
    .unwrap();
    assert_eq!(
        gt_outcome.plugin.mcp_server_names,
        vec!["other-mcp".to_string()]
    );
    assert_eq!(
        gt_outcome.plugin.apps,
        vec![AppConnectorId("connector_sample".to_string())]
    );
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".cx-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".cx-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
    ...
    assert_eq!(
        ...
        ConfiguredMarketplace {
            ...
            interface: Some(MarketplaceInterface {
                display_name: Some("oi Curated".to_string()),
            }),
            plugins: vec![ConfiguredMarketplacePlugin {
...
"#
        ...
        write_openai_curated_marketplace(&curated_root, &["gt-plugin"]);
...
#[tokio::test]
async fn list_marketplaces_uses_gt_curated_manifest_for_bedrock_with_gt_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.cx-plugin/plugin.json"),
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .and(header("gt-account-id", "account_id"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["chat-plugin"]"#))
        .mount(&server)
        .await;

    let mut config = load_config(tmp.path(), tmp.path()).await;
    config.gt_base_url = format!("{}/backend-api/", server.uri());
    let manager = test_plugins_manager_with_options(
        tmp.path().to_path_buf(),
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&CodexAuth::create_dummy_gt_auth_for_testing()),
        )
        .await
        .unwrap();
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "cx"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["cx-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["cx-plugin".to_string()]);
}
...
"#,
    ...
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/ps/plugins/suggested/cx"))
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_cx_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_cx_home(tmp.path())
            .into_iter()
...
"#,
    ...
    let config = load_plugins_config_input(cx_home.path(), &marketplace_root).await;
    let roots = [AbsolutePathBuf::try_from(marketplace_root.clone()).unwrap()];
    let manager = Arc::new(test_plugins_manager(cx_home.path().to_path_buf()));
    manager
...
async fn load_plugins_ignores_project_config_files() {
    let cx_home = TempDir::new().unwrap();
    let project_root = cx_home.path().join("project");
    let plugin_root = cx_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".cx/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_cx_folder: AbsolutePathBuf::try_from(project_root.join(".cx")).unwrap(),
            },
...
"#,
    ...
    for marketplace_name in [
        ...
        write_cached_plugin(cx_home.path(), marketplace_name, "linear");
        write_file(
            &cx_home
                .path()
                .join("plugins/cache")
