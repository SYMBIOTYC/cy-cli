...
use cx_utils_plugins::AgentPluginSchemaStatus;
use cx_utils_plugins::agent_plugin_schema_status;
...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: cx_http_client::HttpError,
    },
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = cx_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(cx_home)?;
...
mod tests {
    ...
    fn install_persists_remote_plugin_install_metadata() {
        ...
        let store = PluginStore::new(cx_home.path().to_path_buf());
        ...
        let metadata_path = store
            ...
            .join(".cx-remote-plugin-install.json");
        assert_eq!(
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".cx-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".cx-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".cx-plugin")).expect("create manifest dir");
        std::fs::write(
