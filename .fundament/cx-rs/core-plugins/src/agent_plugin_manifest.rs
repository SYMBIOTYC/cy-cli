...
use cx_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use cx_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use cx_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    if let Some(extension_contents) = cx_extension.as_deref() {
        apply_cx_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            manifest_path,
            extension_contents,
...
fn apply_cx_agent_plugin_extension(
