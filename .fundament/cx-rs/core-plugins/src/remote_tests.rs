...
#[tokio::test]
async fn recommended_plugins_requests_cx_suggestions_endpoint() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/suggested/cx"))
    ...
    assert_eq!(
        recorded_http_client_urls(&selected_urls),
        vec![format!(
            "{}/backend-api/ps/plugins/suggested/cx?scope=GLOBAL",
            server.uri()
        )]
    );
}
...
fn cached_remote_plugin_catalog_scopes_returns_existing_scopes() {
    ...
    assert_eq!(
        cached_remote_plugin_catalog_scopes(cx_home.path(), &config, Some(&auth)),
...
#[test]
fn recommended_plugins_accept_cx_suggestion_items() {
