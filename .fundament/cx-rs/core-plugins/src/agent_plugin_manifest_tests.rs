...
#[test]
fn legacy_cx_overlay_keeps_portable_components_fixed() {
    let tmp = tempdir().expect("tempdir");
    let plugin_root = tmp.path().join("demo-plugin");
    write_agent_plugin_manifest(
        &plugin_root,
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".cx-plugin")).expect("create overlay dir");
    fs::write(
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline CX"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".cx-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".cx-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy CX"}}"#,
    )
    .expect("write overlay");
