...
pub(crate) fn remote_installed_plugin_bundle_sync_gate(cx_home: &Path) -> Arc<Semaphore> {
    let plugin_cache_root = remote_plugin_cache_root(cx_home);
    let gates =
...
pub fn mark_remote_plugin_cache_mutation_in_flight(
    ...
    let key = RemotePluginCacheMutationKey {
        plugin_cache_root: remote_plugin_cache_root(cx_home),
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = cx_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
...
fn remote_plugin_cache_root(cx_home: &Path) -> PathBuf {
    cx_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let metadata_path = PluginStore::new(cx_home.path().to_path_buf())
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let cx_home = tempfile::tempdir().expect("create cx home");
        let created_by_me_cached_manifest = cx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = cx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
