...
const PERSONAL_MARKETPLACE_NAME: &str = "cx-curated";
...
pub async fn checkout_remote_plugin_share(
    ...
    let local_paths = load_share_local_paths_for_checkout(cx_home)?;
...
fn load_share_local_paths_for_checkout(
    ...
    match local_paths::load_plugin_share_local_paths(cx_home) {
        Ok(paths) => Ok(paths),
