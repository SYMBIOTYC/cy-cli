...
pub(crate) fn load_plugin_share_local_paths(
    ...
    read_plugin_share_local_paths(cx_home)
}
...
pub(crate) fn record_plugin_share_local_path(
    ...
    let _guard = lock_plugin_share_local_paths()?;
    let mut mapping = read_plugin_share_local_paths_for_update(cx_home)?;
    mapping.insert(remote_plugin_id.to_string(), plugin_path);
    write_plugin_share_local_paths(cx_home, mapping)
}
...
fn read_plugin_share_local_paths_for_update(
    ...
    match read_plugin_share_local_paths(cx_home) {
...
fn plugin_share_local_paths_path(cx_home: &Path) -> std::path::PathBuf {
    cx_home.join(PLUGIN_SHARE_LOCAL_PATHS_FILE)
}
