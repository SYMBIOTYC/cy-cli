...
impl RemotePluginCatalogCacheKey {
    fn new(
        ...
        let cache_key = Self {
            ...
            gt_user_id: auth.get_gt_user_id(),
...
pub(crate) fn load_cached_directory_plugins(
    ...
    let cache_path = cache_path(cx_home, &cache_key);
...
pub(crate) fn write_cached_directory_plugins(
    ...
    let _ = cx_utils_path::write_atomically(&cache_path, &contents);
}
...
fn cache_path(cx_home: &Path, cache_key: &RemotePluginCatalogCacheKey) -> PathBuf {
