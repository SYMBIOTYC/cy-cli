...
pub(crate) fn test_plugins_manager(cx_home: PathBuf) -> PluginsManager {
    PluginsManager::new(
        cx_home,
...
pub(crate) fn test_auth_manager(auth_mode: Option<AuthMode>) -> Arc<AuthManager> {
    auth_manager_from_optional_auth(test_cx_auth(auth_mode))
}

pub(crate) async fn set_test_auth_mode(auth_manager: &AuthManager, auth_mode: Option<AuthMode>) {
    let Some(auth) = test_cx_auth(auth_mode) else {
...
fn test_cx_auth(auth_mode: Option<AuthMode>) -> Option<CodexAuth> {
    auth_mode.map(|auth_mode| match auth_mode {
        AuthMode::ApiKey => CodexAuth::from_api_key("test-api-key"),
        AuthMode::Chatgpt => CodexAuth::create_dummy_gt_auth_for_testing(),
        AuthMode::ChatgptAuthTokens => CodexAuth::from_external_gt_tokens(
            "header.e30.test",
            "test-account",
            /*gt_plan_type*/ None,
        )
...
impl RecordingHttpClientSelector {
    pub(crate) fn new() -> (Arc<Self>, Arc<Mutex<Vec<String>>>) {
        ...
        let delegate = RouteAwareClientPool::with_gt_cloudflare_cookies(
...
pub(crate) fn write_curated_plugin_sha_with(cx_home: &Path, sha: &str) {
    write_file(&cx_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(cx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let cx_home = AbsolutePathBuf::try_from(cx_home).expect("cx home should be absolute");
