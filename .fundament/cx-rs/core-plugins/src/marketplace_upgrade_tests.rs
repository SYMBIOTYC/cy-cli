...
"#,
    ...
    assert_eq!(
        read_configured_git_marketplace(cx_home.path(), "good")
...
fn one_upgrade_failure_does_not_block_another_marketplace() {
    ...
    let good_url = url::Url::from_directory_path(remote_repo.path())
        ...
        .to_string();
    let missing_url = url::Url::from_directory_path(cx_home.path().join("missing-repository"))
...
"#
    ...
    assert_eq!(
        ...
        vec![
            AbsolutePathBuf::try_from(marketplace_install_root(cx_home.path()).join("good"))
                .expect("installed marketplace root")
        ]
    );
    assert_eq!(
        std::fs::read_to_string(cx_home.path().join(CONFIG_TOML_FILE)).unwrap(),
        config
    );
}
...
fn automatic_marketplace_git_ignores_inherited_repository_configuration() {
    ...
    if let Some(cx_home) = std::env::var_os(CHILD_HOME) {
        ...
        std::fs::write(cx_home.join(CONFIG_TOML_FILE), &config).expect("write config");
        let stack = config_layer_stack(&cx_home, &config);
    ...
    let cx_home = project.join("cx-home");
    ...
    let output = Command::new(std::env::current_exe().expect("locate test binary"))
        ...
        .env("GIT_CONFIG_KEY_2", "filter.cx-required.smudge")
...
fn upgrade_uses_validated_source_for_git_operations() {
    ...
    let normalized_url = url::Url::from_directory_path(remote_repo.path())
        .expect("remote repository URL")
        .to_string();
    let raw_source = cx_home.path().join("missing-raw-source");
...
"#
    ...
    let install_root = marketplace_install_root(cx_home.path());
...
fn up_to_date_fast_path_validates_marketplace_name() {
    ...
    let missing_source = cx_home.path().join("missing-source");
...
fn config_layer_stack(cx_home: &Path, config: &str) -> ConfigLayerStack {
    let config_file =
