...
pub(super) fn record_added_marketplace_entry(
    ...
    record_user_marketplace(cx_home, marketplace_name, &update).map_err(|err| {
...
mod tests {
    ...
    fn installed_marketplace_root_for_source_propagates_config_read_errors() {
        ...
        let install_root = cx_home.path().join("marketplaces");
    ...
    fn installed_marketplace_root_for_source_uses_local_source_root() {
        let cx_home = TempDir::new().unwrap();
        let install_root = cx_home.path().join("marketplaces");
        let source_root = cx_home.path().join("source");
