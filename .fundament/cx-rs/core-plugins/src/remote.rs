...
use cx_http_client::RouteAwareRequestError;
...
pub const REMOTE_GLOBAL_MARKETPLACE_DISPLAY_NAME: &str = "oi Curated Remote";
...
impl PartialEq for RemotePluginServiceConfig {
    fn eq(&self, other: &Self) -> bool {
        self.gt_base_url == other.gt_base_url
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(cx_home, config, auth, scope, &plugins);
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(cx_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    let Ok(auth) = ensure_gt_auth(auth) else {
        return;
    };
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(cx_home, config, auth, *scope);
    }
}
...
pub async fn fetch_recommended_plugins(
    ...
    let mut url = Url::parse(&format!("{base_url}/ps/plugins/suggested/cx"))
...
pub(crate) fn has_fresh_cached_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_directory_plugins(cx_home, config, auth, scope).is_some_and(
        |cached| {
            matches!(
                cached.freshness,
...
pub(crate) fn cached_remote_plugin_catalog_scopes(
    ...
    RemotePluginScope::CATALOG_CACHE_SCOPES
        .into_iter()
        .filter(|scope| {
            catalog_cache::load_cached_directory_plugins(cx_home, config, auth, *scope).is_some()
        })
        .collect()
}
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(cx_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(cx_home.clone())
    ...
    let legacy_remote_plugin_cache_root = cx_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode == RemotePluginCatalogCacheMode::PreferCache
        && let Some(cx_home) = cx_home
        && let Some(cached) =
            catalog_cache::load_cached_directory_plugins(cx_home, config, auth, scope)
    {
...
fn remote_plugin_skill_detail_url(
    ...
    let mut url = Url::parse(config.gt_base_url.trim_end_matches('/'))
        .map_err(RemotePluginCatalogError::InvalidBaseUrl)?;
    {
...
fn ensure_gt_auth(auth: Option<&CodexAuth>) -> Result<&CodexAuth, RemotePluginCatalogError> {
