...
use crate::loader::configured_curated_plugin_ids_from_cx_home;
...
use cx_config::clear_user_plugin;
use cx_config::set_user_plugin_enabled;
use cx_config::skill_config_rules_from_stack;
use cx_config::types::PluginConfig;
use cx_config::types::ToolSuggestDisabledTool;
use cx_config::types::ToolSuggestDiscoverableType;
use cx_hooks::plugin_hook_declarations;
...
use cx_plugin::PluginIdError;
use cx_plugin::PluginTelemetryMetadata;
use cx_plugin::app_connector_ids_from_declarations;
use cx_plugin::prompt_safe_plugin_description;
...
use cx_skills::SkillRootLoader;
use cx_skills::SkillRootSnapshots;
use cx_tools::DiscoverablePluginInfo;
...
use cx_utils_plugins::PluginIdentity;
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            ...
            && self.auth.as_ref().and_then(CodexAuth::get_gt_user_id)
                == other.auth.as_ref().and_then(CodexAuth::get_gt_user_id)
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        gt_base_url: config.gt_base_url.clone(),
        account_id: auth.and_then(CodexAuth::get_account_id),
        gt_user_id: auth.and_then(CodexAuth::get_gt_user_id),
...
fn target_curated_marketplace(auth_mode: Option<AuthMode>) -> TargetCuratedMarketplace {
    if auth_mode.is_some_and(AuthMode::uses_cx_backend) {
...
impl PluginsManager {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given CX_HOME:
        ...
        // contents of that CX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single CX_HOME is only used by one product.
        let remote_installed_plugin_bundle_sync_gate =
            crate::remote::remote_installed_plugin_bundle_sync_gate(&cx_home);
        Self {
            cx_home: cx_home.clone(),
            store: PluginStore::new(cx_home),
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_cx_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_cx_backend()) else {
            return Vec::new();
        };
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.cx_home.as_path())
                    .ok_or_else(|| {
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(cx_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.cx_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(cx_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if cx_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let curated_marketplace_path = if include_openai_curated {
            match target_curated_marketplace(self.auth_mode()) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root = curated_plugins_repo_path(self.cx_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.cx_home.as_path());
