...
use cx_config::RemoveMarketplaceConfigOutcome;
use cx_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || remove_marketplace_sync(cx_home.as_path(), request))
        .await
        .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(cx_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(cx_home, &marketplace_name).map_err(|err| {
