...
pub enum RemotePluginMutationError {
    ...
    #[error("invalid gt base url for remote plugin mutation: {0}")]
    InvalidBaseUrl(#[source] url::ParseError),

    #[error("gt base url cannot be used for plugin mutation")]
    InvalidBaseUrlPath,
...
pub enum RemotePluginFetchError {
    #[error("invalid gt base url for remote featured plugin request: {0}")]
    InvalidBaseUrl(#[source] url::ParseError),
...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_cx_backend()) {
        request =
            request.headers(cx_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_cx_backend_auth(auth)?;
