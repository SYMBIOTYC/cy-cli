...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".cx-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(cx_home: PathBuf) -> Self {
        Self::try_new(cx_home)
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".cx-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    let (source_manifest_relative_path, source_manifest_contents) = match manifest {
        ...
        InstallManifest::Fallback(contents) => (
            PathBuf::from(".cx-plugin/plugin.json"),
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".cx-plugin/plugin.json");
