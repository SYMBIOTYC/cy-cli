...
use cx_config::MarketplaceAllowedSourceKind;
use cx_config::MarketplaceAllowedSourceToml;
use cx_config::RequirementSource;
use cx_config::types::MarketplaceConfig;
use cx_config::types::MarketplaceSourceType;
use cx_config::types::PluginConfig;
use cx_plugin::PluginId;
use cx_utils_absolute_path::AbsolutePathBuf;
use cx_utils_path::paths_match_after_normalization;
use regex::Regex;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(cx_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(cx_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, cx_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(cx_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, cx_home) else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(cx_home, path)
    {
...
fn managed_marketplace_name(
    ...
    ) && is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(cx_home))
    {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if is_expected_managed_path(root.as_path(), &curated_plugins_repo_path(cx_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(cx_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(cx_home, path)
}

fn managed_local_marketplace_name(cx_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = cx_home
...
fn is_expected_managed_path(path: &Path, expected: &Path) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        let path = cx_utils_absolute_path::normalize_windows_device_path(&path)
            .unwrap_or_else(|| path.into_owned());
        let expected = cx_utils_absolute_path::normalize_windows_device_path(&expected)
...
pub(crate) fn primary_runtime_marketplace_root() -> Option<PathBuf> {
    Some(
        primary_runtime_cache_dir()?
            .join("cx-runtimes/cx-primary-runtime/plugins")
