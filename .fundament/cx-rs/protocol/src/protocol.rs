//! Defines the protocol for a CX session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for CX:";
...
pub struct ConversationStartParams {
    /// Whether CX response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through CX when the session ends.
    ...
    /// Sends automatic CX responses as realtime conversation items instead of handoff appends.
    pub cx_responses_as_items: bool,
    /// Optional prefix added to automatic CX response items when `cx_responses_as_items` is set.
    pub cx_response_item_prefix: Option<String>,
    /// Selects how automatic CX handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub cx_response_handoff_mode: CodexResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub cx_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append CX's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to CX when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to CX when this realtime session ends.
...
pub enum Op {
    ...
    /// Request CX to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down cx instance.
    Shutdown,
...
/// running the command proposed by CX.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.cx`, `.git`, notably `.git/hooks`) under
...
/// CX errors that we expose to clients.
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub cx_error_info: Option<CodexErrorInfo>,
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub cx_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::CX),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older CX
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(cx_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
...
mod tests {
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        assert_eq!(
            SessionSource::Custom("gt".to_string()).restriction_product(),
        ...
        assert_eq!(
            SessionSource::Custom("cx".to_string()).restriction_product(),
            Some(Product::CX)
        );
        assert_eq!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = cx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            cx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".cx")).expect("create .cx");
        let canonical_cwd = cx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            cx_error_info: Some(CodexErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
