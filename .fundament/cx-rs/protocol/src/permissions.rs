...
use cx_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_CODEX_PATH_NAME: &str = ".cx";
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer CX degrades to warn-and-ignore
    /// instead of failing to load. CX 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "cx-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".cx");
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            // Example: if `<root>/.cx -> <root>/decoy`, bwrap must still see
            // `<root>/.cx`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.cx` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .cx read-only to the agent by
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "CX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn writable_roots_proactively_protect_missing_dot_codex() {
        ...
        let expected_dot_codex = expected_root.join(".cx");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".cx".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_cx_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_codex = expected_root.join(".cx");
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".cx".to_string()),
            "explicit .cx rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_codex),
            "explicit .cx rule should win over the default protected carveout"
        );
        assert!(
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_cx_config = cwd.path().join(".cx").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_cx_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_cx_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".cx/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let cx_dir = real_root.join(".cx");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&cx_dir).expect("create .cx");
        ...
        let expected_codex = link_root.join(".cx");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_codex = expected_root.join(".cx");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-cx");
        let dot_codex = root.join(".cx");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_codex).expect("create .cx symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let cx_dir = real_tmpdir.join(".cx");
