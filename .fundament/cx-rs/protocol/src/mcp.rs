...
//! `schemars`) so they can be embedded in CX's own protocol structures.
...
/// Extension ID for oi form elicitation.
...
/// MCP extensions supplied by the client that created a CX session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `cx-protocol` so other crates can convert
