//! Shared model metadata types exchanged between CX services and clients.
//!
...
/// Metadata describing a CX-supported model.
#[derive(Debug, Clone, Deserialize, Serialize, TS, JsonSchema, PartialEq)]
...
/// Model metadata returned by the CX backend `/models` endpoint.
...
impl ModelPreset {
    ...
    ///
    /// In gt mode, all models are visible. Otherwise, only API-supported models are shown.
    pub fn filter_by_auth(models: Vec<ModelPreset>, gt_mode: bool) -> Vec<ModelPreset> {
        models
            .into_iter()
            .filter(|model| gt_mode || model.supported_in_api)
            .collect()
    }
