...
use cx_otel::StatsigMetricsSettings;
use cx_windows_sandbox::SETUP_VERSION;
use cx_windows_sandbox::SetupErrorCode;
use cx_windows_sandbox::SetupErrorReport;
use cx_windows_sandbox::SetupFailure;
use cx_windows_sandbox::add_deny_write_ace;
use cx_windows_sandbox::convert_string_sid_to_sid;
use cx_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use cx_windows_sandbox::ensure_allow_write_aces;
use cx_windows_sandbox::extract_setup_failure;
use cx_windows_sandbox::hide_newly_created_users;
use cx_windows_sandbox::install_wfp_filters;
use cx_windows_sandbox::log_note;
use cx_windows_sandbox::log_writer;
use cx_windows_sandbox::path_mask_allows;
use cx_windows_sandbox::path_write_aces_need_refresh;
use cx_windows_sandbox::sandbox_bin_dir;
use cx_windows_sandbox::sandbox_dir;
use cx_windows_sandbox::sandbox_secrets_dir;
use cx_windows_sandbox::string_from_sid_bytes;
use cx_windows_sandbox::sync_persistent_deny_read_acls;
use cx_windows_sandbox::to_wide;
use cx_windows_sandbox::workspace_write_cap_sid_for_root;
use cx_windows_sandbox::workspace_write_root_overlaps_path;
use cx_windows_sandbox::write_setup_error_report;
...
fn lock_sandbox_dir(
    ...
    // ProvisionOnly accepts another user's CX_HOME; keep its ACL mutation
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(cx_home) = std::env::var("CX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&cx_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.cx_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.cx_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if writes_setup_marker {
        prepare_setup_marker(&payload.cx_home, &payload.real_user)?;
    }
    match payload.mode {
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.cx_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.cx_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.cx_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.cx_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(payload: &Payload, sandbox_group_sid: &[u8]) -> Result<()> {
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.cx_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.cx_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_cx_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.cx_home, &payload.command_cwd, root)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.cx`, and `.agents`.
        //
...
mod tests {
    ...
    use cx_otel::StatsigMetricsSettings;
    use cx_windows_sandbox::ensure_allow_mask_aces;
    use cx_windows_sandbox::ensure_allow_write_aces;
    use cx_windows_sandbox::load_or_create_cap_sids;
    use cx_windows_sandbox::path_mask_allows;
    use cx_windows_sandbox::path_write_aces_need_refresh;
    use cx_windows_sandbox::workspace_write_cap_sid_for_root;
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let cx_home = temp.path().join("cx-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&cx_home).expect("create cx home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&cx_home, &workspace, &workspace)
    ...
    fn write_root_refresh_checks_each_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&cx_home, &workspace, &workspace)
            .expect("workspace sid");
        let other_sid = workspace_write_cap_sid_for_root(&cx_home, &workspace, &other_root)
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let cx_home = temp.path().join("cx-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".cx");
