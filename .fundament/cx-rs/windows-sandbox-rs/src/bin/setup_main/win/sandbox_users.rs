...
use cx_windows_sandbox::dpapi_protect;
...
const SANDBOX_USERS_GROUP_COMMENT: &str = "CX sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(cx_home);
...
pub(super) fn prepare_setup_marker(cx_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(cx_home).join("setup_marker.json");
