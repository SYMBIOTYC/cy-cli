...
use anyhow::Context;
use anyhow::Result;
use cx_utils_pty::JobObject;
use cx_windows_sandbox::ConsoleMode;
use cx_windows_sandbox::ErrorPayload;
use cx_windows_sandbox::ErrorStage;
use cx_windows_sandbox::ExitPayload;
use cx_windows_sandbox::FramedMessage;
use cx_windows_sandbox::IPC_PROTOCOL_VERSION;
use cx_windows_sandbox::LocalSid;
use cx_windows_sandbox::Message;
use cx_windows_sandbox::OutputPayload;
use cx_windows_sandbox::OutputStream;
use cx_windows_sandbox::PipeSpawnHandles;
use cx_windows_sandbox::ResizePayload;
use cx_windows_sandbox::SpawnReady;
use cx_windows_sandbox::SpawnRequest;
use cx_windows_sandbox::StderrMode;
use cx_windows_sandbox::StdinMode;
use cx_windows_sandbox::WindowsSandboxTokenMode;
use cx_windows_sandbox::allow_null_device;
use cx_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use cx_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use cx_windows_sandbox::decode_bytes;
use cx_windows_sandbox::encode_bytes;
use cx_windows_sandbox::get_current_token_for_restriction;
use cx_windows_sandbox::hide_current_user_profile_dir;
use cx_windows_sandbox::log_note;
use cx_windows_sandbox::read_frame;
use cx_windows_sandbox::read_handle_loop;
use cx_windows_sandbox::spawn_process_with_pipes;
use cx_windows_sandbox::to_wide;
use cx_windows_sandbox::token_mode_for_permission_profile;
use cx_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with cx_exec_server::CODEX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--cx-run-as-fs-helper";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<cx_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.cx_home.clone();
    hide_current_user_profile_dir(req.cx_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = cx_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
