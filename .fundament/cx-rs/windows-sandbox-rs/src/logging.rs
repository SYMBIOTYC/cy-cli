...
pub fn current_log_file_path_for_cx_home(cx_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(cx_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_cx_home_uses_sandbox_dir() {
        let cx_home = Path::new("cx-home");

        assert_eq!(
            current_log_file_path_for_cx_home(cx_home),
            current_log_file_path(&cx_home.join(".sandbox"))
        );
    }
}
