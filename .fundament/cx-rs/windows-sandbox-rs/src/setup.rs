...
const SETUP_EXE_FILENAME: &str = "cx-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(cx_home: &Path) -> PathBuf {
    cx_home.join(".sandbox")
}

pub fn sandbox_bin_dir(cx_home: &Path) -> PathBuf {
    cx_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(cx_home: &Path) -> PathBuf {
    cx_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(cx_home: &Path) -> PathBuf {
    sandbox_dir(cx_home).join("setup_marker.json")
}

pub fn sandbox_users_path(cx_home: &Path) -> PathBuf {
    sandbox_secrets_dir(cx_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let deny_read_paths =
        setup_refresh_deny_read_paths(permission_profile, workspace_roots, command_cwd)?;
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, cx_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        cx_home: request.cx_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.cx_home)
    })
}

fn run_setup_refresh_payload(b64: &str, cx_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(cx_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(cx_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| cx_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(cx_home) {
        log_note(
...
fn gather_helper_read_roots(cx_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(cx_home);
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, cx_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<cx_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
fn report_helper_failure(
    ...
    match read_setup_error_report(cx_home) {
...
fn verify_setup_completed(cx_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(cx_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, cx_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(cx_home)?;
        if let Err(err) = clear_setup_error_report(cx_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.cx_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: cx_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.cx_home)
}
...
pub fn run_elevated_provisioning_setup(
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: cx_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, cx_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.cx_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, cx_home: &Path) -> Vec<PathBuf> {
    ...
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(cx_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(cx_home));
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        ...
        let err = verify_setup_completed(cx_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("cx.exe");
        let setup_exe = resources_dir.join("cx-windows-sandbox-setup.exe");
        fs::write(&exe, b"cx").expect("write exe");
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_cx_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let cx_home = user_profile.join("CodexHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&cx_home).expect("create cx home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &cx_home);
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&cx_home);
        fs::create_dir_all(&cx_home).expect("create cx home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_cx_home = dunce::canonicalize(&cx_home).expect("canonical cx home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_cx_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_codex).expect("canonical extra .cx"),
                explicit_deny,
            ]
            .into_iter()
