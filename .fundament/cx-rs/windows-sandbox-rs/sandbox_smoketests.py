...
# Run a suite of smoke tests against the Windows sandbox via the CX CLI
...
def _resolve_cx_cmd() -> List[str]:
    """Resolve the CX CLI to invoke `cx sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run CX.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "cx.exe",
        ws_root / "target" / "release" / "cx.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "cx.exe",
            cargo_base / "release" / "cx.exe",
        ])
    ...
    raise FileNotFoundError(
        "CX CLI not found. Build it first, e.g.\n"
        "  cargo build -p cx-cli --release\n"
        "or for debug:\n"
        "  cargo build -p cx-cli\n"
    )

CODEX_CMD = _resolve_cx_cmd()
print(CODEX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to cx CLI overrides.
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            proxy_home = WS_ROOT / ".cx_proxy_smoke"
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\cx_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.cx artifacts) denied
    cx_home = Path(os.environ["USERPROFILE"]) / ".cx"
    cap_sid_target = cx_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .cx\\policy.json"], WS_ROOT)
    add("WS: .cx cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .cx policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > cx_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://cx-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
