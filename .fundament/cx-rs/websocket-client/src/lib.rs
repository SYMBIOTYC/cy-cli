//! Proxy-aware WebSocket connection setup shared by CX API clients.
...
use cx_http_client::BuildCustomCaTransportError;
use cx_http_client::HttpClientFactory;
use cx_http_client::build_rustls_client_config_with_custom_ca;
use futures::Sink;
use futures::Stream;
...
/// Selects whether WebSocket TLS follows CX custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured CX custom CAs.
    ExplicitCodexTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured CX custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ...
    /// Creates a connector with explicit CX TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build CX TLS configuration when they establish their
