...
use cx_api::ResponsesClient;
use cx_api::ResponsesOptions;
use cx_client::HttpTransport;
use cx_client::Request;
use cx_client::RequestBody;
use cx_client::Response;
use cx_client::StreamResponse;
...
impl AuthProvider for StaticAuth {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        ...
        if let Ok(header) = HeaderValue::from_str(&self.account_id) {
            headers.insert("gt-Account-ID", header);
        }
    }
}
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, cx_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, cx_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
impl AuthProvider for FailsOnceAuth {
    ...
    fn apply_auth(&self, request: Request) -> cx_api::AuthProviderFuture<'_> {
...
async fn streaming_client_adds_auth_headers() -> Result<()> {
    ...
    let account_header = req.headers.get("gt-Account-ID");
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, cx_client::RequestCompression::None);
    Ok(())
}
