...
const OPENAI_FILE_USE_CASE: &str = "cx";
...
pub enum OpenAiFileError {
    ...
    #[error("failed to send oi file request to {url}: {source}")]
    Request {
        url: String,
        #[source]
    ...
    #[error(
        "oi file blob upload to {host} failed after {elapsed_ms} ms ({error_kind}, azure_client_request_id={azure_client_request_id}): {source}"
    )]
    BlobUploadRequest {
        host: String,
        elapsed_ms: u128,
    ...
    #[error(
        "oi file blob upload to {host} failed with status {status} (azure_client_request_id={azure_client_request_id}, azure_request_id={azure_request_id}, azure_error_code={azure_error_code})"
    )]
    BlobUploadStatus {
        host: String,
        status: StatusCode,
    ...
    #[error("oi file request to {url} failed with status {status}: {body}")]
    UnexpectedStatus {
        url: String,
        status: StatusCode,
        body: String,
    },
    #[error("failed to parse oi file response from {url}: {source}")]
    Decode {
        url: String,
        #[source]
        source: serde_json::Error,
    },
    #[error("oi file upload for `{file_id}` is not ready yet")]
    UploadNotReady { file_id: String },
    #[error("oi file upload for `{file_id}` failed: {message}")]
...
pub async fn upload_openai_file(
    ...
    let create_request = match hosted_upload {
        Some(context) => serde_json::json!({
            ...
            "file_size": file_size_bytes,
            "use_case": OPENAI_FILE_USE_CASE,
            "cx_connector_id": context.connector_id,
            "cx_action_name": context.action_name,
            "cx_model": context.model,
        }),
        None => create_request,
    };
    ...
    let upload_response = client_pool
        ...
        .map_err(|source| {
            ...
            tracing::event!(
                ...
                tracing::Level::WARN,
                event.name = "cx.openai_file_blob_upload_failed",
                ...
                "oi file blob upload transport failed"
            );
...
mod tests {
    ...
    impl AuthProvider for ChatGptTestAuth {
        fn add_auth_headers(&self, headers: &mut http::HeaderMap) {
            ...
            headers.insert("gt-Account-ID", HeaderValue::from_static("account_id"));
        }
    }

    fn gt_auth() -> ChatGptTestAuth {
        ChatGptTestAuth
    }
