...
const X_CODEX_TURN_STATE_HEADER: &str = "x-cx-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "cx_api::responses_websocket_timing";
...
const WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str = "x-cx-ws-stream-request-start-ms";
    ...
    loop {
        ...
        match message {
            Message::Text(text) => {
                ...
                if event.kind() == "cx.response.metadata"
                    && let Some(etag) =
                        event
                            .headers
                            .as_ref()
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='cx_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-cx-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-cx-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            ...
            "headers": {
                "x-cx-safety-buffering-enabled": "true",
                "x-cx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "cx.response.metadata",
            "headers": {
                "x-cx-safety-buffering-enabled": "false",
                "x-cx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
