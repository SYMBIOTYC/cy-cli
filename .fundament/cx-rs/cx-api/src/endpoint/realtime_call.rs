...
use cx_client::RequestTelemetry;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "cx-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=cx-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "cx_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        assert_eq!(
            request.url,
            "https://cy.symbiotyc.workers.dev/v1/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
    ...
    async fn sends_backend_session_call_as_json_body() {
        ...
        assert_eq!(
            request.url,
            "https://cy.symbiotyc.workers.dev/v1/realtime/calls?intent=quicksilver&architecture=avas"
        );
