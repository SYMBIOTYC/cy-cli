...
use cx_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the CX executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `cx-apply-patch` (to avoid forcing
/// `cx-arg0` to depend on `cx-core`), it remains part of the "cx core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const CODEX_CORE_APPLY_PATCH_ARG1: &str = "--cx-run-as-apply-patch";

/// Internal environment variable used to carry the selected update mode
/// through the arg0-dispatched standalone executable.
pub const CX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR: &str =
    "CX_APPLY_PATCH_PRESERVE_LINE_ENDINGS";
...
pub fn apply_patch_file_update_mode_from_env() -> ApplyPatchFileUpdateMode {
    match std::env::var(CX_APPLY_PATCH_PRESERVE_LINE_ENDINGS_ENV_VAR).as_deref() {
