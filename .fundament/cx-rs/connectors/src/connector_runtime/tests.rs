...
fn create_cx_apps_tools_cache_context(
    ...
    ConnectorRuntimeManager::<TestTool>::default().context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: gt_user_id.map(ToOwned::to_owned),
...
#[test]
fn cx_apps_tools_cache_is_overwritten_by_last_write() {
    ...
    let cached_gateway_1 =
        read_cached_cx_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_cx_apps_tools(&cache_context, &tools_gateway_2).expect("write second cache");
    let cached_gateway_2 =
        read_cached_cx_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn cx_apps_tools_cache_is_scoped_per_user() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    ...
    let read_user_2 =
        read_cached_cx_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn cx_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_cx_apps_tools(&cache_context, &tools).expect("write cache");
    let cached = read_cached_cx_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn cx_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert!(read_cached_cx_apps_tools(&cache_context).is_none());
}

#[test]
fn cx_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_cx_apps_tools_loads_from_disk_cache() {
    let cx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
#[test]
fn startup_cached_cx_apps_tools_loads_without_server_info_cache() {
    let cx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn cx_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let startup_cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
#[test]
fn cx_apps_tools_cache_context_does_not_reread_disk_after_creation() {
    let cx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert_eq!(
        read_cached_cx_apps_tools(&writer_cache_context).expect("disk tools")[0].callable_name,
        "updated"
    );
}

#[test]
fn cx_apps_tools_cache_publishes_newest_shared_snapshot() {
    let cx_home = tempdir().expect("tempdir");
    ...
    let cache_context_1 = cache.context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: Some("user-one".to_string()),
            is_workspace_account: false,
        },
    );
...
#[test]
fn cx_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let cx_home = tempdir().expect("tempdir");
    let cx_home_file = cx_home.path().join("not-a-directory");
    std::fs::write(&cx_home_file, b"occupied").expect("create cx home file");
...
fn connector_runtime_without_cache_ignores_disk_state() {
    let cx_home = tempdir().expect("tempdir");
    let writer = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
fn cx_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    ...
    let user_two_context = cache.context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: Some("user-two".to_string()),
            is_workspace_account: false,
        },
    );
    let cache_paths = [
...
fn contexts_for_different_identities_keep_isolated_snapshots() {
    ...
    let context_a = manager.context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: Some("user-a".to_string()),
            is_workspace_account: false,
        },
    );
    ...
    let context_b = manager.context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: Some("user-b".to_string()),
            is_workspace_account: false,
        },
    );
...
fn oversized_tools_cache_is_ignored_during_initial_load() {
    let cx_home = tempdir().expect("tempdir");
    let context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
fn personal_and_workspace_contexts_are_distinct_even_with_matching_ids() {
    ...
    let personal_context = manager.context(
        ...
        ConnectorRuntimeContextKey {
            ...
            gt_user_id: Some("user".to_string()),
            is_workspace_account: false,
        },
    );
