...
use cx_rollout::INTERACTIVE_SESSION_SOURCES;
use cx_rollout::RolloutItem;
use cx_rollout::should_persist_response_item_for_memories;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<cx_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            cx_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<cx_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> cx_protocol::error::Result<String> {
        let filtered = items
            .iter()
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: cx_protocol::models::FunctionCallOutputPayload {
                        body: cx_protocol::models::FunctionCallOutputBody::Text(
