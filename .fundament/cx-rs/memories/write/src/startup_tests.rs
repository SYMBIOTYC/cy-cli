...
use cx_git_utils::reset_git_repository;
use cx_login::AuthManager;
use cx_login::CodexAuth;
use cx_model_provider::ModelProvider;
use cx_model_provider::ModelProviderFuture;
use cx_model_provider::ProviderAccountResult;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by CX before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
-> anyhow::Result<()> {
    ...
    core_test_support::submit_thread_settings(
        ...
        cx_protocol::protocol::ThreadSettingsOverrides {
            ...
            permission_profile: Some(cx_protocol::models::PermissionProfile::workspace_write()),
            ..Default::default()
        },
    )
    .await?;
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_cx_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_cx_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.cx_home);
...
async fn build_test_codex(
    ...
    build_test_cx_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_cx_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestCodex> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<cx_state::StateRuntime>> {
    let db = cx_state::StateRuntime::init(
...
async fn trigger_memories_startup(test: &TestCodex) {
    let config_snapshot = test.cx.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<cx_core::config::Config>) {
    let config_snapshot = test.cx.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
async fn seed_stage1_output(
    ...
    let mut metadata_builder = cx_state::ThreadMetadataBuilder::new(
        thread_id,
        cx_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = cx_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = cx_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<cx_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.cx.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        cx_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_codex(test: &TestCodex) -> anyhow::Result<()> {
    ...
    wait_for_event(&test.cx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
