...
Runtime orchestration for Phase 1 and Phase 2 still lives in `cx-core` under
`cx-rs/core/src/memories/`.

## Crates

- `cx-rs/memories/read` (`cx-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `cx-rs/memories/write` (`cx-memories-write`) owns the write path:
...
- In `cx`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.cx/memories/.git` by `cx-git-utils`
