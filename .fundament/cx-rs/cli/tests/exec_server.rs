...
use cx_exec_server::ExecServerClient;
use cx_exec_server::NoiseChannelIdentity;
use cx_exec_server::NoiseChannelPublicKey;
use cx_exec_server::NoiseRendezvousConnectArgs;
...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let cx_home = TempDir::new()?;
    std::fs::write(cx_home.path().join("config.toml"), "not valid toml = [")?;
...
"#
    ...
    client
        .exec(ExecParams {
            ...
            env_policy: Some(cx_exec_server::ExecEnvPolicy {
                inherit: cx_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let cx_bin = cx_utils_cargo_bin::cargo_bin("cx")?;
    let cx_home = cx_home.path().to_path_buf();
