    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    let initialized_response = json!({
        "id": initialize["id"],
        "result": {
            "userAgent": "cx_cli_rs/0.0.0-test",
            "codexHome": cx_home,
        },
    });
    websocket
...
fn queue_rejects_empty_message() -> Result<()> {
    let output = std::process::Command::new(cx_utils_cargo_bin::cargo_bin("cx")?)
...
async fn run_remote_queue_command(response: QueueResponse) -> Result<(Output, Value)> {
    ...
    let server_home = cx_home.path().to_path_buf();
...
async fn queue_rejects_local_daemon_that_does_not_support_queueing() -> Result<()> {
    let cx_home = tempfile::tempdir_in("/tmp")?;
    let socket_path = cx_app_server::app_server_control_socket_path(cx_home.path())?;
    ...
    assert!(!cx_home.path().join("queue_1.sqlite").exists());
    Ok(())
}
