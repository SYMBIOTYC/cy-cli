...
fn write_file_auth_config(cx_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(cx_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(cx_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(cx_home.path())?;
    ...
    let auth = read_auth_json(cx_home.path())?;
...
fn login_status_reports_auth_storage_errors() -> Result<()> {
    let cx_home = TempDir::new()?;
    write_file_auth_config(cx_home.path())?;
    std::fs::write(cx_home.path().join("auth.json"), "{invalid json")?;
