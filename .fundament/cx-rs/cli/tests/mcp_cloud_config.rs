...
impl CloudManagedMcpFixture {
    ...
    async fn output(&self, args: &[&str]) -> Result<Output> {
        let output = self.command(args)?.output().await?;
            ...
            "cx {} failed with status {}: stdout={}; stderr={}",
            args.join(" "),
            output.status,
    ...
    fn assert_user_config_unchanged(&self) -> Result<()> {
        assert_eq!(
            std::fs::read_to_string(self.cx_home.path().join("config.toml"))?,
            self.user_config
        );
        Ok(())
    }
...
async fn login_and_logout_persist_only_cloud_managed_mcp_oauth_credentials() -> Result<()> {
    ...
    let credentials_path = fixture.cx_home.path().join(".credentials.json");
...
async fn add_and_remove_do_not_copy_or_delete_cloud_managed_mcp_servers() -> Result<()> {
    ...
    fixture
        ...
        .await?;
    let local_servers = load_global_mcp_servers(fixture.cx_home.path()).await?;
    ...
    assert!(
        load_global_mcp_servers(fixture.cx_home.path())
            .await?
            .is_empty()
    );
    fixture
