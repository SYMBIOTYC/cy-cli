...
fn write_installed_marketplace(cx_home: &Path, marketplace_name: &str) -> Result<()> {
    let root = marketplace_install_root(cx_home).join(marketplace_name);
...
async fn marketplace_remove_deletes_config_and_installed_root() -> Result<()> {
    ...
    record_user_marketplace(cx_home.path(), "debug", &configured_marketplace_update())?;
    write_installed_marketplace(cx_home.path(), "debug")?;
...
async fn marketplace_remove_json_prints_remove_outcome() -> Result<()> {
    ...
    record_user_marketplace(cx_home.path(), "debug", &configured_marketplace_update())?;
    write_installed_marketplace(cx_home.path(), "debug")?;
    let installed_root = marketplace_install_root(cx_home.path()).join("debug");
