...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_cx_app_path(&cx_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_cx_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_cx_to_user_applications(&download_url)
        .await
    ...
    open_cx_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_cx_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["gt.app", "CX.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_cx_app_bundle(candidate))
}

fn cx_app_search_dirs() -> Vec<PathBuf> {
...
fn is_cx_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
...
async fn open_cx_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = cx_new_thread_url(workspace);
...
fn cx_new_thread_url(workspace: &Path) -> String {
    let workspace = workspace.as_os_str().to_string_lossy();
    let mut serializer = url::form_urlencoded::Serializer::new(String::new());
    serializer.append_pair("path", workspace.as_ref());
    let query = serializer.finish();
    format!("cx://threads/new?{query}")
}

async fn download_and_install_cx_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("cx-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("CX.dmg");
...
async fn install_cx_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("CX.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install CX.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install CX.app to any applications directory");
}
...
fn find_cx_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("CX.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
#[cfg(test)]
mod tests {
    use super::cx_new_thread_url;
    use super::find_existing_cx_app_path;
    ...
    #[test]
    fn finds_gt_app_with_cx_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("gt.app");
        write_app_bundle(&app_path, "com.openai.cx");

        assert_eq!(
            find_existing_cx_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }

    #[test]
    fn ignores_classic_gt_app() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        write_app_bundle(&temp_dir.path().join("gt.app"), "com.openai.chat");
        let cx_app_path = temp_dir.path().join("CX.app");
        write_app_bundle(&cx_app_path, "com.openai.cx");
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex\t/Volumes/CX\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex Installer\t/Volumes/CX Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/CX Installer")
        );
    }

    #[test]
    fn cx_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&cx_new_thread_url(Path::new("/tmp/cx workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/cx workspace/#1".to_string())],
            )
        );
    }
}
