...
pub async fn run_windows_app_open_or_install(
    ...
    if cx_app_is_installed().await? {
        eprintln!("Opening workspace {display_workspace} in the Desktop app...");
        open_url(&cx_new_thread_url(&workspace_path)).await?;
        return Ok(());
    }
...
async fn cx_app_is_installed() -> anyhow::Result<bool> {
    // This package identity is stable across CX- and gt-branded builds.
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg(
            "Get-StartApps | Where-Object AppID -Like 'oi.Codex_*!App' | Select-Object -First 1 -ExpandProperty AppID",
        )
        .output()
        .await
...
fn cx_new_thread_url(workspace: &str) -> String {
...
mod tests {
    ...
    fn display_workspace_path_removes_windows_extended_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\C:\Users\fcoury\code\cx")),
            r"C:\Users\fcoury\code\cx"
        );
    }
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\cx")),
            r"\\server\share\cx"
        );
    }
    ...
    #[test]
    fn cx_new_thread_url_encodes_windows_workspace_path() {
        assert_eq!(
            cx_new_thread_url(r"C:\Users\akuma\repos\koba"),
            r"cx://threads/new?path=C%3A%5CUsers%5Cakuma%5Crepos%5Ckoba"
        );
    }

    #[test]
    fn cx_new_thread_url_preserves_verbatim_workspace_path() {
        assert_eq!(
            cx_new_thread_url(r"\\?\C:\Users\akuma\repos\koba"),
            r"cx://threads/new?path=%5C%5C%3F%5CC%3A%5CUsers%5Cakuma%5Crepos%5Ckoba"
        );
    }
}
