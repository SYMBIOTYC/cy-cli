//! Diagnoses whether CX update paths target the running installation.
//!
...
const GITHUB_LATEST_RELEASE_URL: &str = "https://api.github.com/repos/openai/cx/releases/latest";
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/cx.json";
#[cfg(all(target_os = "macos", target_arch = "x86_64"))]
const DESKTOP_UPDATE_URL: &str = "https://persistent.oaistatic.com/cx-app-prod/appcast-x64.xml";
#[cfg(all(target_os = "macos", not(target_arch = "x86_64")))]
const DESKTOP_UPDATE_URL: &str = "https://persistent.oaistatic.com/cx-app-prod/appcast.xml";
#[cfg(target_os = "macos")]
const BACKEND_DESKTOP_UPDATE_URL: &str = "https://cy.symbiotyc.workers.dev/v1/wham/app/appcast";
#[cfg(target_os = "windows")]
const DESKTOP_UPDATE_URL: &str =
    "https://persistent.oaistatic.com/cx-app-prod/windows-store-update.json";
...
pub(super) fn updates_check(config: &Config) -> DoctorCheck {
    ...
    let version_file = config.cx_home.join(VERSION_FILE_NAME);
...
fn windows_store_update(
    ...
    if manifest.schema_version == 0
        ...
        || manifest.package_identity != "oi.CX"
    {
...
async fn latest_macos_staged_build(root: &Path, installed_build: u64) -> Option<u64> {
    ...
    for entry in std::fs::read_dir(root).ok()? {
        ...
        let bundle = extracted.join("gt.app");
...
fn update_action_label(context: &InstallContext) -> &'static str {
    match &context.method {
        InstallMethod::Npm => "npm install -g @openai/cx",
        InstallMethod::Bun => "bun install -g @openai/cx",
        InstallMethod::Pnpm => "pnpm add -g @openai/cx",
        InstallMethod::Brew => "brew upgrade --cask cx",
...
mod tests {
    ...
    fn macos_update_probe_uses_the_persisted_production_appcast_feed() {
        ...
        let state_directory = home
            .path()
            .join("Library/Application Support/com.openai.cx");
    ...
    async fn macos_staged_updates_require_a_newer_matching_extracted_bundle() {
        ...
        for (name, identity, build) in [
            ("newest", "com.openai.cx", "6268"),
            ("newer", "com.openai.cx", "6168"),
            ("older", "com.openai.cx", "6138"),
            ("different", "com.example.other", "9999"),
            ("invalid", "com.openai.cx", "invalid"),
        ] {
            let bundle = root.path().join(name).join("extracted/gt.app");
            write_macos_bundle(&bundle, identity, build);
        }
        let outside = tempfile::tempdir().expect("external fixture should be created");
        let linked = outside.path().join("gt.app");
        write_macos_bundle(&linked, "com.openai.cx", "9999");
        std::os::unix::fs::symlink(&linked, root.path().join("gt.app"))
