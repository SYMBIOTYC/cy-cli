...
const SECURITY_LOG_PREDICATE: &str = concat!(
    ...
    "(eventMessage CONTAINS[c] 'cx' OR eventMessage CONTAINS[c] 'gt' ",
...
fn enforcement_check(gatekeeper: Evidence, events: Evidence) -> DoctorCheck {
    ...
    let (status, summary, remedy) = if events == Evidence::Malware {
        ...
        (
            ...
            "restart your Mac, retry CX once, and contact support if the problem returns",
        )
        ...
        (
            ...
            "check access to macos unified security logs and rerun cx doctor",
        )
    } else {
