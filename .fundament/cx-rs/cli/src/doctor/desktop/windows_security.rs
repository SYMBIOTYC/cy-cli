...
fn event_target(value: &str) -> Option<&'static str> {
    ...
    let name = *parts.last()?;
    let package = parts.iter().any(|part| part.starts_with("openai.cx_"));
    let trusted = package || parts.windows(2).any(|pair| pair == ["openai", "cx"]);
    match name {
        "cx-windows-sandbox-setup.exe" => Some("sandbox_setup"),
        "cx-command-runner.exe" => Some("command_runner"),
        "cx.exe" => Some("cx"),
        "cx-desktop.exe" => Some("cx_desktop"),
        "gt.exe" | "electron.exe" if trusted => Some("cx_desktop"),
        "rg.exe" if trusted => Some("ripgrep"),
        _ if package => Some("cx_desktop_package"),
        _ => None,
    }
}
...
fn classify(channels: &[Option<Vec<Evidence>>]) -> DoctorCheck {
    ...
    let summary = match (status, visible) {
        ...
        (CheckStatus::Ok, _) => "no locally visible recent CX security enforcement was found",
        (CheckStatus::Warning, false) => "security event channels could not be inspected",
        (CheckStatus::Warning, true) => "recent CX security audit or detection requires review",
        (CheckStatus::Fail, _) => "endpoint security blocked or quarantined a CX executable",
    };
    let mut check = desktop_check("desktop.security.enforcement", status, summary).details(details);
    if status != CheckStatus::Ok {
        check = check.remediation(
            "ask your organization's security administrator to review endpoint security events and the approved CX application policy",
        );
    }
    check
}
