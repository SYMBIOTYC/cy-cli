...
pub(super) fn endpoint_check(inspection: EndpointInspection) -> DoctorCheck {
    ...
    let targets = if cfg!(target_os = "windows") {
        "signed CX app; cx.exe; cx-windows-sandbox-setup.exe; cx-command-runner.exe; cx-mode-host.exe"
    } else if cfg!(target_os = "macos") {
        "signing team 2DC432GLL2 plus the installed CX app identity, signed cx agent, and required helpers"
    } else {
        "verified CX app, cx agent, and required helpers"
    };
    let mut check = DoctorCheck::new(
        "security.endpoint",
        "security",
        CheckStatus::Warning,
        "endpoint protection detected; CX exclusions are unverified",
    )
    .detail(format!("endpoint products: {}", products.join(", ")))
    .detail(format!("exclusion targets: {targets}"))
    .detail("CX exclusions: not verified")
    .remediation(
        "ask your security administrator to verify CX exclusions and required helper allowances",
    );
    ...
    for product in products {
        let remedy = match product {
            ...
            "Microsoft Defender" => {
                "Microsoft Defender: Add a certificate or executable-path exclusion for CX and its helpers. If Attack Surface Reduction blocks CX, add a rule exclusion. If Controlled Folder Access blocks CX, allow the app."
            }
            "SentinelOne" => {
                "SentinelOne: Add a signer, file-hash, or executable-path exclusion for CX and its helpers."
            }
            "Jamf Protect" => {
        ...
        check = check.issue(
            DoctorIssue::new(
                CheckStatus::Warning,
                format!("{product} can interfere with CX. Verify CX exclusions."),
            )
            .measured("not verified")
            .expected("CX application and helper exclusions")
            .remedy(remedy)
            .field("CX exclusions"),
        );
    }
