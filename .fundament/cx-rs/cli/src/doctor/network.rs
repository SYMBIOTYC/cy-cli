...
#[cfg(target_os = "macos")]
use cx_http_client::MacosSystemProxyConfiguration;
use cx_http_client::RouteAwareClientPool;
use cx_http_client::RouteAwareRequestError;
use cx_http_client::RouteFailureClass;
#[cfg(target_os = "macos")]
use cx_http_client::macos_system_proxy_configuration;
...
pub(super) fn check(config: Option<&Config>) -> DoctorCheck {
    ...
    #[cfg(target_os = "macos")]
    {
        let request_url = config
            ...
            .or_else(|| config.map(|config| config.gt_base_url.as_str()))
...
pub(super) fn with_system_proxy_remediation(
        ...
        })
    {
        check.remediation = Some(
            "A macOS system proxy is configured but unused. If your organization requires it, ask your administrator whether to enable the under-development feature with `cx features enable respect_system_proxy`."
                .to_string(),
        );
    }
    check
