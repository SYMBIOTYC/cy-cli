...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.cx_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match cx_app_server::app_server_control_socket_path(&config.cx_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run cx app-server daemon version for more details.");
    }
    check
}
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match cx_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    async fn test_config(cx_home: PathBuf) -> Config {
