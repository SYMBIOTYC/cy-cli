...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run cx doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.cx_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.cx_version);
...
fn auth_reachability_note(report: &DoctorReport) -> Option<DoctorNote> {
    ...
    if auth_mode_lower.contains("gt") && reachability_mode_lower.contains("api key") {
        return Some(DoctorNote {
            status: DisplayStatus::Warning,
            name: "auth".to_string(),
            summary: "mixed auth signals: gt login plus API key env var; HTTP reachability uses API-key mode".to_string(),
        });
    }
    None
}
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: cx"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
...
Notes
   ...
   ✗ auth         token expired - Run `cx login`.
─────────────────────────────────────────────────────────────
...
",
    ...
    fn render_human_report_snapshot_covers_environment_rows() {
        ...
        *targets = "exclusion targets: verified CX app and required helpers".into();
        ...
        report.checks.extend([
            ...
            .detail("running: true")
            .detail("log directory: $HOME/Library/Logs/com.openai.cx"),
            DoctorCheck::new(
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `cx login`.
-------------------------------------------------------------
...
",
    ...
    fn render_human_report_promotes_notes_without_changing_statuses() {
        ...
        assert!(rendered.contains(
            "⚠ auth         mixed auth signals: gt login plus API key env var; HTTP reachability uses API-key mode"
        ));
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/cx/target/debug/cx · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/cx/target/debug/cx"));
    ...
    fn redact_detail_preserves_secret_presence_booleans() {
        assert_eq!(
            redact_detail("stored gt tokens: true"),
            "stored gt tokens: true"
        );
        assert_eq!(
            redact_detail("stored gt tokens: false"),
            "stored gt tokens: false"
        );
    }
