...
use cx_sandboxing::landlock::allow_network_for_proxy;
use cx_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use cx_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use cx_sandboxing::seatbelt::create_seatbelt_command_args;
use cx_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _cx_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<cx_mcp::SandboxState>)
        .transpose()
    ...
    let cx_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
    ...
    let network = network_proxy
        .as_ref()
        .map(cx_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Landlock => {
            ...
            let cx_linux_sandbox_exe = config
                .cx_linux_sandbox_exe
                .expect("cx-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use cx_windows_sandbox::WindowsSandboxProxySettingsMode;
    use cx_windows_sandbox::WindowsSandboxSessionRequest;
    use cx_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        cx_home: config.cx_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = cx_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_cx_home(
        cli_overrides,
        cx_linux_sandbox_exe,
        options,
        /*cx_home*/ None,
...
async fn load_debug_sandbox_config_with_cx_home(
    cli_overrides: Vec<(String, TomlValue)>,
    cx_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `cx sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            cx_linux_sandbox_exe: cx_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        cx_home.clone(),
        managed_requirements_mode,
...
"#;
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = cx_home.path().join("work.config.toml");
