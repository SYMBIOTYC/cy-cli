//! Implements the `cx doctor` diagnostic report.
//!
...
use clap::Parser;
use cx_api::ApiError;
use cx_api::ResponsesWebsocketClient;
use cx_api::is_azure_responses_provider;
...
use cx_install_context::CodexPackageLayout;
use cx_install_context::InstallContext;
use cx_install_context::InstallMethod;
...
use cx_terminal_detection::Multiplexer;
...
const OPENAI_BETA_HEADER: &str = "oi-Beta";
...
/// Options for building a local CX diagnostic report.
///
...
///
/// This is the CLI entry point for cx doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("auth", progress.clone(), || match &auth_manager_result {
                        ...
                        .remediation(
                            "Fix the reported authentication error, then rerun cx doctor.",
                        ),
                    })
                },
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun cx doctor.")
                    })
                },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        cx_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load CX config")
}
...
/// JSON support report emitted by `cx doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        cx_version: report.cx_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH cx entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH cx #{}: {path}", index + 1)),
        );
    }
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("cx");
...
fn cx_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["cx"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "cx"]);
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.cx_home.join(cx_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.cx_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run cx login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no CX credentials were found",
        )
        .details(details)
        .remediation("Run cx login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run cx login again."),
    }
}
...
fn provider_specific_auth_check(
    ...
    details.push(format!(
        "model provider requires oi auth: {requires_openai_auth}"
    ));
    ...
    match provider_env_key {
        ...
        None => Some(
            DoctorCheck::new(
                ...
                "auth",
                CheckStatus::Ok,
                "oi auth is not required for the active model provider",
            )
            .details(details),
        ),
    }
...
fn stored_auth_mode(auth: &cx_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => "api_key",
        AuthMode::Chatgpt => "gt",
        AuthMode::ChatgptAuthTokens => "gt_auth_tokens",
    ...
    match stored_auth_mode_value(auth) {
            ...
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
                        issues.push("gt auth is missing an access token");
                    }
                    if tokens.refresh_token.trim().is_empty() {
                        issues.push("gt auth is missing a refresh token");
                    }
                }
                None => issues.push("gt auth is missing token data"),
            }
            if auth.last_refresh.is_none() {
                issues.push("gt auth is missing refresh metadata");
            }
        }
        AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
                        issues.push("external gt auth is missing an access token");
                    }
                    if tokens.account_id.is_none() && tokens.id_token.gt_account_id.is_none() {
                        issues.push("external gt auth is missing a gt account id");
                    }
                }
                None => issues.push("external gt auth is missing token data"),
            }
            if auth.last_refresh.is_none() {
                issues.push("external gt auth is missing refresh metadata");
            }
        }
        AuthMode::Headers => {
...
async fn state_check(config: &Config) -> DoctorCheck {
    ...
    rollout_stats_details(&mut details, &config.cx_home);
...
async fn sqlite_integrity_detail(
    ...
    match cx_state::sqlite_integrity_check(sqlite, path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, cx_home: &Path) {
    let active = collect_rollout_stats(&cx_home.join("sessions"));
    let archived = collect_rollout_stats(&cx_home.join("archived_sessions"));
...
fn provider_reachability_plan_from_parts(
    ...
    let endpoints = match (mode, provider_base_url) {
        ...
        (ProviderAuthReachabilityMode::Chatgpt, None) => vec![ReachabilityEndpoint {
            label: "gt".to_string(),
            url: provider_url_for_path(gt_base_url, "cx/responses", provider_query_params),
            required: true,
...
async fn provider_reachability_check(plan: ReachabilityPlan) -> DoctorCheck {
    ...
    let client = RouteAwareClientPool::with_gt_cloudflare_cookies_without_request_logging(
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/cx");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("cx")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/cx");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("cx"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].cx_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            cx_self_exe: Some(PathBuf::from("/bin/cx")),
            cx_linux_sandbox_exe: Some(PathBuf::from("/bin/cx-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/cx-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.cx_self_exe, arg0_paths.cx_self_exe);
        assert_eq!(
            overrides.cx_linux_sandbox_exe,
            arg0_paths.cx_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn provider_specific_auth_allows_non_openai_provider_without_env_key() {
        ...
        .expect("non-oi provider should produce a provider-specific check");
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running CX."),
            Vec::new(),
            |_| false,
        )
        .expect("non-oi provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running CX.".to_string())
        );
    }
    ...
    fn stored_auth_validation_rejects_missing_gt_tokens() {
        ...
        assert_eq!(
            stored_auth_issues(&auth, |_| false),
            vec![
                "gt auth is missing token data",
                "gt auth is missing refresh metadata",
            ]
        );
    }
    ...
    fn provider_reachability_adds_models_route_probe_for_openai_compatible_base_urls() {
        ...
        for (mode, description) in [
            ...
            (ProviderAuthReachabilityMode::Chatgpt, "gt auth"),
        ] {
            assert_eq!(
                provider_reachability_plan_from_parts(
                    mode,
    ...
    #[test]
    fn provider_reachability_api_key_does_not_require_gt() {
        let plan = provider_reachability_plan_from_parts(
            ProviderAuthReachabilityMode::ApiKey,
            "openai",
            "oi",
    ...
    #[test]
    fn provider_reachability_gt_uses_inference_endpoint() {
        assert_eq!(
            default_reachability_plan().endpoints,
            vec![ReachabilityEndpoint {
                label: "gt".to_string(),
                url: "https://cy.symbiotyc.workers.dev/v1/responses".to_string(),
                required: true,
                route_probe_url: None,
            }]
        );
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-cx-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-cx-doctor-mcp\" is not resolvable",
            )
        }));
    }
