...
use cx_exec_server::HttpClient;
use cx_exec_server::RouteAwareHttpClient;
use cx_login::AuthManager;
use cx_mcp::McpOAuthLoginSupport;
use cx_mcp::McpRuntimeContext;
use cx_mcp::ResolvedMcpOAuthScopes;
use cx_mcp::apply_http_headers_helper;
use cx_mcp::compute_auth_statuses;
...
use cx_rmcp_client::delete_oauth_tokens;
use cx_rmcp_client::perform_oauth_login;
use cx_utils_cli::CliConfigOverrides;
use cx_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.cx/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "cx mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: cx_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: cx_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    ConfigEditsBuilder::new(&cx_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", cx_home.display()))?;
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `cx mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: cx mcp remove {}", get_args.name);
