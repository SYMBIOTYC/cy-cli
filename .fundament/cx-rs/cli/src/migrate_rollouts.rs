...
use cx_thread_store::RolloutMigrationMode;
use cx_thread_store::RolloutMigrationOptions;
use cx_thread_store::RolloutMigrationProgress;
use cx_thread_store::RolloutMigrationReport;
...
pub(crate) async fn run(
    ...
    cx_core::otel_init::record_process_start(otel.as_ref(), "cx_migrate_rollouts");
    ...
    let state_db = if mode == RolloutMigrationMode::Apply {
        Some(
            cx_rollout::state_db::try_init(&config)
                .await
...
fn print_human_report(
    ...
    if mode == RolloutMigrationMode::DryRun && counts.eligible > 0 {
        println!("Run `cx migrate-rollouts --apply` to migrate eligible sessions.");
    }
...
fn print_outcome(outcome: &cx_thread_store::RolloutMigrationOutcome) {
...
async fn thread_storage_bytes(cx_home: &Path, thread_history_db_path: &Path) -> io::Result<u64> {
    let mut bytes = 0_u64;
    let mut directories = vec![
        cx_home.join(cx_rollout::SESSIONS_SUBDIR),
        cx_home.join(cx_rollout::ARCHIVED_SESSIONS_SUBDIR),
    ];
