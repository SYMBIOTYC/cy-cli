...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run CX after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the CX user.
    #[arg(
        long = "current-user",
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`cx sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .cx_home(identity.cx_home.clone())
        .fallback_cwd(Some(identity.cx_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_cx_home(
        ...
        .build()
        .await
        .context("failed to load target user's CX config for sandbox provisioning")?;

    cx_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.cx_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.cx_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let cx_home = match cmd.cx_home.clone() {
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.cx_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.cx"))
        );
    }
    ...
    #[test]
    fn requires_cx_home_for_managed_user() {
        let err =
