...
use cx_core_plugins::ConfiguredMarketplace;
use cx_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use cx_core_plugins::PluginInstallOutcome;
use cx_core_plugins::PluginInstallRequest;
use cx_core_plugins::PluginsConfigInput;
use cx_core_plugins::PluginsManager;
use cx_core_plugins::allowed_configured_marketplace_names;
use cx_core_plugins::installed_marketplaces::marketplace_install_root;
use cx_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use cx_core_plugins::marketplace::MarketplaceListError;
use cx_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use cx_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use cx_core_plugins::marketplace::MarketplacePluginSource;
use cx_core_plugins::marketplace::find_marketplace_manifest_path;
use cx_login::AuthManager;
use cx_plugin::PluginId;
use cx_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin add",
    after_help = "Examples:\n  cx plugin add sample@debug\n  cx plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin list",
    after_help = "Examples:\n  cx plugin list\n  cx plugin list --marketplace debug\n  cx plugin list --json\n  cx plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin remove",
    after_help = "Examples:\n  cx plugin remove sample@debug\n  cx plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplaces = outcome
        ...
        .collect::<Vec<_>>();
    let marketplace_sources = configured_marketplace_sources(&plugins_input, cx_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    cx_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    cx_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    cx_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<cx_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, cx_home);
...
pub(crate) async fn load_cli_auth_manager(config: &Config) -> Result<Arc<AuthManager>> {
    Ok(AuthManager::shared_from_config(config, /*enable_cx_api_key_env*/ true).await?)
}
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(cx_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, cx_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
