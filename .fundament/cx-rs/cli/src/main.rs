...
use cx_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use cx_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
...
use cx_tui::AppExitInfo;
use cx_tui::Cli as TuiCli;
use cx_tui::ExitReason;
use cx_tui::UpdateAction;
use cx_utils_absolute_path::AbsolutePathBuf;
use cx_utils_cli::CliConfigOverrides;
use cx_utils_cli::ProfileV2Name;
...
use cx_core::config::ConfigLoadOptions;
...
use cx_login::read_cx_access_token_from_env;
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = cx_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = cx_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = cx_cli::WindowsCommand;
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | cx login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = cx_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: cx_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: cx_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: cx_exec_server::RequestDispatchMode,
...
async fn run_session_archive_cli_command(
    action: cx_tui::SessionArchiveAction,
    ...
    cx_tui::run_session_archive_command(
        action,
        target,
        cx_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<cx_tui::SessionArchiveAction> {
    if force && cx_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => cx_tui::DeleteConfirmation::Skip,
        false => cx_tui::DeleteConfirmation::Prompt,
    };
    Ok(cx_tui::SessionArchiveAction::Delete(confirmation))
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = cx_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    {
        anyhow::bail!("`cx agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        ...
        Some(Subcommand::Agents(_)) => {
            if open_agents_overview {
                ...
                {
                    anyhow::bail!(
                        "`cx agents` cannot attach to shared sessions with invocation-specific configuration overrides"
                    );
                }
                ...
                if root_remote.is_none() {
                    ...
                    #[cfg(not(unix))]
                    anyhow::bail!("`cx agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::McpServer(McpServerCommand { strict_config })) => {
            eprintln!(
                "warning: `cx mcp-server` is deprecated and will be removed in a future release."
            );
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    let runtime_options = cx_app_server::AppServerRuntimeOptions {
                        ...
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            ...
                            (false, false) => {
                                cx_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        ..Default::default()
                    };
                    cx_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        cx_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        cx_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            ...
                            cx_app_server::app_server_control_socket_path(&cx_home)?
                        }
                    };
                    cx_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = cx_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    cx_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    cx_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    cx_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                cx_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                cx_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Model(ModelCommand {
            ...
            if let Some(m) = model {
                ...
                let config = ConfigBuilder::default()
                    ...
                    .build()
                    .await?;
                let config_path = config.cx_home.join("config.toml");
        ...
        Some(Subcommand::ListModels(ListModelsCommand {
            ...
            let catalog = if bundled {
                cx_models_manager::bundled_models_response()?
            } else {
                let cli_overrides = config_overrides
                    .parse_overrides()
                    .map_err(anyhow::Error::msg)?;
        ...
        Some(Subcommand::History(HistoryCommand {
            ...
            let app_server_handle = start(InProcessStartArgs {
                ...
                strict_config: false,
                cloud_config_bundle: cx_cloud_config::cloud_config_bundle_loader_for_storage(
                    config.auth_config(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | cx login --with-api-key`."
                        );
                        std::process::exit(1);
                        ...
                        run_login_with_gt(login_cli.config_overrides).await;
                    }
                }
            }
        }
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            cx_cli::run_command_under_seatbelt(
                sandbox_cli,
            ...
            .await?;
            #[cfg(target_os = "linux")]
            cx_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.cx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            cx_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.cx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`cx sandbox` is not supported on this operating system");
            }
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `cx mcp`: `cx`, `cx exec`, `cx review`, `cx resume`, `cx queue`, `cx archive`, `cx delete`, `cx unarchive`, `cx fork`, `cx mcp`, `cx sandbox`, and `cx debug prompt-input`."
        ),
    }
}
    ...
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = cx_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
            config.http_client_factory(),
            ...
            async move {
                ...
                match cmd.command {
                    Some(ExecServerSubcommand::Forward { connect }) => {
                        cx_exec_server::run_remote_environment_forward_until_shutdown(
                            remote_config,
                            connect,
                            shutdown,
                        )
        ...
        let http_client_factory = config
            .as_ref()
            .map(cx_core::config::Config::http_client_factory)
            .unwrap_or_else(|| {
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| cx_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(
            async move {
                cx_exec_server::run_main_with_telemetry(
                    &listen_url,
                    runtime_paths,
                    telemetry,
                    http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    ...
    if !is_supported_exec_server_remote_auth(&auth) {
        anyhow::bail!(
            "remote exec-server registration requires gt authentication or API key authentication; Agent Identity auth requires --use-agent-identity-auth"
        );
    }
    ...
    if auth_manager.is_workload_identity_selected() {
        Ok(cx_model_provider::auth_provider_from_auth_manager(
            auth_manager,
            &auth,
        ))
    } else {
        Ok(cx_model_provider::auth_provider_from_auth(&auth))
    }
}

fn is_supported_exec_server_remote_auth(auth: &CodexAuth) -> bool {
    auth.is_gt_auth() || auth.is_api_key_auth()
}
...
) -> anyhow::Result<cx_core::config::Config> {
...
) -> anyhow::Result<(Arc<AuthManager>, cx_login::CodexAuth)> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_cx_api_key_env*/ true).await?;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&cx_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let cx_home = find_cx_home()?;
            Ok(loader_overrides_for_profile_at_cx_home(
                Some(profile_v2),
                &cx_home,
            ))
        }
...
fn loader_overrides_for_profile_at_cx_home(
    profile_v2: Option<&ProfileV2Name>,
    cx_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(cx_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(cx_home: &std::path::Path, feature: &str) {
    ...
    let config_path = cx_home.join(cx_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(cx_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        cx_self_exe: arg0_paths.cx_self_exe,
        cx_linux_sandbox_exe: arg0_paths.cx_linux_sandbox_exe,
    ...
    let mut extensions = cx_extension_api::ExtensionRegistryBuilder::new();
    cx_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.gt_base_url.clone(),
        config.http_client_factory(),
    );
    cx_skills_extension::install(&mut extensions, |config: &Config| {
        cx_skills_extension::SkillsExtensionConfig {
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.cx_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `cx {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `cx {subcommand}`"
        );
    }
    Ok(())
}
...
/// `cx --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `cx {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = cx_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<cx_core::config::Config>,
) -> cx_http_client::HttpClientFactory {
    match config {
...
async fn print_app_server_remote_control_output(
    ...
    let output = cx_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = cx_terminal_detection::terminal_info();
    ...
    if interactive.agents_overview && remote.is_none() {
        ...
        cx_app_server_daemon::run(AppServerLifecycleCommand::Start)
            .await
            .map_err(std::io::Error::other)?;
    }
    ...
    let start_tui = || {
        cx_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            cx_config::LoaderOverrides::default(),
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<cx_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(cx_tui::resolve_remote_addr)
        .transpose()
        .map_err(std::io::Error::other)?;
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !cx_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let cx_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `cx resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `cx` without additional flags.
...
/// Build the final `TuiCli` for a `cx fork` invocation.
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let cx_home = tempfile::tempdir().expect("temporary CX home");
        let config = ConfigBuilder::default()
    ...
    fn exec_server_remote_api_key_auth_rejects_http_openai_domain() {
        for base_url in [
            ...
            let error = validate_api_key_remote_host(base_url)
                .expect_err("reject plaintext oi destination");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_cx_home() -> anyhow::Result<()> {
        ...
        assert_eq!(
            ...
            Some(resolve_profile_v2_config_path(cx_home.path(), &profile))
        );
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        ...
        let Some(cx_exec::Command::Resume(args)) = exec.command else {
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(cx_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(cx_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let cx_home = find_cx_home().expect("cx home");
        cx_app_server::app_server_control_socket_path(&cx_home)
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        for (subcommand, usage) in [
            ("add", "Usage: cx plugin marketplace add"),
            ("list", "Usage: cx plugin marketplace list"),
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: cx_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run cx resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run cx resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(cx_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(cx_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `cx remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `cx app-server proxy`"
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://cx.sock"));
    }
    ...
    fn app_server_listen_unix_socket_path_parses() {
        ...
        assert_eq!(
            app_server.listen,
            cx_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/cx.sock")
    ...
    fn app_server_proxy_sock_path_parses() {
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("cx.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(cx_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/cx-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(cx_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/cx-secret"))
        );
