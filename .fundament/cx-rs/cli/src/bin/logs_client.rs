...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(cx_home) = args.cx_home.as_ref() {
        config_builder = config_builder.cx_home(cx_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["cx-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["cx-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["cx-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["cx-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden CX home.
    #[tokio::test]
    async fn direct_db_skips_cx_home_config() {
        let cx_home = tempfile::tempdir().expect("create CX home");
        std::fs::write(cx_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("cx-state-logs"),
            OsString::from("--cx-home"),
            cx_home.path().as_os_str().to_owned(),
            OsString::from("--db"),
