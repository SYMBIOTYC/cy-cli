...
//! interactive-session layers. Direct `cx login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `cx-login.log` artifact that
...
use cx_login::login_with_access_token;
use cx_login::login_with_api_key;
use cx_login::logout_with_revoke;
use cx_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `cx login` flows.
///
...
/// command produce a durable `cx-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match cx_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("cx-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("cx_cli=info,cx_core=info,cx_login=info"));
    ...
    // Direct `cx login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `cx login --device-auth` instead."
    );
}
...
pub async fn run_login_with_gt(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    let effective_gt_workspaces = config.auth_config().effective_gt_workspaces();
    match login_with_gt(
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | cx login --with-api-key`.",
...
/// `cx login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    if is_workload_identity_selected() {
        match AuthManager::shared_from_config(&config, /*enable_cx_api_key_env*/ false).await {
            Ok(_) => {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let cx_home = tempdir().expect("create temporary CX home");
        login_with_api_key(
            cx_home.path(),
            "sk-existing",
