...
use cx_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use cx_core_plugins::marketplace::marketplace_root_dir;
use cx_core_plugins::marketplace_add::MarketplaceAddOutcome;
use cx_core_plugins::marketplace_add::MarketplaceAddRequest;
use cx_core_plugins::marketplace_add::add_marketplace;
use cx_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use cx_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "cx plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces CX is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin marketplace add",
    after_help = "Examples:\n  cx plugin marketplace add ./path/to/marketplace\n  cx plugin marketplace add owner/repo --ref main\n  cx plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "cx plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin marketplace upgrade",
    after_help = "Examples:\n  cx plugin marketplace upgrade\n  cx plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "cx plugin marketplace remove",
    after_help = "Example:\n  cx plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.cx_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<cx_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, cx_home);
