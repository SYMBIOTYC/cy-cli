...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\cx.zip").as_deref(),
            Some("/mnt/c/Temp/cx.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/cx.tgz").as_deref(),
            Some("/mnt/d/Work/cx.tgz")
        );
        assert!(win_path_to_wsl("/home/user/cx").is_none());
    }
