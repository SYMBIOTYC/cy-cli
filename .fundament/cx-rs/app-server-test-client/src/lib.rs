...
use cx_app_server_protocol::JSONRPCRequest;
...
use cx_otel::OtelProvider;
use cx_otel::current_span_w3c_trace_context;
use cx_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
#[derive(Parser)]
#[command(author = "CX", version, about = "Bootstrap CX app-server", long_about = None)]
struct Cli {
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `cx app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/cx-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `cx app-server --listen`.
    ...
    /// Send a user message through the CX app-server.
    SendMessage {
        /// User message to send to CX.
        user_message: String,
    },
    ...
    /// Trigger a gt or Amazon Bedrock login flow.
    TestLogin {
        ...
        device_code: bool,
        /// Use a CX-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the CX app-server.
    GetAccountRateLimits,
    /// List the available models from the CX app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the CX app-server.
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let cx_bin = cx_bin.unwrap_or_else(|| PathBuf::from("cx"));
            serve(&cx_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(cx_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(cx_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(cx_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --cx-bin and does not support --url");
            }
            let cx_bin = cx_bin.context("plugin-analytics-smoke requires --cx-bin")?;
            plugin_analytics_smoke::run(&cx_bin, &config_overrides, &plugin_id, capture_file)
        }
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --cx-bin and does not support --url");
            }
            let cx_bin = cx_bin.context("plugin-remote-uninstall requires --cx-bin")?;
...
fn resolve_endpoint(cx_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if cx_bin.is_some() && url.is_some() {
        bail!("--cx-bin and --url are mutually exclusive");
    }
    if let Some(cx_bin) = cx_bin {
        return Ok(Endpoint::SpawnCodex(cx_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if cx_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --cx-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(cx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(cx_bin);
        if let Some(cx_bin_parent) = cx_bin.parent() {
            let mut path = OsString::from(cx_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", cx_bin.display()))?;
...
fn serve(cx_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/cx-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,cx_=trace {}",
        shell_quote(&cx_bin.display().to_string())
    );
    for override_kv in config_overrides {
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnCodex(cx_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn test_login(
    ...
    with_client("test-login", endpoint, config_overrides, |client| {
        ...
        let login_response = match mode {
            TestLoginMode::ChatgptBrowser => client.login_account_gt()?,
            TestLoginMode::ChatgptDeviceCode => client.login_account_gt_device_code()?,
            TestLoginMode::AmazonBedrock { api_key, region } => {
                let request_id = client.request_id();
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        request_id: request_id.clone(),
                        params: cx_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
        ...
        let login_id = match login_response {
            ...
            _ => bail!("expected gt login response"),
        };
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (cx_bin, url) {
        (Some(_), Some(_)) => bail!("--cx-bin and --url are mutually exclusive"),
        (Some(cx_bin), None) => {
            let server = BackgroundAppServer::spawn(&cx_bin, config_overrides)?;
...
impl CodexClient {
    ...
    fn spawn_stdio(cx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(cx_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let cx_bin_display = cx_bin.display();
        let mut cmd = Command::new(cx_bin);
        if let Some(cx_bin_parent) = cx_bin.parent() {
            let mut path = OsString::from(cx_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut cx_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{cx_bin_display}` app-server"))?;

        let stdin = cx_app_server
            .stdin
            .take()
            .context("cx app-server stdin unavailable")?;
        let stdout = cx_app_server
            .stdout
            .take()
            .context("cx app-server stdout unavailable")?;
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `cx-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "cx-toy-app-server".to_string(),
                    title: Some("CX Toy App Server".to_string()),
    ...
    fn login_account_gt(&mut self) -> Result<LoginAccountResponse> {
        let request_id = self.request_id();
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: cx_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
    ...
    fn login_account_gt_device_code(&mut self) -> Result<LoginAccountResponse> {
        let request_id = self.request_id();
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: cx_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to cx app-server")?;
                    return Ok(());
                }
                bail!("cx app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from cx app-server")?;
                if bytes == 0 {
                    bail!("cx app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = cx_core::otel_init::build_provider(
            &config,
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[cx app-server exited: {status}]");
            return;
        }
