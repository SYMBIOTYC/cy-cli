...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("cx-plugin-analytics-{}.jsonl", process::id()))
    });
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "cx_plugin_used"
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug CX binary",
                path.display()
            );
        }
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("cx_plugin_used event has null or missing `{field}`");
        }
    }
