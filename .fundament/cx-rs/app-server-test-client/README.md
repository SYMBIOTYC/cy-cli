...
Quickstart for running and hitting `cx app-server`.

## Quickstart

Run from `<reporoot>/cx-rs`.

```bash
# 1) Build debug cx binary
cargo build -p cx-cli --bin cx

# 2) Start websocket app-server in background
cargo run -p cx-app-server-test-client -- \
  --cx-bin ./target/debug/cx \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p cx-app-server-test-client -- model-list
```

`send-message` and `send-message-v2` handle `request_user_input` server requests interactively.
When CX asks a question, choose a numbered option (or `o` for a free-form answer when offered)
and the client will send the response and continue streaming the same turn.

## Testing CX-managed Amazon Bedrock login
...
The test client redacts `apiKey` from its outbound request log. After login, start a fresh CX
...
active CX profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug CX binary; analytics capture is unavailable in release builds.
cargo build -p cx-cli --bin cx
...
The command validates one `cx_plugin_disabled`, `cx_plugin_enabled`, and
`cx_plugin_used` event with the expected local and remote plugin identities
...
installed, installs it, validates `cx_plugin_installed`, uninstalls it, and
validates `cx_plugin_uninstalled`, and verifies that the original
uninstalled state was restored.

The mutation events include the local CX ID in `plugin_id` and the backend ID
in `remote_plugin_id`.
...
Cleanup does not require analytics capture or a debug CX binary. When the
...
```bash
cargo run -p cx-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/cx-app-server-test-client/app-server.log`
...
```bash
cargo run -p cx-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p cx-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin cx-app-server-test-client -- \
...
```bash
cargo run --bin cx-app-server-test-client -- thread-resume <THREAD_ID>
```
