...
use cx_backend_client as backend;
use cx_backend_client::CodeTaskDetailsResponseExt;
use cx_git_utils::ApplyGitRequest;
use cx_git_utils::apply_git_patch;
...
impl HttpClient {
    ...
    pub fn with_gt_account_id(mut self, account_id: impl Into<String>) -> Self {
        self.backend = self.backend.clone().with_gt_account_id(account_id);
        self
    }
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/cx/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("CX_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    fn details_path(base_url: &str, id: &str) -> Option<String> {
        ...
        } else if base_url.contains("/api/cx") {
