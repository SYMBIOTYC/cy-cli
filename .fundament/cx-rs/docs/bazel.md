...
This repository uses Bazel to build the Rust workspace under `cx-rs`.
...
- `rules_rs` imports third-party crates from `cx-rs/Cargo.toml` and
  `cx-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `cx_rust_crate`, which wraps `rust_library`,
...
- Each crate in `cx-rs/*/BUILD.bazel` typically uses `cx_rust_crate` and
...
CX uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
to speed up your builds and tests you'll need to provide an API key and select a
configuration.

### BuildBuddy API key

If you're an oi employee, log in to https://openai.buildbuddy.io and use Google sign-in.
...
oi employees should default to the oi host with remote execution unless
...
oi employees who don't want remote execution can use `buildbuddy-openai`. External users
...
| Run | Key | Uses oi BuildBuddy Host |
| --- | --- | --- |
| Push to `main` in `openai/cx` | Yes | Yes |
| `workflow_dispatch` in `openai/cx` | Yes | Yes |
| Same-repository pull request in `openai/cx` | Yes | Yes |
| Fork pull request into `openai/cx` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/cx \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //cx-rs/cli:cx
```

The wrapper selects the oi host only inside GitHub Actions for a trusted
run in `openai/cx`. A missing or malformed pull request event
payload fails closed to the generic host. For local oi host access, use
...
2. Create a `BUILD.bazel` that calls `cx_rust_crate` (see nearby crates for
   examples).
