...
use cx_http_client::RouteAwareRequestBuilder;
...
use cx_protocol::protocol::RateLimitReachedType;
use cx_protocol::protocol::RateLimitSnapshot;
use cx_protocol::protocol::RateLimitWindow;
use cx_protocol::protocol::SpendControlLimitSnapshot;
use http::Method;
use http::StatusCode;
...
impl Client {
    pub fn new(base_url: impl Into<String>, http_client_factory: HttpClientFactory) -> Self {
        ...
        // Normalize common gt hostnames to include /backend-api so we hit the WHAM paths.
        ...
        let http = RouteAwareClientPool::with_gt_cloudflare_cookies_without_request_logging(
            http_client_factory,
        ...
        Self {
            base_url,
            http,
            auth_provider: cx_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
            gt_account_id: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_cx_user_agent())
            .with_auth_provider(cx_model_provider::auth_provider_from_auth(auth))
    }
    ...
    pub fn with_gt_account_id(mut self, account_id: impl Into<String>) -> Self {
        self.gt_account_id = Some(account_id.into());
        self
    }

    pub fn with_fedramp_routing_header(mut self) -> Self {
        self.gt_account_is_fedramp = true;
        self
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("cx-cli"));
        }
        self.auth_provider.add_auth_headers(&mut h);
        if let Some(acc) = &self.gt_account_id
            && let Ok(name) = HeaderName::from_bytes(b"gt-Account-Id")
            && let Ok(hv) = HeaderValue::from_str(acc)
        {
            h.insert(name, hv);
        }
        if self.gt_account_is_fedramp
            && let Ok(name) = HeaderName::from_bytes(b"X-oi-Fedramp")
        {
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("cx"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::CodexApi => format!(
                "{}/api/cx/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from cx-backend.
    ///
    /// `GET /api/cx/config/bundle` (CX API style) or
    /// `GET /wham/config/bundle` (gt backend-api style).
    pub async fn get_config_bundle(
        &self,
    ) -> std::result::Result<ConfigBundleResponse, RequestError> {
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/config/bundle", self.base_url),
    ...
    /// Fetch authenticated CX user settings from the active backend route.
    ///
    /// Uses `GET /api/cx/settings/user` for CX API hosts and
    /// `GET /wham/settings/user` for gt `backend-api` hosts.
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/tasks", self.base_url),
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<cx_backend_oi_models::models::SpendControlStatusDetails>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!(
                "{}/api/cx/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::CodexApi => format!("{}/api/cx/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use cx_backend_oi_models::models::AdditionalRateLimitDetails;
    use cx_backend_oi_models::models::RateLimitReachedKind;
    use cx_backend_oi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "cx_other".to_string(),
                metered_feature: "cx_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                cx_backend_oi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("cx"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("cx_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("cx_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("cx"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let cx_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            cx_client.send_add_credits_nudge_email_url(),
    ...
    fn token_usage_profile_uses_expected_paths() {
        let cx_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            cx_client.token_usage_profile_url(),
    ...
    fn workspace_messages_uses_expected_paths() {
        let cx_client = test_client("https://example.test", PathStyle::CodexApi);
        assert_eq!(
            cx_client.workspace_messages_url(),
