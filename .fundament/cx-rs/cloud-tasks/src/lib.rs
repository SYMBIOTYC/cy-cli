...
use anyhow::anyhow;
use chrono::Utc;
use cx_cloud_tasks_client::TaskStatus;
use cx_git_utils::current_branch_name;
use cx_git_utils::default_branch_name;
...
use cx_login::default_client::get_cx_user_agent;
...
struct BackendContext {
    backend: Arc<dyn cx_cloud_tasks_client::CloudBackend>,
    base_url: String,
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let use_mock = matches!(
        std::env::var("CX_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    ...
    if use_mock {
        ...
        return Ok(BackendContext {
            backend: Arc::new(cx_cloud_tasks_mock_client::MockClient),
            base_url,
    ...
    let mut http = cx_cloud_tasks_client::HttpClient::new(base_url.clone(), http_client_factory)
        .with_user_agent(ua);
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'cx login' to sign in with gt, then re-run 'cx cloud'."
            );
            std::process::exit(1);
        }
    };
    ...
    if let Some(acc) = auth.get_account_id() {
        append_error_log(format!("auth: mode=gt account_id={acc}"));
    }

    if !auth.uses_cx_backend() {
        eprintln!(
            "Not signed in. Please run 'cx login' to sign in with gt, then re-run 'cx cloud'."
        );
        std::process::exit(1);
    ...
    let auth_provider = cx_model_provider::auth_provider_from_auth(&auth);
    http = http.with_auth_provider(auth_provider);
    if let Some(acc) = auth.get_account_id() {
        append_error_log(format!("auth: set gt-Account-Id header: {acc}"));
    }
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("cx_cloud_tasks_exec").await?;
    ...
    let created = cx_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    let headers = util::build_gt_headers().await;
    let environments =
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `cx cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `cx cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn parse_task_id(raw: &str) -> anyhow::Result<cx_cloud_tasks_client::TaskId> {
    ...
    Ok(cx_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = cx_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &cx_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &cx_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[cx_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("cx_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        cx_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    let ctx = init_backend("cx_cloud_tasks_list").await?;
    let env_filter = if let Some(env) = args.environment {
        Some(resolve_environment_id(&ctx, &env).await?)
    } else {
        None
    };
    let page = cx_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("cx cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("cx_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("cx_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = cx_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
...
fn level_from_status(status: cx_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        cx_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        cx_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        cx_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = cx_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = cx_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `cx cloud` subcommand.
pub async fn run_main(cli: Cli, _cx_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext {
        ...
        base_url,
        environment_http,
    } = init_backend("cx_cloud_tasks_tui").await?;
    ...
    let force_internal = matches!(
        std::env::var("CX_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
    ...
    // Do this concurrently so the initial list shows quickly; on success we refetch with filter.
    {
        ...
        tokio::spawn(async move {
            ...
            // Build headers: UA + gt auth if available
            let headers = util::build_gt_headers().await;
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx
                            .send(Instant::now() + cx_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match cx_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let cx_tui::ComposerAction::Submitted(text) =
                                        page.composer.input(key)
                                    {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = cx_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, /*qa_mode*/ false, best_of_n).await;
                                    ...
                                    if page.composer.is_in_paste_burst() {
                                        let _ = frame_tx.send(
                                            Instant::now()
                                                + cx_tui::ComposerInput::recommended_flush_delay(),
                                        );
                                    }
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match cx_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match cx_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = cx_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match cx_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
// extract_gt_account_id moved to util.rs
...
mod tests {
    ...
    use cx_cloud_tasks_client::DiffSummary;
    use cx_cloud_tasks_client::TaskId;
    use cx_cloud_tasks_client::TaskStatus;
    use cx_cloud_tasks_client::TaskSummary;
    use cx_cloud_tasks_mock_client::MockClient;
    ...
    fn format_task_list_lines_formats_urls() {
        ...
        assert_eq!(
            lines,
            vec![
                "https://chatgpt.com/cx/tasks/task_1".to_string(),
                "  [READY] Example task".to_string(),
                "  Env  •  0s ago".to_string(),
                "  +5/-2 • 3 files".to_string(),
                String::new(),
                "https://chatgpt.com/cx/tasks/task_2".to_string(),
                "  [PENDING] No diff task".to_string(),
                "  env-2  •  0s ago".to_string(),
                "  no diff".to_string(),
            ]
    ...
    async fn collect_attempt_diffs_includes_sibling_attempts() {
        ...
        let task_id = parse_task_id("https://chatgpt.com/cx/tasks/T-1000").expect("id");
    ...
    fn parse_task_id_from_url_and_raw() {
        ...
        let url =
            parse_task_id("https://chatgpt.com/cx/tasks/task_i_123456?foo=bar").expect("url id");
