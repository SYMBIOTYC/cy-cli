...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = cx_login::default_client::USER_AGENT_SUFFIX.lock() {
...
/// - trims trailing '/'
/// - appends '/backend-api' for gt hosts when missing
...
pub async fn load_auth_manager(
    ...
    auth_config.gt_base_url = gt_base_url.or(Some(config.gt_base_url.clone()));
...
/// Build headers for gt-backed requests: `User-Agent`, optional `Authorization`,
/// and optional `gt-Account-Id`.
pub async fn build_gt_headers() -> HeaderMap {
    use http::header::HeaderValue;
    use http::header::USER_AGENT;

    set_user_agent_suffix("cx_cloud_tasks_tui");
    let ua = cx_login::default_client::get_cx_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("cx-cli")),
    );
    if let Some(am) = load_auth_manager(/*gt_base_url*/ None).await.0
        && let Some(auth) = am.auth().await
        && auth.uses_cx_backend()
    {
        headers.extend(cx_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/cx/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/cx") {
        return format!("{root}/cx/tasks/{task_id}");
    }
    if normalized.ends_with("/cx") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/cx/tasks/{task_id}")
}
