...
pub enum Command {
    /// Submit a new CX Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a CX Cloud task.
    Status(StatusCommand),
    /// List CX Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a CX Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a CX Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in CX Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `cx cloud` to browse).
    ...
    /// Git branch to run in CX Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// CX Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// CX Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// CX Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
