...
This crate implements the business logic for CX. It is designed to be used by the various CX UIs written in Rust.
...
Note that `cx-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.cx` read-only.
...
Expects the binary containing `cx-core` to run the equivalent of `cx sandbox` when `arg0` is `cx-linux-sandbox`. See the `cx-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `cx-resources/bwrap`
binary shipped with CX and CX surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
CX also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so CX rejects sandboxed shell commands that would enter the
...
Expects the binary containing `cx-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--cx-run-as-apply-patch`. See the
`cx-arg0` crate for details.
