...
pub use cx_core::test_support::TestCodexResponsesRequestKind;
pub use cx_core::test_support::responses_metadata;
use cx_utils_absolute_path::AbsolutePathBuf;
pub use cx_utils_absolute_path::test_support::PathBufExt;
pub use cx_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    cx_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    cx_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(cx_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = cx_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("cx-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\cx\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.cx`.
pub async fn load_default_config_for_test(cx_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        cx_linux_sandbox_exe: Some(
            find_cx_linux_sandbox_exe().expect("should find binary for cx-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_cx_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    F: FnMut(&cx_protocol::protocol::EventMsg) -> bool,
{
    ...
    wait_for_event_with_timeout(cx, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(cx: &CodexThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    cx: &CodexThread,
    thread_settings: cx_protocol::protocol::ThreadSettingsOverrides,
) -> anyhow::Result<()> {
    use cx_protocol::protocol::EventMsg;
    ...
    let submission_id = cx.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), cx.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(cx: &CodexThread, matcher: F) -> T
where
    F: Fn(&cx_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(cx, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&cx_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), cx.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    cx_core::spawn::CODEX_SANDBOX_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    cx_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    cx_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! cx_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_cx_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("cx-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
