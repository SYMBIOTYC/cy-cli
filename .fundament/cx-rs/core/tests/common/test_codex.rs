...
use cx_core::shell::Shell;
use cx_core::shell::get_shell_by_model_provided_path;
...
use cx_extension_api::LoadUserInstructionsFuture;
...
use cx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
#[derive(Debug)]
pub struct TestEnv {
    environment: cx_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: cx_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
                config: EnvironmentConfigState::FromThread,
            },
    ...
    pub fn environment(&self) -> &cx_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                cx_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestCodexBuilder {
    ...
    pub async fn restart(
        ...
        previous.cx.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let cx_linux_sandbox_exe = Some(
            crate::find_cx_linux_sandbox_exe()
                .context("should find binary for cx-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let cx_linux_sandbox_exe = None;
        let local_runtime_paths = cx_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            cx_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            cx_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            cx_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        environment_manager: Arc<cx_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestCodex> {
        ...
        let auth_manager = cx_core::test_support::auth_manager_from_auth_with_home(
            auth.clone(),
        ...
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| cx_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            cx_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    cx_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    cx_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = cx_utils_cargo_bin::cargo_bin("cx") {
            config.cx_self_exe = Some(path);
        } else if let Ok(path) = cx_utils_cargo_bin::cargo_bin("cx-exec") {
            // `cx-exec` also supports `--cx-run-as-apply-patch`, so use it
        ...
        {
            let cx = bin_dir.join("cx");
            let cx_exec = bin_dir.join("cx-exec");
            if cx.is_file() {
                config.cx_self_exe = Some(cx);
            } else if cx_exec.is_file() {
                config.cx_self_exe = Some(cx_exec);
            }
        }
...
impl TestCodex {
    ...
    pub fn cx_home_path(&self) -> &Path {
        self.config.cx_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.cx, |event| match event {
