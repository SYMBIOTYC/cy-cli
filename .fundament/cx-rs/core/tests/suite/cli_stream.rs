...
fn repo_root() -> std::path::PathBuf {
    cx_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn mount_personal_access_token_startup(server: &MockServer) {
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            ...
            "gt_user_id": "user-pat",
            "gt_account_id": PERSONAL_ACCESS_TOKEN_ACCOUNT_ID,
...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    let bin = cx_utils_cargo_bin::cargo_bin("cx").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg(format!("openai_base_url=\"{}/api/cx\"", server.uri()))
        .arg("-c")
        .arg(format!("gt_base_url=\"{}/backend-api\"", server.uri()))
        .arg("-C")
        .arg(repo_root())
        .arg("hello?");
...
impl Drop for ChildProcessCleanupGuard {
    fn drop(&mut self) {
        #[cfg(unix)]
        {
            let _ = cx_utils_pty::process_group::kill_process_group(self.0);
        }
...
// Use this for new `cx exec` subprocess tests in this file. These commands
...
async fn responses_mode_stream_cli_supports_personal_access_tokens() {
    ...
    assert_eq!(
        request.header("gt-account-id").as_deref(),
...
/// received by a mock oi Responses endpoint.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn exec_cli_applies_model_instructions_file() {
    ...
    // CX to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `cx exec --profile ...` preserves the active user config
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the cx CLI and invoke `exec`, which is what records a session.
    let bin = cx_utils_cargo_bin::cargo_bin("cx").unwrap();
    let mut cmd = Command::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = cx_utils_cargo_bin::cargo_bin("cx").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("CX_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume cx-cli run failed");
