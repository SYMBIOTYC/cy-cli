...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    let EventMsg::Error(error) =
        wait_for_event(&test.cx, |event| matches!(event, EventMsg::Error(_))).await
    else {
        unreachable!()
    };
    assert_eq!(error.message, "shell is unavailable in this session");
    assert_eq!(error.cx_error_info, None);
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(cx, |ev| match ev {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.cx, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.cx.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:CX_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${CX_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.cx, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.cx, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.cx, |ev| match ev {
    ...
    let _ = wait_for_event(&test.cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.cx, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
