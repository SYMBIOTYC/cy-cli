...
async fn root_reconciliation_reuses_pending_apps_startup() -> Result<()> {
    ...
    extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
        ...
        url: format!("{}/api/cx/ps/mcp", gated_apps_server.gt_base_url),
        root_resolved: Some(Arc::clone(&root_resolved)),
    }));
    let mut builder = search_capable_apps_builder(apps_server.gt_base_url)
        .with_extensions(Arc::new(extensions.build()))
        .with_config(|config| {
            config
                .features
...
async fn out_of_band_resource_read_reconciles_the_published_mcp_runtime() -> Result<()> {
    ...
    let user_config_path = refresh_config.cx_home.join("config.toml");
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, cx_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.cx.refresh_runtime_config(refresh_config).await;
    test.cx.submit(Op::RefreshMcpServers).await?;
...
async fn elevated_apps_catalog_limit_requires_host_owned_registration() -> Result<()> {
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        if let Some(id) = extension_id {
            ...
            extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
                id,
                url: format!("{}/api/cx/ps/mcp", apps_server.gt_base_url),
                root_resolved: None,
            }));
        ...
        if extension_id == Some("test-extension") {
            ...
            assert!(
                ...
                "an extension named cx_apps must not inherit the trusted Apps limit: {error}"
            );
            continue;
        }
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__cx_apps__calendar_create_event(args:")
        }),
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    initial.cx.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__cx_apps__calendar": "Plan events and manage your calendar."
        })
    );
    let mut resume_builder = search_capable_apps_builder(apps_server.gt_base_url)
        .with_config(enable_deferred_tool_world_state_without_agents);
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    test.cx
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.cx, |event| {
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.cx_home_path().join("cache/cx_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.cx.submit(Op::RefreshMcpServers).await?;
    test.cx
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
