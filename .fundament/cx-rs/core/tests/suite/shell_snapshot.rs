...
async fn wait_for_snapshot(cx_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = cx_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let cx_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&cx_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&cx, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&cx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.cx_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestCodexHarness::with_builder(builder).await?;
    let cx_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(cx_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let cx_home = home.path().to_path_buf();
