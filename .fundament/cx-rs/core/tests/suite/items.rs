...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let TestCodex { cx, .. } = test_codex().build(&server).await?;
    ...
    .await;
    let completed_item = wait_for_event_match(&cx, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&cx, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(cx_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn missing_streamed_reasoning_id_is_reused_for_completion() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let TestCodex { cx, .. } = test_codex().build_with_auto_env(&server).await?;
    ...
    .await;
    let completed_id = wait_for_event_match(&cx, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.id.clone()),
...
async fn web_search_item_is_emitted() -> anyhow::Result<()> {
    ...
    .await;
    let begin = wait_for_event_match(&cx, |ev| match ev {
    ...
    cx.flush_rollout().await?;
    let rollout_path = cx.rollout_path().expect("paginated rollout path");
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let delta_event = wait_for_event_match(&cx, |ev| match ev {
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    let plan_completed = wait_for_event_match(&cx, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while plan_delta.is_none() || agent_item.is_none() || plan_item.is_none() {
        let ev = wait_for_event(&cx, |_| true).await;
        match ev {
...
async fn sequential_cutoff_renders_done_summaries_for_active_reasoning_item() -> anyhow::Result<()>
{
    ...
    loop {
        match wait_for_event(&cx, |_| true).await {
