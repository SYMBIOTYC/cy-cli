...
use cx_protocol::items::AgentMessageDelivery;
...
async fn send_user_message_async_emits_item_and_does_not_end_the_turn() -> Result<()> {
    ...
    let started = wait_for_event_match(test.cx.as_ref(), |event| {
    ...
    let completed = wait_for_event_match(test.cx.as_ref(), |event| {
    ...
    wait_for_event(test.cx.as_ref(), |event| {
