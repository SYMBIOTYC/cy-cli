...
async fn search_tool_enabled_by_default_adds_tool_search() -> Result<()> {
    ...
    let mut builder = configured_builder(apps_server.gt_base_url.clone());
...
async fn app_search_sources_are_hidden_for_api_key_auth() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            configure_search_capable_apps(config, apps_server.gt_base_url.as_str())
        });
    let test = builder.build_with_auto_env(&server).await?;
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.cx, |event| {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.cx, |event| {
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_cx_apps"),
        Some(&json!({
            "call_id": "calendar-call-1",
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-cx-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-cx-turn-metadata/thread_id"),
        Some(&json!(test.session_configured.thread_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-cx-turn-metadata/turn_id")
            .and_then(Value::as_str)
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-cx-turn-metadata/model")
            .and_then(Value::as_str),
        Some("gpt-5.4")
    );
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-cx-turn-metadata/reasoning_effort")
            .and_then(Value::as_str),
        Some(first_request_reasoning_effort)
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-cx-turn-metadata/turn_started_at_unix_ms")
...
async fn tool_search_indexes_only_enabled_non_app_mcp_tools() -> Result<()> {
    ...
    let mut builder =
        configured_builder(apps_server.gt_base_url.clone()).with_config(move |config| {
