                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "cx_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["cx_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
        ...
        wait_for_event(&test.cx, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.cx_error_info == Some(CodexErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.cx, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["cx_rollout_budget_units"] = json!(10.0);
    }
    ...
    let test = test_codex()
        .with_config(move |config| {
            ...
            if remote_v2 {
                ...
                config.model_provider.name = "oi-compatible test provider".to_string();
            }
        })
        .build(&server)
        .await?;
...
async fn restates_the_current_remainder_after_compaction() -> Result<()> {
    ...
    model_provider.name = "oi-compatible test provider".to_string();
