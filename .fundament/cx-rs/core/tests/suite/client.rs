...
use cx_core::resolve_installation_id;
use cx_core::thread_store_from_config;
...
use cx_protocol::models::LocalShellAction;
use cx_protocol::models::LocalShellExecAction;
use cx_protocol::models::LocalShellStatus;
use cx_protocol::models::MessagePhase;
use cx_protocol::models::ReasoningItemContent;
use cx_protocol::models::ReasoningItemReasoningSummary;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> cx_core::CodexResponsesMetadata {
...
fn assert_cx_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-cx-window-id"].as_str(),
...
async fn sends_audio_urls_to_responses() {
    ...
    cx.start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Audio {
...
async fn sends_local_audio_to_responses() -> anyhow::Result<()> {
    ...
    cx.start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::LocalAudio {
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.cx, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex();
    let first = builder
        .resume(&server, Arc::clone(&cx_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.cx.submit(Op::Shutdown).await?;
    wait_for_event(&first.cx, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, cx_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_codex()
        .with_auth(CodexAuth::create_dummy_gt_auth_for_testing())
        .with_config(|config| {
...
/// Writes an `auth.json` into the provided `cx_home` with the specified parameters.
...
fn write_auth_json(
    ...
    let payload = json!({
        ...
        "https://api.cy.symbiotyc.workers.dev/auth": {
            "gt_plan_type": gt_plan_type,
            "gt_account_id": account_id.unwrap_or("acc-123")
        }
    });
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: cx_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = cx_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = cx_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![cx_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = cx_protocol::models::ResponseItem::Message {
        id: None,
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.cx_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&cx_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = cx_core::test_support::get_model_offline(config.model.as_deref());
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &cx_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn gt_auth_sends_correct_request() {
    skip_if_no_network!();
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/cx", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_codex()
        .with_auth(create_dummy_cx_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/cx/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
    let request_gt_account_id = request
        .header("gt-account-id")
        .expect("gt-account-id header");
    let request_body = request.body_json();
    ...
    assert_eq!(request_gt_account_id, "account_id");
    assert_cx_client_metadata(
        &request_body,
        installation_id.as_str(),
        session_id_string.as_str(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn prefers_apikey_when_config_prefers_apikey_even_with_gt_tokens() {
    skip_if_no_network!();
    ...
    // Expect API key header, no gt account header required.
    Mock::given(method("POST"))
        .and(path("/v1/responses"))
    ...
    // Init session
    let cx_home = TempDir::new().unwrap();
    // Write auth.json that contains both API key and gt tokens for a plan that should prefer gt,
    ...
    .expect("No CodexAuth found in cx_home");
    let auth_manager = cx_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.cx_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        cx_core::build_models_manager(&config, auth_manager),
        cx_core::CodexAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(cx_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(cx_core::test_support::EmptyUserInstructionsProvider),
    ...
    let NewThread { thread: cx, .. } = thread_manager
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn includes_apps_guidance_as_developer_message_for_gt_auth() {
    skip_if_no_network!();
    let server = MockServer::start().await;
    let apps_server = AppsTestServer::mount(&server)
        .await
        .expect("mount apps MCP mock");
    let apps_base_url = apps_server.gt_base_url.clone();
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            ...
            config.gt_base_url = apps_base_url;
        });
    let cx = builder
        .build(&server)
        .await
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__cx_apps"),
        "did not expect cx_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect cx_apps resource calls: {resource_methods:?}"
    );
}
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { cx, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { cx, config, .. } = test_codex().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_does_not_store_and_preserves_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        cx_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-cx-primary-used-percent", "12.5")
        .insert_header("x-cx-secondary-used-percent", "40.0")
        .insert_header("x-cx-primary-window-minutes", "10")
        .insert_header("x-cx-secondary-window-minutes", "60")
        .insert_header("x-cx-primary-reset-at", "1704069000")
        .insert_header("x-cx-secondary-reset-at", "1704074400")
    ...
    wait_for_event(&cx, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-cx-primary-used-percent", "100.0")
        .insert_header("x-cx-secondary-used-percent", "87.5")
        .insert_header("x-cx-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-cx-primary-window-minutes", "15")
        .insert_header("x-cx-secondary-window-minutes", "60")
        .insert_header("x-cx-credits-has-credits", "true")
        .insert_header("x-cx-credits-unlimited", "false")
        .insert_header("x-cx-credits-balance", "")
        .insert_header(
            "x-cx-rate-limit-reached-type",
    ...
    let mut builder = test_codex();
    let cx_fixture = builder.build(&server).await?;
    let cx = cx_fixture.cx.clone();
    ...
    let token_event = wait_for_event(&cx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&cx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&cx, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&cx, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = CxErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_cx_auth() -> CodexAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under CX sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
