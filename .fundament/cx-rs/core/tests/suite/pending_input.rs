...
use cx_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    submit_queue_only_agent_mail(test.cx.as_ref(), "pending mailbox input").await;
    let submission = test
        .cx
    ...
    wait_for_turn_complete(test.cx.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.cx.as_ref(), |event| {
...
async fn build_codex(server: &StreamingSseServer) -> Arc<CodexThread> {
    test_codex()
        ...
        .await
        .expect("build streaming CX test session")
        .cx
}
...
async fn steer_user_input(cx: &CodexThread, text: &str) {
    let submission = cx
...
async fn wait_for_reasoning_item_started(cx: &CodexThread) {
    wait_for_event(cx, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(cx: &CodexThread, text: &str) {
...
async fn wait_for_turn_complete(cx: &CodexThread) {
    wait_for_event(cx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(cx: &CodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(cx, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(cx: &CodexThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(cx, |event| {
        matches!(
            event,
...
impl cx_extension_api::ThreadLifecycleContributor<cx_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: cx_extension_api::ThreadStartInput<'a, cx_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions =
        cx_extension_api::ExtensionRegistryBuilder::<cx_core::config::Config>::new();
    ...
    let cx = test_codex()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build CX test session")
        .cx;

    enqueue_queue_only_agent_mail(&cx, CHILD_MESSAGE).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            ...
            RolloutItem::ResponseItem(envelope)
                if matches!(
                    &envelope.item,
                    cx_protocol::models::ResponseItem::AgentMessage { content, .. }
                        if content.iter().any(|content| matches!(
                            content,
                            cx_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&cx, INITIAL_PROMPT).await;
    wait_for_event(&cx, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&cx, STEER_PROMPT).await;
    wait_for_turn_complete(&cx).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&cx, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&cx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&cx, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&cx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&cx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&cx, "new mailbox input").await;
    wait_for_sleep_item_completed(&cx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&cx).await;
    ...
    cx.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&cx, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = cx.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let cx = build_codex(&server).await;

    submit_user_input(&cx, "first prompt").await;

    wait_for_reasoning_item_started(&cx).await;

    submit_queue_only_agent_mail(&cx, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&cx, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&cx, "second prompt").await;
    wait_for_turn_complete(&cx).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&cx, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&cx, "resumed old task").await;
    wait_for_turn_complete(&cx).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&cx, "processed steered prompt").await;
    wait_for_turn_complete(&cx).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&cx, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&cx, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
