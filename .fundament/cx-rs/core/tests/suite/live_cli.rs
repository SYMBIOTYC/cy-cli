//! Optional smoke tests that hit the real oi /v1/responses endpoint. They are `#[ignore]` by
//! default so CI stays deterministic and free. Developers can run them locally with
//! `just test -p cx-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let mut cmd = Command::new(cx_utils_cargo_bin::cargo_bin("cx-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn cx-rs");
