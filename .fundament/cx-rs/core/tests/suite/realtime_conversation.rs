...
use cx_core::test_support::auth_manager_from_auth;
...
use cx_protocol::protocol::RealtimeConversationVersion;
use cx_protocol::protocol::RealtimeEvent;
use cx_protocol::protocol::RealtimeHandoffRequested;
use cx_protocol::protocol::RealtimeNoopRequested;
use cx_protocol::protocol::RealtimeOutputModality;
use cx_protocol::protocol::RealtimeTranscriptEntry;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from CX.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = cx_prompts::BACKEND_PROMPT;
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in cx_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.cx.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = cx_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.cx, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.cx, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.cx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.cx.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.cx, |msg| match msg {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_gt_sends_cx_headers_to_backend() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let capture = RealtimeCallRequestCapture::new();
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/cx/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let backend_base_url = format!("{}/backend-api/cx", server.uri());
    ...
    let created = wait_for_event_match(&test.cx, |msg| match msg {
    ...
    test.cx.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.cx, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=cx-realtime-call-boundary")
    );
...
async fn conversation_webrtc_live_reconnects_sideband_after_unclean_disconnect() -> Result<()> {
    ...
    let handoff = timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.cx.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.cx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.cx, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        test.cx.submit(Op::Shutdown).await?;
        loop {
            match test.cx.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                    payload: RealtimeEvent::Error(message),
        ...
        test.cx.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.cx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_start_uses_openai_env_key_fallback_with_gt_auth() -> Result<()> {
    if std::env::var_os(REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR).is_none() {
        return run_realtime_conversation_test_in_subprocess(
            "suite::realtime_conversation::conversation_start_uses_openai_env_key_fallback_with_gt_auth",
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.cx_error_info, Some(CodexErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.cx, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.cx
        ...
        .await?;
    let _ = wait_for_event_match(&test.cx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let history = user_turns
        ...
        .collect::<Vec<_>>();
    test.cx.shutdown_and_wait().await?;
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("cx-rs/core"))?;
    ...
    assert!(startup_context.contains("cx-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.cx, |event| match event {
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.cx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            cx_responses_as_items: false,
            cx_response_item_prefix: None,
            cx_response_handoff_mode: cx_protocol::protocol::CodexResponseHandoffMode::BemTags,
            cx_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.cx.next_event().await?;
