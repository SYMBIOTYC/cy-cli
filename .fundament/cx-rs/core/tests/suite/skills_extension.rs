...
use cx_skills_extension::HostSkillProvider;
use cx_skills_extension::OrchestratorSkillProvider;
use cx_skills_extension::SkillProvider;
use cx_skills_extension::SkillProviderSource;
use cx_skills_extension::SkillProviders;
use cx_skills_extension::SkillsExtensionConfig;
use cx_skills_extension::catalog::SkillAuthority;
use cx_skills_extension::catalog::SkillCatalog;
use cx_skills_extension::catalog::SkillCatalogEntry;
use cx_skills_extension::catalog::SkillPackageId;
use cx_skills_extension::catalog::SkillProviderError;
use cx_skills_extension::catalog::SkillReadResult;
use cx_skills_extension::catalog::SkillResourceId;
use cx_skills_extension::catalog::SkillSearchResult;
use cx_skills_extension::catalog::SkillSourceKind;
use cx_skills_extension::install;
use cx_skills_extension::install_with_providers;
use cx_skills_extension::provider::SkillListQuery;
use cx_skills_extension::provider::SkillProviderFuture;
use cx_skills_extension::provider::SkillReadRequest;
use cx_skills_extension::provider::SkillSearchRequest;
...
fn write_host_skills(cx_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<cx_extension_api::ExtensionRegistry<Config>>,
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.cx_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    if !host_skills.is_empty() {
        write_host_skills(cx_home.path(), host_skills)?;
    }
    ...
    for _ in 0..turn_count {
        test.cx
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.cx, |_| true).await {
...
async fn capability_sections_render_in_order_with_host_repo_and_plugin_skills() -> Result<()> {
    ...
    let cx_home = Arc::new(TempDir::new()?);
    let host_skill_dir = cx_home.path().join("skills/host-search");
    ...
    let plugin_root = cx_home.path().join("plugins/cache/test/sample/local");
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            ...
            config.gt_base_url = apps_server.gt_base_url;
        });
    let test = builder.build_with_auto_env(&server).await?;
...
async fn agent_plugin_skill_prompt_stays_bounded_without_skills_extension() -> Result<()> {
    ...
    let mut builder = test_codex().with_home(cx_home);
    let test = builder.build_with_auto_env(&server).await?;
    ...
    test.cx
        ...
        .await?;
    let warning = core_test_support::wait_for_event(&test.cx, |event| {
        matches!(
            event,
...
async fn explicit_only_orchestrator_skill_is_hidden_but_can_be_invoked() -> Result<()> {
    ...
    let mut builder = apps_enabled_builder(apps_server.gt_base_url)
...
async fn executor_skill_invocation_is_environment_scoped_and_deduplicated() -> Result<()> {
    ...
    let server = responses::start_mock_server().await;
    let cx_home = Arc::new(TempDir::new()?);
    let skill_path = cx_home.path().join("executor-skill/SKILL.md");
...
async fn production_turn_aliases_combined_skill_catalogs_under_shared_budget() -> Result<()> {
    ...
    let cx_home = Arc::new(
        tempfile::Builder::new()
            .prefix("cx-integration-shared-skill-catalog-roots-")
            .tempdir()?,
    );
    write_host_skills(
        cx_home.path(),
    ...
    let developer_text = response
        ...
        .message_input_texts("developer")
        .join("\n");
    let host_root = dunce::canonicalize(cx_home.path().join("skills"))?
        .to_string_lossy()
        .replace('\\', "/");
    for (alias, root) in [
        ("e0", EXECUTOR_ROOT),
...
async fn production_turn_uses_configured_skill_catalog_token_budget() -> Result<()> {
    ...
    write_host_skills(cx_home.path(), &HOST_CATALOG)?;
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("cx-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let cx_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_uses_provider_host_catalog_and_core_snapshot_injection() -> Result<()> {
    ...
    write_host_skills(cx_home.path(), &[(skill_name, snapshot_description)])?;
    ...
    let app_mentioned_events =
        wait_for_analytics_events(&server, "cx_app_mentioned", /*expected_count*/ 1).await;
...
async fn production_turn_suppresses_only_the_superseded_host_skill_prompt() -> Result<()> {
    ...
    let first_skill_path = cx_home.path().join("skills/first-host/SKILL.md");
    let second_skill_path = cx_home.path().join("skills/second-host/SKILL.md");
...
async fn production_turn_warns_and_omits_unreadable_host_skill() -> Result<()> {
    ...
    let missing_skill_path =
        dunce::canonicalize(cx_home.path().join("skills/missing-host/SKILL.md"))?;
    let available_skill_path =
        dunce::canonicalize(cx_home.path().join("skills/available-host/SKILL.md"))?;
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let cx_home = Arc::new(TempDir::new()?);
    let skill_dir = cx_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_keeps_orchestrator_world_state_incremental_across_turns() -> Result<()> {
    ...
    let skill_resource = "skill://cx_apps/orchestrator-search/SKILL.md";
    let catalog = SkillCatalog {
        entries: vec![
            SkillCatalogEntry::new(
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. CX can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );
