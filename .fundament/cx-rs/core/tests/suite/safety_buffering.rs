...
-> anyhow::Result<()> {
    ...
    mount_response_once(
        ...
        .insert_header("x-cx-safety-buffering-enabled", "true")
        .insert_header("x-cx-safety-buffering-faster-model", FASTER_MODEL),
    )
    .await;
    ...
    let event = wait_for_event_match(&test.cx, |event| match event {
