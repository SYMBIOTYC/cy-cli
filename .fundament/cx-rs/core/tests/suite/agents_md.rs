...
async fn submit_thread_turn(thread: &Arc<cx_core::CodexThread>, prompt: &str) -> Result<()> {
    thread
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.cx.instruction_sources().await, creation_sources);
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    assert_eq!(
        initial.cx.instruction_sources().await,
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    assert_eq!(
        parent.cx.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.cx.ensure_rollout_materialized().await;
    parent.cx.flush_rollout().await?;
    let rollout_path = parent.cx.rollout_path().expect("rollout path");
    ...
    fork_config.cx_self_exe = parent.config.cx_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(
