...
use cx_core::shell::ShellType;
use cx_core::shell::get_shell;
...
const NETWORK_TEST_HOST: &str = "cx-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://cx-network-test.invalid:80";
...
-> Result<()> {
    ...
    .await?;
    let EventMsg::RequestPermissions(request) = wait_for_event(&test.cx, |event| {
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.cx.next_event())
            .await
...
#[test_case("GET", "http://cx-network-test.invalid/"; "plain_http")]
#[test_case("CONNECT", "cx-network-test.invalid:443"; "connect")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg_attr(
    not(target_os = "linux"),
    ignore = "requires the trusted Linux proxy bridge"
...
async fn background_network_approval_uses_active_turn_after_original_turn_completes() -> Result<()>
{
    ...
    assert_eq!(test.cx.list_background_terminals().await.len(), 1);
    ...
    test.cx
        ...
        .await?;
    let assessment = wait_for_event(&test.cx, |event| {
        matches!(
            event,
            EventMsg::GuardianAssessment(assessment)
    ...
    assert_eq!(
        test.cx.list_background_terminals().await.len(),
        1,
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#cx-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("cx-network-test.invalid is not in the allowed_domains")
    );
    test.cx
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="cx-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): cx-network-test.invalid",
        )
    }));
    mount_exec_network_turn(
        &server,
...
async fn denying_network_policy_amendment_persists_and_blocks_request() -> Result<()> {
    ...
    let policy = fs::read_to_string(test.home.path().join("rules/default.rules"))?;
    assert!(policy.contains(
        r#"network_rule(host="cx-network-test.invalid", protocol="http", decision="deny""#
    ));
    let output = responses
        .requests()
        .iter()
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.cx.list_background_terminals().await.is_empty() {
                break;
            }
