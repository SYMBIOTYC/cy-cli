...
use cx_core::context::NodeReplReviewEvidence;
use cx_extension_api::ExtensionData;
use cx_extension_api::ExtensionRegistryBuilder;
use cx_extension_api::ToolCallOutcome;
use cx_extension_api::ToolContributor;
use cx_extension_api::ToolFinishInput;
use cx_extension_api::ToolLifecycleContributor;
...
use cx_tools::FreeformTool;
use cx_tools::FreeformToolFormat;
use cx_tools::FunctionCallError;
use cx_tools::JsonToolOutput;
use cx_tools::ResponsesApiNamespace;
use cx_tools::ResponsesApiNamespaceTool;
use cx_tools::ToolCall;
use cx_tools::ToolExecutor;
use cx_tools::ToolExecutorFuture;
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "test-gpt-5.1-cx", configure)
        .await
}
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_code_mode_host_program("cx-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("cx-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("cx-mode-host-does-not-exist"),
...
"#,
    ...
    let auth_manager = cx_core::test_support::auth_manager_from_auth(auth.clone());
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-cx").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.cx, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__cx_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__cx_apps__calendar_timezone_option_99",
...
async fn code_mode_wait_timeout_reconnects_on_next_exec() -> Result<()> {
    ...
    test.cx
        ...
        .await?;
    wait_for_event_match(&test.cx, |event| match event {
...
impl ToolLifecycleContributor for InterruptedNestedToolObserver {
    fn on_tool_start<'a>(&'a self, input: ToolStartInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            let cx_extension_api::ToolCallSource::CodeMode { cell_id, .. } = input.source else {
                return;
            };
    ...
    fn on_tool_finish<'a>(&'a self, input: ToolFinishInput<'a>) -> ToolLifecycleFuture<'a> {
        Box::pin(async move {
            if input.tool_name.name != "test_sync_tool"
                || !matches!(
                    input.source,
                    cx_extension_api::ToolCallSource::CodeMode { .. }
                )
            {
                return;
            }
...
"#,
    ...
    assert!(!exec_description.contains("cx/imageDetail"));
...
-> Result<()> {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    core_test_support::wait_for_mcp_server(&test.cx, "node_repl").await?;
    let evidence = test
        .cx
...
async fn code_mode_node_repl_image_flag_without_enhanced_stays_disabled() -> Result<()> {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.cx, "node_repl").await?;
            ...
            if reviewer_constraint.is_some_and(|value| value != "large_prompt") || check_detail {
                ...
                if check_detail {
                    ...
                    reviewer.input_modalities =
                        vec![cx_protocol::openai_models::InputModality::Text];
                } else {
...
"#,
        ...
        test.cx
            .start_or_steer_turn(
                ...
                .with_thread_settings(ThreadSettingsOverrides {
                    environments: Some(cx_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        let turn_id = wait_for_event_match(&test.cx, |event| match event {
        ...
        wait_for_event(&test.cx, |event| match event {
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-cx-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "cx_app__hidden_dynamic_tool");
const out = await tools.cx_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("cx_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("cx_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("CX app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("cx_app__hidden_dynamic_tool(args:")
            })
    );
