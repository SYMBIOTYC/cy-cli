...
async fn build_cx_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestCodex> {
    let mut builder = test_codex().with_model("test-gpt-5.1-cx");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_cx_with_test_tool(&server).await?;
