...
const FIXED_CWD: &str = "/tmp/cx_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> CodexAuth {
        match self {
            AuthCase::ChatGpt => CodexAuth::create_dummy_gt_auth_for_testing(),
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&cx, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().cx_home_path();
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<CX_HOME>");
            search_start = start_index + "<CX_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <CX_HOME>/skills/.system/imagegen/SKILL.md and \
         <CX_HOME>/skills/custom/SKILL.md"
    );
}
