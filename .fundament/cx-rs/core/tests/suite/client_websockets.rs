...
use cx_core::ModelClientSession;
use cx_core::Prompt;
use cx_core::ResponseEvent;
use cx_core::TurnInputRequest;
use cx_core::X_CODEX_ROUTING_HINT_HEADER;
use cx_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use cx_core::test_support::with_parent_turn;
...
use cx_rollout_trace::ConversationPart;
...
async fn responses_websocket_streams_request() {
    ...
    let harness = websocket_harness_for_cx_backend(&server).await;
    ...
    assert_eq!(
        ...
        Some(cx_login::default_client::get_cx_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-cx-installation-id"].as_str(),
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-cx-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-cx-primary-used-percent": "100.0",
            "x-cx-secondary-used-percent": "87.5",
            "x-cx-primary-over-secondary-limit-percent": "95.0",
            "x-cx-primary-window-minutes": "15",
            "x-cx-secondary-window-minutes": "60",
            "x-cx-credits-has-credits": "true",
            "x-cx-credits-unlimited": "false",
            "x-cx-credits-balance": "",
            "x-cx-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let error_event = wait_for_event(&test.cx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options_and_auth(
    ...
    let model_info = cx_core::test_support::construct_model_info_offline(MODEL, &config);
    let thread_id = ThreadId::new();
    let session_id = SessionId::new();
    let client_auth_manager = auth.map(cx_core::test_support::auth_manager_from_auth);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "cx-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
