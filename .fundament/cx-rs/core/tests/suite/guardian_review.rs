...
#[test_case(CodexAuth::create_dummy_gt_auth_for_testing(), "cx-auto-review"; "gt_uses_cx_auto_review")]
async fn guardian_session_prewarms_and_is_reused_for_first_review(
    ...
    let bundled_models = cx_models_manager::bundled_models_response()?.models;
    let catalog_auto_review = bundled_models
        .iter()
        .find(|model| model.slug == "cx-auto-review")
    ...
    let mut builder = test_codex()
        ...
        .with_config(move |config| {
            let rules_dir = config.cx_home.join("rules");
...
async fn guardian_session_is_reused_for_consecutive_tool_reviews_without_prewarm() -> Result<()> {
    ...
    let mut builder = test_codex()
        ...
        .with_workspace_setup(|cwd, fs| async move {
            fs.write_file(
                &cx_utils_path_uri::PathUri::from_abs_path(&cwd.join("guardian-secret.txt")),
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the CX agent history whose request action you are assessing."
        ),
