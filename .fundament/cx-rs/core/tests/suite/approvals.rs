...
use cx_protocol::protocol::ExecPolicyAmendment;
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured CX helper binary
...
/// same sandbox with `cx sandbox -P`. The nested invocation must also pass
...
/// profile names before passing one to `cx sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current CX sandbox remotely."
    )
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected cx sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the CX sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/cx-fallback-rule-test.txt && echo hello > /tmp/cx-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://cx-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
