    ...
    fn refresh(&self, context: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, CodexAuth> {
        let auth = if context.previous_account_id.as_deref() != Some(CHATGPT_ACCOUNT_ID) {
            Err(std::io::Error::other(
                "external auth refresh changed the gt workspace",
            ))
        } else {
            self.current
                .lock()
...
fn external_gt_auth(access_token: &str) -> std::io::Result<CodexAuth> {
    CodexAuth::from_external_gt_tokens(access_token, CHATGPT_ACCOUNT_ID, Some("enterprise"))
}
...
async fn custom_provider_does_not_receive_ambient_auth_headers() -> anyhow::Result<()> {
    ...
    assert_eq!(request.header("gt-account-id"), None);
    Ok(())
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn external_auth_401_retry_uses_refreshed_gt_headers() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    Mock::given(method("POST"))
        ...
        .and(header("gt-account-id", CHATGPT_ACCOUNT_ID))
    ...
    let external_auth = Arc::new(ScriptedExternalAuth {
        current: Mutex::new(external_gt_auth(INITIAL_ACCESS_TOKEN)?),
        refreshed: external_gt_auth(REFRESHED_ACCESS_TOKEN)?,
    });
    test.thread_manager
        .auth_manager()
