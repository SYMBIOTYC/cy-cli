...
use cx_core::EnvironmentNetworkPolicy;
use cx_core::StartThreadOptions;
use cx_core::TurnInputRequest;
use cx_core::WaitForEnvironmentToolConfig;
...
use cx_exec_server::EnvironmentReadyInfo;
...
use cx_exec_server::NoiseChannelPublicKey;
use cx_exec_server::NoiseRendezvousConnectBundle;
use cx_exec_server::NoiseRendezvousConnectProvider;
use cx_exec_server::REMOTE_ENVIRONMENT_ID;
use cx_exec_server::RemoteEnvironmentConfig;
use cx_exec_server::RemoveOptions;
use cx_extension_api::ContextContributor;
...
use cx_protocol::models::SandboxPermissions;
...
use cx_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_cx_with_wait_for_environment() -> TestCodexBuilder {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = cx_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.cx, |event| {
        matches!(
            event,
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/cx-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\cx-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn environment_permissions_follow_configuration_ownership() -> Result<()> {
    ...
    test.cx
        ...
        .await?;
    let persisted_settings = wait_for_event_match(&test.cx, |event| match event {
        EventMsg::ThreadSettingsApplied(event) => Some(event.thread_settings.clone()),
        _ => None,
    })
    .await;
    let snapshot = test.cx.config_snapshot().await;
    ...
    test.cx
        .restore_thread_settings(test.cx.restorable_thread_settings().await)
        .await?;
    let (mcp_config, _) = test.cx.current_mcp_config_and_runtime_context().await;
    assert_eq!(
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_cx_with_wait_for_environment()
            } else {
                test_codex()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    let initial_environments = test.cx.environment_selections().await;
    ...
    test.cx
        ...
        .await?;
    let request = wait_for_event_match(&test.cx, |event| match event {
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    test.cx.ensure_rollout_materialized().await;
    test.cx.flush_rollout().await?;
    let rollout = fs::read_to_string(test.cx.rollout_path().context("rollout path")?)?;
...
async fn shared_executor_keeps_ready_capability_roots_scoped_to_each_attachment() -> Result<()> {
    ...
    assert_eq!(
        test.cx.inspect_selected_capability_roots().ready_roots,
        vec![root("first-root")]
    );
    assert_eq!(
        second
...
async fn owner_network_policy_is_rejected_until_runtime_enforcement_exists() -> Result<()> {
    ...
    let selections = test.cx.environment_selections().await;
    ...
    assert_eq!(test.cx.environment_selections().await, selections);
    Ok(())
}
...
-> Result<()> {
    ...
    let remote_environment = tokio::spawn(cx_exec_server::run_remote_environment(
        remote_config,
        runtime_paths,
    ));
...
async fn deferred_executor_stays_pending_after_materialization() -> Result<()> {
    ...
    .await;
    let mut builder = test_cx_with_wait_for_environment().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        assert!(config.features.enable(Feature::UnifiedExec).is_ok());
    });
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_cx_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(test.cx.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    let mut builder = test_cx_with_wait_for_environment()
        ...
        .with_config(|config| {
            ...
            config.model_provider.name = "oi (test)".to_string();
    ...
    test.cx.ensure_rollout_materialized().await;
    test.cx.flush_rollout().await?;
    let rollout_path = test.cx.rollout_path().context("rollout path")?;
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/cx-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/cx-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/cx-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/cx-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/cx-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/cx-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/cx-remote-dotdot-{}", std::process::id()));
