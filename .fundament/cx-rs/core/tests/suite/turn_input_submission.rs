...
async fn submit_user_message(
    ...
    text: &str,
) -> cx_protocol::error::Result<TurnInputSubmission> {
    cx.start_or_steer_turn(user_message_request(text)).await
}
...
async fn start_turn_if_idle_rejects_non_user_input_that_requests_plan_mode() {
    ...
    let original_collaboration_mode = test.cx.config_snapshot().await.collaboration_mode;
    ...
    assert_eq!(
        test.cx.config_snapshot().await.collaboration_mode,
...
async fn turn_input_submission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    let first_submission = tokio::spawn({
        ...
        async move {
            barrier.wait().await;
            submit_user_message(cx.as_ref(), "first message").await
        }
    });
    let second_submission = tokio::spawn({
        ...
        async move {
            barrier.wait().await;
            submit_user_message(cx.as_ref(), "second message").await
        }
    });
    barrier.wait().await;
    ...
    wait_for_event(cx.as_ref(), |event| {
...
async fn turn_input_submission_applies_thread_settings_only_after_accepted_input() {
    ...
    let started = submit_user_message(cx, "start turn")
        .await
    ...
    assert_eq!(
        cx.config_snapshot().await.approval_policy,
        AskForApproval::Never
    );
    assert_eq!(
        cx.environment_selections().await,
