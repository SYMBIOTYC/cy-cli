...
use cx_config::types::McpServerAuth;
...
use cx_core::EnvironmentMcpPolicy;
...
use cx_protocol::mcp_policy::McpServerIdentity;
use cx_protocol::mcp_policy::McpServerRequirement;
...
use cx_protocol::models::PermissionProfileSnapshot;
use cx_protocol::models::ResponseItem;
use cx_protocol::openai_models::ConfigShellToolType;
use cx_protocol::openai_models::InputModality;
use cx_protocol::openai_models::ModelInfo;
use cx_protocol::openai_models::ModelVisibility;
...
use cx_protocol::protocol::McpStartupStatus;
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        cx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/cx-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn assert_openai_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    ...
    wait_for_mcp_server(&fixture.cx, server_name).await?;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        ...
        .await?;
    wait_for_mcp_server(&fixture.cx, "bounded").await?;

    fixture
        .cx
        .start_or_steer_turn(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.cx, |event| {
...
async fn environment_mcp_policy_filters_runtime_config_and_model_tools(
    ...
    .await?;
    let (pending_config, _) = fixture.cx.current_mcp_config_and_runtime_context().await;
    ...
    let (runtime_config, _) = fixture.cx.current_mcp_config_and_runtime_context().await;
    ...
    fixture
        ...
        .await?;
    let (failed_config, _) = fixture.cx.current_mcp_config_and_runtime_context().await;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.cx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_codex()
        .with_pre_build_hook(move |cx_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.cx, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.cx.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    .await
    .context("CX Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.cx.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.cx.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.cx, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta(
    ...
    fixture
        ...
        .list_models(
            ...
            cx_core::test_support::default_http_client_factory(),
        )
        .await;
    ...
    let response_metadata: Value = serde_json::from_str(
        initial_request["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
    ...
    assert_eq!(
        output_json.pointer("/x-cx-turn-metadata/node_repl_auto_review_required"),
        Some(&json!(node_repl_auto_review_required))
    );
    assert_eq!(
        output_json.pointer("/x-cx-turn-metadata/node_repl_disabled"),
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            cx_linux_sandbox_exe: fixture.config.cx_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.cx, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: cx_protocol::openai_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that CX should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: CX can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip(with_headers_helper: bool) -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so CX will call the MCP echo tool
    ...
    // Phase 3: configure CX with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert CX begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.cx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_configured_auth_precedes_gt_auth() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    wait_for_mcp_server(&configured_auth_fixture.cx, "configured_auth").await?;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_gt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;
    let untrusted_server = MockServer::start().await;
    let untrusted_apps = AppsTestServer::mount(&untrusted_server).await?;
    let untrusted_mcp_url = format!("{}/api/cx/ps/mcp", untrusted_apps.gt_base_url);
    let untrusted_gt_base_url = untrusted_apps.gt_base_url;

    let fixture = test_codex()
        .with_auth(CodexAuth::create_dummy_gt_auth_for_testing())
        .with_config(move |config| {
            config.gt_base_url = untrusted_gt_base_url;
            insert_mcp_server(
                config,
                "untrusted_origin",
                McpServerTransportConfig::StreamableHttp {
    ...
    wait_for_mcp_server(&fixture.cx, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/cx/ps/mcp")
        .filter_map(|request| {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn configured_gt_base_url_does_not_grant_mcp_gt_auth() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let fixture = test_codex()
        ...
        .with_pre_build_hook(move |cx_home| {
            fs::write(
                ...
                format!(
                    r#"
gt_base_url = "{untrusted_gt_base_url}"
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so CX will call the OAuth-backed
    ...
    // Phase 4: configure CX with the OAuth-backed Streamable HTTP MCP
    ...
    let recovery_hint = if credential_config.is_local_environment() {
        format!("Run `cx mcp login {server_name}`.")
    } else {
    ...
    let startup = wait_for_event(&fixture.cx, |event| {
    ...
    let (contended_turn, refreshed_failure, refreshed_failed_servers) =
        tokio::time::timeout(Duration::from_secs(5), async {
            ...
            {
                match fixture.cx.next_event().await?.msg {
    ...
    let discovered_turn = tokio::time::timeout(Duration::from_secs(5), async {
        ...
        fixture
            ...
            .await?;
        loop {
            if let EventMsg::TurnComplete(turn) = fixture.cx.next_event().await?.msg {
    ...
    tokio::time::timeout(
        ...
        wait_for_mcp_server(&fixture.cx, discovered_server_name),
    )
    .await
    .context("the newly discovered OAuth server did not recover after its store was unlocked")??;

    assert!(cx_rmcp_client::delete_oauth_tokens(
        discovered_credential_name.as_ref(),
        http_server.url(),
        OAuthCredentialsStoreMode::File,
        cx_config::types::AuthKeyringBackendKind::default(),
    )?);
    fixture.cx.refresh_runtime_config(refreshed_config).await;
    ...
    // Phase 7: assert CX begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.cx, |ev| {
        matches!(
            ev,
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the CX fixture.
...
fn write_fallback_oauth_tokens(
    ...
    cx_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        cx_config::types::AuthKeyringBackendKind::default(),
