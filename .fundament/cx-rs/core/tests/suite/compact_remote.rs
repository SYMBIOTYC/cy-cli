...
use cx_core::compact::SUMMARY_PREFIX;
...
use cx_login::auth::AgentIdentityAuth;
use cx_login::auth::AgentIdentityAuthRecord;
use cx_login::auth::BedrockApiKeyAuth;
use cx_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
...
use cx_protocol::protocol::RealtimeConversationRealtimeEvent;
...
const TURN_STATE_HEADER: &str = "x-cx-turn-state";
...
fn remote_realtime_test_cx_builder(
...
async fn close_realtime_conversation(cx: &cx_core::CodexThread) -> Result<()> {
    cx.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(cx, |msg| match msg {
...
async fn wait_for_turn_complete(cx: &cx_core::CodexThread) {
...
async fn amazon_bedrock_uses_remote_compaction_endpoint() -> Result<()> {
    ...
    harness.test().cx.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().cx).await;
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    initial.cx.shutdown_and_wait().await?;
    annotate_retained_user_in_rollout(&rollout_path, retained_text)?;

    let resumed = builder()
        .resume(&server, home, rollout_path.clone())
        .await?;
    resumed.cx.submit(Op::Compact).await?;
    wait_for_turn_complete(&resumed.cx).await;
    resumed.cx.shutdown_and_wait().await?;
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    assert_eq!(compact_request.path(), "/v1/responses/compact");
    assert_eq!(
        compact_request.header("gt-account-id").as_deref(),
        Some("account_id")
    );
    assert_eq!(
        compact_request.header("authorization").as_deref(),
    ...
    assert_eq!(
        ...
        compact_request.header("x-cx-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-cx-window-id").as_deref()
    );
    assert_ne!(
        ...
        test_codex().with_auth(CodexAuth::AgentIdentity(
            AgentIdentityAuth::from_record(
                AgentIdentityAuthRecord {
                    ...
                    gt_user_id: "user-compact".to_string(),
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let uses_cx_backend = auth.uses_cx_backend();
...
async fn remote_manual_compact_gt_auth_reuses_service_tier_and_prompt_cache_key() -> Result<()>
{
    ...
    assert_remote_manual_compact_request_parity(
        ...
        Some(ServiceTier::Fast),
        Some("priority"),
        "remote_manual_compact_gt_auth_service_tier_prompt_cache_key_request_diff",
        "After five varied gt-auth turns, remote manual compaction reuses service_tier and prompt_cache_key while omitting responses-only fields.",
    )
    .await?;
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-cx-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_codex().with_auth(CodexAuth::create_dummy_gt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "cx_app".to_string(),
        description: "CX app tools.".to_string(),
        tools: vec![
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "cx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "cx_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "cx_app", tool_name, "{}"),
    ...
    test.cx
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.cx, |event| {
    ...
    test.cx
        ...
        .await?;
    wait_for_turn_complete(&test.cx).await;
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    .await?;
    let baseline_codex = baseline_harness.test().cx.clone();
    ...
    .await?;
    let override_codex = override_harness.test().cx.clone();
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = cx.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    wait_for_event(&cx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.cx.submit(Op::Compact).await?;
    wait_for_event(&initial.cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_cx_builder(&realtime_server).with_config(|config| {
        config.model_auto_compact_token_limit = Some(200);
    });
    let test = builder.build(&server).await?;
    ...
    start_realtime_conversation(test.cx.as_ref()).await?;
    ...
    close_realtime_conversation(test.cx.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_cx_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_cx_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.cx.as_ref()).await?;
    ...
    start_realtime_conversation(resumed.cx.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.cx.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_cx_builder(&realtime_server);
    let test = builder.build(&server).await?;
