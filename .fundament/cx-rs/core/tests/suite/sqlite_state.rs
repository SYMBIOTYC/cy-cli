...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.cx.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |cx_home| {
            let rollout_path = cx_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.sqlite.state_db_path();
    let rollout_path = test.config.cx_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.cx, server_name).await?;
    let db = test.cx.state_db().expect("state db enabled");
    let thread_id = test.session_configured.thread_id;
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = cx_state::log_db::start(db.clone());
