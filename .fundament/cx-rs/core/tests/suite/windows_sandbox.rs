...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("cx-resources");
    ...
    for helper_name in ["cx-windows-sandbox-setup", "cx-command-runner"] {
        let helper = cx_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    ...
    let public = cwd.join("public.txt");
    let setup_marker = cx_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(cx_home.path()),
...
async fn windows_elevated_shell_and_unified_exec_enforce_managed_deny_reads() -> anyhow::Result<()>
{
    ...
    let builder = test_codex()
        ...
        .with_config(move |config| {
            config.cx_home = configured_cx_home;
