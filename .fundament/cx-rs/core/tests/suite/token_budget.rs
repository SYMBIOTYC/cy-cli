...
async fn assert_context_compaction_item_lifecycle(cx: &std::sync::Arc<cx_core::CodexThread>) {
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_codex()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.cx, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert_context_compaction_item_lifecycle(&test.cx).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.cx, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.cx, |event| match event {
        EventMsg::HookCompleted(completed)
...
async fn token_budget_mid_turn_auto_compaction_resets_before_active_follow_up(
    ...
    .await;
    let mut model_provider = built_in_model_providers(/*openai_base_url*/ None)["openai"].clone();
    model_provider.name = "oi (test)".into();
    model_provider.base_url = Some(format!("{}/v1", server.uri()));
    model_provider.supports_websockets = false;
    let test = test_codex()
        .with_model_info_override("gpt-5.2", |model_info| {
...
async fn token_budget_auto_compact_fallback_uses_buffer_until_new_context() -> Result<()> {
    ...
    let test = test_codex()
        .with_config(|config| {
            config.model_provider.name = "oi (test)".into();
