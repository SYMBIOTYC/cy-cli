...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.model_provider.name = "Non-oi Model provider".to_string();
        config.compact_prompt = Some(SUMMARIZATION_PROMPT.to_string());
    });
    let initial = builder.build(&server).await?;
    let initial_thread = Arc::clone(&initial.cx);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.cx, "after resume").await?;
    shutdown_thread(&resumed.cx).await?;
...
async fn submit_compact_turn(cx: &Arc<CodexThread>) -> Result<()> {
    cx.submit(Op::Compact).await?;
    let warning_event = wait_for_event(cx, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(cx: &Arc<CodexThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-cx-window-id header");
