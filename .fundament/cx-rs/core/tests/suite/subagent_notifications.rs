...
fn write_home_skill(cx_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = cx_home.join("skills").join(dir);
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.cx_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.cx_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.cx_home_path(), "stop_hook_log.jsonl")?;
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    if matches!(selection, FullHistoryV2ModelSelection::WorldStateIdentity) {
        ...
        let child_rollout = cx_rollout::RolloutRecorder::get_rollout_history(
            &child_thread
                .rollout_path()
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.cx_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.cx_home.join("instructions-only-role.toml");
    ...
    let test = test_codex()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.cx_home.join("model-only-role.toml");
