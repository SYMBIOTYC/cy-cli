...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = CodexAuth::from_external_gt_tokens(
        "header.e30.first",
        "test-account",
        /*gt_plan_type*/ None,
    )?;
    ...
    // correctly strips gt auth from untrusted localhost origins.
    let mut hosted_plugin_runtime_config = cx_mcp::hosted_plugin_runtime_mcp_server_config(
        &apps_server.gt_base_url,
    ...
    *bearer_token_env_var = None;
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        cx_apps_tools_cache: CodexAppsToolsCache::default(),
        tool_catalog_cache: McpToolCatalogCache::default(),
        cx_apps_tools_cache_key: cx_mcp::cx_apps_tools_cache_key(Some(&expected_auth)),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: Some(expected_auth.clone()),
        cx_apps_auth_manager: Some(Arc::clone(&auth_manager)),
    ...
    auth_manager
        .set_external_auth(Arc::new(StaticExternalAuth(
            CodexAuth::from_external_gt_tokens(
                "header.e30.reloaded",
                "test-account",
                /*gt_plan_type*/ None,
            )?,
    ...
    // so this tool call only sees the new token if the CX Apps provider
    ...
    let tool_call_request = requests
        .iter()
        .find(|request| {
            request.url.path() == "/api/cx/ps/mcp"
        ...
        .expect("CX Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")
