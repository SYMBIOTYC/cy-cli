...
const REMOTE_MODEL: &str = "cx-test-ttl";
const VERSIONED_MODEL: &str = "cx-test-versioned";
const MISSING_VERSION_MODEL: &str = "cx-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "cx-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.cx_home.join(CACHE_FILE);
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.cx_home.join(CACHE_FILE);
