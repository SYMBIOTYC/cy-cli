...
use cx_core::find_thread_path_by_id_str;
...
use cx_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use cx_protocol::protocol::EventMsg;
use cx_protocol::protocol::ExitedReviewModeEvent;
use cx_protocol::protocol::Op;
use cx_protocol::protocol::ReviewCodeLocation;
use cx_protocol::protocol::ReviewFinding;
use cx_protocol::protocol::ReviewLineRange;
use cx_protocol::protocol::ReviewOutputEvent;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under CX sandbox network restrictions.
    skip_if_no_network!();
    ...
    let entered_started = wait_for_event(&cx, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&cx, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&cx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&cx, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&cx, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let path = cx.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match cx.next_event().await.expect("review event").msg {
    ...
    cx.submit(Op::Interrupt).await.unwrap();
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = cx_home.path().join("resume.jsonl");
    {
        ...
        let user = cx_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = cx_protocol::models::ResponseItem::Message {
            id: None,
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let cx = new_conversation_for_server(&server, cx_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, cx_home, resume_path)
        .await
