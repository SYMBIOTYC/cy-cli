...
use cx_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(cx_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = cx_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(cx_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(cx_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = cx_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_cx_home_directory() {
    let repo = TempDir::new().unwrap();
    let cx_home = repo.path().join(".cx");
    std::fs::create_dir_all(&cx_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".cx/**\n").unwrap();
    let id = Uuid::new_v4();
