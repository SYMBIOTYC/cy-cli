    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _context: Context<'_, S>) {
        if event.metadata().target() == "cx_http_client::transport" {
            self.record_request_after_retry();
            return;
        }

        if event.metadata().target() != "cx_otel.trace_safe" {
            return;
        }

        let mut visitor = RetryTelemetryVisitor::default();
        event.record(&mut visitor);
        if visitor.name.as_deref() != Some("cx.retry") {
            return;
        }
    ...
    loop {
        match wait_for_event(&test.cx, |_| true).await {
            ...
            EventMsg::TurnComplete(event) => {
                assert_eq!(
                    event.error.and_then(|error| error.cx_error_info),
