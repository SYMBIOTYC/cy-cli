...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__cx_apps__google_calendar";
...
fn write_sample_plugin_manifest_and_config_at_root(
    ...
    std::fs::create_dir_all(plugin_root.join(".cx-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context(
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local CX home cache"
    );
...
"#,
    ...
    let mut builder = builder
        .with_home(Arc::clone(&cx_home))
        .with_auth(CodexAuth::create_dummy_gt_auth_for_testing())
        .with_model("gpt-5.2")
        .with_config(move |config| config.gt_base_url = gt_base_url);
    ...
    cx.start_or_steer_turn(
        TurnInputRequest::user_input(vec![cx_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&cx, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&cx, |event| match event {
    ...
    let measurement = wait_for_analytics_event(&server, "cx_plugin_measurement_event").await;
    assert_eq!(
        serde_json::json!({
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let cx_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_agent_plugin_skill_plugin(cx_home.as_ref()))?;
    let mut builder = test_codex()
        .with_home(Arc::clone(&cx_home))
    ...
    test_codex
        ...
        .await?;
    let warning = wait_for_event(&test_codex.cx, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_codex.cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn plugin_skill_product_policy_and_migrated_command_precedence_reach_agent_turns(
    ...
    .await;
    let cx_home = Arc::new(TempDir::new()?);
    let plugin_root = write_sample_plugin_manifest_and_config(cx_home.as_ref());
    ...
    if include_migrated_command {
        let migrated_skill_dir =
            plugin_root.join(".cx-plugin/migrated-command-skills/source-command-review");
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let cx_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(cx_home.as_ref());
...
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths() -> Result<()> {
    ...
    .await;
    let cx_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(cx_home.as_ref());
    ...
    let mut builder = test_codex().with_home(Arc::clone(&cx_home));
    let test_codex = builder.build_with_remote_and_local_env(&server).await?;
    wait_for_mcp_server(&test_codex.cx, "agent").await?;
    let data_root = dunce::canonicalize(
        std::fs::read_dir(cx_home.path().join("plugins/data/agent-plugins"))?
            .next()
    ...
    test_codex
        ...
        .await?;
    let end = wait_for_event(&test_codex.cx, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_codex.cx, |event| {
...
async fn curated_plugin_skills_follow_auth_switch() -> Result<()> {
    const CHATGPT_CURATED_PLUGIN_SKILL: &str = "gt-plugin:gt-skill";
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            name: "gt with a custom provider",
            target_auth: TargetAuth::Chatgpt,
...
[plugins."gt-plugin@openai-curated"]
enabled = true
...
"#,
        ...
        for (marketplace_name, plugin_name, skill_name, description) in [
            ("openai-curated", "gt-plugin", "gt-skill", "gt description"),
            (
                "openai-api-curated",
                "api-plugin",
                "api-skill",
        ...
        assert!(initial_skills.contains("gt description"));
        ...
        let expected_auth_mode = match fixture.target_auth {
            ...
            TargetAuth::ApiKey => {
                cx_login::login_with_api_key(
                    cx_home.path(),
                    "test-api-key",
                    cx_login::AuthCredentialsStoreMode::File,
                    cx_login::AuthKeyringBackendKind::default(),
                )?;
                test_codex.thread_manager.auth_manager().reload().await;
                Some(AuthMode::ApiKey)
            }
            TargetAuth::BedrockApiKey => {
                cx_login::login_with_bedrock_api_key(
                    cx_home.path(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn explicit_plugin_mentions_use_apps_for_gt_dual_surface_plugins(
    app_enabled: bool,
) -> Result<()> {
    skip_if_no_network!(Ok(()));
    let server = start_mock_server().await;
    ...
    write_plugin_skill_plugin(cx_home.as_ref());
    write_plugin_mcp_plugin(cx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(cx_home.as_ref());
    ...
    let test_codex =
        build_apps_enabled_plugin_test_codex(&server, cx_home, apps_server.gt_base_url).await?;
    let cx = Arc::clone(&test_codex.cx);
    wait_for_mcp_server(&cx, CX_APPS_MCP_SERVER_NAME).await?;

    cx.start_or_steer_turn(TurnInputRequest::user_input(vec![
        cx_protocol::user_input::UserInput::Mention {
            name: "sample".into(),
    ...
    assert!(
        ...
        "expected plugin MCP guidance to be suppressed for gt auth: {developer_messages:?}"
    );
    assert_eq!(
        developer_messages
            .iter()
    ...
    assert!(
        request
            ...
            .is_none(),
        "plugin MCP tool should not leak into the request for gt auth"
    );
    ...
    assert!(
        echo_tool.is_none(),
        "plugin MCP tool should be suppressed for gt auth"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_gt_auth() -> Result<()> {
    skip_if_no_network!(Ok(()));
    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount_with_connector_name(&server, "Google Calendar").await?;
    let mock = mount_plugin_tool_search_turn(&server).await;
    ...
    write_plugin_skill_plugin(cx_home.as_ref());
    write_plugin_mcp_plugin(cx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(cx_home.as_ref(), "sample_app");

    let test_codex =
        build_apps_enabled_plugin_test_codex(&server, cx_home, apps_server.gt_base_url).await?;
    let cx = Arc::clone(&test_codex.cx);
    wait_for_mcp_server(&cx, "sample").await?;
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(write_plugin_skill_plugin(cx_home.as_ref()))?;
    write_plugin_mcp_plugin(cx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(cx_home.as_ref());
    let initialize_barrier = block_plugin_mcp_startup(cx_home.as_ref(), &rmcp_test_server_bin);
    ...
    cx.start_or_steer_turn(TurnInputRequest::user_input(vec![input]))
        .await?;
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let cx_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(cx_home.as_ref());
    let test_codex = build_analytics_plugin_test_codex(&server, cx_home).await?;
    let cx = Arc::clone(&test_codex.cx);
    ...
    let event = wait_for_analytics_event(&server, "cx_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(cx_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let cx_home = Arc::new(TempDir::new()?);
    let skill_path = dunce::canonicalize(write_remote_plugin_skill_plugin(cx_home.as_ref()))?;
    persist_sample_remote_plugin_id(cx_home.as_ref());
    let test_codex = build_analytics_plugin_test_codex(&server, cx_home).await?;
    let cx = Arc::clone(&test_codex.cx);

    cx.start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Skill {
        name: "sample:sample-search".into(),
        path: skill_path,
    }]))
    .await?;
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id(
    ...
    let server = start_mock_server().await;
    let cx_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(cx_home.as_ref());
    persist_sample_remote_plugin_id(cx_home.as_ref());
...
async fn wait_for_analytics_event(server: &MockServer, event_type: &str) -> serde_json::Value {
    ...
    loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/cx/analytics-events/events")
            .find_map(|request| {
