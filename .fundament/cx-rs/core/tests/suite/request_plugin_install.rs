...
use cx_core_plugins::startup_sync::curated_plugins_repo_path;
use cx_features::Feature;
use cx_login::CodexAuth;
use cx_models_manager::bundled_models_response;
use cx_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__cx_apps__calendar";
...
fn configure_apps_without_search_tool(config: &mut Config, apps_base_url: &str) {
    ...
    config.gt_base_url = apps_base_url.to_string();
...
async fn start_gated_step_preparation(test: &TestCodex, server: &MockServer) -> Result<PathUri> {
    let prior_recommendation_count = server
        ...
        .unwrap_or_default()
        .into_iter()
        .filter(|request| request.url.path() == "/ps/plugins/suggested/cx")
        .count();
...
async fn start_install_turn(test: &TestCodex, prompt: &str) -> Result<ElicitationRequestEvent> {
    ...
    Ok(wait_for_event_match(&test.cx, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "cx_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": cx_login::default_client::originator().value,
            }
        })
    );
...
async fn run_remote_plugin_install_refresh_case(refreshed_tools: RefreshedAppsTools) -> Result<()> {
    ...
    drop(requests);
    test.cx.refresh_runtime_config(test.config.clone()).await;
