...
use cx_protocol::openai_models::MultiAgentModeMessages;
...
async fn ultra_reasoning_uses_max_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.cx, "hello", /*effort*/ None).await?;
...
async fn mode_hints_override_reasoning_effort(source: ModeHintSource) -> Result<()> {
    ...
    let test = test_codex()
        ...
        .build(&server)
        .await?;
    submit_turn(&test.cx, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.cx, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn model_switch_refreshes_catalog_role_and_explicit_mode() -> Result<()> {
    ...
    let test = test_codex()
        ...
        .await?;
    submit_turn(&test.cx, "first model", Some(ReasoningEffort::High)).await?;
    ...
    .await?;
    submit_turn(&test.cx, "second model", Some(ReasoningEffort::High)).await?;
...
async fn empty_configured_mode_hint_emits_no_mode_message() -> Result<()> {
    ...
    submit_turn(&test.cx, "hello", Some(ReasoningEffort::High)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    submit_turn(&initial.cx, "before resume", /*effort*/ None).await?;
    ...
    drop(initial);
    submit_turn(&resumed.cx, "after resume", /*effort*/ None).await?;
...
async fn live_mode_change_appends_mode_without_reappending_usage_hint() -> Result<()> {
    ...
    submit_turn(&test.cx, "proactive", /*effort*/ None).await?;
    submit_turn(&test.cx, "explicit", Some(ReasoningEffort::High)).await?;
...
async fn leaving_ultra_after_cold_resume_emits_explicit_mode() -> Result<()> {
    ...
    let resumed = resume_builder.restart(&server, &initial).await?;
    drop(initial);
    submit_turn(&resumed.cx, "after resume", Some(ReasoningEffort::High)).await?;
