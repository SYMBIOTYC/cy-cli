...
fn local_compaction_provider(server: &wiremock::MockServer) -> ModelProviderInfo {
    ...
    provider.name = "oi-compatible test provider".to_string();
    provider.base_url = Some(format!("{}/v1", server.uri()));
    provider.supports_websockets = false;
    provider
}
...
async fn assert_compaction_uses_turn_lifecycle_id(cx: &std::sync::Arc<cx_core::CodexThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = cx.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn CX.
    ...
    cx.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&cx, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down CX to flush rollout entries before inspecting the file.
    cx.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    cx.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&cx, |ev| match ev {
    ...
    wait_for_event(&cx, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.cx_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs = read_hook_inputs(&test.cx_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let cx = builder.build(&server).await.unwrap().cx;
    ...
    let first = wait_for_event_match(&cx, |ev| match ev {
    ...
    let last = wait_for_event_match(&cx, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        cx_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    wait_for_event(&resumed.cx, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test cx");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.cx).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using CX with a gt account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.cx).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&cx, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&cx, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-cx-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-cx-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let cx = builder.build(&server).await.unwrap();

    cx.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    cx.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    cx.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&cx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
-> Result<()> {
    ...
    let rollout_path = test.cx.rollout_path().expect("rollout path");
    ...
    assert_eq!(
        resumed.cx.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
