...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let base_path = cx.rollout_path().expect("rollout path");
    ...
    let fork1_path = cx_fork1.rollout_path().expect("rollout path");
    expected_after_first.push(thread_settings_applied_item(
        cx_fork1.thread_settings_snapshot().await,
    ));
    ...
    let fork2_path = cx_fork2.rollout_path().expect("rollout path");
    ...
    expected_after_second.push(thread_settings_applied_item(
        cx_fork2.thread_settings_snapshot().await,
    ));
...
async fn assert_copied_fork_persists_inherited_history(history_mode: ThreadHistoryMode) {
    ...
    let source_meta = cx_rollout::read_session_meta_line(source_path.as_path())
        .await
    ...
    if matches!(history_mode, ThreadHistoryMode::Paginated) {
        ...
        let resumed_history = cx_rollout::RolloutRecorder::get_rollout_history(&forked_path)
            .await
