...
pub async fn resolve_installation_id(cx_home: &AbsolutePathBuf) -> Result<String> {
    let path = cx_home.join(INSTALLATION_ID_FILENAME);
    fs::create_dir_all(cx_home).await?;
...
mod tests {
    ...
    async fn resolve_installation_id_generates_and_persists_uuid() {
        ...
        let persisted_path = cx_home.path().join(INSTALLATION_ID_FILENAME);

        let installation_id = resolve_installation_id(&cx_home_abs)
            .await
    ...
    async fn resolve_installation_id_rewrites_invalid_file_contents() {
        ...
        assert_eq!(
            std::fs::read_to_string(cx_home.path().join(INSTALLATION_ID_FILENAME))
