...
use cx_connectors::merge::plugin_connector_to_app_info;
use cx_connectors::metadata::connector_install_url;
use cx_connectors::metadata::sanitize_name;
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    let mut synthetic_tool = cx_app_tool("gmail_batch_read_email", "gmail", Some("Gmail"), &[]);
    ...
    let tools = vec![
        synthetic_tool,
        cx_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__cx_apps__calendar".to_string(),
