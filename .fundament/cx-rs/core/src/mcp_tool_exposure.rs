...
impl McpHandlerCache {
    pub(crate) fn append_mcp_tools(
        ...
        config: &Config,
        apps_enabled: bool,
        mcp_server_catalog: &cx_mcp::ResolvedMcpCatalog,
...
fn append_mcp_tools(
    ...
    let non_app_tools = filter_non_cx_apps_mcp_tools_only(all_mcp_tools);
    let app_tools = apps_enabled
        .then(|| filter_cx_apps_mcp_tools(all_mcp_tools, config))
        .into_iter()
        .flatten();
