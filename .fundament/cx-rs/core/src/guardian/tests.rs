...
use cx_analytics::GuardianApprovalRequestSource;
...
use cx_protocol::protocol::GuardianRiskLevel;
use cx_protocol::protocol::GuardianUserAuthorization;
...
// The memories extension depends on cx-core, so this probe verifies the nested Guardian config
...
impl cx_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: cx_extension_api::ThreadStartInput<'a, Config>,
...
impl cx_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a cx_extension_api::ExtensionData,
        thread_store: &'a cx_extension_api::ExtensionData,
    ) -> cx_extension_api::ExtensionFuture<'a, Vec<cx_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![cx_extension_api::PromptFragment::developer_policy(
...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/cx-rs/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"openai/cx\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: cx_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/cx-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[cx_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            cx_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The CX agent has requested the following action:\n"));
...
async fn build_guardian_prompt_truncates_oversized_approval_reason() -> anyhow::Result<()> {
    ...
    let expected_reason = cx_utils_output_truncation::truncate_text(
        &approval_reason,
        cx_utils_output_truncation::TruncationPolicy::Tokens(/*tokens*/ 512),
    );
    ...
    let reason_item = prompt
        ...
        .find_map(|item| match item {
            cx_protocol::user_input::UserInput::Text { text, .. }
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: cx_protocol::permissions::FileSystemSpecialPath::Root,
                },
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: cx_protocol::permissions::FileSystemSpecialPath::project_roots(Some(
                        "private".to_string(),
                    )),
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The CX agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "cx_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    cx_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    let mut extensions = cx_extension_api::ExtensionRegistryBuilder::<Config>::new();
    ...
    assert!(matches!(
        ...
        Some(cx_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    let request_body = request.body_json();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "cx_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            cx_protocol::user_input::UserInput::Text { text, .. } => text,
            cx_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
        ...
        .replace_history(
            vec![
                ResponseItem::Compaction {
                    id: Some(cx_protocol::ResponseItemId::from_server(
    ...
    assert!(matches!(
        ...
        Some(cx_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "cx_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
async fn escalated_retry_bypasses_extension_approval_and_runs_guardian() -> anyhow::Result<()> {
    ...
    impl cx_extension_api::ApprovalReviewContributor for AutoApprovingReviewContributor {
        fn contribute<'a>(
            &'a self,
            _session_store: &'a cx_extension_api::ExtensionData,
            _thread_store: &'a cx_extension_api::ExtensionData,
            _prompt: &'a str,
            _extension_metrics: Option<Arc<dyn cx_extension_api::ExtensionMetrics>>,
        ) -> cx_extension_api::ExtensionFuture<'a, Option<ReviewDecision>> {
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(cx_protocol::openai_models::ReasoningEffort::Low),
        /*model_messages*/ None,
    )
...
async fn guardian_review_session_config_clears_context_overrides_for_distinct_effective_model() {
    ...
    let (session, mut turn) = guardian_test_session_and_turn(&server).await;
    let mut config = (*turn.config).clone();
    config.model = Some("cx-auto-review".to_string());
