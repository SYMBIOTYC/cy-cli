...
use anyhow::anyhow;
use cx_analytics::GuardianReviewAnalyticsResult;
use cx_analytics::GuardianReviewSessionAnalyticsParams;
use cx_analytics::GuardianReviewSessionKind;
use cx_extension_api::UserInstructions;
...
use crate::cx_delegate::run_cx_thread_interactive;
...
use cx_protocol::turn_input::TurnStartOptions;
use cx_protocol::user_input::UserInput;
use cx_thread_store::PersistContext;
use cx_tools::normalize_output_image_detail;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            cx_linux_sandbox_exe: spawn_config.cx_linux_sandbox_exe.clone(),
...
async fn spawn_guardian_review_session(
    ...
    let (session, io) = Box::pin(run_cx_thread_interactive(
        spawn_config,
        ...
        cx_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
    ))
    .await?;
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.cx_error_info,
                                    },
                                    true,
                                    true,
                                );
...
fn read_only_guardian_permission_profile(
    ...
    permission_profile
        ...
        .unwrap_or(PermissionProfile::External {
            network: cx_protocol::permissions::NetworkSandboxPolicy::Restricted,
        })
}

pub(crate) fn build_guardian_review_session_config(
    parent_config: &Config,
    live_network_config: Option<cx_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
mod tests {
    use super::*;
    use cx_protocol::openai_models::AutoReviewMessages;
    ...
    async fn test_review_params() -> GuardianReviewSessionParams {
        ...
        GuardianReviewSessionParams {
            ...
            model,
            reasoning_effort,
            guardian_default_review_model_id: "cx-auto-review".to_string(),
