...
use cx_analytics::GuardianReviewDecision;
use cx_analytics::GuardianReviewFailureReason;
use cx_analytics::GuardianReviewTerminalStatus;
use cx_analytics::GuardianReviewTrackContext;
use cx_analytics::GuardianReviewedAction;
...
use cx_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &cx_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        cx_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("cx-approval-review".to_string())
        .spawn(move || {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == cx_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == cx_protocol::openai_models::ReasoningEffort::Low),
