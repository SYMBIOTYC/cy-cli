...
use cx_protocol::protocol::ExecCommandOutputDeltaEvent;
use cx_protocol::protocol::ExecOutputStream;
use cx_sandboxing::SandboxCommand;
use cx_sandboxing::SandboxManager;
use cx_sandboxing::SandboxTransformRequest;
use cx_sandboxing::SandboxType;
use cx_sandboxing::SandboxablePreference;
use cx_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use cx_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use cx_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use cx_sandboxing::record_filesystem_sandbox_violation;
#[cfg(test)]
use cx_sandboxing::resolve_windows_elevated_filesystem_overrides;
#[cfg(test)]
use cx_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use cx_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
#[cfg(any(test, target_os = "windows"))]
use cx_sandboxing::windows_sandbox_uses_elevated_backend;
use cx_utils_absolute_path::AbsolutePathBuf;
use cx_utils_path_uri::PathUri;
use cx_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use cx_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: cx_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: cx_protocol::config_types::WindowsSandboxLevel,
...
pub fn build_exec_request(
    ...
    let request = manager
        .transform(SandboxTransformRequest {
            ...
            cx_linux_sandbox_exe: cx_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        cx_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = cx_otel::global() {
        let _ = metrics.counter(
            "cx.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_cx_home;
    use cx_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use cx_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                cx_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    cx_home: cx_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        cx_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                cx_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
