...
use cx_shell_command::shell_snapshot::snapshot_script;
...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("cx.shell_snapshot.duration_ms", &[]);
            ...
            config
                .session_telemetry
                .counter("cx.shell_snapshot", /*inc*/ 1, &counter_tags);
...
async fn run_script_with_timeout(
    ...
    cx_protocol::shell_environment::scrub_non_inheritable_env_vars(handler.as_std_mut());
    handler.args(&args[1..]);
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(cx_home, session_id, state_db.as_deref()).await?;
