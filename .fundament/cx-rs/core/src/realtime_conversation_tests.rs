...
async fn clears_active_handoff_explicitly() {
    ...
    let state = RealtimeHandoffState {
        ...
        cx_response_handoff_mode: CodexResponseHandoffMode::Thinking,
        cx_response_handoff_channel_prefixes: Arc::new(BTreeMap::new()),
...
fn realtime_headers_include_only_non_default_originator() {
    let default_originator = cx_login::default_client::originator();
    for (originator, expected_header) in [
        ("cx_work_desktop", Some("cx_work_desktop")),
