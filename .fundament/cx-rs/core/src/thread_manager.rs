...
use cx_agent_graph_store::AgentGraphStore;
use cx_agent_graph_store::LocalAgentGraphStore;
...
use cx_code_mode::CodeModeSessionProvider;
use cx_code_mode::DisabledCodeModeSessionProvider;
use cx_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use cx_protocol::mcp::OPENAI_STANDARD_FORM_INPUT_EXTENSION_ID;
...
use cx_thread_store::LoadThreadHistoryParams;
use cx_thread_store::LocalThreadStore;
use cx_thread_store::LocalThreadStoreConfig;
use cx_thread_store::MoveThreadToSectionParams;
use cx_thread_store::PreparedFork;
use cx_thread_store::ReadThreadByRolloutPathParams;
use cx_thread_store::ReadThreadParams;
use cx_thread_store::StoredModelContext;
use cx_thread_store::StoredThread;
use cx_thread_store::ThreadMetadataPatch;
use cx_thread_store::ThreadStore;
use cx_thread_store::ThreadStoreError;
use cx_thread_store::UpdateThreadMetadataParams;
...
/// Represents a newly created CX thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_cx_home_guard: Option<TempCodexHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<cx_protocol::dynamic_tools::DynamicToolSpec>,
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            if has_state_db && background_migration_enabled {
                ...
                let cx_home = config.cx_home.to_path_buf();
                tokio::spawn(async move {
                    ...
                    if compression_enabled {
                        cx_rollout::spawn_rollout_compression_worker(cx_home);
                    }
                });
            } else if compression_enabled {
                cx_rollout::spawn_rollout_compression_worker(config.cx_home.to_path_buf());
            }
            store
        }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        cx_apps_tools_cache: CodexAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        manager._test_cx_home_guard = Some(TempCodexHomeGuard { path: cx_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided CodexAuth and cx home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_cx_home = match AbsolutePathBuf::from_absolute_path_checked(&cx_home) {
            Ok(cx_home) => cx_home,
            Err(err) => panic!("test cx_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        let restriction_product = SessionSource::Exec.restriction_product();
        let skills_service = Arc::new(HostSkillsService::new_with_restriction_product(
            absolute_cx_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(cx_home, /*config_model_catalog*/ None),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
                plugins_manager,
    ...
    pub async fn list_models(
        ...
        http_client_factory: cx_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    CxErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<CodexThread>> {
        ...
        match threads.get(&thread_id) {
            ...
            Some(_) | None => Err(CxErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => CxErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(CX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(CxErr::SessionConfiguredNotFirstEvent);
            }
        };
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> cx_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return cx_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(cx_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> CxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => CxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => CxErr::InvalidRequest(message),
        err => CxErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> CxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => CxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => CxErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => CxErr::UnsupportedOperation(format!(
