...
use cx_code_mode::CodeModeNestedToolCall;
use cx_code_mode::CodeModeSession;
...
use cx_utils_audio::estimate_audio_token_count;
use cx_utils_output_truncation::TruncationPolicy;
use cx_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use cx_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = cx_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = cx_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = cx_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) fn take_unavailable_warning(&self, tool_mode: ToolMode) -> Option<String> {
        ...
        .then(|| {
            format!(
                "Code Mode is unavailable because {error}. {behavior}; enable `features.code_mode_host` and install `cx-mode-host`."
            )
        })
    }
    ...
    pub(crate) async fn execute(
        &self,
        mut request: cx_code_mode::ExecuteRequest,
    ) -> Result<cx_code_mode::StartedCell, String> {
        request
            .yield_time_ms
    ...
    ) -> Result<cx_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &cx_code_mode::CellId) {
...
async fn call_nested_tool(
    ...
    exec.session
        ...
        .track_code_mode_tool_call(cx_analytics::CodeModeToolCallFact::ChildStarted {
