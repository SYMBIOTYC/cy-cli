...
use cx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_LUNA_MODEL_ID;
use cx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_SOL_MODEL_ID;
...
use cx_protocol::openai_models::ApplyPatchToolType;
...
fn zsh_fork_config_for_spec_plan_tests() -> cx_tools::ZshForkConfig {
    let placeholder_exe = cx_utils_absolute_path::AbsolutePathBuf::try_from(
...
fn use_gt_auth(turn: &mut TurnContext) {
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(cx_tools::ResponsesApiNamespace {
    ...
    fn handle(&self, _call: ExtensionToolCall) -> cx_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(cx_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: cx_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    cx_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = cx_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(cx_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    cx_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(cx_code_mode::PUBLIC_TOOL_NAME) else {
    ...
    for (tool_name, guardian) in [
        ...
        for allow_login_shell in [false, true] {
            let plan = probe(|turn| {
                ...
                if guardian {
                    turn.session_source = cx_protocol::protocol::SessionSource::SubAgent(
                        cx_protocol::protocol::SubAgentSource::Other(
...
async fn shell_command_is_not_registered_without_a_single_local_environment() {
    let remote_environment = probe(|turn| {
        ...
        environment.environment = Arc::new(
            cx_exec_server::Environment::create_for_tests(Some(
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if cx_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            cx_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
                ...
                crate::session::turn_context::TurnEnvironment::new(
                    TurnEnvironmentSelection {
                        ...
                        config: EnvironmentConfigState::Ready(
                            cx_protocol::protocol::EnvironmentConfig {
    ...
    let cases = [
        ...
        (
            ...
            ToolPlanInputs {
                ...
                dynamic_tools: vec![dynamic_tool(
                    /*namespace*/ None,
                    cx_tools::TOOL_SEARCH_TOOL_NAME,
                    /*defer_loading*/ false,
                )],
                ..ToolPlanInputs::default()
            },
