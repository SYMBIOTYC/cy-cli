...
use cx_analytics::ControlToolCallFact;
...
impl<'a> ControlToolCallGuard<'a> {
    pub(crate) fn new(invocation: &'a ToolInvocation) -> Self {
        Self {
            invocation,
            started_at_ms: cx_analytics::now_unix_millis(),
    ...
    pub(crate) fn finish(&mut self, status: ControlToolCallStatus) {
        self.completed_at_ms = Some(cx_analytics::now_unix_millis());
        self.status = status;
    }
}
        ...
        invocation
            ...
            .track_control_tool_call(ControlToolCallFact {
                ...
                completed_at_ms: self
                    .completed_at_ms
                    .unwrap_or_else(cx_analytics::now_unix_millis),
                status: self.status,
            });
    }
}
