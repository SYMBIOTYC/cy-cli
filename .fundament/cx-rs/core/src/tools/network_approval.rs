...
use cx_network_proxy::BlockedRequest;
...
use cx_network_proxy::NetworkRequestDisconnect;
...
use cx_sandboxing::record_network_sandbox_violation;
        ...
        let approval_decision = match session.request_approval(action, approval_context).await {
            ...
            Err(ToolError::CX(err)) => {
                let telemetry_decision = if matches!(
                    err.details(),
                    cx_protocol::error::CodexErrorDetails::TurnAborted
                ) {
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::CX(cx_protocol::error::CxErr::Io(io::Error::other(format!(
