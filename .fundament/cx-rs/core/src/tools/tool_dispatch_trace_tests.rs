...
use cx_rollout_trace::ExecutionStatus;
use cx_rollout_trace::ThreadStartedTraceMetadata;
use cx_rollout_trace::ToolCallRequester;
...
impl cx_code_mode::CodeModeSessionProvider for MissingCellCodeModeSessionProvider {
    fn create_session<'a>(
        &'a self,
        _delegate: Arc<dyn cx_code_mode::CodeModeSessionDelegate>,
    ) -> cx_code_mode::CodeModeSessionProviderFuture<'a> {
        Box::pin(async {
            Ok(Arc::new(MissingCellCodeModeSession) as Arc<dyn cx_code_mode::CodeModeSession>)
        })
    }
}
...
impl cx_code_mode::CodeModeSession for MissingCellCodeModeSession {
    fn execute<'a>(
        &'a self,
        _request: cx_code_mode::ExecuteRequest,
    ) -> cx_code_mode::CodeModeSessionResultFuture<'a, cx_code_mode::StartedCell> {
        Box::pin(async { Err("test session cannot execute cells".to_string()) })
    }

    fn wait<'a>(
        &'a self,
        request: cx_code_mode::WaitRequest,
    ) -> cx_code_mode::CodeModeSessionResultFuture<'a, cx_code_mode::WaitOutcome> {
        self.terminate(request.cell_id)
    }

    fn terminate<'a>(
        &'a self,
        cell_id: cx_code_mode::CellId,
    ) -> cx_code_mode::CodeModeSessionResultFuture<'a, cx_code_mode::WaitOutcome> {
        Box::pin(async move {
            Ok(cx_code_mode::WaitOutcome::MissingCell(
                cx_code_mode::RuntimeResponse::Result {
    ...
    fn shutdown<'a>(&'a self) -> cx_code_mode::CodeModeSessionResultFuture<'a, ()> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let registry = ToolRegistry::with_handler_for_test(Arc::new(TestHandler {
        tool_name: cx_tools::ToolName::plain("test_tool"),
    }));
    ...
    let replayed = cx_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
    ...
    let result = registry
        .dispatch_any_with_terminal_outcome(
            test_invocation_with_payload(
                ...
                turn,
                "incompatible-call",
                cx_tools::ToolName::plain("test_tool"),
                ToolCallSource::Direct,
                ToolPayload::Custom {
                    input: "{}".to_string(),
                },
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        cx_tools::ToolName::plain(tool_name),
        source,
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace = cx_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
        root,
    ...
    rollout_thread_trace.record_cx_turn_started(turn.sub_id.as_str());
