...
use cx_protocol::protocol::FileSystemPath;
use cx_protocol::protocol::FileSystemSandboxEntry;
use cx_protocol::protocol::FileSystemSandboxPolicy;
use cx_protocol::protocol::InterAgentCommunication;
use cx_protocol::protocol::ItemCompletedEvent;
use cx_protocol::protocol::NetworkSandboxPolicy;
...
fn invocation(
    ...
    ToolInvocation {
        ...
        tool_name: cx_tools::ToolName::plain(tool_name),
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    cx_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    let mut config = (*turn.config).clone();
    let provider_info = built_in_model_providers()["cy"].clone();
...
async fn multi_agent_v2_spawn_fork_turns_all_applies_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: cx_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn spawn_agent_role_service_tier_falls_back_to_supported_parent_tier() {
    ...
    tokio::fs::create_dir_all(&turn.config.cx_home)
        .await
        .expect("cx home should be created");
    let role_config_path = turn.config.cx_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.cx_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
