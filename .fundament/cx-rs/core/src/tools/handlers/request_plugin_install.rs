...
use cx_analytics::PluginInstallRequestSource;
use cx_analytics::PluginInstallRequested;
use cx_analytics::PluginInstallRequestedPlugin;
...
use cx_tools::DiscoverableToolAction;
use cx_tools::DiscoverableToolType;
use cx_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use cx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use cx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use cx_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use cx_tools::RequestPluginInstallArgs;
use cx_tools::RequestPluginInstallResult;
use cx_tools::ToolName;
use cx_tools::ToolSpec;
use cx_tools::all_requested_connectors_picked_up;
use cx_tools::build_request_plugin_install_elicitation_request;
use cx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use cx_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("cx-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in cx-tui yet".to_string(),
            ));
        }
        ...
        let tool = if self.presentation == ToolSuggestPresentation::RecommendationContext {
            ...
            match cx_core_plugins::hydrate_selected_recommended_plugin_install_metadata(
                &plugins_config,
                auth.as_ref(),
                tool,
            )
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.cx_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    cx_home: &cx_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&cx_login::CodexAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_cx_apps_tools().await {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh cx apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
