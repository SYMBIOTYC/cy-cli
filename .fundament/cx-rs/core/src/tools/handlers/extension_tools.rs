...
pub(crate) struct ExtensionToolAdapter(Arc<dyn cx_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn cx_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let cx_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    impl cx_extension_api::ToolExecutor<cx_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> cx_tools::ToolName {
            cx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> cx_tools::ToolSpec {
            cx_tools::ToolSpec::Function(cx_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: cx_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: cx_tools::ToolCall) -> cx_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(cx_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn cx_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<cx_tools::ToolCall>>>,
    }

    impl cx_extension_api::ToolExecutor<cx_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> cx_tools::ToolName {
            cx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn handle(&self, call: cx_tools::ToolCall) -> cx_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: cx_tools::ToolCall,
        ) -> Result<Box<dyn cx_tools::ToolOutput>, cx_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: cx_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = cx_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        cx_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        cx_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
