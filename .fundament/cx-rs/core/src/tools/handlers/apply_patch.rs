...
use cx_apply_patch::ApplyPatchFileChange;
use cx_apply_patch::ApplyPatchFileUpdateMode;
use cx_apply_patch::Hunk;
use cx_apply_patch::StreamingPatchParser;
...
use cx_protocol::protocol::PatchApplyUpdatedEvent;
...
use cx_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[cx_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &cx_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    cx_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        cx_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match cx_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match cx_apply_patch::verify_apply_patch_args_with_mode(
            args,
        ...
        .await
        {
            cx_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            cx_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            cx_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            cx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match cx_apply_patch::maybe_parse_apply_patch_verified_with_mode(
        command,
        cwd,
        apply_patch_file_update_mode(turn),
        fs,
    ...
    .await
    {
        ...
        cx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
