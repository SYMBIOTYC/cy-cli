...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    write_curated_plugin_sha(cx_home.path());
    write_plugins_feature_config(cx_home.path());
    ...
    let refreshed_config = load_plugins_config(cx_home.path()).await;
    assert!(verified_plugin_install_completed(
        "sample@openai-api-curated",
        &refreshed_config,
        &plugins_manager,
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&cx_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(cx_home.path().join(CONFIG_TOML_FILE)).expect("read config");
