...
#[cfg(unix)]
use cx_protocol::approvals::GuardianCommandSource;
    ...
    pub(crate) fn permission_request_payload(&self) -> PermissionRequestPayload {
        match self {
            ...
            Self::Execve { command, .. } => PermissionRequestPayload::bash(
                cx_shell_command::parse_command::shlex_join(command),
                /*description*/ None,
            ),
...
fn guardian_cwd(environment_id: &str, cwd: PathUri) -> std::io::Result<AbsolutePathBuf> {
    match cwd.to_abs_path() {
        Ok(cwd) => Ok(cwd),
        Err(err) if environment_id != cx_exec_server::LOCAL_ENVIRONMENT_ID => Err(err),
        Err(_) => {
        ...
        match action {
            ...
            | ApprovalAction::ExecCommand {
                ...
                let policy_fingerprint = ctx
                    ...
                    .map(cx_execpolicy::RequirementsExecPolicy::fingerprint);
