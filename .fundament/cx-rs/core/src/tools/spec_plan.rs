...
use cx_tools::TOOL_SEARCH_TOOL_NAME;
use cx_tools::ToolCall as ExtensionToolCall;
use cx_tools::ToolEnvironmentMode;
use cx_tools::ToolExecutor;
use cx_tools::ToolExposures;
use cx_tools::ToolName;
use cx_tools::ToolSearchInfo;
use cx_tools::ToolSpec;
use cx_tools::UnifiedExecShellMode;
use cx_tools::can_request_original_image_detail;
use cx_tools::collect_code_mode_exec_prompt_tool_definitions;
use cx_tools::collect_request_plugin_install_entries;
use cx_tools::default_namespace_description;
use cx_tools::request_user_input_available_modes;
use cx_tools::shell_command_backend_for_features;
use cx_tools::shell_type_for_model_and_features;
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(cx_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(cx_code_mode::WAIT_TOOL_NAME),
        ] {
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && cx_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&cx_code_mode::normalize_code_mode_identifier(
                &cx_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        cx_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && cx_code_mode::is_code_mode_nested_tool(&cx_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !cx_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(cx_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let execute_handler = CodeModeExecuteHandler::new(
        create_code_mode_tool(
            ...
            {
                cx_code_mode::ImageDetailVisibility::Hidden
            } else {
                cx_code_mode::ImageDetailVisibility::Visible
            },
        ),
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, cx_code_mode::ToolNamespaceDescription> {
    let mut namespace_descriptions = BTreeMap::new();
    for spec in specs {
        let ToolSpec::Namespace(namespace) = spec else {
            continue;
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| cx_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &cx_code_mode::ToolDefinition,
    right: &cx_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, cx_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &cx_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, cx_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
