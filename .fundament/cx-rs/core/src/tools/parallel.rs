...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        if self
            ...
            .config
            .features
            .enabled(cx_features::Feature::ExecutedToolCallMetadata)
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: cx_protocol::models::FunctionCallOutputPayload {
                    body: cx_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
    ...
    fn drop(&mut self) {
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    event.name = "cx.tool_call",
                    trace_id = %cx_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = cx_tools::ToolName::plain("test_tool");
        ...
        let timing_events = logs
            .lines()
            .filter(|line| line.contains("event.name=\"cx.tool_call\""))
            .collect::<Vec<_>>();
        assert_eq!(
            timing_events.len(),
            1,
    ...
    impl cx_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl cx_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: cx_extension_api::ToolFinishInput<'a>,
        ) -> cx_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let mut builder =
            cx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = cx_tools::ToolName::plain("cleanup_tool");
