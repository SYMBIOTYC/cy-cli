...
#[cfg(target_os = "macos")]
use cx_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = cx_network_proxy::build_config_state(
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/cx-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/cx-path:/usr/bin:/bin"),
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/cx-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/cx-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/cx-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/cx-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/cx-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_cx_marker() {
    let env = HashMap::from([
        (
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_cx_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_cx_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_cx_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{CX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("cx-path");
...
fn maybe_wrap_shell_lc_with_snapshot_does_not_embed_override_values_in_argv() {
    ...
    assert!(!rewritten[2].contains("oi_identity_token_file"));
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
