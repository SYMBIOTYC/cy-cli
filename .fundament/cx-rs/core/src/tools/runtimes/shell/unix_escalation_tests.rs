...
use cx_execpolicy::PolicyParser;
...
use cx_sandboxing::policy_transforms::effective_permission_profile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(cx_execpolicy::Policy::empty())),
        ...
        stopwatch: cx_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = cx_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = cx_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.cx_home)
        .context("recreate cx home for hook fixtures")?;
    ...
    let expected_hook_command =
        cx_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    ...
    session
        ...
        .await;
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(cx_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        cx_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        cx_shell_escalation::EscalationDecision::Escalate(
            cx_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, cx_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        cx_shell_escalation::EscalationDecision::Deny {
