...
use cx_execpolicy::MatchOptions;
...
use cx_protocol::protocol::NetworkPolicyRuleAction;
...
use cx_sandboxing::policy_transforms::merge_permission_profiles;
use cx_sandboxing::record_filesystem_sandbox_violation;
use cx_shell_command::bash::parse_shell_lc_plain_commands;
use cx_shell_escalation::EscalateServer;
use cx_shell_escalation::EscalationDecision;
use cx_shell_escalation::EscalationExecution;
use cx_shell_escalation::EscalationPermissions;
use cx_shell_escalation::EscalationPolicy;
use cx_shell_escalation::EscalationPolicyFuture;
use cx_shell_escalation::EscalationSession;
use cx_shell_escalation::ExecParams;
use cx_shell_escalation::ExecResult;
use cx_shell_escalation::PreparedExec;
use cx_shell_escalation::ResolvedPermissionProfile;
use cx_shell_escalation::ShellCommandExecutor;
use cx_shell_escalation::ShellCommandExecutorFuture;
use cx_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        cx_linux_sandbox_exe: ctx.step_context.turn.config.cx_linux_sandbox_exe.clone(),
...
impl CoreShellActionProvider {
    ...
    async fn prompt(
            ...
            .await
        {
            ...
            Err(ToolError::CX(err)) => Err(err.into()),
        }
    }
...
struct CoreShellCommandExecutor {
    ...
    network: Option<cx_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            cx_linux_sandbox_exe: self.cx_linux_sandbox_exe.as_deref(),
