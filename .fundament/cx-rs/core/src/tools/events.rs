...
use cx_analytics::ArtifactOperation;
use cx_analytics::ArtifactOperationLifecycle;
use cx_analytics::build_track_events_context;
use cx_apply_patch::AppliedPatchDelta;
use cx_core_plugins::PluginCommandAttribution;
use cx_core_plugins::recognize_artifact_operation;
use cx_otel::ARTIFACT_OPERATION_EXPECTED_OUTPUT_COUNT_METRIC;
use cx_otel::ARTIFACT_OPERATION_STARTED_METRIC;
...
use cx_protocol::items::CommandExecutionStatus;
        ...
        ctx.session
            ...
            .track_artifact_operation(
                ...
                ArtifactOperation {
                    ...
                    occurred_at_ms: cx_analytics::now_unix_millis(),
...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::CX(err)) => match err.details() {
...
mod tests {
    ...
    async fn assert_failed_apply_patch_tracks_committed_delta(
        ...
        let delta = cx_apply_patch::apply_patch(
