...
fn exec_env_policy_excludes_non_inheritable_and_runtime_variables() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "cx_permission_profile".to_string(),
                "stale-profile".to_string(),
            ),
            (
                "oi_identity_token_file".to_string(),
                "/run/identity-token".to_string(),
            ),
            (
                "cx_apply_patch_preserve_line_endings".to_string(),
                "1".to_string(),
            ),
            (
                "cx_plugin_metrics_output".to_string(),
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: cx_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = cx_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: cx_exec_server::ExecEnvPolicy {
                inherit: cx_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: cx_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = cx_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    assert_eq!(
        ...
        Some(cx_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: cx_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let cx_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let cx_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        cx_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            cx_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            cx_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process
