...
async fn reserved_thread_id_is_used_without_changing_normal_id_generation() {
    ...
    config.cx_home = temp_dir.path().join("cx-home").abs();
    config.cwd = config.cx_home.abs();
    std::fs::create_dir_all(&config.cx_home).expect("create cx home");
    ...
    assert!(matches!(
        ...
        cx_protocol::error::CodexErrorDetails::InvalidRequest(message)
...
impl cx_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: cx_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> cx_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<cx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> cx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<cx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> cx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: cx_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl cx_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: cx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> cx_extension_api::ExtensionFuture<'a, Vec<cx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
    ...
    impl cx_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        fn contribute<'a>(
            &'a self,
            context: cx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> cx_extension_api::ExtensionFuture<'a, Vec<cx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = cx_mcp::cx_apps_mcp_server_config(
                ...
                vec![cx_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = cx_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("cx_work_desktop".to_string()),
            environments: Some(Vec::new()),
    ...
    let selected_servers = |config: &cx_mcp::McpConfig| {
        cx_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let cx_apps_headers = match cx_apps_server.transport {
        ...
        cx_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("CX Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        cx_apps_headers
            .expect("CX Apps headers should be configured")
            .get("originator"),
        Some(&"cx_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_cx_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.cx_home = temp_dir.path().join("cx-home").abs();
    config.cwd = config.cx_home.abs();
    ...
    assert!(!config.cx_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.cx_home.join("models_cache.json").exists());
}
