...
use cx_model_provider::BearerAuthProvider;
...
use cx_rollout_trace::RolloutTrace;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: cx_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_gt_auth_json(cx_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn gt_auth_manager(
    ...
    write_gt_auth_json(cx_home.path());
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(cx_protocol::ResponseItemId::with_suffix("msg", id)),
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart CX, url: {url}"
        )
    );
}
...
impl ModelProvider for TestRecoveryProvider {
    ...
    fn recover_from_unauthorized(
        ...
        Box::pin(async move {
            if self.should_fail {
                Err(CxErr::Io(std::io::Error::other("provider recovery failed")))
            } else {
    ...
    fn models_manager(
        ...
        self.inner.models_manager(cx_home, config_model_catalog)
    }
}
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_gt_cx_responses() {
...
#[tokio::test]
async fn non_gt_cx_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
