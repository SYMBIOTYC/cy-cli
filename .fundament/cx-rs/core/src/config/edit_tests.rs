...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = cx_home.join("config-link.toml");
    let config_path = cx_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let cx_home = tmp.path();
    let link_a = cx_home.join("a.toml");
    let link_b = cx_home.join("b.toml");
    let config_path = cx_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(cx_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(cx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_cx_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let cx_home = tmp.path();
    std::fs::write(
        cx_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        cx_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-cx-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-cx-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(cx_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !cx_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(cx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(cx_home.join(CONFIG_TOML_FILE)).expect("read config");
