...
use crate::config::permissions::get_readable_roots_required_for_cx_runtime;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(cx_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(cx_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl AuthManagerConfig for Config {
    ...
    fn forced_gt_workspace_id(&self) -> Option<Vec<String>> {
        self.forced_gt_workspace_id.clone()
    }
...
impl ConfigBuilder {
    pub fn cx_home(mut self, cx_home: PathBuf) -> Self {
        self.cx_home = Some(cx_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let cx_home = match cx_home {
            Some(cx_home) => AbsolutePathBuf::from_absolute_path(cx_home)?,
            None => find_cx_home()?,
        };
...
impl Config {
    pub fn sqlite_config(&self) -> &cx_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub fn http_client_factory(&self) -> HttpClientFactory {
        ...
        if self.features.enabled(Feature::Psp) {
            factory.with_gt_cookies([HeaderValue::from_static("oai-chat-psp=true")])
        } else {
            factory
        }
    }
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            gt_base_url: self.gt_base_url.clone(),
            apps_mcp_product_sku: self.apps_mcp_product_sku.clone(),
            cx_home: self.cx_home.to_path_buf(),
            ...
            cx_linux_sandbox_exe: self.cx_linux_sandbox_exe.clone(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.cx_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides_for_cx_home(
        cx_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &cx_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// in [ConfigToml], such as `cwd`, `cx_self_exe`, `cx_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(cx_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(cx_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, cx_home).map_err(|e| {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&cx_home);
        ...
        let forced_gt_workspace_id = cfg
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| cx_home.join("log").to_path_buf());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_cx_runtime(
            &cx_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            cx_home,
            sqlite: cx_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            config_layer_stack,
            history,
            ...
            gt_base_url: cfg
                .gt_base_url
                .unwrap_or("https://cy.symbiotyc.workers.dev/v1/".to_string()),
