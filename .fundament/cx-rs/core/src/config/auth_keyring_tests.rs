...
use cx_config::config_toml::ForcedChatgptWorkspaceIds;
...
fn managed_auth_restrictions_intersect_workspaces_and_fail_closed() {
    let config = ConfigToml {
        ...
        forced_gt_workspace_id: Some(ForcedChatgptWorkspaceIds::Multiple(vec![
            " denied ".to_string(),
            " allowed ".to_string(),
        ])),
        ..Default::default()
    ...
    let auth_config = bootstrap_auth_config(Path::new("cx-home"), &bootstrap_config)
    ...
    assert_eq!(
        auth_config.forced_gt_workspace_id,
        Some(vec!["denied".to_string(), "allowed".to_string()])
    );
    assert_eq!(
        auth_config.effective_gt_workspaces(),
    ...
    assert_eq!(
        bootstrap_auth_config(Path::new("cx-home"), &bootstrap_config)
            .expect_err("gt-only policy without an allowed workspace must fail")
            .kind(),
...
#[test]
fn bootstrap_auth_config_applies_managed_store_and_gt_base_url() {
    ...
    let config_toml = ConfigToml {
        ...
        gt_base_url: Some(configured_url.to_string()),
        ..Default::default()
    };
    ...
    assert_eq!(auth_config.gt_base_url.as_deref(), Some(managed_url));
}
