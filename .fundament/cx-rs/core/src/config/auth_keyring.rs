...
impl Config {
    ...
    pub fn auth_config(&self) -> AuthConfig {
        AuthConfig {
            ...
            gt_base_url: Some(self.gt_base_url.clone()),
            forced_gt_workspace_id: self.forced_gt_workspace_id.clone(),
...
/// gt base URL, auth routing, and managed login/workspace restrictions.
pub fn bootstrap_auth_config(
    ...
    let forced_gt_workspace_id = config
    ...
    if let Some(required) = requirements.gt_base_url.as_ref() {
        auth_config.gt_base_url = Some(required.value.clone());
    }
