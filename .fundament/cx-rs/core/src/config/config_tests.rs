...
use cx_config::McpServerCommandMatcher;
use cx_config::McpServerIdentity;
use cx_config::McpServerRequirement;
use cx_config::McpServerValueMatcher;
use cx_config::ProfileV2Name;
use cx_config::RequirementSource;
use cx_config::Sourced;
use cx_config::config_toml::AgentRoleToml;
use cx_config::config_toml::AgentsToml;
use cx_config::config_toml::AutoReviewToml;
use cx_config::config_toml::ConfigToml;
use cx_config::config_toml::ExperimentalRequestUserInput;
use cx_config::config_toml::ProjectConfig;
use cx_config::config_toml::RealtimeConfig;
use cx_config::config_toml::RealtimeToml;
use cx_config::config_toml::RealtimeTransport;
use cx_config::config_toml::RealtimeWsMode;
use cx_config::config_toml::RealtimeWsVersion;
use cx_config::config_toml::ToolsToml;
...
use cx_config::permissions_toml::NetworkDomainPermissionToml;
use cx_config::permissions_toml::NetworkDomainPermissionsToml;
use cx_config::permissions_toml::NetworkMitmActionToml;
use cx_config::permissions_toml::NetworkMitmHookToml;
use cx_config::permissions_toml::NetworkMitmToml;
...
use cx_config::permissions_toml::WorkspaceRootsToml;
use cx_config::types::AppToolApproval;
use cx_config::types::ApprovalsReviewer;
use cx_config::types::BundledSkillsConfig;
use cx_config::types::FeedbackConfigToml;
use cx_config::types::HistoryPersistence;
use cx_config::types::McpServerEnvVar;
use cx_config::types::McpServerOAuthConfig;
use cx_config::types::McpServerToolConfig;
use cx_config::types::McpServerTransportConfig;
use cx_config::types::MemoriesConfig;
use cx_config::types::MemoriesToml;
use cx_config::types::ModelAvailabilityNuxConfig;
use cx_config::types::Notice;
use cx_config::types::NotificationCondition;
use cx_config::types::NotificationMethod;
use cx_config::types::Notifications;
use cx_config::types::OtelConfigToml;
use cx_config::types::OtelExporterKind;
use cx_config::types::ResumeCwdMode;
use cx_config::types::SandboxWorkspaceWrite;
use cx_config::types::SessionPickerViewMode;
use cx_config::types::SkillsConfig;
use cx_config::types::ToolSuggestDisabledTool;
use cx_config::types::ToolSuggestDiscoverableType;
use cx_config::types::Tui;
...
use cx_config::types::WindowsToml;
...
use cx_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
...
use cx_protocol::protocol::NetworkAccess;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: cx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
excluded_tool_namespaces = ["mcp__cx_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__cx_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
"#,
        ...
        assert_eq!(
            static_headers
                .get("X-oi-Internal-CX-Residency")
                .map(String::as_str),
...
"#,
    ...
    assert_eq!(
        ...
        cx_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    assert_eq!(
        config
            .auth_route_config()
    ...
    assert_eq!(
        ...
        cx_core_plugins::remote::RemotePluginServiceConfig::new(
            config.gt_base_url,
...
-> std::io::Result<()> {
    ...
    let memories_root = cx_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let cx_home = temp_dir.path().join("cx-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".cx"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".cx".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".cx"), cwd.as_path()),
            "expected .cx carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".cx"), cwd.path()),
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of CX and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of CX and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of CX."
        )),
        "{:?}",
        config.startup_warnings
    );
...
-> std::io::Result<()> {
    let cases: Vec<(&str, &str, Option<Vec<&str>>)> = vec![
        ...
        (
            "whitespace string",
            r#"forced_gt_workspace_id = "   ""#,
            None,
        ),
        ("empty list", r#"forced_gt_workspace_id = []"#, None),
        (
            "blank list entries",
            r#"forced_gt_workspace_id = ["", "  "]"#,
            None,
        ...
        (
            "mixed list entries",
            r#"forced_gt_workspace_id = ["", " 123e4567-e89b-42d3-a456-426614174000 ", "123e4567-e89b-42d3-a456-426614174001"]"#,
            Some(vec![
    ...
    for (name, toml, expected) in cases {
        let cfg_toml: ConfigToml = toml::from_str(toml)
            .unwrap_or_else(|err| panic!("{name} should parse forced_gt_workspace_id: {err}"));
        let config = Config::load_from_base_config_with_overrides(
            cfg_toml,
            ConfigOverrides::default(),
            tempdir().expect("tempdir").abs(),
        ...
        assert_eq!(config.forced_gt_workspace_id, expected, "{name}");
    }
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    cx_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            cx_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, cx_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.cx", cx_home.path());
    ...
    let requirements_toml = cx_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = cx_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_cx_folder: project_dot_codex.clone(),
                },
                toml::toml! {
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_cx_folder: project_dot_codex,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            cx_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let default_zsh_path = cx_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_cx_home_for_workspace_write() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), cx_home.path());
...
-> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let memories_root = cx_home.path().join("memories");
    let writable_root = cx_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
#[tokio::test]
async fn config_applies_managed_auth_store_and_gt_base_url() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        requirements,
        cx_config::ConfigRequirementsToml::default(),
    )?;
    ...
    let config = Config::load_config_with_layer_stack(
        ...
        ConfigToml {
            ...
            gt_base_url: Some("https://user.example/backend-api/".to_string()),
            ..Default::default()
        },
        ConfigOverrides {
            cwd: Some(cx_home.path().to_path_buf()),
    ...
    assert_eq!(config.gt_base_url, managed_url);
...
"#,
    ...
    let project_config_dir = workspace.path().join(".cx");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    ...
    let config_path = cx_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), cx_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(cx_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let cx_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(cx_home.path());
    ...
    let loaded = load_global_mcp_servers(cx_home.path()).await?;
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, cx_mcp::McpProtocolMode::Legacy);
...
"#,
    ...
    let err = load_global_mcp_servers(cx_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/cx-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/cx-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-cx-123".to_string()),
                callback_port: None,
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-cx-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-cx-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(cx_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(cx_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let cx_home = TempDir::new()?;
    let base_config = cx_home.path().join(CONFIG_TOML_FILE);
    let selected_config = cx_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(cx_home.path(), &profile_name),
        cx_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn cx_home(&self) -> AbsolutePathBuf {
        self.cx_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let workspace = cx_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, cx_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        cx_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let missing_path = cx_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let role_config_path = cx_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = cx_config::ConfigLayerStack::new(
        vec![cx_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: cx_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".cx").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let researcher_path = cx_home.path().join("agents").join("researcher.toml");
    let reviewer_path = cx_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = cx_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let catalog_path = cx_home.path().join("catalog.json");
...
"#;
    ...
    let cx_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        cx_home: cx_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: cx_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = cx_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![cx_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = cx_config::RequirementSource::Unknown;
    ...
    let requirements = cx_config::ConfigRequirements {
        web_search_mode: cx_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        cx_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
"#,
    ...
    assert_eq!(
        ...
        Some(cx_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let cwd = cx_home.path().join("workspace");
    let outside_root = cx_home.path().join("outside-write");
...
-> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let selected_config = cx_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let catalog_path = cx_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = cx_home.path().join("required-state");
    let required_log_dir = cx_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&cx_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let config = load_with_enterprise_requirement(&cx_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let cx_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(cx_home.path().join("required-state"))?;
    let environment = cx_home.path().join("environment-state");
