...
pub fn apply_blocking(cx_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(cx_home: &Path) -> Self {
        Self::for_config_path(&cx_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(cx_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.cx_home.join(CONFIG_TOML_FILE).to_path_buf());
