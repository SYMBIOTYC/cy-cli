...
use cx_config::ConfigError;
use cx_config::ConfigLayerEntry;
use cx_config::ConfigLayerSource;
use cx_config::ConfigLoadError;
use cx_config::ConfigLoadOptions;
use cx_config::ConfigRequirements;
use cx_config::ConfigRequirementsToml;
use cx_config::ConfigRequirementsWithSources;
use cx_config::FilesystemDenyReadPattern;
use cx_config::LoaderOverrides;
use cx_config::RequirementSource;
use cx_config::RequirementsLayerEntry;
use cx_config::SessionThreadConfig;
use cx_config::StaticThreadConfigLoader;
use cx_config::ThreadConfigSource;
use cx_config::compose_requirements;
use cx_config::config_error_from_ignored_toml_fields;
use cx_config::config_error_from_toml;
use cx_config::config_error_from_typed_toml;
use cx_config::config_toml::ConfigToml;
use cx_config::config_toml::ProjectConfig;
use cx_config::loader::load_config_layers_state;
use cx_config::loader::load_requirements_toml;
...
use cx_protocol::config_types::EnvironmentVariablePattern;
...
async fn write_project_hook_config(
    ...
    tokio::fs::create_dir_all(dot_cx_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_cx_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when CX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn ignore_project_config_skips_project_discovery() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(cx_home.as_path()).await?;
    ...
    .await?;
    let project_config_dir = workspace.join(".cx");
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = cx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        cx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = cx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = cx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_packaged_defaults_when_other_layers_are_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when CX_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_cx_home() -> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let cx_home = tmp.path().join("cx-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.cx/logs", &cx_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(cx_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(cx_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![cx_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(cx_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(cx_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.cx".to_string(),
...
"#,
    ...
    let mut config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let config_toml = AbsolutePathBuf::from_absolute_path(cx_home.join(CONFIG_TOML_FILE))?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(cx_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_cx_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let cx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&cx_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", cx_home.display()))
                .expect("bundle requirements path should resolve against cx_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let cx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&cx_home)
        .await
        .expect("create cx home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            cx_config::SandboxModeRequirement::ReadOnly,
            cx_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    cx_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".cx")).await?;
    tokio::fs::create_dir_all(project_root.join(".cx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".cx").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".cx").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_cx_folder } => Some(dot_cx_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".cx").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".cx").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".cx")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".cx")).await?;
    ...
    .await?;
    write_project_hook_config(
        &worktree_root.join(".cx"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".cx"),
        Some("worktree-child"),
...
async fn forged_linked_worktree_does_not_inherit_repo_trust() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(attacker_root.join(".cx")).await?;
    tokio::fs::write(
    ...
    .await?;
    tokio::fs::write(
        attacker_root.join(".cx").join(CONFIG_TOML_FILE),
        r#"foo = "attacker"
"#,
    )
    .await?;
...
async fn malformed_untrusted_linked_worktree_does_not_read_root_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".cx")).await?;
    tokio::fs::create_dir_all(repo_root.join(".cx")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    tokio::fs::write(worktree_root.join(".cx").join(CONFIG_TOML_FILE), "foo =").await?;
    tokio::fs::write(repo_root.join(".cx").join(CONFIG_TOML_FILE), [0xff]).await?;
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".cx"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
...
"#;
    tokio::fs::write(project_root.join(".cx").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".cx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".cx").join("root.txt"),
        "root instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let cx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&cx_home).await?;
    tokio::fs::write(cx_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_cx_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_cx_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".cx"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn cx_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let cx_home = home_dir.join(".cx");
    tokio::fs::create_dir_all(&cx_home).await?;
    tokio::fs::write(
        cx_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn cx_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".cx");
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_cx_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_codex)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let cx_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&cx_home_untrusted).await?;
    make_config_for_test(
        &cx_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
    ...
    .await?;
    let untrusted_config_path = cx_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let cx_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&cx_home_unknown).await?;
    tokio::fs::write(
        cx_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    // `.cx` folder. The malformed profile value below would fail typed path
...
gt_base_url = "https://attacker.example/backend-api"
apps_mcp_product_sku = "attacker"
responses_api_metadata = { cx_security_surface = "attacker" }
...
"#,
    ...
    // still use the project-local `.cx` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".cx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".cx").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let cx_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&cx_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let cx_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&cx_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &cx_home,
...
mod requirements_exec_policy_tests {
    ...
    use cx_config::ConfigRequirementsWithSources;
    use cx_config::RequirementSource;
    use cx_config::RequirementsExecPolicyDecisionToml;
    use cx_config::RequirementsExecPolicyParseError;
    use cx_config::RequirementsExecPolicyPatternTokenToml;
    use cx_config::RequirementsExecPolicyPrefixRuleToml;
    use cx_config::RequirementsExecPolicyToml;
    use cx_execpolicy::Decision;
    use cx_execpolicy::Evaluation;
    ...
    fn config_stack_for_dot_cx_folder_with_requirements(
        dot_cx_folder: &Path,
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_cx_folder_with_requirements(temp_dir.path(), requirements);
