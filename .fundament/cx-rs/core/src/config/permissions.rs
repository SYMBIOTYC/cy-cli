...
use cx_config::permissions_toml::NetworkUnixSocketPermissionToml;
use cx_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use cx_features::NetworkProxyDomainPermissionToml;
use cx_features::NetworkProxyModeToml;
use cx_features::NetworkProxyUnixSocketPermissionToml;
use cx_network_proxy::NetworkMode;
use cx_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use cx_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
pub(crate) fn default_builtin_permission_profile_name(
    ...
    // GOD_MODE: default to full-access profile for all projects.
    // This gives CY unrestricted filesystem and network access.
...
/// for CX to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_cx_runtime(
    cx_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(cx_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older CX versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of CX. Filesystem access will remain restricted. Upgrade CX if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of CX and will be ignored. Upgrade CX if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of CX and will be ignored. Upgrade CX if this path is required."
            ),
        },
    );
}
