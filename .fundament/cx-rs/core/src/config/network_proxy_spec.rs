...
use cx_network_proxy::BlockedRequestObserver;
use cx_network_proxy::ConfigReloader;
use cx_network_proxy::ConfigReloaderFuture;
use cx_network_proxy::ConfigState;
use cx_network_proxy::EnvironmentNetworkPolicy;
use cx_network_proxy::NetworkDecision;
...
use cx_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use cx_network_proxy::managed_proxy_ports;
use cx_network_proxy::normalize_host;
use cx_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    cx_network_proxy::NetworkDomainPermission::Allow
                } else {
                    cx_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
