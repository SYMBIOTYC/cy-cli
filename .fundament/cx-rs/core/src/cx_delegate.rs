...
/// Start an interactive sub-CX thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_cx_thread_interactive(
    mut config: Config,
    ...
    let extensions = if crate::guardian::is_guardian_reviewer_source(&session_source) {
        cx_extension_api::empty_extension_registry()
    } else {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_cx_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
