...
//!
//! `ModelClient` is intended to live for the lifetime of a CX session and holds the stable
...
//! per-turn state such as the `x-cx-turn-state` token used for sticky routing.
//!
...
use cx_api::AgentIdentityTelemetry;
use cx_api::ApiError;
use cx_api::AuthProvider;
use cx_api::CompactClient as ApiCompactClient;
use cx_api::CompactionInput as ApiCompactionInput;
use cx_api::Compression;
use cx_api::MemoriesClient as ApiMemoriesClient;
use cx_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use cx_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use cx_api::Provider as ApiProvider;
use cx_api::RawMemory as ApiRawMemory;
use cx_api::RealtimeCallClient as ApiRealtimeCallClient;
use cx_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use cx_api::Reasoning;
use cx_api::ReasoningContext;
use cx_api::RequestTelemetry;
use cx_api::ReqwestTransport;
use cx_api::ResponseCreateWsRequest;
use cx_api::ResponsesApiRequest;
use cx_api::ResponsesClient as ApiResponsesClient;
use cx_api::ResponsesOptions as ApiResponsesOptions;
use cx_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use cx_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use cx_api::ResponsesWsRequest;
use cx_api::SharedAuthProvider;
use cx_api::SseTelemetry;
use cx_api::StreamOptions;
use cx_api::TransportError;
use cx_api::WebsocketTelemetry;
use cx_api::auth_header_telemetry;
use cx_api::build_session_headers;
use cx_api::create_text_param_for_request;
use cx_api::response_create_client_metadata;
use cx_http_client::ClientRouteClass;
use cx_http_client::HttpClientFactory;
use cx_login::AuthManager;
use cx_login::CodexAuth;
use cx_login::RefreshTokenError;
use cx_login::UnauthorizedRecovery;
use cx_login::default_client::add_originator_header;
use cx_login::default_client::create_client_for_route;
...
use cx_protocol::config_types::Verbosity as VerbosityConfig;
...
use cx_protocol::protocol::InternalSessionSource;
use cx_protocol::protocol::SessionSource;
use cx_protocol::protocol::W3cTraceContext;
use cx_rollout_trace::CompactionTraceContext;
use cx_rollout_trace::InferenceTraceAttempt;
use cx_rollout_trace::InferenceTraceContext;
use cx_tools::create_tools_json_for_responses_api;
use cx_tools::create_tools_json_for_responses_lite;
use cx_tools::create_tools_raw_json_for_responses_api;
...
use cx_feedback::FeedbackRequestTags;
use cx_feedback::emit_feedback_request_tags_with_auth_env;
use cx_login::auth::AgentIdentityAuthPolicy;
use cx_login::auth_env_telemetry::AuthEnvTelemetry;
use cx_login::auth_env_telemetry::collect_auth_env_telemetry;
use cx_model_provider::AgentIdentitySessionFallback;
...
use cx_response_debug_context::telemetry_api_error_message;
use cx_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "oi-Beta";
pub const X_CODEX_INSTALLATION_ID_HEADER: &str = "x-cx-installation-id";
pub const X_CODEX_ROUTING_HINT_HEADER: &str = "x-cx-routing-hint";
pub const X_CODEX_TURN_STATE_HEADER: &str = "x-cx-turn-state";
pub const X_CODEX_TURN_METADATA_HEADER: &str = "x-cx-turn-metadata";
pub const X_CODEX_PARENT_THREAD_ID_HEADER: &str = "x-cx-parent-thread-id";
pub const X_CODEX_WINDOW_ID_HEADER: &str = "x-cx-window-id";
...
const X_CODEX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-cx-ws-stream-request-start-ms";
...
const X_OPENAI_INTERNAL_CODEX_RESPONSES_LITE_HEADER: &str = "x-openai-internal-cx-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a CX session
...
/// - The `x-cx-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each CX turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-cx-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-cx-turn-state` request header for
...
///
/// API-key sessions send that API bearer. gt-auth sessions send their bearer plus account id;
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a CX session. Per-turn values
    ...
    pub fn new(
        ...
        let cx_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.cx_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), cx_api_key_env_enabled);
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: cx_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    fn build_routing_hint_header(
        ...
        if !auth.is_some_and(CodexAuth::uses_cx_backend)
            || !provider.is_openai()
    ...
    async fn connect_websocket(
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                cx_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&CodexAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(CodexAuth::uses_cx_backend)
    ...
    /// Streams a turn via the oi Responses API.
    ///
    ...
    /// Permanently disables WebSockets for this CX session and resets WebSocket state.
    ///
...
///
/// These headers implement CX-specific conventions:
///
/// - `x-cx-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-cx-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-cx-beta-features", header_value);
    }
...
fn map_response_stream(
    api_stream: cx_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let cx_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = cx_api::ResponseStream {
        rx_event,
