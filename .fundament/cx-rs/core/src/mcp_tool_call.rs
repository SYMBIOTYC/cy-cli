...
use cx_analytics::AppInvocation;
use cx_analytics::InvocationType;
use cx_analytics::build_track_events_context;
use cx_api::HostedFileUploadContext;
use cx_config::ConfigLayerSource;
use cx_config::types::AppToolApproval;
use cx_connectors::AppToolPolicy;
...
use cx_mcp::ToolInfo;
use cx_mcp::auth_elicitation_completed_result;
use cx_mcp::build_auth_elicitation_plan;
use cx_mcp::mcp_permission_prompt_is_auto_approved;
...
use cx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use cx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use cx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use cx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use cx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use cx_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use cx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use cx_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use cx_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use cx_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use cx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use cx_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use cx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use cx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use cx_protocol::request_user_input::RequestUserInputQuestion;
use cx_protocol::request_user_input::RequestUserInputQuestionOption;
...
use cx_utils_output_truncation::TruncationPolicy;
use cx_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "cx/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "cx.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str = "cx.mcp.server_user_flow.triggered";
    ...
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        ...
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.cx_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let result = async {
            ...
            Ok(maybe_request_cx_apps_auth_elicitation(
                sess,
                turn_context,
    ...
    .await;
    maybe_track_cx_app_used(sess, turn_context, &server, &metadata).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        cx.mcp.target.id = Empty,
        cx.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        cx.mcp.error.code = Empty,
    );
...
async fn maybe_request_cx_apps_auth_elicitation(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    ...
    let install_url = connector_id.map(|connector_id| {
        cx_connectors::metadata::connector_install_url(
    ...
    refresh_cx_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_cx_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    let mcp_tools_result = sess.hard_refresh_latest_cx_apps_tools().await;
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh CX Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        cx_linux_sandbox_exe: prepared_call.config().cx_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                cx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == cx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_cx_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    cx_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, cx_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == CX_APPS_MCP_SERVER_NAME {
        ...
        request_meta.insert(
            ...
            serde_json::Value::Object(cx_apps_meta),
        );
    }
...
async fn maybe_request_mcp_tool_approval(
    ...
    Some(
        match sess.request_approval(action, approval_context).await {
            ...
            Err(ToolError::CX(_)) => ReviewDecision::Abort,
        },
    )
}
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == CX_APPS_MCP_SERVER_NAME {
        ...
        persist_cx_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_cx_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, cx_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
