...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.cx_linux_sandbox_exe = Some(PathBuf::from("/tmp/cx-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/cx-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.cx_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/cx-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/cx-execve-wrapper"))
    );
...
"#,
    ...
    assert_eq!(config.gt_base_url, parent.gt_base_url);
