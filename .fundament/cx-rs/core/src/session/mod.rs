...
use cx_analytics::ImagePreparationFact;
use cx_analytics::ImagePreparationMetadata;
use cx_analytics::SubAgentThreadStartedInput;
use cx_analytics::TurnCodexErrorFact;
use cx_async_utils::OrCancelExt;
use cx_connectors::connector_runtime_context_key;
use cx_exec_server::Environment;
use cx_exec_server::EnvironmentManager;
use cx_execpolicy::prefix_rule_migration;
use cx_extension_api::ConversationHistorySnapshot;
use cx_extension_api::ExtensionDataInit;
use cx_extension_api::LoadedUserInstructions;
use cx_extension_api::PromptFragment;
use cx_extension_api::PromptSlot;
use cx_extension_api::TurnContextContributionInput;
use cx_features::FEATURES;
use cx_features::Feature;
use cx_features::unstable_features_warning_event;
use cx_history::RolloutItem;
use cx_hooks::Hooks;
use cx_hooks::HooksConfig;
use cx_login::AuthManager;
use cx_login::CodexAuth;
use cx_login::auth_env_telemetry::collect_auth_env_telemetry;
...
use cx_network_proxy::NetworkProxyAuditMetadata;
use cx_network_proxy::normalize_host;
use cx_otel::current_span_trace_id;
use cx_otel::current_span_w3c_trace_context;
use cx_otel::set_parent_from_w3c_trace_context;
use cx_protocol::SessionId;
use cx_protocol::ThreadId;
use cx_protocol::approvals::ElicitationRequest;
use cx_protocol::approvals::ElicitationRequestEvent;
...
use cx_protocol::dynamic_tools::DynamicToolResponse;
use cx_protocol::dynamic_tools::DynamicToolSpec;
use cx_protocol::items::EnteredReviewModeItem;
use cx_protocol::items::TurnItem;
use cx_protocol::items::UserMessageItem;
...
use cx_protocol::protocol::HasLegacyEvent;
use cx_protocol::protocol::HistoryPosition;
...
use cx_protocol::protocol::RawResponseItemEvent;
...
use cx_protocol::protocol::TurnContextNetworkItem;
use cx_protocol::protocol::TurnEnvironmentSelection;
use cx_protocol::protocol::W3cTraceContext;
use cx_protocol::protocol::WorldStateItem;
use cx_protocol::request_permissions::PermissionGrantScope;
use cx_protocol::request_permissions::RequestPermissionProfile;
use cx_protocol::request_permissions::RequestPermissionsArgs;
use cx_protocol::request_permissions::RequestPermissionsEvent;
use cx_protocol::request_permissions::RequestPermissionsResponse;
use cx_protocol::request_user_input::RequestUserInputArgs;
use cx_protocol::request_user_input::RequestUserInputResponse;
use cx_rmcp_client::ElicitationResponse;
use cx_rollout::state_db;
use cx_rollout_trace::AgentResultTracePayload;
use cx_rollout_trace::ThreadStartedTraceMetadata;
use cx_rollout_trace::ThreadTraceContext;
use cx_sandboxing::policy_transforms::intersect_permission_profiles;
use cx_shell_command::parse_command::parse_command;
use cx_terminal_detection::user_agent;
use cx_thread_store::CreateThreadParams;
use cx_thread_store::LiveThread;
use cx_thread_store::LiveThreadInitGuard;
use cx_thread_store::LocalThreadStore;
use cx_thread_store::PersistContext;
use cx_thread_store::ReadThreadParams;
use cx_thread_store::ResumeThreadParams;
use cx_thread_store::ThreadPersistenceMetadata;
use cx_thread_store::ThreadStore;
use cx_utils_audio::prepare_response_items as prepare_audio_response_items;
...
use cx_core_plugins::PluginCommandAttribution;
use cx_core_plugins::PluginsManager;
use cx_core_plugins::RecommendedPluginCandidatesInput;
use cx_git_utils::get_git_repo_root;
use cx_history::CompactedItem;
use cx_history::InitialHistory;
use cx_history::ResponseItemEnvelope;
use cx_mcp::McpConfig;
use cx_mcp::effective_mcp_servers;
use cx_otel::SessionTelemetry;
use cx_otel::THREAD_STARTED_METRIC;
...
use cx_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
...
use cx_protocol::models::LocalImagePreparation;
...
use cx_protocol::protocol::SessionNetworkProxyRuntime;
use cx_protocol::protocol::StreamErrorEvent;
use cx_protocol::protocol::Submission;
use cx_protocol::protocol::ThreadMemoryMode;
use cx_protocol::protocol::TokenCountEvent;
use cx_protocol::protocol::TokenUsage;
use cx_protocol::protocol::TokenUsageInfo;
use cx_protocol::protocol::TurnModerationMetadataEvent;
...
use cx_tools::UnifiedExecShellMode;
use cx_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use cx_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn cx_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<cx_extension_api::ExtensionRegistry<crate::config::Config>>,
...
impl Session {
    ...
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if cx_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            {
                let cx_home = config.cx_home.clone();
                let policy_path = default_policy_path(cx_home.as_path());
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            cx_models_manager::manager::RefreshStrategy::Offline
        } else {
            cx_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.cx_home)
        })?;
...
impl Session {
    ...
    /// Builds the `x-cx-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &cx_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn cx_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn cx_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn cx_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.cx_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect CX performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<cx_extension_api::UserInstructions> {
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        self.services
            ...
            .record_cx_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &cx_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| cx_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => cx_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&cx_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the CX host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
    ...
    pub(crate) async fn capture_step_context_with_required_mcp_servers(
        ...
        let executor_capability_discovery = self
            ...
            .await?;
        let extension_data = cx_extension_api::ExtensionData::new(turn_context.sub_id.clone());
        ...
        let mut selected_plugins = self
            ...
            .get::<cx_extension_api::SelectedPluginSnapshot>()
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-cx, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(cx_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            separate_developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(cx_protocol::AgentPath::root),
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let cx_error_info = CodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: cx_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<cx_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
