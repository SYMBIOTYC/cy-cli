...
use cx_analytics::TurnResolvedConfigFact;
...
use cx_extension_api::TurnInputContext;
use cx_extension_api::TurnInputEnvironment;
use cx_features::Feature;
use cx_file_system::FindUpErrorPolicy;
use cx_file_system::find_nearest_ancestor_with_markers;
use cx_login::CodexAuth;
use cx_model_provider::RemoteCompactionSupport;
...
use cx_protocol::protocol::AgentMessageContentDeltaEvent;
use cx_protocol::protocol::AgentReasoningSectionBreakEvent;
use cx_protocol::protocol::CodexErrorInfo;
use cx_protocol::protocol::ErrorEvent;
use cx_protocol::protocol::EventMsg;
use cx_protocol::protocol::PlanDeltaEvent;
use cx_protocol::protocol::RawResponseCompletedEvent;
use cx_protocol::protocol::ReasoningContentDeltaEvent;
use cx_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use cx_skills::ToolMentionKind;
use cx_skills::app_id_from_path;
use cx_skills::build_skill_name_counts;
use cx_skills::collect_explicit_skill_mentions;
use cx_skills::tool_kind_for_path;
use cx_skills_extension::HostSkillPrompts;
use cx_skills_extension::InjectedHostSkillPrompts;
use cx_thread_store::PersistContext;
use cx_tools::DiscoverableTool;
use cx_tools::ToolName;
use cx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use cx_utils_path_uri::PathUri;
use cx_utils_stream_parser::AssistantTextChunk;
use cx_utils_stream_parser::AssistantTextStreamParser;
use cx_utils_stream_parser::ProposedPlanSegment;
use cx_utils_stream_parser::extract_proposed_plan_text;
use cx_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "cx_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        ...
        let error = err.to_cx_protocol_error();
    ...
    // Although from the perspective of cx.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        match sampling_request_result {
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_cx_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_cx_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &cx_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = cx_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
...
///
/// Returns `None` when the active authentication does not use the CX backend, the provider is
/// not oi, or the previous and current model are the same.
async fn capture_current_model_fallback_step_context(
    ...
    let uses_cx_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(cx_login::AuthManager::current_auth_uses_cx_backend);
    if !uses_cx_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = cx_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<cx_extension_api::ExtensionData>,
    ...
    loop {
        ...
        {
            cx_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
...
pub(super) fn agent_message_text(item: &cx_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            cx_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: cx_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(cx_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &cx_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            cx.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            cx.usage.reasoning_output_tokens = field::Empty,
            cx.usage.total_tokens = field::Empty,
        );
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(cx_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![cx_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        cx_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
