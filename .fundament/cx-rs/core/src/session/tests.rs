...
use cx_config::NetworkConstraints;
use cx_config::NetworkDomainPermissionToml;
use cx_config::NetworkDomainPermissionsToml;
...
use cx_http_client::RouteAwareClientPool;
...
use cx_models_manager::model_info;
use cx_models_manager::test_support::construct_model_info_offline_for_tests;
use cx_models_manager::test_support::get_model_offline_for_tests;
use cx_protocol::AgentPath;
use cx_protocol::ResponseItemId;
...
use cx_protocol::models::AgentMessageInputContent;
...
use cx_protocol::openai_models::ModelServiceTier;
...
use cx_protocol::protocol::ErrorEvent;
...
use cx_protocol::turn_input::TurnInput as SubmittedTurnInput;
use cx_protocol::turn_input::TurnInputMode;
use cx_protocol::turn_input::TurnInputRequest;
use cx_protocol::turn_input::TurnInputSubmission;
...
use cx_config::permissions_toml::FilesystemPermissionsToml;
use cx_config::permissions_toml::NetworkToml;
use cx_config::permissions_toml::PermissionProfileToml;
use cx_config::permissions_toml::PermissionsToml;
use cx_execpolicy::Decision;
use cx_execpolicy::NetworkRuleProtocol;
use cx_execpolicy::Policy;
use cx_history::CodexHarnessMetadata;
use cx_history::CompactedItem;
...
use cx_otel::MetricsClient;
use cx_otel::MetricsConfig;
...
use cx_protocol::items::HookPromptFragment;
use cx_protocol::items::build_hook_prompt_message;
use cx_protocol::models::BaseInstructions;
use cx_protocol::models::ContentItem;
use cx_protocol::models::InternalChatMessageMetadataPassthrough;
...
use cx_protocol::protocol::CreditsSnapshot;
use cx_protocol::protocol::GranularApprovalConfig;
use cx_protocol::protocol::InterAgentCommunication;
use cx_protocol::protocol::MultiAgentVersion;
use cx_protocol::protocol::NetworkApprovalProtocol;
use cx_protocol::protocol::RateLimitSnapshot;
use cx_protocol::protocol::RateLimitWindow;
use cx_protocol::protocol::RealtimeAudioFrame;
use cx_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use cx_protocol::protocol::RealtimeVoice;
use cx_protocol::protocol::RealtimeVoicesList;
use cx_protocol::protocol::SessionMeta;
use cx_protocol::protocol::SessionMetaLine;
use cx_protocol::protocol::Submission;
...
use cx_protocol::protocol::TurnAbortedEvent;
...
use cx_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<cx_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &cx_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    extension_metrics.counter(
        "cx.test.extension.counter",
        /*inc*/ 2,
        &[("component", "skills"), ("model", "extension-model")],
    );

    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "cx.test.extension");
    let counter = find_metric(&snapshot, "cx.test.extension.counter");
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl cx_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: cx_extension_api::WorldStateContributionInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<
            'a,
            Vec<cx_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("cx.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder = cx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "cx.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("cx-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        cx_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        cx_home.join(cx_config::CONFIG_TOML_FILE),
...
) -> std::io::Result<Vec<cx_protocol::protocol::HookRunSummary>> {
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn cx_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { cx_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: cx_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> cx_sandboxing::SandboxablePreference {
            cx_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(cx_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: cx_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let config_toml_path = cx_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let cx_home = session.cx_home().await;
    std::fs::create_dir_all(&cx_home).expect("create cx home");
    let base_config_path = cx_home.join(CONFIG_TOML_FILE);
    let profile_config_path = cx_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(cx_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable CX hooks");
    })
    .await?;
    let cx_home = session.cx_home().await;
    std::fs::create_dir_all(&cx_home)?;
    let config_toml_path = cx_home.join(CONFIG_TOML_FILE);
    let user_config: cx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = cx_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = cx_hooks::list_hooks(cx_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..cx_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        cx_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: cx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: cx_config::MatcherGroup {
                matcher: None,
                hooks: vec![cx_config::HookHandlerConfig::Command {
        ...
        let identity = cx_config::TomlValue::try_from(identity)?;
        cx_config::version_for_toml(&identity)
    };
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = cx_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = cx_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: cx_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl cx_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a cx_extension_api::ExtensionData,
            thread_store: &'a cx_extension_api::ExtensionData,
            turn_store: &'a cx_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl cx_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: cx_extension_api::TurnStartInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl cx_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: cx_extension_api::TurnErrorInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl cx_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &cx_extension_api::ExtensionData,
            thread_store: &cx_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: cx_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_codex().with_config(|config| {
        config.permissions.approval_policy =
            cx_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial.cx.ensure_rollout_materialized().await;
    initial
        .cx
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        cx_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "cx_core__cx_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: cx_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            cx_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            cx_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            cx_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    let cx_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(cx_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(cx_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("cx_other".to_string()),
        limit_name: Some("cx_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(cx_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.cx_error_info == Some(CodexErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(cx_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/cx/analytics-events/events"))
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("cx-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "cx_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = cx_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        cx_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        cx_home.path().join(cx_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let cx_home = parent_config.cx_home.clone();
    let skill_dir = cx_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&cx_exec_server::LOCAL_FS));
    ...
    let role_path = cx_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let cx_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(cx_home.path()).await;
    config
        .features
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(cx_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(cx_extension_api::ExtensionRegistryBuilder::new().build()),
        cx_extension_api::ExtensionDataInit::default(),
        ...
        Arc::new(cx_thread_store::LocalThreadStore::new(
            cx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        cx_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        cx_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(cx_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: cx_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(cx_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: cx_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: cx_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: cx_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(cx_thread_store::LocalThreadStore::new(
            cx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .cx_home(config.cx_home.to_path_buf())
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            cx_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(cx_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(cx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            cx_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(cx_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = cx_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(cx_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    turn_context.as_ref(),
                    call_id,
                    cx_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(cx_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: cx_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(cx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = cx_utils_path_uri::PathUri::from_abs_path(&second_cwd);
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        cx_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = cx_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(cx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn cx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    result_sender
        .try_send(
            serde_json::from_value::<cx_protocol::protocol::HookCompletedEvent>(json!({
                "turn_id": "turn-1",
                "run": {
    ...
    assert_eq!(
        cx_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: cx_extension_api::ThreadStopInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: cx_extension_api::ThreadStopInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl cx_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: cx_extension_api::TurnAbortInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(cx_home).await;
    configure_config(&mut config);
    let state_db = None;
    let config = Arc::new(config);
    let thread_id = ThreadId::default();
    let auth_manager = AuthManager::from_auth_for_testing_with_home(auth, cx_home.to_path_buf());
    let models_manager = models_manager_with_provider(
        config.cx_home.to_path_buf(),
        auth_manager.clone(),
        config.model_provider.clone(),
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    let configured_servers = cx_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !cx_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        cx_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        cx_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = cx_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: cx_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("cx_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("cx_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_detects_shared_auth_manager_changes() {
    ...
    assert_eq!(
        ...
        Some(cx_protocol::auth::AuthMode::ApiKey)
    );
    session.refresh_mcp_if_dirty().await;
    assert!(
        session
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    let (session, turn_context) = make_session_and_context().await;
    let selected_root =
        |environment_id: &str, path: &str| cx_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: cx_protocol::capabilities::CapabilityRootLocation::Environment {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(cx_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(cx_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl cx_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a cx_extension_api::ExtensionData,
        thread_store: &'a cx_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<cx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
...
-> Arc<cx_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = cx_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl cx_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: cx_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<cx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<cx_thread_store::InMemoryThreadStore> {
    let store = Arc::new(cx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn cx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &cx_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: cx_extension_api::ThreadIdleInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = cx_protocol::user_input::TextElement::new(
        cx_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl cx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: cx_extension_api::ThreadIdleInput<'a>,
        ) -> cx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    session
        .emit_thread_idle_lifecycle_if_idle(cx_extension_api::ThreadIdleCause::Completed)
        .await;
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(cx_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: cx_tools::ToolName::plain("exec_command"),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let cx_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_codex = project_root.join(".cx");
    let nested_dot_codex = nested.join(".cx");
    ...
    write_project_trust_config(&cx_home, &[(&nested, TrustLevel::Trusted)]).await?;
    ...
    let expected_source_path = cx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_codex = project_root.join(".cx");
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let cx_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&cx_home)?;
        write_project_trust_config(&cx_home, &trust_entries).await?;
