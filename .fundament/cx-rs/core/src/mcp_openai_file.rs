//! Bridges Apps SDK-style `openai/fileParams` metadata into CX's MCP flow.
//!
//! Strategy:
...
//!   upload them to oi file storage,
//!   and rewrite only the declared arguments into the provided-file payload
//!   shape expected by the downstream Apps tool.
//!
//! The model-facing local-path schema is owned by `cx-mcp` alongside MCP tool inventory, so this
...
use cx_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use cx_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("gt auth is required to upload files for CX Apps tools".to_string());
    };
    ...
    let upload_auth = cx_model_provider::auth_provider_from_auth(auth);
    let uploaded = upload_openai_file(
        turn_context.config.gt_base_url.trim_end_matches('/'),
        upload_auth.as_ref(),
