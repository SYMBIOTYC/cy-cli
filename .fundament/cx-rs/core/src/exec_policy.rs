...
use cx_execpolicy::AmendError;
use cx_execpolicy::Decision;
use cx_execpolicy::Error as ExecPolicyRuleError;
...
use cx_execpolicy::RequirementsExecPolicy;
use cx_execpolicy::RuleMatch;
use cx_execpolicy::blocking_append_allow_prefix_rule;
use cx_execpolicy::blocking_append_network_rule;
...
use cx_shell_command::is_dangerous_command::DangerousCommandMatch;
use cx_shell_command::is_dangerous_command::dangerous_command_match;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(cx_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(cx_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &cx_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            cx_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub(crate) fn default_policy_path(cx_home: &Path) -> PathBuf {
    cx_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            cx_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and cx prompts the user to run outside the sandbox
