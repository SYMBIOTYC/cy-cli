...
pub use cx_connectors::AppBranding;
pub use cx_connectors::AppInfo;
pub use cx_connectors::AppMetadata;
use cx_connectors::ConnectorDirectoryCacheContext;
use cx_connectors::ConnectorDirectoryCacheKey;
use cx_connectors::apps_config_from_layer_stack;
...
use cx_mcp::McpStartupPolicy;
use cx_mcp::ToolInfo;
use cx_mcp::ToolPluginProvenance;
use cx_mcp::effective_mcp_servers;
use cx_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = cx_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        cx_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(CodexAuth::uses_cx_backend))
    {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_cx_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    let mcp_runtime = McpRuntime::new(McpRuntimeInput {
        ...
        runtime_context,
        cx_apps_tools_cache: mcp_manager.cx_apps_tools_cache(),
        tool_catalog_cache: mcp_manager.tool_catalog_cache(),
        cx_apps_tools_cache_key: connector_runtime_context_key(auth.as_ref()),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: auth.clone(),
        cx_apps_auth_manager,
    ...
    if cx_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let gt_user_id = auth.and_then(CodexAuth::get_gt_user_id);
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + cx_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    cx_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        ...
        Some(cx_connectors::accessible::AccessibleConnectorTool {
    ...
    cx_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &cx_config::ConfigLayerStack,
