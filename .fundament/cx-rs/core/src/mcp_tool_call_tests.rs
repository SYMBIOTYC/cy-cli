...
use cx_config::types::AppConfig;
use cx_config::types::AppToolConfig;
use cx_config::types::AppToolsConfig;
use cx_config::types::ApprovalsReviewer;
use cx_config::types::AppsConfigToml;
...
fn approval_config(turn_context: &TurnContext) -> cx_mcp::McpConfig {
...
fn write_sample_plugin_mcp(cx_home: &std::path::Path) {
    let plugin_root = cx_home.join("plugins/cache/test/sample/local");
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.cx_home)
        .expect("create cx home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.cx_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = cx_hooks::list_hooks(HooksConfig {
...
#[test]
fn openai_file_params_are_only_honored_for_cx_apps() {
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("cx.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("cx.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("cx.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("cx.mcp.target.id=")
            && !invalid_logs.contains("cx.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("cx.mcp.target.id=")
            && !missing_logs.contains("cx.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("cx.mcp.target.id=")
            && !no_meta_logs.contains("cx.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("cx.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_cx_apps_identity() {
    let mut metadata = approval_metadata(
...
#[tokio::test]
async fn cx_apps_tool_call_request_meta_includes_turn_metadata_and_cx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
#[tokio::test]
async fn cx_apps_tool_call_request_meta_includes_call_id_without_existing_cx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = expected_mcp_turn_metadata(&turn_context);
...
fn cx_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn cx_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn cx_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = cx_apps_auth_failure_result();
    let metadata = cx_apps_auth_failure_metadata();

    let returned = maybe_request_cx_apps_auth_elicitation(
        &session,
        &turn_context,
...
#[tokio::test]
async fn cx_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn cx_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn cx_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_cx_apps_auth_elicitation(
                &session,
                &turn_context,
                turn_context.approval_policy(),
                "call_123",
    ...
    assert!(matches!(
        request.request,
        cx_protocol::approvals::ElicitationRequest::Url { .. }
    ));
...
#[tokio::test]
async fn persist_cx_app_tool_approval_writes_tool_override() {
    ...
    persist_cx_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(cx_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .cx_home(cx_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_cx_dir = project_dir.path().join(".cx");
    std::fs::create_dir_all(&project_cx_dir).expect("create project .cx dir");
    std::fs::write(
        project_cx_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&cx_home)
        .set_project_trust_level(
            project_dir.path(),
            cx_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_cx_dir.join(CONFIG_TOML_FILE))
