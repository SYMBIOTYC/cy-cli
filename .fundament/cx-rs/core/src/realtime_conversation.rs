...
use cx_api::RealtimeAudioFrame;
use cx_api::RealtimeContextAppendChannel;
use cx_api::RealtimeEvent;
use cx_api::RealtimeEventParser;
use cx_api::RealtimeSessionConfig;
use cx_api::RealtimeSessionMode;
use cx_api::RealtimeWebsocketClient;
use cx_api::RealtimeWebsocketEvents;
use cx_api::RealtimeWebsocketWriter;
use cx_api::build_session_headers;
use cx_api::map_api_error;
...
use cx_protocol::protocol::RealtimeConversationClosedEvent;
use cx_protocol::protocol::RealtimeConversationRealtimeEvent;
use cx_protocol::protocol::RealtimeConversationSdpEvent;
use cx_protocol::protocol::RealtimeConversationStartedEvent;
...
use cx_utils_output_truncation::approx_bytes_for_tokens;
use cx_utils_string::approx_token_count;
use cx_utils_string::take_bytes_at_char_boundary;
use http::HeaderMap;
...
const STANDALONE_HANDOFF_ID: &str = "cx";
...
struct RealtimeHandoffState {
    ...
    cx_responses_as_items: bool,
    cx_response_item_prefix: Option<String>,
    cx_response_handoff_mode: CodexResponseHandoffMode,
    cx_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.cx_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.cx_response_handoff_mode == CodexResponseHandoffMode::BemTags
    }
}
...
impl RealtimeConversationManager {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> CodexResult<RealtimeStartOutput> {
        ...
        let handoff = RealtimeHandoffState {
            ...
            cx_response_item_prefix,
            cx_response_handoff_mode,
            cx_response_handoff_channel_prefixes: Arc::new(
                cx_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.cx_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.cx_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn handoff_complete(&self) -> CodexResult<()> {
        ...
        let output = if handoff.cx_responses_as_items {
...
async fn prepare_realtime_start(
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        cx_responses_as_items: params.cx_responses_as_items,
        cx_response_item_prefix: params.cx_response_item_prefix,
        cx_response_handoff_mode: params.cx_response_handoff_mode,
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.cx_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
