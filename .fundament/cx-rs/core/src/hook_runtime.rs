...
use cx_analytics::HookRunFact;
use cx_analytics::build_track_events_context;
use cx_hooks::PermissionRequestDecision;
use cx_hooks::PermissionRequestOutcome;
use cx_hooks::PermissionRequestRequest;
use cx_hooks::PostToolUseOutcome;
use cx_hooks::PostToolUseRequest;
use cx_hooks::PreToolUseOutcome;
use cx_hooks::PreToolUseRequest;
use cx_hooks::SessionStartOutcome;
use cx_hooks::StartHookTarget;
use cx_hooks::StopHookTarget;
use cx_hooks::StopOutcome;
use cx_hooks::SubagentHookContext;
use cx_hooks::UserPromptSubmitOutcome;
use cx_hooks::UserPromptSubmitRequest;
use cx_hooks::hook_execution_mode_label;
use cx_hooks::hook_handler_type_label;
use cx_otel::HOOK_RUN_DURATION_METRIC;
use cx_otel::HOOK_RUN_METRIC;
...
use cx_protocol::protocol::HookCompletedEvent;
...
use cx_protocol::protocol::HookRunSummary;
use cx_protocol::protocol::HookSource;
use cx_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = cx_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = cx_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = cx_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = cx_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(cx_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: cx_hooks::HookEvent::AfterAgent {
                event: cx_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            cx_hooks::HookResult::Success => continue,
            cx_hooks::HookResult::FailedContinue(error) => (error, false),
            cx_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(cx_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (cx_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use cx_protocol::protocol::HookHandlerType;
    use cx_protocol::protocol::HookRunStatus;
    use cx_protocol::protocol::HookScope;
    use cx_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    cx_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
    ...
    async fn hook_lifecycle_notifications_only_report_synchronous_runs() {
        ...
        assert!(matches!(
            started.msg,
            cx_protocol::protocol::EventMsg::HookStarted(event)
        ...
        assert!(matches!(
            completed.msg,
            cx_protocol::protocol::EventMsg::HookCompleted(event)
                if event.run.id == synchronous_run.id
        ));
        assert!(events.try_recv().is_err());
    }
