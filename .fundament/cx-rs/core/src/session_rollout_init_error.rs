...
pub(crate) fn map_session_init_error(err: &anyhow::Error, cx_home: &Path) -> CodexErr {
    ...
    if let Some(mapped) = err
        ...
        .find_map(|io_err| map_rollout_io_error(io_err, cx_home))
    {
        return mapped;
    }
...
fn map_rollout_io_error(io_err: &std::io::Error, cx_home: &Path) -> Option<CodexErr> {
    ...
    let hint = match io_err.kind() {
        ErrorKind::PermissionDenied => format!(
            "CX cannot access session files at {} (permission denied). If sessions were created using sudo, fix ownership: sudo chown -R $(whoami) {}",
            sessions_dir.display(),
            cx_home.display()
        ),
        ErrorKind::NotFound => format!(
            "Session storage missing at {}. Create the directory or choose a different CX home.",
            sessions_dir.display()
        ),
        ErrorKind::AlreadyExists => format!(
            "Session storage path {} is blocked by an existing file. Remove or rename it so CX can create sessions.",
            sessions_dir.display()
        ),
        ...
        ErrorKind::IsADirectory | ErrorKind::NotADirectory => format!(
            "Session storage path {} has an unexpected type. Ensure it is a directory CX can use for session files.",
            sessions_dir.display()
        ),
        _ => return None,
    };
