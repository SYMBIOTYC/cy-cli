...
`cx-network-proxy` is CX's local network policy enforcement proxy. It runs:
...
`cx-network-proxy` reads from CX's merged `config.toml` (via `cx-core` config loading).
...
`cx-network-proxy` can be embedded as a library with a thin API:

```rust
use cx_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `cx-network-proxy` is embedded in managed CX runtime, policy decisions emit structured
OTEL-compatible events with `target=cx_otel.network_proxy`.

Event name:

- `cx.network_proxy.policy_decision`
...
This section documents the protections implemented by `cx-network-proxy`, and the boundaries of
