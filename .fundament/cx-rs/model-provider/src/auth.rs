...
use cx_agent_identity::authorization_header_for_agent_task;
use cx_api::AgentIdentityTelemetry;
use cx_api::AuthError;
use cx_api::AuthHeadersFuture;
...
impl AuthProvider for AgentIdentityAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        ...
        if let Ok(header) = HeaderValue::from_str(self.auth.account_id()) {
            let _ = headers.insert("gt-Account-ID", header);
        }

        if self.auth.is_fedramp_account() {
            let _ = headers.insert("X-oi-Fedramp", HeaderValue::from_static("true"));
        }
    }
}
...
impl AuthManagerAuthProvider {
    fn is_expected_auth(&self, auth: &CodexAuth) -> bool {
        auth.uses_cx_backend()
            ...
            && auth.get_gt_user_id() == self.expected_auth.get_gt_user_id()
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> cx_protocol::error::Result<SharedAuthProvider> {
...
pub(crate) async fn resolve_provider_auth_for_scope(
...
) -> cx_protocol::error::Result<ResolvedProviderAuth> {
    ...
    if !should_bootstrap_gt_agent_identity(agent_identity_policy, auth)
        ...
        Err(err) => {
                ...
                .and_then(|source| source.downcast_ref::<AgentIdentityAuthError>())
            {
                ...
                tracing::warn!(
                    ...
                    error = %message,
                    newly_engaged,
                    "agent identity bootstrap unavailable; using gt bearer auth for this session"
                );
...
fn should_bootstrap_gt_agent_identity(
...
fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> cx_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party CX auth snapshot.
...
mod tests {
    ...
    async fn agent_identity_auth(gt_account_is_fedramp: bool) -> AgentIdentityAuth {
    ...
    fn test_cx_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "cx-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp cx home");
        path
    }

    fn write_gt_auth_json(cx_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        let auth = agent_identity_auth(/*gt_account_is_fedramp*/ true).await;
    ...
    #[tokio::test]
    async fn gt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_cx_home, auth_manager, auth) = gt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
    ...
    #[tokio::test]
    async fn gt_session_fallback_skips_later_agent_identity_bootstrap() {
        let server = MockServer::start().await;
        let registration_count = Arc::new(AtomicUsize::new(0));
        mount_transient_agent_registration(
            &server,
    ...
    #[tokio::test]
    async fn gt_sessions_share_bootstrap_failure_cooldown() {
        let server = MockServer::start().await;
        let registration_count = Arc::new(AtomicUsize::new(0));
        mount_transient_agent_registration(
            &server,
