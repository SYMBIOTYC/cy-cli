...
use cx_login::collect_auth_env_telemetry;
use cx_login::default_client::create_client_for_route_async;
...
/// Provider-owned oi-compatible `/models` endpoint.
#[derive(Debug)]
...
impl OpenAiModelsEndpoint {
    ...
    fn auth_env(&self) -> AuthEnvTelemetry {
        let cx_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.cx_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, cx_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_cx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_cx_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            ...
            auth.env_cx_api_key_present = self.auth_env.cx_api_key_env_present,
            auth.env_cx_api_key_enabled = self.auth_env.cx_api_key_env_enabled,
