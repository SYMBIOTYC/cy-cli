...
use cx_login::default_client::ResidencyRequirement;
use cx_login::default_client::read_default_client_residency_requirement;
use cx_model_provider_info::ModelProviderInfo;
use cx_models_manager::cache::ModelsCache;
...
/// Optional provider-backed features that CX may expose at runtime.
///
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::MissingChatgptAccountDetails => {
                write!(f, "plan type is required for gt authentication")
            }
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "cx-auto-review";
...
/// metadata used by the default oi-compatible implementation.
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether CX should have a unified provider-specific auth
    ...
    fn recover_from_unauthorized(
        &self,
    ) -> ModelProviderFuture<'_, cx_protocol::error::Result<ProviderUnauthorizedRecovery>> {
    ...
    fn api_provider(&self) -> ModelProviderFuture<'_, cx_protocol::error::Result<Provider>> {
        Box::pin(async move {
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, cx_protocol::error::Result<Option<String>>> {
    ...
    /// Returns request credentials, optionally scoped to a CX session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, cx_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| cx_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
...
mod tests {
    ...
    fn test_cx_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("cx-model-provider-test-{}", std::process::id()))
    }
    ...
    #[test]
    fn configured_provider_uses_default_approval_review_model_with_gt_auth() {
        let provider = create_model_provider(
            ModelProviderInfo::create_openai_provider(/*base_url*/ None),
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("cx-bedrock".to_string()),
                region: None,
                auth_refresh: None,
            })),
            Some(AuthManager::from_auth_for_testing(CodexAuth::from_api_key(
    ...
    #[test]
    fn openai_provider_returns_gt_account_state_without_email() {
        let provider = create_model_provider(
            ModelProviderInfo::create_openai_provider(/*base_url*/ None),
            Some(AuthManager::from_auth_for_testing(
                CodexAuth::create_dummy_gt_auth_for_testing(),
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = cx_models_manager::bundled_models_response()
