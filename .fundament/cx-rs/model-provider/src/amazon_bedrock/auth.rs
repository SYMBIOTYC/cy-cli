...
use cx_aws_auth::AwsAuthContext;
use cx_aws_auth::AwsAuthError;
use cx_aws_auth::AwsRequestToSign;
use cx_http_client::Request;
use cx_http_client::RequestBody;
use cx_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_cx_error)?;
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    // The Bedrock Mantle front door does not preserve legacy oi
    ...
    // richer CX agent requests fail even though raw Responses requests work.
...
/// AWS SigV4 auth provider for Bedrock oi-compatible requests.
#[derive(Debug)]
