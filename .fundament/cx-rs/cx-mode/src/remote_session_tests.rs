...
use cx_code_mode_protocol::CodeModeSessionProvider;
use cx_code_mode_protocol::ExecuteRequest;
use cx_code_mode_protocol::FunctionCallOutputContentItem;
use cx_code_mode_protocol::RuntimeResponse;
...
use cx_code_mode_protocol::host::WireCellId;
...
fn missing_host_error_limits_the_displayed_path_to_512_bytes() {
    let executable = "cx-mode-host-does-not-exist";
...
async fn websocket_provider_executes_over_shared_connector() {
    ...
    assert_eq!(
        ...
        RuntimeResponse::Result {
            cell_id: cx_code_mode_protocol::CellId::new("1".to_string()),
...
async fn provider_returns_missing_host_error() {
    let provider = ProcessOwnedCodeModeSessionProvider::with_host_program(
        "cx-mode-host-does-not-exist".into(),
    );
    ...
    assert!(error.contains("failed to spawn code-mode host cx-mode-host-does-not-exist"));
}
...
async fn shutdown_before_open_does_not_spawn_the_host() {
    ...
    let error = session
        .execute(cx_code_mode_protocol::ExecuteRequest {
