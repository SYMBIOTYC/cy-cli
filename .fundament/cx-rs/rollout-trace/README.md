...
> **Privacy:** Rollout tracing is not telemetry. CX does **not** upload or
...
during a CX session. It records raw runtime evidence into a local bundle on
...
Hot-path CX code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how CX got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["cx-core runtime"]
...
CX session fail just because diagnostic recording failed. Core emits raw
