...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.cx_turns.get(&started.cx_turn_id) else {
            bail!(
                "compaction request {} referenced unknown cx turn {}",
                started.compaction_request_id,
                started.cx_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but cx turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.cx_turn_id,
                turn.thread_id
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but cx turn {cx_turn_id} belongs to {}",
                turn.thread_id
            );
        }
