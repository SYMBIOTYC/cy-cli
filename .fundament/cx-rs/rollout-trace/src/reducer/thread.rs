...
impl TraceReducer {
    ...
    /// Starts a CX turn inside an existing thread.
    pub(super) fn start_cx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.cx_turns.contains_key(&cx_turn_id) {
            bail!("duplicate cx turn start for {cx_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.cx_turns.insert(
            cx_turn_id.clone(),
            CodexTurn {
                cx_turn_id,
                thread_id,
    ...
    /// Marks a CX turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_cx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.cx_turns.get(&cx_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "cx turn end for {cx_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.cx_turns.get_mut(&cx_turn_id) else {
            bail!("cx turn end referenced unknown turn {cx_turn_id}");
        };
