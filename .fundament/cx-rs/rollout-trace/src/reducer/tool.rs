...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, cx_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, cx_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_cx_turn_id: cx_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        cx_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(cx_turn_id) = cx_turn_id else {
            bail!("tool call start did not include thread or CX turn context");
        };
        self.rollout
            .cx_turns
            .get(cx_turn_id)
    ...
    fn validate_tool_turn(&self, thread_id: &str, cx_turn_id: Option<&str>) -> Result<()> {
        ...
        if let Some(cx_turn_id) = cx_turn_id {
            let Some(turn) = self.rollout.cx_turns.get(cx_turn_id) else {
                bail!("tool call start referenced unknown CX turn {cx_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but CX turn {cx_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }
