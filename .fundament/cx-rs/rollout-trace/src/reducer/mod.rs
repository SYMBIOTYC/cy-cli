    ...
    fn apply_event(&mut self, event: RawTraceEvent) -> Result<()> {
        ...
        match event.payload {
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(cx_turn_id) = event.cx_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a cx turn id"
                    );
                };
