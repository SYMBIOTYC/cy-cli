...
//! - runtime work starts as soon as CX dispatches the tool.
//!
...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) cx_turn_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(cx_turn_id) = cx_turn_id else {
            bail!(
                "code cell start {} did not include a CX turn id",
                started.code_cell_id
            );
        };
        self.validate_code_cell_turn(&thread_id, &cx_turn_id)?;
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, cx_turn_id: &str) -> Result<()> {
        ...
        let Some(turn) = self.rollout.cx_turns.get(cx_turn_id) else {
            bail!("code cell start referenced unknown CX turn {cx_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but CX turn {cx_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
