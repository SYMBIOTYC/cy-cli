//! Mapping from CX protocol events into raw rollout-trace events.
//!
...
//! `cx-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `cx-rollout-trace`.
//!
...
use cx_protocol::protocol::PatchApplyStatus;
use cx_protocol::protocol::SubAgentActivityEvent;
...
pub(crate) fn cx_turn_trace_event(
    ...
    match event {
        EventMsg::TurnStarted(event) => {
            let cx_turn_id = event.turn_id.clone();
            Some(CodexTurnTraceEvent {
                context_turn_id: cx_turn_id.clone(),
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a cx_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a cx_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a cx_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a cx_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a cx_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a cx_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a cx_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a cx_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [cx_protocol::parse_command::ParsedCommand],
