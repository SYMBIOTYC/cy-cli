...
use crate::protocol_event::cx_turn_trace_event;
...
impl ThreadTraceContext {
    ...
    /// Emits typed CX turn lifecycle events from protocol lifecycle events.
    pub fn record_cx_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            cx_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what CX did while executing that boundary.
    pub fn record_tool_call_event(&self, cx_turn_id: impl Into<CodexTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(cx_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_cx_turn_id: impl Into<CodexTurnId>,
    ...
    pub fn record_cx_turn_started(&self, cx_turn_id: impl Into<CodexTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let cx_turn_id = cx_turn_id.into();
        context.append_with_context_best_effort(
            cx_turn_id.clone(),
    ...
    /// Builds reusable inference trace context for one CX turn.
    ///
