...
const INFERENCE_CALL_ID_HEADER: &str = "x-cx-inference-call-id";
...
///
/// A CX turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one CX turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        cx_turn_id: CodexTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                cx_turn_id: attempt.context.cx_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps cx-core focused on
    ...
    /// Records a provider stream that CX intentionally stopped consuming.
    ///
...
/// CX received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        cx_turn_id: Some(context.cx_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.cx_turn_id, "turn-1");
