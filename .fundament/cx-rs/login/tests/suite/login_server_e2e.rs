...
use cx_login::LoginCallbackResult;
...
fn start_mock_issuer(gt_account_id: &str) -> (SocketAddr, thread::JoinHandle<()>) {
    // Bind to a random available port
    let listener = TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let addr = listener.local_addr().unwrap();
    let server = tiny_http::Server::from_listener(listener, None).unwrap();
    let gt_account_id = gt_account_id.to_string();
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let gt_account_id = "12345678-0000-0000-0000-000000000000";
    let (issuer_addr, issuer_handle) = start_mock_issuer(gt_account_id);
    ...
    let server_home = cx_home.clone();

    let opts = ServerOptions {
        cx_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: cx_login::test_support::transport_default_auth_route_config(),
        client_id: cx_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
        force_state: Some(state),
        forced_gt_workspace_id: Some(vec![gt_account_id.to_string()]),
    ...
    assert!(
        server
            .auth_url
            .contains(format!("allowed_workspace_id={gt_account_id}").as_str()),
    ...
    // Validate auth.json
    let auth_path = cx_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/cx/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_cx_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let cx_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn login_server_includes_forced_workspaces_as_one_query_param() -> Result<()> {
    ...
    let opts = ServerOptions {
        ...
        open_browser: false,
        force_state: Some(state),
        forced_gt_workspace_id: Some(vec![
...
#[tokio::test]
async fn forced_gt_workspace_id_mismatch_blocks_login() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = HttpClientBuilder::new().build_direct()?;
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_cx_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to CX"),
        "error body should clearly explain the CX access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_cx_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to CX to retry"),
    ...
    assert!(
        !body.contains("get access to CX"),
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not cx"));
            }
        })
    };
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_cx_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        cx_home: first_cx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: cx_login::test_support::transport_default_auth_route_config(),
        client_id: cx_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_cx_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        cx_home: second_cx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: cx_login::test_support::transport_default_auth_route_config(),
        client_id: cx_login::CLIENT_ID.to_string(),
        issuer,
