...
use cx_utils_template::Template;
use rand::RngCore;
...
pub(super) const DEFAULT_ISSUER: &str = "https://auth.symbiotyc.workers.dev";
...
pub fn run_login_server(opts: ServerOptions) -> io::Result<LoginServer> {
    ...
    let auth_url = build_authorize_url(
        ...
        &pkce,
        &state,
        opts.forced_gt_workspace_id.as_deref(),
    );
    ...
    match path.as_str() {
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to CX failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "cx_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    pkce: &PkceCodes,
    state: &str,
    forced_gt_workspace_ids: Option<&[String]>,
) -> String {
    let mut query = vec![
        ...
        ("cx_cli_simplified_flow".to_string(), "true".to_string()),
        ("state".to_string(), state.to_string()),
        ("originator".to_string(), originator().value),
    ];
    if let Some(workspace_ids) = forced_gt_workspace_ids {
...
pub(crate) async fn persist_tokens_async(
    ...
    let cx_home = cx_home.to_path_buf();
    tokio::task::spawn_blocking(move || {
        let mut tokens = TokenData {
            id_token: parse_gt_jwt_claims(&id_token).map_err(io::Error::other)?,
            access_token,
            refresh_token,
            account_id: None,
        };
...
pub(crate) fn ensure_workspace_allowed(
    ...
    let Some(actual) = claims.get("gt_account_id").and_then(JsonValue::as_str) else {
        return Err("Login is restricted to a specific workspace, but the token did not include an gt_account_id claim.".to_string());
    };

    ensure_workspace_account_allowed(Some(expected), actual)
}

/// Validates an already known gt account ID against an optional workspace restriction.
///
...
/// Returns true when the OAuth callback represents a missing CX entitlement.
fn is_missing_cx_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_cx_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_cx_entitlement_error(error_code, error_description) {
        return "CX is not enabled for your workspace. Contact your workspace administrator to request access to CX.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_cx_entitlement_error(code, error_description) {
            (
                "You do not have access to CX".to_string(),
                "This account is not currently authorized to use CX in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to CX.".to_string(),
                "Contact your workspace administrator to get access to CX, then return to CX and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to CX to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_cx_entitlement_error;
    ...
    fn redact_sensitive_url_parts_preserves_safe_url_shape() {
        ...
        assert_eq!(
            url.as_str(),
            "https://auth.symbiotyc.workers.dev/oauth/token?code=%3Credacted%3E&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback"
        );
    }
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_cx_entitlement");
        assert!(is_missing_cx_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to CX"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_cx_entitlement"));
    }
}
