    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this CX server. Use the browser login or verify the server URL.",
            ));
        }
...
\n{ANSI_GRAY}Continue only if you started this login in CX. If a website or another person gave you this code, cancel.{ANSI_RESET}\n",
    )
}
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    Ok(DeviceCode {
        verification_url: format!("{base_url}/cx/device"),
