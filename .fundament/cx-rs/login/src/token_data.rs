...
pub struct IdTokenInfo {
    ...
    /// The gt subscription plan type
    /// (e.g., "free", "plus", "pro", "business", "enterprise", "edu").
    /// (Note: values may vary by backend.)
    pub gt_plan_type: Option<PlanType>,
    /// gt user identifier associated with the token, if present.
    ...
    /// Whether the selected gt workspace must route through the FedRAMP edge.
    pub gt_account_is_fedramp: bool,
    pub raw_jwt: String,
}

impl IdTokenInfo {
    pub fn get_gt_plan_type(&self) -> Option<String> {
        self.gt_plan_type.as_ref().map(|t| match t {
            PlanType::Known(plan) => plan.display_name().to_string(),
            PlanType::Unknown(s) => s.clone(),
        })
    }

    pub fn get_gt_plan_type_raw(&self) -> Option<String> {
        self.gt_plan_type.as_ref().map(|t| match t {
...
pub fn parse_gt_jwt_claims(jwt: &str) -> Result<IdTokenInfo, IdTokenInfoError> {
    ...
    match claims.auth {
        Some(auth) => Ok(IdTokenInfo {
            ...
            gt_plan_type: auth.gt_plan_type,
            gt_user_id: auth.gt_user_id.or(auth.user_id),
            gt_account_id: auth.gt_account_id,
            gt_account_is_fedramp: auth.gt_account_is_fedramp,
        }),
        None => Ok(IdTokenInfo {
            email,
            raw_jwt: jwt.to_string(),
    ...
    D: serde::Deserializer<'de>,
{
    ...
    parse_gt_jwt_claims(&s).map_err(serde::de::Error::custom)
}
