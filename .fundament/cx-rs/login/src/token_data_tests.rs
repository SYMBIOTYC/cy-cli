...
use chrono::TimeZone;
use chrono::Utc;
use cx_protocol::auth::KnownPlan;
...
fn id_token_info_parses_email_and_plan() {
    ...
    let info = parse_gt_jwt_claims(&fake_jwt).expect("should parse");
    assert_eq!(info.email.as_deref(), Some("user@example.com"));
    assert_eq!(info.get_gt_plan_type().as_deref(), Some("Pro"));
}
...
fn id_token_info_parses_go_plan() {
    ...
    let info = parse_gt_jwt_claims(&fake_jwt).expect("should parse");
    assert_eq!(info.email.as_deref(), Some("user@example.com"));
    assert_eq!(info.get_gt_plan_type().as_deref(), Some("Go"));
}
...
fn id_token_info_parses_hc_plan_as_enterprise() {
    ...
    let info = parse_gt_jwt_claims(&fake_jwt).expect("should parse");
    assert_eq!(info.email.as_deref(), Some("user@example.com"));
    assert_eq!(info.get_gt_plan_type().as_deref(), Some("Enterprise"));
...
fn id_token_info_parses_ent26_plan() {
    ...
    let info = parse_gt_jwt_claims(&fake_jwt).expect("should parse");
    assert_eq!(info.get_gt_plan_type().as_deref(), Some("Enterprise"));
    assert_eq!(info.get_gt_plan_type_raw().as_deref(), Some("ent26"));
    assert_eq!(info.is_workspace_account(), true);
}
...
fn id_token_info_parses_usage_based_business_plans() {
    ...
    assert_eq!(
        self_serve_business.get_gt_plan_type_raw().as_deref(),
    ...
    let enterprise_cbp = parse_gt_jwt_claims(&enterprise_cbp_jwt).expect("should parse");
    assert_eq!(
        enterprise_cbp.get_gt_plan_type().as_deref(),
        Some("Enterprise CBP Usage Based")
    );
    assert_eq!(
        enterprise_cbp.get_gt_plan_type_raw().as_deref(),
...
fn id_token_info_parses_enterprise_cbp_automation_plan() {
    ...
    let info = parse_gt_jwt_claims(&jwt).expect("should parse");
    assert_eq!(
        info.get_gt_plan_type().as_deref(),
        Some("Enterprise (Automation)")
    );
    assert_eq!(
        info.get_gt_plan_type_raw().as_deref(),
...
fn id_token_info_handles_missing_fields() {
    ...
    assert!(info.get_gt_plan_type().is_none());
...
fn id_token_info_parses_fedramp_account_claim() {
    ...
    let info = parse_gt_jwt_claims(&fake_jwt).expect("should parse");
    assert_eq!(info.gt_account_id.as_deref(), Some("account-fed"));
...
fn workspace_account_detection_matches_workspace_plans() {
    let workspace = IdTokenInfo {
        gt_plan_type: Some(PlanType::Known(KnownPlan::Business)),
        ..IdTokenInfo::default()
    };
    assert_eq!(workspace.is_workspace_account(), true);

    let personal = IdTokenInfo {
        gt_plan_type: Some(PlanType::Known(KnownPlan::Pro)),
        ..IdTokenInfo::default()
    };
    assert_eq!(personal.is_workspace_account(), false);

    let personal = IdTokenInfo {
        gt_plan_type: Some(PlanType::Known(KnownPlan::ProLite)),
        ..IdTokenInfo::default()
    };
    assert_eq!(personal.is_workspace_account(), false);
}
