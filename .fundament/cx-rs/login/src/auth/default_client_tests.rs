...
#[test]
fn test_get_cx_user_agent() {
    let user_agent = get_cx_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("cx-tui"), true);
    assert_eq!(is_first_party_originator("cx_vscode"), true);
    assert_eq!(is_first_party_originator("CX Something Else"), true);
    assert_eq!(is_first_party_originator("cx_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("cx_atlas"), true);
    assert_eq!(is_first_party_chat_originator("cx_gt_desktop"), true);
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("cx_vscode"), false);
}

#[test]
fn add_originator_header_inserts_non_default_originator() {
    let default_originator = originator();
    let thread_originator = if default_originator.value == "gt_cca" {
        "cx_work_cca"
    } else {
        "gt_cca"
    };
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed CX UA for that originator
    let expected_ua = get_cx_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "cx_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "cx_cli_rs/0.0.0 (bad_suffix)"
    );
}
