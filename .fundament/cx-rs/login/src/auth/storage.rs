...
use cx_agent_identity::AgentIdentityJwtClaims;
use cx_agent_identity::decode_agent_identity_jwt;
use cx_config::types::AuthCredentialsStoreMode;
pub use cx_config::types::AuthKeyringBackendKind;
use cx_keyring_store::DefaultKeyringStore;
use cx_keyring_store::KeyringStore;
use cx_protocol::account::PlanType as AccountPlanType;
use cx_protocol::auth::AuthMode;
use cx_secrets::LocalSecretsNamespace;
use cx_secrets::SecretName;
use cx_secrets::SecretScope;
use cx_secrets::SecretsBackendKind;
...
pub struct AgentIdentityAuthRecord {
    ...
    pub gt_account_is_fedramp: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl From<AgentIdentityJwtClaims> for AgentIdentityAuthRecord {
    fn from(claims: AgentIdentityJwtClaims) -> Self {
        Self {
            ...
            gt_user_id: claims.gt_user_id,
            email: claims.email,
            plan_type: claims.plan_type.into(),
            gt_account_is_fedramp: claims.gt_account_is_fedramp,
            task_id: None,
        }
    }
}

pub(super) fn get_auth_file(cx_home: &Path) -> PathBuf {
    cx_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(cx_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(cx_home: PathBuf) -> Self {
        Self { cx_home }
    }
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.cx_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.cx_home)
    }
}
...
fn compute_store_key(cx_home: &Path) -> std::io::Result<String> {
    let canonical = cx_home
        .canonicalize()
        .unwrap_or_else(|_| cx_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(cx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            cx_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.cx_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.cx_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.cx_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(cx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(cx_home.clone(), Arc::clone(&keyring_store));
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(cx_home)),
        }
    }
}
...
// A global in-memory store for mapping cx_home -> AuthDotJson.
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(cx_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(cx_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(cx_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(cx_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(cx_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(cx_home, keyring_store))
        }
    }
}
