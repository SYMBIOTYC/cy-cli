//! Default CX HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`cx_login::default_client`] from other crates in this
//! workspace.

use cx_http_client::BuildRouteAwareHttpClientError;
...
pub use cx_http_client::RequestBuilder as CodexRequestBuilder;
...
/// See https://github.com/openai/cx/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by CX. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "cx_cli_rs";
pub const CX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "CX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-cx-residency";

pub use cx_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(CX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(CX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "cx-tui"
        || originator_value == "cx_vscode"
        || originator_value.starts_with("CX ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "cx_atlas" || originator_value == "cx_gt_desktop"
}

pub fn get_cx_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized CX user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base CX user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default CX originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// CX's route-aware system/PAC resolution.
pub fn create_client() -> HttpClient {
    build_default_client(default_http_client_builder())
}

/// Creates the default client with configured gt cookies and no sensitive-response logging.
pub fn create_client_with_gt_cookies(http_client_factory: &HttpClientFactory) -> HttpClient {
    build_default_client(
        default_http_client_builder()
            .with_gt_cookies(http_client_factory)
...
/// Builds the default CX HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the CX
...
/// Builds the default CX HTTP client for a concrete outbound route without blocking the
...
fn default_http_client_builder() -> HttpClientBuilder {
    HttpClientBuilder::new()
        ...
        .with_gt_cloudflare_cookie_store()
}
...
/// Builds an HTTP client for an auth endpoint without CX default headers.
...
/// Builds the default CX HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_cx_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("CX_SANDBOX").as_deref() == Ok("seatbelt")
}
