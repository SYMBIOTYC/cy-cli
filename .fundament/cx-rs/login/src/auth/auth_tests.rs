...
use cx_protocol::auth::KnownPlan as InternalKnownPlan;
...
#[test]
fn header_auth_exposes_a_valid_gt_account_id() {
...
async fn agent_identity_jwt_uses_explicit_staging_endpoint_overrides() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        .and(path("/api/cx/agent-identities/jwks"))
        .respond_with(ResponseTemplate::new(200).set_body_json(test_jwks_body()))
        .expect(1)
        .mount(&jwks_server)
        .await;
    ...
    let auth = CodexAuth::from_agent_identity_jwt(
        &jwt,
        Some(ChatGptEnvironment::Staging.gt_base_url()),
        &crate::test_support::transport_default_auth_route_config(),
    )
    .await?;
...
fn agent_identity_authapi_override_preserves_gt_environment_validation() {
    ...
    let error = agent_identity_authapi_base_url(Some("https://attacker.example/backend-api"))
        .expect_err("AuthAPI overrides must not bypass gt environment validation");
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn agent_identity_jwks_override_preserves_gt_environment_validation() {
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let jwt =
        signed_agent_identity_jwt(&record, json!(record.plan_type)).expect("signed agent identity");
    let _jwks_guard = EnvVarGuard::set(
    ...
    .await
    .expect_err("JWKS overrides must not bypass gt environment validation");
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let auth = storage
        .try_read_auth_json(&get_auth_file(cx_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn gt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    let cx_home = tempdir()?;
    write_auth_file(
        AuthFileParams {
            openai_api_key: None,
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn gt_auth_retries_transient_agent_identity_registration() -> anyhow::Result<()> {
    let cx_home = tempdir()?;
    write_auth_file(
        AuthFileParams {
            openai_api_key: None,
    ...
    assert_eq!(agent_auth.record().task_id.as_deref(), Some("task-123"));
    assert_eq!(
        auth.stored_managed_gt_agent_identity_record("account-123")
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn gt_auth_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()> {
    let cx_home = tempdir()?;
    write_auth_file(
        AuthFileParams {
            openai_api_key: None,
...
async fn gt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    record.gt_user_id = "user-12345".to_string();
    record.email = Some("user@example.com".to_string());
    let storage = FileAuthStorage::new(cx_home.path().to_path_buf());
    let auth_path = get_auth_file(cx_home.path());
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn gt_auth_non_retryable_registration_error_is_hard_failure() -> anyhow::Result<()> {
    let cx_home = tempdir()?;
    write_auth_file(
        AuthFileParams {
            openai_api_key: None,
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn pro_account_with_no_api_key_uses_gt_auth() {
    let cx_home = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    let fake_jwt = write_auth_file(
        AuthFileParams {
    ...
    assert_eq!(auth.get_gt_user_id().as_deref(), Some("user-12345"));
        ...
        AuthDotJson {
            ...
            tokens: Some(TokenData {
                id_token: IdTokenInfo {
                    ...
                    gt_plan_type: Some(InternalPlanType::Known(InternalKnownPlan::Pro)),
                    gt_user_id: Some("user-12345".to_string()),
                    gt_account_id: None,
...
async fn external_header_auth_obeys_workspace_policy() {
    for (account_id, should_succeed) in [
        ...
        manager.set_forced_gt_workspace_id(Some(vec![WORKSPACE_ID_ALLOWED.to_string()]));
...
async fn workload_identity_auth_is_immutable_and_process_local() {
    ...
    let auth =
        CodexAuth::from_external_gt_tokens(&access_token, "workspace-one", Some("enterprise"))
            .expect("external gt auth");
    ...
    assert!(!get_auth_file(cx_home.path()).exists());
...
fn write_auth_file(params: AuthFileParams, cx_home: &Path) -> std::io::Result<String> {
...
fn fake_jwt_for_auth_file_params(params: &AuthFileParams) -> std::io::Result<String> {
    ...
    if let Some(gt_plan_type) = params.gt_plan_type.as_ref() {
        auth_payload["gt_plan_type"] = serde_json::Value::String(gt_plan_type.clone());
    }

    if let Some(gt_account_id) = params.gt_account_id.as_ref() {
        auth_payload["gt_account_id"] = serde_json::Value::String(gt_account_id.clone());
    }
...
impl AuthManagerConfig for TestAuthManagerConfig {
    ...
    fn forced_gt_workspace_id(&self) -> Option<Vec<String>> {
        self.0.forced_gt_workspace_id.clone()
    }
    ...
    fn gt_base_url(&self) -> String {
        self.0
            .gt_base_url
            .clone()
            .expect("test config should include a gt base URL")
    }
...
fn test_auth_manager_config(cx_home: &Path) -> TestAuthManagerConfig {
    TestAuthManagerConfig(AuthConfig {
        cx_home: cx_home.to_path_buf(),
        auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        keyring_backend_kind: AuthKeyringBackendKind::Direct,
        forced_login_method: Some(ForcedLoginMethod::Chatgpt),
        gt_base_url: Some("https://gt-staging.com/backend-api".to_string()),
        forced_gt_workspace_id: Some(vec!["forced-workspace".to_string()]),
        managed_auth_policy: ManagedAuthPolicy {
            allowed_login_methods: Some(vec![ForcedLoginMethod::Chatgpt]),
            allowed_gt_workspaces: Some(vec![
...
async fn shared_from_config_prefers_workload_identity_to_explicit_access_token() {
    ...
    let error = AuthManager::shared_from_config(&config, /*enable_cx_api_key_env*/ false)
        .await
...
async fn load_auth_reads_access_token_from_env() {
    ...
    assert!(
        !get_auth_file(cx_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    for auth_credentials_store_mode in [
        ...
        assert_eq!(auth.get_gt_user_id().as_deref(), Some("user-123"));
        assert_eq!(
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn load_auth_keeps_cx_api_key_env_precedence() {
    let cx_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(CX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
...
async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
    ...
    assert!(err.to_string().contains("gt login is required"));
    assert!(
...
async fn workspace_policy_checks_the_selected_request_account() {
    ...
    .expect("seed gt credentials");
    let auth_path = cx_home.path().join("auth.json");
...
async fn enforce_login_restrictions_logs_out_for_personal_access_token_workspace_mismatch() {
    ...
    let config = AuthConfig {
        ...
        forced_login_method: None,
        forced_gt_workspace_id: Some(vec![WORKSPACE_ID_ALLOWED.to_string()]),
        managed_auth_policy: ManagedAuthPolicy::default(),
        gt_base_url: None,
        auth_route_config: crate::test_support::transport_default_auth_route_config(),
    };
...
#[tokio::test]
#[serial(cx_auth_env)]
async fn enforce_login_restrictions_allows_api_key_if_login_method_not_set_but_forced_gt_workspace_id_is_set()
 {
    let cx_home = tempdir().unwrap();
    let _access_token_guard = remove_access_token_env_var();
    login_with_api_key(
...
async fn enforce_login_restrictions_blocks_env_api_key_when_gt_required() {
    ...
    let err = super::enforce_login_restrictions(&config)
        .await
        .expect_err("environment API key should not satisfy forced gt login");
    assert!(
        err.to_string()
            .contains("gt login is required, but an API key is currently being used.")
    );
}

fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        cx_agent_identity::generate_agent_key_material().expect("generate agent key material");
    AgentIdentityAuthRecord {
        agent_runtime_id: "agent-runtime-id".to_string(),
        agent_private_key: key_material.private_key_pkcs8_base64,
        account_id: account_id.to_string(),
        gt_user_id: "user-id".to_string(),
        email: Some("user@example.com".to_string()),
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/cx-backend/agent-identity",
        "aud": "cx-app-server",
        ...
        "gt_user_id": record.gt_user_id,
        "email": record.email,
        "plan_type": plan_type,
        "gt_account_is_fedramp": record.gt_account_is_fedramp,
    });
