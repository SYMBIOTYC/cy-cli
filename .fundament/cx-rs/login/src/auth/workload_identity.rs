...
use cx_protocol::shell_environment::OI_WORKLOAD_IDENTITY_CONTEXT_ENV_VAR;
use cx_workload_identity::WorkloadIdentityConfig;
use cx_workload_identity::WorkloadIdentityError;
use cx_workload_identity::WorkloadIdentityExchange;
use cx_workload_identity::WorkloadIdentityToken;
use thiserror::Error;
use url::Url;
...
fn resolve_config(
    ...
    if !gt_login_allowed {
        return Err(invalid_config(
            "workload identity requires a login policy that permits gt authentication",
        ));
    }
    ...
    let workload_identity_context = optional_unicode(
        ...
        OI_WORKLOAD_IDENTITY_CONTEXT_ENV_VAR,
    )?;
    let auth_environment = classify_auth_environment(gt_base_url)?;
...
impl ProcessEnvironment {
    fn read() -> Self {
        Self {
            federation_rule_id: std::env::var_os(OI_FEDERATION_RULE_ID_ENV_VAR),
            identity_token_file: std::env::var_os(OI_IDENTITY_TOKEN_FILE_ENV_VAR),
            workload_identity_context: std::env::var_os(OI_WORKLOAD_IDENTITY_CONTEXT_ENV_VAR),
        }
    }
...
impl From<&WorkloadIdentityToken> for WorkloadIdentitySubject {
    fn from(token: &WorkloadIdentityToken) -> Self {
        Self {
            account_id: token.gt_account_id.clone(),
            account_user_id: token.gt_account_user_id.clone(),
...
impl WorkloadIdentitySession {
    ...
    fn accept_subject(
        ...
        if previous_account_id.is_some_and(|account_id| account_id != token.gt_account_id) {
...
impl WorkloadIdentityExternalAuth {
    ...
    fn validate_auth(
        ...
        let auth = CodexAuth::from_external_gt_tokens(
            &token.access_token,
            &token.gt_account_id,
            token.gt_plan_type.as_deref(),
        )
        .map_err(|_| std::io::Error::other(WorkloadIdentityError::InvalidExchangeResponse))?;
        if auth.get_gt_user_id().as_deref() != Some(token.user_id.as_str()) {
