...
use cx_agent_identity::AgentIdentityKey;
use cx_agent_identity::ChatGptEnvironment;
use cx_agent_identity::agent_identity_jwks_url;
use cx_agent_identity::agent_registration_url;
use cx_agent_identity::agent_task_registration_url;
use cx_agent_identity::build_abom;
use cx_agent_identity::decode_agent_identity_jwt;
use cx_agent_identity::fetch_agent_identity_jwks;
use cx_agent_identity::generate_agent_key_material;
use cx_agent_identity::is_retryable_registration_error;
use cx_agent_identity::public_key_ssh_from_private_key_pkcs8_base64;
use cx_agent_identity::register_agent_identity;
use cx_agent_identity::register_agent_task;
...
fn agent_identity_jwks_base_url_matches(gt_base_url: &str, jwks_base_url: &str) -> bool {
    gt_base_url.trim().trim_end_matches('/') == jwks_base_url
}

pub(super) fn agent_identity_authapi_base_url(
    gt_base_url: Option<&str>,
) -> std::io::Result<String> {
    let environment = match gt_base_url {
        Some(gt_base_url) => ChatGptEnvironment::from_gt_base_url(gt_base_url),
    ...
    match (environment, authapi_base_url) {
        ...
        (Err(_), Some(base_url))
            if gt_base_url.is_some_and(|gt_base_url| {
                jwks_base_url.as_deref().is_some_and(|jwks_base_url| {
                    agent_identity_jwks_base_url_matches(gt_base_url, jwks_base_url)
                })
            }) =>
        {
            Ok(base_url)
...
pub(super) struct ManagedChatGptAgentIdentityBinding {
    ...
    pub(super) gt_user_id: String,
    pub(super) email: Option<String>,
    pub(super) plan_type: AccountPlanType,
    pub(super) gt_account_is_fedramp: bool,
...
impl AgentIdentityAuth {
    ...
    pub async fn from_jwt(
        ...
        let record = verified_record_from_jwt(jwt, gt_base_url, auth_route_config).await?;
    ...
    pub fn gt_user_id(&self) -> &str {
        &self.record.gt_user_id
    }
    ...
    pub fn is_fedramp_account(&self) -> bool {
        self.record.gt_account_is_fedramp
    }
}

pub(super) async fn register_managed_gt_agent_identity(
    ...
    let record = AgentIdentityAuthRecord {
        ...
        gt_user_id: binding.gt_user_id,
        email: binding.email,
        plan_type: binding.plan_type,
        gt_account_is_fedramp: binding.gt_account_is_fedramp,
        task_id: None,
    };
...
pub(super) fn record_matches_managed_gt_binding(
    record: &AgentIdentityAuthRecord,
    binding: &ManagedChatGptAgentIdentityBinding,
) -> bool {
    record.account_id == binding.account_id
        && record.gt_user_id == binding.gt_user_id
