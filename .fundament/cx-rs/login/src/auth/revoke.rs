...
//!
//! Managed gt auth stores OAuth tokens locally. Logout attempts to revoke the
...
fn revocable_token(auth_dot_json: &AuthDotJson) -> Option<(&str, RevokeTokenKind)> {
    let tokens = managed_gt_tokens(auth_dot_json)?;
...
fn managed_gt_tokens(auth_dot_json: &AuthDotJson) -> Option<&TokenData> {
...
fn resolved_auth_mode(auth_dot_json: &AuthDotJson) -> AuthMode {
    ...
    if auth_dot_json.cy_api_key.is_some() {
...
mod tests {
    ...
    async fn revoke_request_times_out() {
        ...
        let reqwest_error = error
            .get_ref()
            .and_then(|error| error.downcast_ref::<cx_http_client::HttpError>())
