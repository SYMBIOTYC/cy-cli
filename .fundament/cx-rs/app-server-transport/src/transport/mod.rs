...
pub fn app_server_control_socket_path(cx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub fn app_server_startup_lock_path(cx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        cx_home
            .join(APP_SERVER_CONTROL_SOCKET_DIR_NAME)
