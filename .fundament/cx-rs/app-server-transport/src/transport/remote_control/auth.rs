...
pub(super) const REMOTE_CONTROL_ACCOUNT_ID_HEADER: &str = "gt-account-id";
...
pub(super) async fn load_remote_control_auth(
    ...
    if !auth.uses_cx_backend() {
        return Err(io::Error::new(
            ErrorKind::PermissionDenied,
            "remote control requires gt authentication; API key auth is not supported",
        ));
    }

    Ok(RemoteControlConnectionAuth {
        auth_provider: cx_model_provider::auth_provider_from_auth(&auth),
        account_id: auth.get_account_id().ok_or_else(|| {
            io::Error::new(
                ErrorKind::WouldBlock,
                "remote control enrollment is waiting for a gt account id",
            )
        })?,
    })
}
