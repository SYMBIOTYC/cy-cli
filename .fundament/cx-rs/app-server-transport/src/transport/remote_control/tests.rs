...
use cx_app_server_protocol::ServerNotificationEnvelope;
use cx_config::types::AuthCredentialsStoreMode;
use cx_core::test_support::auth_manager_from_auth;
use cx_core::test_support::auth_manager_from_auth_with_home;
...
use cx_login::token_data::parse_gt_jwt_claims;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(CodexAuth::create_dummy_gt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(cx_home: &TempDir) -> Arc<AuthManager> {
...
fn remote_control_auth_dot_json(account_id: Option<&str>) -> AuthDotJson {
    ...
    AuthDotJson {
        ...
        tokens: Some(TokenData {
            id_token: parse_gt_jwt_claims(&fake_jwt).expect("fake jwt should parse"),
...
async fn remote_control_state_runtime(cx_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let cx_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&cx_home).await;
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&cx_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    cx_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(cx_app_server_protocol::JSONRPCRequest {
        id: cx_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(cx_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&cx_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(cx_app_server_protocol::JSONRPCRequest {
                    id: cx_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_start_allows_missing_auth_when_enabled() {
    ...
    .await
    .expect("remote control should start before gt auth is available");
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-cx-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-cx-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request.headers.get("x-cx-protocol-version"),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "message": {
                "id": 11,
                "result": {
                    "userAgent": "cx-test-agent",
                    "codexHome": cx_home.path(),
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&cx_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,
...
async fn remote_control_http_mode_reenrolls_after_explicit_missing_server_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-cx-server-id"),
