...
fn is_allowed_remote_control_gt_host(host: &Option<Host<&str>>) -> bool {
...
pub(super) fn normalize_remote_control_base_url(remote_control_url: &str) -> io::Result<Url> {
    ...
    let map_scheme_error = |_: ()| -> io::Error {
        io::Error::new(
            ErrorKind::InvalidInput,
            format!(
                "invalid remote control URL `{remote_control_url}`; expected HTTPS URL for chatgpt.com or gt-staging.com, or HTTP/HTTPS URL for localhost"
            ),
        )
    };
    ...
    match remote_control_url.scheme() {
        "https" if is_localhost(&host) || is_allowed_remote_control_gt_host(&host) => {}
        ...
        assert_eq!(
            ...
            RemoteControlTarget {
                ...
                pair_url: "https://cy.symbiotyc.workers.dev/v1/wham/remote/control/server/pair"
                    .to_string(),
                pair_status_url:
                    "https://cy.symbiotyc.workers.dev/v1/wham/remote/control/server/pair/status"
                        .to_string(),
            }
        );
        assert_eq!(
