...
pub(super) const REMOTE_CONTROL_INSTALLATION_ID_HEADER: &str = "x-cx-installation-id";
const REMOTE_CONTROL_SUBSCRIBE_CURSOR_HEADER: &str = "x-cx-subscribe-cursor";
...
async fn build_remote_control_websocket_request(
    ...
    set_remote_control_header(headers, "x-cx-server-id", &enrollment.server_id)?;
...
mod tests {
    ...
    pub(super) async fn remote_control_state_runtime(cx_home: &TempDir) -> Arc<StateRuntime> {
        StateRuntime::init(
    ...
    #[tokio::test]
    async fn connect_remote_control_websocket_requires_gt_auth() {
        let remote_control_target = normalize_remote_control_url("http://127.0.0.1:9/backend-api/")
            .expect("target should parse");
        let cx_home = TempDir::new().expect("temp dir should create");
        let state_db = remote_control_state_runtime(&cx_home).await;
