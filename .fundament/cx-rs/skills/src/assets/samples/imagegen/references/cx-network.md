# CX network approvals / sandbox notes
...
The fallback CLI uses the oi Image API, so it needs outbound network access. In many CX setups, network access is disabled by default and/or the approval policy requires confirmation before networked commands run.
...
- In `workspace-write`, network access still depends on your CX configuration (for example `[sandbox_workspace_write] network_access = true`).
...
Example `~/.cx/config.toml` pattern:
