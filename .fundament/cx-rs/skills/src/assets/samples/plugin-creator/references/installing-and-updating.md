...
- the user wants CX to see the updated plugin without manually editing marketplace files
...
2. Update the plugin manifest to a single CX cachebuster suffix:
...
outside CX depends on a specific token:
...
```bash
cx plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `cx plugin marketplace add` for that
path, and `cx plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
cx plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that CX picks up new skills
   and tools.
...
```text
<base-version>+cx.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+cx.local-20260519-184516`
- `0.1.0+cx.old-token` → `0.1.0+cx.local-20260519-184516`
- `1.2.3-beta.1+cx.prev` → `1.2.3-beta.1+cx.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+cx.local-20260519-184516`

Replace the existing CX cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `cx plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `cx plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by CX.
...
- Use `cx plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `cx plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `cx plugin add <plugin-name>@<marketplace-name>`.
