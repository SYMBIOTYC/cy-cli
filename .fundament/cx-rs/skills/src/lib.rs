...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".cx-system-skills.marker";
...
/// install is skipped.
pub fn install_system_skills(cx_home: &AbsolutePathBuf) -> Result<(), SystemSkillsError> {
    let skills_root_dir = cx_home.join(SKILLS_DIR_NAME);
    fs::create_dir_all(skills_root_dir.as_path())
        .map_err(|source| SystemSkillsError::io("create skills root dir", source))?;

    let dest_system = system_cache_root_dir(cx_home);
