...
- From the CX CLI, run `cx execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
cx execpolicy check --rules path/to/policy.rules git status
```
...
```bash
cargo run -p cx-execpolicy -- check --rules path/to/policy.rules git status
```
