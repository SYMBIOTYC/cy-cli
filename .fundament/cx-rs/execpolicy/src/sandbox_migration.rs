...
/// removes legacy allow rules that newer cx versions no longer offer.
///
/// this migration is intentionally one-shot. once complete, a marker in `cx_home` prevents
/// policies saved by newer cx versions from being removed on later startups.
pub async fn prefix_rule_migration(
    cx_home: &Path,
    policy_path: &Path,
    banned_prefixes: &[&[&str]],
) -> io::Result<()> {
    let marker_path = cx_home.join(MIGRATION_MARKER_FILENAME);
    if tokio::fs::try_exists(&marker_path).await? {
        return Ok(());
    }
    clean_rules_file(policy_path, banned_prefixes).await?;

    write_migration_marker(cx_home, &marker_path).await?;
    Ok(())
}

// atomically writes the marker after creating cx home when needed.
async fn write_migration_marker(cx_home: &Path, marker_path: &Path) -> io::Result<()> {
    tokio::fs::create_dir_all(cx_home).await?;
    let cx_home = cx_home.to_owned();
    let marker_path = marker_path.to_owned();
    tokio::task::spawn_blocking(move || {
        let mut marker = tempfile::NamedTempFile::new_in(cx_home)?;
