...
/// single CX run, while the agent runtime id and private key are the
...
pub async fn register_agent_identity(
    ...
    if is_fedramp_account {
        request_builder = request_builder.header("X-oi-Fedramp", "true");
    }
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "cx-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "cx-cli".to_string(),
        },
...
-----END PRIVATE KEY-----"#,
    ...
    #[test]
    fn gt_environment_rejects_custom_urls() {
        assert!(ChatGptEnvironment::from_gt_base_url("http://localhost:8080").is_err(),);
    }
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/cx"),
            "http://localhost:8080/api/cx/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/cx/"),
            "http://localhost:8080/api/cx/agent-identities/jwks"
