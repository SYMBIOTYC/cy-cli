...
use cx_exec_server::ExecBackend;
use cx_exec_server::ExecEnvPolicy;
use cx_exec_server::ExecParams;
use cx_exec_server::ExecProcess;
...
use cx_utils_pty::process_group::kill_process_group;
#[cfg(target_os = "macos")]
use cx_utils_pty::process_group::kill_process_group_with_member_fallback as kill_process_group;
#[cfg(all(unix, not(target_os = "macos")))]
use cx_utils_pty::process_group::terminate_process_group;
#[cfg(target_os = "macos")]
use cx_utils_pty::process_group::terminate_process_group_with_member_fallback as terminate_process_group;
...
impl LocalStdioServerLauncher {
    fn launch_server(
        ...
        #[cfg(windows)]
        let job = match cx_utils_pty::JobObject::create_without_breakaway() {
            Ok(job) => {
    ...
    fn terminate(&self) {
        let result = match self {
            ...
            Self::Process(process_handle) => {
                cx_utils_pty::JobObject::terminate_process_handle(process_handle)
            }
        };
...
impl ExecutorStdioServerLauncher {
    ...
    fn remote_env_policy(remote_env_vars: &[String]) -> ExecEnvPolicy {
        let include_only = if remote_env_vars.is_empty() {
            ...
            // environment, not copied from CX. Start from `All` only so the
...
mod tests {
    ...
    use cx_protocol::shell_environment;
