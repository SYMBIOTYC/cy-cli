...
const LOCK_CONTENTION_EVENT_TARGET: &str = "cx_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    ...
    fn acquire_with_timeout(
        ...
        // authority. Those stores are CX_HOME-backed today: if CX_HOME is unset they use
    ...
    fn acquire_in_with_mode(
        ...
        let path = oauth_store_lock_path(cx_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve CX_HOME for MCP OAuth {store} aggregate-store lock")]
    CodexHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(cx_home: &Path, store: OAuthStore) -> PathBuf {
    cx_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}
