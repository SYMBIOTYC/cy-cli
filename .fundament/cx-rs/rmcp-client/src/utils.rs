...
pub(crate) const MCP_USER_AGENT: &str = concat!("cx-mcp-client/", env!("CARGO_PKG_VERSION"));
...
mod tests {
    ...
    async fn create_env_honors_overrides() {
        ...
        let env = create_env_for_mcp_server(
            Some(HashMap::from([
                ...
                (
                    OsString::from("oi_identity_token_file"),
        ...
        assert!(!env.contains_key(OsStr::new("oi_identity_token_file")));
    }
    ...
    fn create_remote_env_overlay_only_forwards_explicit_variables() {
        ...
        let env = create_env_overlay_for_remote_mcp_server(
            Some(HashMap::from([(
                OsString::from("oi_Federation_Rule_Id"),
                OsString::from("rule"),
            )])),
            &[custom_var.into()],
        );
    ...
    fn remote_mcp_env_var_names_returns_remote_source_names() {
        let names = remote_mcp_env_var_names(&[
            ...
            McpServerEnvVar::Config {
                name: "oi_identity_token_file".to_string(),
                source: Some("remote".to_string()),
            },
        ]);
    ...
    fn create_env_preserves_path_when_it_is_not_utf8() {
        ...
        let raw_path = std::ffi::OsStr::from_bytes(b"/tmp/cx-\xFF/bin");
