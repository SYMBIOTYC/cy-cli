...
const MEMO_URI: &str = "memo://cx/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "cx/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of CX TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (CX TUI):
    /// - Build this binary: `cargo build -p cx-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `cx mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in CX TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://cx/{slug}", "cx-memo")
            .with_title("CX Memo")
            .with_description("Template for memo://cx/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise CX TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("cx-rmcp-test-server", env!("CARGO_PKG_VERSION"))
        ...
        match request.name.as_ref() {
            "js" => {
                ...
                if args.code == "nodeRepl.fail()" {
                    ...
                    meta.insert("cx/imageDetail".to_string(), json!("low"));
                    Ok(CallToolResult::success(vec![
            ...
            "encrypted_output" => {
                let mut meta = MetaObject::new();
                meta.insert("cx/encryptedContent".to_string(), json!(true));
