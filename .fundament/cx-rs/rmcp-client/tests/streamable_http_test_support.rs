...
use cx_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    cx_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("cx-test", "0.0.0-test").with_title("CX rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<cx_http_client::HttpClient> {
    Ok(cx_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `cx exec-server --listen`.
