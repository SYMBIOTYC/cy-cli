...
//!
//! The cache is scoped to the authenticated gt user and account, has a
...
const CLOUD_CONFIG_BUNDLE_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"cx-cloud-config-bundle-cache-v1-6160ae70-bcfd-4ca8-a99b-40f73b3b072e";
...
impl CloudConfigBundleCache {
    pub(super) fn new(cx_home: AbsolutePathBuf) -> Self {
        Self {
            path: cx_home.join(CLOUD_CONFIG_BUNDLE_CACHE_FILENAME),
        }
    }
    ...
    pub(super) async fn load(
        ...
        let (Some(gt_user_id), Some(account_id)) = (gt_user_id, account_id) else {
        ...
        let (Some(cached_gt_user_id), Some(cached_account_id)) = (
            cache_file.signed_payload.gt_user_id.as_deref(),
            cache_file.signed_payload.account_id.as_deref(),
        ) else {
            return Err(CacheLoadStatus::CacheIdentityIncomplete);
        };

        if cached_gt_user_id != gt_user_id || cached_account_id != account_id {
...
pub(super) struct CloudConfigBundleCacheSignedPayload {
    ...
    pub(super) gt_user_id: Option<String>,
