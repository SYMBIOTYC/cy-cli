...
use cx_backend_client::ConfigBundleResponse;
use cx_backend_client::DeliveredTomlFragment;
use cx_config::AbsolutePathBuf;
use cx_config::CloudConfigFragment;
use cx_config::CloudConfigTomlBundle;
use cx_config::CloudRequirementsFragment;
use cx_config::CloudRequirementsTomlBundle;
...
use cx_login::auth::ExternalAuth;
use cx_login::auth::ExternalAuthRefreshContext;
...
fn write_auth_json(cx_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(cx_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(cx_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(cx_home, "/"))
}
    ...
    AuthManager::from_auth_for_testing(CodexAuth::AgentIdentity(
        AgentIdentityAuth::from_record(
            AgentIdentityAuthRecord {
                ...
                gt_user_id: "user-12345".to_string(),
...
fn gt_auth_json(
    ...
    gt_auth_json_with_last_refresh(
        plan_type,
        gt_user_id,
        account_id,
        access_token,
...
fn gt_auth_json_with_last_refresh(
    plan_type: &str,
    gt_user_id: Option<&str>,
    account_id: Option<&str>,
    access_token: &str,
    ...
    let fake_jwt = fake_gt_jwt(plan_type, gt_user_id, b"sig");
    json!({
        "OPENAI_API_KEY": null,
        "tokens": {
            "id_token": fake_jwt,
...
fn fake_gt_jwt(plan_type: &str, gt_user_id: Option<&str>, signature: &[u8]) -> String {
...
#[tokio::test]
async fn get_bundle_skips_non_gt_auth() {
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = CodexAuth::from_external_gt_tokens(
        &fake_gt_jwt("enterprise", Some("user-12345"), b"initial"),
        "account-12345",
        Some("enterprise"),
    )
...
async fn dropping_loader_cancels_in_flight_startup_and_refresh() {
    for starts_from_cache in [false, true] {
        ...
        if starts_from_cache {
            create_test_cache(cx_home.path())
                .save(
                    Some("user-12345".to_string()),
                    Some("account-12345".to_string()),
                    test_bundle(),
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(cx_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            cx_backend_client::DeliveredRequirementsToml {
