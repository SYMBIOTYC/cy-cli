...
use cx_config::CloudConfigBundle;
...
fn auth_identity(auth: &CodexAuth) -> (Option<String>, Option<String>) {
    (auth.get_gt_user_id(), auth.get_account_id())
}
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let cx_home = AbsolutePathBuf::resolve_path_against_base(cx_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(cx_home.clone()),
            cx_home,
            timeout,
    ...
    async fn load_startup_bundle(
        ...
        let (gt_user_id, account_id) = auth_identity(&auth);
        match self
            .load_valid_cached_bundle(gt_user_id.as_deref(), account_id.as_deref())
            .await
        {
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(gt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.cx_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.cx_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",
        ...
        if let Err(err) = self
            .cache
            .save(gt_user_id, account_id, bundle.clone())
            .await
        {
            tracing::warn!(
                error = %err,
