...
impl RemoteAppServerClient {
    ...
    pub fn cx_home(&self) -> Option<&str> {
        self.cx_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, cx_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            cx_home = response
                                ...
                                .get("codexHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|cx_home| !cx_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, cx_home))
}
