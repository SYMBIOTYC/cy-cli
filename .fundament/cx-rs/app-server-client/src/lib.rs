...
//!
//! This crate wraps [`cx_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](cx_app_server::in_process::InProcessClientHandle),
...
pub use cx_app_server::app_server_control_socket_path;
pub use cx_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use cx_app_server::in_process::InProcessServerEvent;
use cx_app_server::in_process::InProcessStartArgs;
use cx_app_server::in_process::LogDbLayer;
pub use cx_app_server::in_process::StateDbHandle;
use cx_app_server_protocol::ClientInfo;
use cx_app_server_protocol::ClientNotification;
...
use cx_app_server_protocol::Result as JsonRpcResult;
...
pub use cx_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `cx-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use cx_core::config::*;

        pub mod edit {
            pub use cx_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `CX_API_KEY` environment variable.
    pub enable_cx_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_cx_api_key_env: self.enable_cx_api_key_env,
            initialize,
...
/// - [`cx_app_server::in_process::InProcessClientHandle`], which speaks to
                    ...
                    event = handle.next_event(), if event_stream_enabled => {
                                ...
                                request.as_ref()
                        {
                            let send_result = request_sender.fail_server_request(
                                ...
                                JSONRPCErrorError {
                                    code: -32000,
                                    message: "gt auth token refresh is not supported for in-process app-server clients".to_string(),
                                    data: None,
                                },
                            );
                            if let Err(err) = send_result {
                                warn!(
                                    "failed to reject unsupported gt auth token refresh request: {err}"
                                );
                            }
                            continue;
                        }
...
impl AppServerClient {
    pub fn cx_home(&self, local_cx_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_cx_home.display().to_string(),
            )),
            Self::Remote(client) => client.cx_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use cx_app_server_protocol::ServerNotification;
    use cx_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    async fn build_test_config_for_cx_home(cx_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_cx_home(
                cx_home.to_path_buf(),
                Vec::new(),
            )
            .await
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config_for_cx_home(cx_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "cx-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "cx_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "codexHome": "/server/.cx",
                }),
            }),
        )
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            cx_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(cx_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: cx_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(cx_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: cx_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: cx_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: cx_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: cx_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<cx_app_server_protocol::ThreadReadResponse>(
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.cx_home(), Some("/server/.cx"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: cx_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("cx.sock"))
            ...
            match event {
                ...
                AppServerEvent::ServerNotification(notification) => match notification.as_ref() {
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            cx_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        ) =>
                    {
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == cx_app_server_protocol::TurnStatus::Completed =>
                    {
