...
use cx_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == cx_features::Feature::Artifact {
            continue;
        }
        ...
        if feature.id == cx_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<cx_features::FeatureToml<
                    cx_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        ...
        if feature.id == cx_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<cx_features::FeatureToml<
                    cx_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == cx_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<cx_features::FeatureToml<
                    cx_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == cx_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<cx_features::ToolRegistryConfigToml>(),
    );
