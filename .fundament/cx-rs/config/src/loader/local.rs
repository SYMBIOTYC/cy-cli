...
pub async fn load_local_config_layers(
    ...
    load_local_config_layers_with_overrides(fs, cx_home, cwd, &LoaderOverrides::default()).await
}
    ...
    for layer in layers {
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_cx_folder: layer.dot_cx_folder.clone(),
            },
            base_dir: layer.dot_cx_folder,
            toml: config,
        });
        ...
        output.push(LocalTomlLayer {
            source: ConfigLayerSource::Project {
                dot_cx_folder: hooks_config_folder.clone(),
            },
...
async fn local_requirements_layers(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        entries.extend(
            ...
            .await?
            .map(|layer| layer.with_base_dir(cx_home)),
        );
    }
