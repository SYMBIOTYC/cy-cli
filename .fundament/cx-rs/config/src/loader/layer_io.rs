...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/cx/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(cx_home)),
    )?;
...
pub(super) fn managed_config_default_path(cx_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = cx_home;
