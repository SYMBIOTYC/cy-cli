use super::*;
use cx_file_system::CopyOptions;
use cx_file_system::CreateDirectoryOptions;
use cx_file_system::ExecutorFileSystemFuture;
use cx_file_system::FileMetadata;
use cx_file_system::FileSystemReadStream;
use cx_file_system::FileSystemSandboxContext;
use cx_file_system::GetMetadataOptions;
use cx_file_system::ReadDirectoryEntry;
use cx_file_system::ReadFileOptions;
use cx_file_system::RemoveOptions;
use cx_file_system::WriteFileOptions;
...
"#,
    ...
    assert_eq!(requirements.allowed_gt_workspaces, None);
    assert_eq!(
        requirements
...
async fn default_windows_managed_config_is_ignored_with_warning() {
    ...
    let managed_config_path = cx_home.join("managed_config.toml");
    std::fs::write(
        &managed_config_path,
        r#"
model = "legacy-model"
...
"#,
    ...
    std::fs::write(cx_home.join(CONFIG_TOML_FILE), r#"model = "user-model""#)
...
fn windows_local_managed_configuration_ignores_legacy_file_but_detects_requirements() {
    ...
    let cx_home = tmp.path().join("cx-home");
    std::fs::create_dir_all(&cx_home).expect("create cx home");
    std::fs::write(cx_home.join("managed_config.toml"), "")
        .expect("write default legacy managed config");
...
async fn local_layers_keep_raw_paths_order_and_legacy_requirements() {
    ...
    for dir in [&cx_home, &dot_codex, &system_dir, &managed_dir] {
    ...
    let user_file = cx_home.join(CONFIG_TOML_FILE);
