...
use cx_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/cx/config.toml";
...
///
/// - system    `/etc/cx/requirements.toml` (Unix) or
///   `%ProgramData%\oi\CX\requirements.toml` (Windows)
/// - cloud:    enterprise-managed cloud config bundle requirements
/// - legacy:   `/etc/cx/managed_config.toml` (Unix) reinterpreted as
///   requirements.toml
/// - admin:    managed preferences (*)
///
/// For backwards compatibility, Unix continues to load
/// `/etc/cx/managed_config.toml` and map it to `requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - package:  optional default configuration supplied with the CX package
/// - admin:    managed preferences (*)
...
pub async fn load_config_layers_state(
    ...
    let packaged_defaults_layer = if let Some(file) = &overrides.packaged_defaults_path {
        ...
        let config = toml::from_str(raw_toml).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("invalid embedded packaged defaults; this is a CX build error: {error}"),
            )
        })?;
        ConfigLayerEntry::new_with_raw_toml(
            ...
            config,
            raw_toml.to_owned(),
            AbsolutePathBuf::from_absolute_path(cx_home)?,
        )
    };
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(cx_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(cx_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, cx_home, overrides.clone(), strict_config)
            .await?;
    ...
    if overrides.ignore_login_requirements {
        ...
        config_requirements_toml.allowed_gt_workspaces = None;
    }
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(cx_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, cx_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(cx_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/cx/requirements.toml"))
}
...
pub fn has_local_managed_configuration(cx_home: &Path) -> io::Result<bool> {
...
fn has_local_managed_configuration_with_system_requirements_path(
    ...
    #[cfg(not(windows))]
    if layer_io::managed_config_default_path(cx_home).try_exists()? {
        return Ok(true);
    }
...
#[cfg(windows)]
fn windows_cx_system_dir() -> PathBuf {
...
fn requirements_layers_from_legacy_scheme(
    ...
    let cx_home = AbsolutePathBuf::from_absolute_path(cx_home)?;
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for CX to function correctly.
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_cx_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    for layer in discovered.layers {
        let config =
            resolve_relative_paths_in_config_toml(layer.config, layer.dot_cx_folder.as_path())?;
...
async fn discover_project_layers(
    ...
    let cx_home_abs = AbsolutePathBuf::from_absolute_path(cx_home)?;
    let cx_home_normalized =
        normalize_path(cx_home_abs.as_path()).unwrap_or_else(|_| cx_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_cx_abs = dir.join(".cx");
        let dot_cx_uri = PathUri::from_abs_path(&dot_cx_abs);
        if !fs
            .get_metadata(&dot_cx_uri, Default::default(), /*sandbox*/ None)
            .await
        ...
        let dot_cx_normalized =
            normalize_path(dot_cx_abs.as_path()).unwrap_or_else(|_| dot_cx_abs.to_path_buf());
        if dot_cx_abs == cx_home_abs || dot_cx_normalized == cx_home_normalized {
            continue;
        }
        let config_file = dot_cx_abs.join(CONFIG_TOML_FILE);
...
/// from a file like `/etc/cx/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("oi").join("CX").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("oi").join("CX").join("config.toml"))
        );
    }
}
