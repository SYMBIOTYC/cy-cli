//! Schema-heavy configuration TOML types used by CX.
...
/// Backward-compatible shape for gt workspace login restrictions in config.toml.
        ...
        D: Deserializer<'de>,
    {
        ...
        match Repr::deserialize(deserializer)? {
            Repr::Single(value) if value.contains(',') => Err(D::Error::custom(
                "forced_gt_workspace_id must be a single workspace ID string or a TOML list \
of strings; comma-separated strings are not supported. Use \
`forced_gt_workspace_id = [\"123e4567-e89b-42d3-a456-426614174000\", \
...
/// Base config deserialized from ~/.cx/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by CX will likely degrade model performance.
    ...
    #[serde(default)]
    pub forced_gt_workspace_id: Option<ForcedChatgptWorkspaceIds>,
    ...
    /// file: Use a file in the CX home directory.
    ...
    /// Optional product SKU forwarded on host-owned CX Apps MCP requests.
    ...
    /// directories for `.cx` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use cx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use cx_protocol::protocol::RealtimeVoice;
...
mod tests {
    ...
    #[test]
    fn forced_gt_workspace_id_accepts_string_list() {
        let config: ConfigToml = toml::from_str(&format!(
            r#"forced_gt_workspace_id = ["{WORKSPACE_ID_A}", "{WORKSPACE_ID_B}"]"#
        ))
    ...
    #[test]
    fn forced_gt_workspace_id_rejects_comma_separated_string() {
