...
use cx_execpolicy::RuleRef;
use cx_execpolicy::rule::PatternToken;
use cx_execpolicy::rule::PrefixPattern;
use cx_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: CX merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `cx-execpolicy`.
