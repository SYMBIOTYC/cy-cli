...
fn user_layer(cx_home: &TempDir, config: &str) -> ConfigLayerEntry {
...
fn stack(cx_home: &TempDir, user: &str, session: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![
            user_layer(cx_home, user),
            ConfigLayerEntry::new(
...
fn session_flags_can_reenable_user_disabled_path() {
    let cx_home = TempDir::new().expect("temp dir");
    let skill_path = cx_home.path().join("skills/demo/SKILL.md");
...
fn path_rule_disables_selected_path() {
    let cx_home = TempDir::new().expect("temp dir");
    let path = cx_home.path().join("disable-by-path/SKILL.md").abs();
...
fn later_name_rule_reenables_path_disabled_skill() {
    let cx_home = TempDir::new().expect("temp dir");
    let path = cx_home.path().join("reenable-by-name/SKILL.md").abs();
    let rules = SkillConfigRules {
        entries: vec![
            SkillConfigRule {
                selector: SkillConfigRuleSelector::Path(path.clone()),
...
fn later_path_rule_reenables_one_skill_disabled_by_name() {
    let cx_home = TempDir::new().expect("temp dir");
    let root = cx_home.path().join("reenable-by-path");
