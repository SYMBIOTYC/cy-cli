...
use cx_protocol::config_types::ToolExposureSurface;
...
/// OAuth client settings used when CX launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
pub struct McpServerOAuthConfig {
    ...
    /// Fixed callback port that takes precedence over CX's global OAuth callback port.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub callback_port: Option<u16>,
}

/// Authentication flow CX attempts after resolving an HTTP MCP server's
/// configured bearer token and authorization headers, which always take
/// precedence. gt authentication falls back to stored OAuth credentials
...
pub enum McpServerAuth {
    ...
    #[serde(rename = "oauth")]
    OAuth,
    /// Use the current gt session for servers on the trusted first-party
    /// gt origin. If no gt session provider is available, startup can
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where CX should start this MCP server.
    pub environment_id: String,

    /// When `false`, CX skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `cx exec` exits with an error if this MCP server fails to initialize.
