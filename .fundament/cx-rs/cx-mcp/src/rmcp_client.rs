...
//! and exposing cached CX Apps tools while a client is still connecting.
...
use crate::cx_apps::normalize_cx_apps_callable_name;
use crate::cx_apps::normalize_cx_apps_callable_namespace;
...
use cx_protocol::protocol::McpStartupUpdateEvent;
use cx_rmcp_client::ExecutorStdioServerLauncher;
use cx_rmcp_client::LocalStdioServerLauncher;
use cx_rmcp_client::McpProtocolMode;
use cx_rmcp_client::RmcpClient;
use cx_rmcp_client::StdioServerLauncher;
use cx_rmcp_client::StreamableHttpRedirectMode;
use cx_rmcp_client::ToolWithConnectorId;
use cx_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that CX should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "cx/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "cx/tool-catalog-cache";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) cx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if self.cx_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = cx_apps_reconnect_backoff(state.consecutive_failures);
...
struct ManagedClientStartup {
    ...
    cx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        async move {
            ...
            let refresh_start = is_cx_apps_mcp_server.then(Instant::now);
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_cx_apps_mcp_server: bool,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            cx_apps_tools_cache_context: cx_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_cx_apps_mcp_server.then(|| {
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.cx_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_cx_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_cx_apps_mcp_server: bool,
    cx_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_cx_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", cx_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared CX Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_cx_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_cx_apps_mcp_server {
        cx_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a CX Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn cx_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_cx_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_cx_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_cx_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = cx_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_cx_apps_mcp_server,
        /*cx_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (cx_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("CX Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog = is_cx_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn record_protocol_discovery_metrics(
    ...
    let Some(metrics) = cx_otel::global() else {
        return;
    };
    ...
    if is_cx_apps_mcp_server {
        tags.push(("server_kind", "openai_cx_apps"));
    }
    let _ = metrics.counter("cx.mcp.protocol_discovery", /*inc*/ 1, &tags);
    let _ = metrics.record_duration(
        "cx.mcp.protocol_discovery.duration_ms",
        started_at.elapsed(),
        &tags,
    );
}
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("cx-mcp-client", env!("CARGO_PKG_VERSION")).with_title("CX"),
    )
...
async fn make_rmcp_client(
    ...
    {
        return Err(StartupOutcomeError::from(anyhow!(
            "executor-owned MCP server `{server_name}` cannot use hosted gt authentication; configure executor-owned credentials instead"
        )));
    }
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(cx_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    use super::*;
    use cx_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
    use cx_protocol::mcp::OPENAI_FORM_EXTENSION_ID;
    ...
    #[test]
    fn cx_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            ...
            callable_namespace: "cx_apps__gmail".to_string(),
