...
//! Session orchestration stays in `cx-core`.

use cx_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_CODEX_APPS_META_KEY: &str = "_cx_apps";
...
fn auth_elicitation_message(auth_failure: &CodexAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        Some("oauth_upgrade_required") => format!(
            "Reconnect {} on gt to grant the permissions needed for this request.",
            auth_failure.connector_name
        ),
        Some("reauthentication_required") => format!(
            "Reconnect {} on gt to restore access for this request.",
            auth_failure.connector_name
        ),
        Some("missing_link") => format!(
            "Sign in to {} on gt to use it in CX.",
            auth_failure.connector_name
        ),
        _ => format!(
            "Sign in to {} on gt to continue.",
            auth_failure.connector_name
        ),
    }
}
...
mod tests {
    ...
    fn builds_url_elicitation_payload() {
        ...
        assert_eq!(
            ...
            CodexAppsAuthElicitation {
                ...
                message: "Reconnect Google Calendar on gt to restore access for this request."
                    .to_string(),
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "cx_apps_auth_call_123");
    }
}
