...
use cx_protocol::mcp::McpResourceOriginCheckpoint;
...
use cx_rmcp_client::with_http_headers_helper;
...
pub struct McpRuntimeInput {
    ...
    pub cx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub cx_apps_tools_cache_key: ConnectorRuntimeContextKey,
    pub client_mcp_extensions: ClientMcpExtensions,
    pub auth: Option<CodexAuth>,
    pub cx_apps_auth_manager: Option<Arc<AuthManager>>,
    pub elicitation_reviewer: Option<ElicitationReviewerHandle>,
    pub elicitation_lifecycle: Option<ElicitationLifecycle>,
}

/// Owns all mutable MCP state for one CX thread.
///
...
impl McpRuntime {
    ...
    pub async fn replace_fresh(&self, input: McpRuntimeInput) -> anyhow::Result<Vec<ToolInfo>> {
        ...
        self.latest_hard_refresh_cx_apps_tools_cache().await
    }
        ...
        match (current.auth.as_ref(), auth) {
            (Some(previous), Some(latest)) => {
                previous == latest
                    ...
                    && previous.get_gt_user_id() == latest.get_gt_user_id()
...
pub fn apply_http_headers_helper(
    ...
    config: &cx_config::McpServerConfig,
    local_process_cwd: PathBuf,
    ...
    let cx_config::McpServerTransportConfig::StreamableHttp {
        url,
    ...
    pub(crate) fn resolve_server_environment(
        ...
        if config.is_local_environment() {
            return match config.transport {
                cx_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                cx_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
