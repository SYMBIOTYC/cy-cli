...
impl McpConnectionSet {
    pub(crate) async fn stable_catalog_revision(&self) -> Option<u64> {
        for (server_name, view) in &self.servers {
            ...
            let Some(client) = view.connection.client.ready_transport() else {
                if !view.connection.client.is_cx_apps_mcp_server
            ...
            let server_tools = async {
                match catalog_override {
                    Some(tools) => {
                        ...
                        Some(prepare_cx_apps_tools_for_model(
                            tools,
    ...
    /// Force-refresh CX Apps tools and publish one new exact catalog revision.
    #[expect(
        clippy::await_holding_invalid_type,
        reason = "catalog publication must remain serialized with captured tool calls"
    )]
    pub async fn hard_refresh_cx_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let _refresh = self.cx_apps_refresh_lock.lock().await;
        ...
        let tools = match (
            managed_client.cx_apps_tools_cache_context.as_ref(),
            fetch_ticket,
        ) {
        ...
        *self.cx_apps_tools_override.write().await = Some(client_tools);
        *tool_catalog_revision += 1;
        ...
        let tools = prepare_cx_apps_tools_for_model(
