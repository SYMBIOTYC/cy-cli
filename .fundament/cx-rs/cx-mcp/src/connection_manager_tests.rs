...
use crate::rmcp_client::CX_APPS_RECONNECT_INITIAL_BACKOFF;
...
use cx_config::McpServerAuth;
use cx_config::McpServerConfig;
use cx_config::McpServerEnvVar;
use cx_config::McpServerToolConfig;
use cx_config::types::AuthKeyringBackendKind;
use cx_config::types::OAuthCredentialsStoreMode;
use cx_connectors::ConnectorRuntimeContext;
use cx_connectors::ConnectorRuntimeContextKey;
use cx_connectors::ConnectorRuntimeFetchSource;
use cx_connectors::ConnectorRuntimeManager;
use cx_exec_server_test_support::environment_manager_without_environments;
use cx_login::AuthHeaders;
use cx_login::CodexAuth;
use cx_protocol::ToolName;
use cx_protocol::approvals::ElicitationRequest;
use cx_protocol::mcp::ClientMcpExtensions;
use cx_protocol::mcp::McpServerInfo;
use cx_protocol::models::PermissionProfile;
use cx_protocol::protocol::GranularApprovalConfig;
use cx_protocol::protocol::McpStartupFailureReason;
use cx_rmcp_client::ElicitationResponse;
use cx_rmcp_client::InProcessTransportFactory;
use cx_rmcp_client::McpAuthState;
use cx_rmcp_client::McpLoginRequirement;
use cx_rmcp_client::RmcpClient;
...
impl McpConnectionSet {
    fn new_uninitialized(
        ...
        Self {
            ...
            cx_apps_tools_override: RwLock::new(None),
            cx_apps_refresh_lock: Mutex::new(()),
...
fn create_cx_apps_tools_cache_context(
    cx_home: PathBuf,
    account_id: Option<&str>,
    gt_user_id: Option<&str>,
    ...
    ConnectorRuntimeManager::<ToolInfo>::default().context(
        ...
        ConnectorRuntimeContextKey::personal(
            ...
            gt_user_id.map(ToOwned::to_owned),
        ),
    )
}
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("cx-test", "0.0.0-test"),
            )
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        cx_apps_tools_cache_context: None,
    }
}
...
async fn create_test_manager_with_ready_apps_client(
    ...
    let tool = create_test_tool(CX_APPS_MCP_SERVER_NAME, tool_name);
    let client = Arc::new(
        RmcpClient::new_in_process_client(Arc::new(RefreshTestTransportFactory {
    ...
    let managed_client = ManagedClient {
        client,
        server_info: create_test_server_info("CX Apps"),
        tools: vec![tool],
        tool_timeout: Some(Duration::from_secs(5)),
        server_instructions: None,
        server_supports_sandbox_state_meta_capability: false,
        cx_apps_tools_cache_context: Some(cache_context.clone()),
    };
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            cached_server_info: Some(create_test_server_info("CX Apps")),
            cx_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
            cancel_token: CancellationToken::new(),
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        cx_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn assert_elicitation_declined_with_reviewer_calls(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            cx_rmcp_client::Elicitation::Mcp(elicitation),
...
async fn assert_requested_user_input_is_declined(
    ...
    let response = tokio::select! {
        ...
        response = sender(
            NumberOrString::Number(1),
            cx_rmcp_client::Elicitation::Mcp(
    ...
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "cx_approval_kind": "tool_suggestion",
            })),
...
#[tokio::test]
async fn disabled_permissions_surface_requested_user_input_with_non_cx_approval_metadata() {
...
async fn assert_disabled_permissions_surface_requested_user_input(
    ...
    let cx_protocol::mcp::RequestId::String(request_id) = request.id else {
        panic!("expected CX-owned string request ID");
    };
...
async fn disabled_permissions_decline_requested_user_input_with_approval_metadata() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "cx_approval_kind": "mcp_tool_call",
...
async fn on_request_approval_forms_remain_with_the_reviewer() {
    assert_elicitation_declined_with_reviewer_calls(
        ...
        ElicitRequestParams::FormElicitationParams {
            meta: elicitation_meta(serde_json::json!({
                "cx_request_type": "approval_request",
                "cx_approval_kind": "mcp_tool_call",
                "tool_name": "test_tool",
            })),
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        cx_protocol::mcp::RequestId::String(request_a_id),
        cx_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected CX-owned string request IDs");
    };
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        cx_protocol::mcp::RequestId::String(cancelled_id),
        cx_protocol::mcp::RequestId::String(pending_id),
...
fn test_normalize_tools_selects_raw_server_name() {
    let mut tool = create_test_tool("cx_apps", "search");
    tool.callable_namespace = "cx_apps__calendar".to_string();
    ...
    assert_eq!(
        model_tool_names(&model_tools),
        HashSet::from([ToolName::namespaced("cx_apps__calendar", "search")])
    );
}
...
#[test]
fn cx_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_cx_apps_tools_cache(
        CX_APPS_MCP_SERVER_NAME,
...
async fn cx_apps_extension_does_not_share_host_owned_tools_cache() -> anyhow::Result<()> {
    ...
    let mut config = crate::mcp::tests::test_mcp_config(cx_home.path().to_path_buf());
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            cx_apps_tools_cache_key: cache_key,
            client_mcp_extensions: ClientMcpExtensions::default(),
            auth: None,
            cx_apps_auth_manager: None,
    ...
    let client = manager.test_client(CX_APPS_MCP_SERVER_NAME);
    assert!(
        client.cx_apps_tools_cache_context.is_none(),
...
#[tokio::test]
async fn list_all_tools_uses_shared_cx_apps_cache_while_client_is_pending() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__cx_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, CX_APPS_MCP_SERVER_NAME);
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    let mut ready_client = create_test_managed_client(vec![
        create_test_tool(CX_APPS_MCP_SERVER_NAME, "client_local_tool"),
        create_test_tool(CX_APPS_MCP_SERVER_NAME, "client_local_blocked"),
    ])
    .await;
    ...
    ready_client.cx_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
        &approval_policy,
    ...
    manager
        .servers
        .get_mut(CX_APPS_MCP_SERVER_NAME)
        .expect("test server exists")
    ...
    assert!(
        step.prepare_call(CX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        step.prepare_call(CX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    assert!(
        step.prepare_call(CX_APPS_MCP_SERVER_NAME, "client_local_blocked")
            .is_none()
    );
}
...
-> anyhow::Result<()> {
    ...
    let cache_context_a = shared_cache.context(cx_home.path().to_path_buf(), cache_key.clone());
    let cache_context_b = shared_cache.context(cx_home.path().to_path_buf(), cache_key);
    ...
    let tools_b = manager_b.hard_refresh_cx_apps_tools_cache().await?;
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<cx_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(CX_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn capture_binding_exposes_cached_tools_before_startup() {
    ...
    let mut cached_tool = create_test_tool(CX_APPS_MCP_SERVER_NAME, "shared_cached_tool");
    ...
    assert!(
        cached_binding
            .prepare_call(CX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    ...
    let startup = tokio::spawn(async move {
        manager_for_startup
            .wait_for_server_startup(CX_APPS_MCP_SERVER_NAME)
            .await
    });
...
async fn capture_binding_resolves_concurrently_and_rechecks_cached_clients() {
    ...
    apps_client.is_cx_apps_mcp_server = true;
    apps_client.cx_apps_tools_cache_context = Some(cache_context);
    let first_client =
    ...
    manager.insert_test_client(CX_APPS_MCP_SERVER_NAME, apps_client);
    ...
    tokio::time::timeout(Duration::from_secs(1), apps_started)
        .await
        .expect("CX Apps startup should begin")
        .expect("signal CX Apps startup");
    ...
    release_apps.send(()).expect("release CX Apps startup");
    assert!(startup.await.expect("CX Apps startup task"));
    ...
    assert!(
        binding
            .prepare_call(CX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        binding
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_cx_apps_cache_is_empty() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_cx_apps_cache_when_client_startup_fails() {
    let cx_home = tempdir().expect("tempdir");
    let cache_context = create_cx_apps_tools_cache_context(
        cx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_cx_apps_startup_and_reuses_client() {
    ...
    let step = capture_binding(&manager).await;
    let prepared = step
        .prepare_call(CX_APPS_MCP_SERVER_NAME, "drive_search")
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(CX_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_cx_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        CX_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::cx_apps_mcp_server_config(
        "https://docs.example",
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    let bootstrap_auth = CodexAuth::create_dummy_gt_auth_for_testing();
    let mut actor_headers = Default::default();
    cx_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let provider = cx_model_provider::auth_provider_from_auth(&hosted_auth);
    let mut local_config = crate::cx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    ...
    assert!(
        gt_auth_provider_for_server(&remote_server, Some(provider)).is_none(),
...
async fn executor_owned_gt_mcp_accepts_only_safe_explicit_authorization() -> anyhow::Result<()>
{
    ...
    let hosted_auth = CodexAuth::Headers(AuthHeaders::new(actor_headers));
    let runtime_config = crate::mcp::tests::test_mcp_config(cx_home.path().to_path_buf());
    let cases = [
        ...
        (
            ...
            None,
            Some(("aUtHoRiZaTiOn", "CX_TEST_HOSTED_SECRET")),
            false,
        ),
        (
            "environment bearer",
            Some(("Authorization", "Bearer executor-secret")),
            Some("CX_TEST_HOSTED_SECRET"),
            None,
            false,
        ),
        (
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        let mut server_json = serde_json::json!({
            "url": "https://cy.symbiotyc.workers.dev/v1/ps/mcp",
            "auth": "gt",
        ...
        assert!(
            gt_auth_provider_for_server(
                remote_server,
                Some(cx_model_provider::auth_provider_from_auth(&hosted_auth)),
            )
            .is_none(),
        ...
        assert!(
            direct_keyring_identity
                .oauth_credentials()
                .expect("executor-owned gt authentication must skip OAuth lookup")
                .is_none(),
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                cx_apps_tools_cache: ConnectorRuntimeManager::default(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                cx_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                    /*account_id*/ None, /*gt_user_id*/ None,
                ),
                client_mcp_extensions: ClientMcpExtensions::default(),
        ...
        if allows_executor_auth {
            ...
            assert_eq!(
                error,
                "executor-owned MCP server `fake-first-party` cannot use hosted gt authentication; configure executor-owned credentials instead",
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            cx_apps_tools_cache: ConnectorRuntimeManager::<ToolInfo>::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            cx_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                /*account_id*/ None, /*gt_user_id*/ None,
            ),
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("CX_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
            http_headers_helper: None,
        },
...
fn connection_identity_uses_effective_authorization_headers() {
    let runtime_context = reusable_server_runtime_context();
    let missing_env_var = format!("CX_TEST_UNSET_MCP_AUTHORIZATION_{}", std::process::id());
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    .await;
    let cx_home = tempdir().expect("tempdir");
    let mut mcp_config = crate::mcp::tests::test_mcp_config(cx_home.path().to_path_buf());
    mcp_config.protocol_mode = cx_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = cx_login::auth::AgentIdentityAuthRecord {
        ...
        gt_user_id: "user-a".to_string(),
        email: Some("agent@example.com".to_string()),
        plan_type: cx_protocol::account::PlanType::Plus,
        gt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = cx_login::test_support::transport_default_auth_route_config();
    ...
    for changed_record in [
        cx_login::auth::AgentIdentityAuthRecord {
            account_id: "account-b".to_string(),
            ..record.clone()
        },
        cx_login::auth::AgentIdentityAuthRecord {
            gt_user_id: "user-b".to_string(),
            ..record.clone()
        },
        cx_login::auth::AgentIdentityAuthRecord {
