//! Aggregates MCP server connections for CX.
//!
...
use crate::rmcp_client::prepare_cx_apps_tools_for_model;
...
use cx_protocol::protocol::McpStartupCompleteEvent;
use cx_protocol::protocol::McpStartupFailure;
...
use cx_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Some(if self.connection.client.is_cx_apps_mcp_server {
            prepare_cx_apps_tools_for_model(tools, tool_plugin_provenance)
        } else {
...
pub(crate) struct McpConnectionSet {
    ...
    cx_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    cx_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_gt_auth_provider = auth
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_cx_apps_tools_cache
                    .then(|| (cx_home.clone(), cx_apps_tools_cache_key.clone())),
    ...
    /// CX Apps metadata may come from its existing cache; regular MCP server information is
