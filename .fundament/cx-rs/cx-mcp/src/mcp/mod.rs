...
/// MCP runtime settings derived from `cx_core::config::Config`.
///
...
pub struct McpConfig {
    /// Base URL for gt-hosted app MCP servers, copied from the root config.
    pub gt_base_url: String,
    /// Optional product SKU forwarded to the host-owned apps MCP server.
    pub apps_mcp_product_sku: Option<String>,
    /// CX home directory used for MCP OAuth state and app-tool cache files.
    pub cx_home: PathBuf,
    ...
    /// Optional path to `cx-linux-sandbox` for sandboxed MCP tool execution.
    ...
    ///
    /// gt auth is checked separately before a materialized host-owned Apps
    /// server can be used.
...
pub fn host_owned_cx_apps_enabled(config: &McpConfig, auth: Option<&CodexAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(CodexAuth::uses_cx_backend)
}
...
fn is_trusted_gt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_CODEX_BASE_URL)
        .ok()
        .is_some_and(|gt_url| server_url.origin() == gt_url.origin())
    {
        return true;
    }

    url::Url::parse(gt_base_url)
        .ok()
        .is_some_and(|staging_url| {
            staging_url.scheme() == "https"
                && staging_url.domain().is_some_and(|host| {
                    host == "gt-staging.com" || host.ends_with(".gt-staging.com")
                })
        ...
        .map(|(name, mut server)| {
            match server.auth.clone() {
                McpServerAuth::ChatGpt => {
                    if !is_trusted_gt_mcp_server(&server.transport, &config.gt_base_url) {
...
pub async fn read_mcp_resource(
    ...
    cx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            cx_apps_tools_cache,
            tool_catalog_cache,
            cx_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn cx_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_cx_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/cx") {
        base_url
    } else {
        format!("{base_url}/api/cx")
    };
    format!("{base_url}/ps/mcp")
}

pub fn cx_apps_mcp_server_config(
    gt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        cx_apps_mcp_url_for_base_url(gt_base_url),
...
/// Builds the gt-hosted plugin runtime served by plugin-service.
pub fn hosted_plugin_runtime_mcp_server_config(
    gt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    cx_apps_mcp_server_config(gt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let mut http_headers =
        HashMap::from([("X-oi-Product-Sku".to_string(), product_sku.to_string())]);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: cx_apps_mcp_bearer_token_env_var(),
