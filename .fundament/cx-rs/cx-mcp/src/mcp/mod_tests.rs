...
pub(crate) fn test_mcp_config(cx_home: PathBuf) -> McpConfig {
    McpConfig {
        gt_base_url: "https://chatgpt.com".to_string(),
        ...
        config_layer_stack: cx_config::ConfigLayerStack::default(),
        approvals_reviewer: cx_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: cx_connectors::ConnectorSnapshot::default(),
    }
}
...
fn cx_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    ...
    assert_eq!(
        ...
        "http://localhost:8080/api/cx/ps/mcp"
    );
    assert_eq!(
        cx_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/cx/ps/mcp"
    );
}

#[test]
fn cx_apps_server_config_uses_plugin_service_path() {
    let config = cx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for cx apps"),
    };
...
#[test]
fn cx_apps_server_config_forwards_thread_originator_header() {
    let config = cx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-oi-Product-Sku".to_string(), "cx".to_string()),
                ]))
            );
...
#[test]
fn cx_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "cx"), (Some("tpp"), "tpp")] {
        let config = cx_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn cx_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = cx_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
    ...
    match &config.transport {
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ("originator".to_string(), "thread_originator".to_string()),
                    ("X-oi-Product-Sku".to_string(), "tpp".to_string()),
                ]))
            );
            assert!(env_http_headers.is_none());
        }
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(cx_home.path().to_path_buf());
