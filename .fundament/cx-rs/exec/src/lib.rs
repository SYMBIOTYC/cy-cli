...
use cx_app_server_client::EnvironmentManager;
use cx_app_server_client::ExecServerRuntimePaths;
use cx_app_server_client::InProcessAppServerClient;
use cx_app_server_client::InProcessClientStartArgs;
use cx_app_server_client::InProcessServerEvent;
...
use cx_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use cx_core::StateDbHandle;
use cx_core::check_execpolicy_for_warnings;
use cx_core::config::Config;
use cx_core::config::ConfigBuilder;
use cx_core::config::ConfigOverrides;
use cx_core::config::ConfigTomlLoadResult;
...
use cx_core::read_session_meta_line;
...
enum StdinPromptBehavior {
    ...
    /// `cx exec` behavior for `cx exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `cx exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<cx_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("cx_exec".to_string()) {
        tracing::warn!(?err, "Failed to set cx exec originator override {err:?}");
    }
    ...
    #[allow(clippy::print_stderr)]
    let cx_home = match find_cx_home() {
        Ok(cx_home) => cx_home,
        Err(err) => {
            eprintln!("Error finding cx home: {err}");
            std::process::exit(1);
        }
    };
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&cx_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        cx_core::otel_init::build_provider(
            &config,
    ...
    cx_core::otel_init::record_process_start(otel.as_ref(), "cx_exec");
    cx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "cx_exec");
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.cx_self_exe.clone(),
        arg0_paths.cx_linux_sandbox_exe.clone(),
    )?;
    let state_db = cx_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_cx_home(
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = cx_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what CX
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            cx_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("CX initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        cx_app_server_protocol::TurnStatus::Failed
                            | cx_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<cx_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<cx_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != cx_app_server_protocol::TurnItemsView::Full
}
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.cx_home, session_id, Some(state_db.as_ref()))
                .await?
    ...
    let handle_result = match request {
        ...
        ServerRequest::ChatgptAuthTokensRefresh { request_id, .. } => {
            reject_server_request(
                ...
                request_id,
                &method,
                "gt auth token refresh is not supported in exec mode".to_string(),
            )
            .await
        }
