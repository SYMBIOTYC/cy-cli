...
#[command(
    version,
    override_usage = "cx exec [OPTIONS] [PROMPT]\n       cx exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running CX outside a Git repository.
    ...
    /// Do not load `$CX_HOME/config.toml`; auth still uses `CX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "cx resume --last <prompt>" treats the positional as a prompt, not a session id.
