...
/// While we may add an `apply-patch` subcommand to the `cx` CLI multitool
/// at some point, we must ensure that the smaller `cx-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(cx_utils_cargo_bin::cargo_bin("cx-exec")?)
...
async fn shutdown_flushes_completed_turn_and_file_diff() -> anyhow::Result<()> {
    ...
    test.cmd_with_server(&server)
        ...
        .env_remove("CX_ANALYTICS_EVENTS_CAPTURE_FILE")
        .arg("--skip-git-repo-check")
        .arg("-s")
        .arg("danger-full-access")
        .arg("-c")
        ...
        .arg("-c")
        .arg(format!("gt_base_url={}", server.uri()))
        .arg("write the file")
        .assert()
        .success();
    ...
    assert!(
        event_types.iter().any(|event| event == "cx_turn_event"),
