...
pub(super) async fn spawn_command_under_sandbox(
    ...
    use cx_core::exec::build_exec_request;
    ...
    let cx_linux_sandbox_exe = Some(
        core_test_support::find_cx_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_cx_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .cx && echo 'sandbox_mode = \"danger-full-access\"' > .cx/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .cx");

    let status = child.wait().await.expect("should wait for .cx command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .cx creation: {status:?}"
    );
    let dot_cx_metadata = tokio::fs::symlink_metadata(&dot_codex).await;
    if let Ok(metadata) = dot_cx_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_codex.display()
        );
    } else if let Err(err) = &dot_cx_metadata {
        assert_eq!(
            err.kind(),
