...
use cx_otel::OtelExporter;
use cx_otel::OtelHttpProtocol;
use cx_otel::OtelProvider;
use cx_otel::OtelSettings;
use cx_otel::Result;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("cx.turns", /*inc*/ 1, &[("source", "test")])?;
    metrics.counter("cx.api_request", /*inc*/ 1, &[("status", "200")])?;
    metrics.record_duration(
        "cx.api_request.duration_ms",
        Duration::from_millis(100),
        &[("status", "200")],
    )?;
    metrics.counter("cx.conversation.turn.count", /*inc*/ 1, &[])?;
    metrics.record_duration(
    ...
    metrics.counter("cx.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "cx.tool.call.duration_ms",
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let otel = OtelProvider::try_new(&OtelSettings {
        ...
        service_name: "cx-cli".to_string(),
        service_version: env!("CARGO_PKG_VERSION").to_string(),
        cx_home: PathBuf::from("."),
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            target: "cx_otel.log_only",
            tracing::Level::INFO,
            event.name = "cx.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("cx.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            target: "cx_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "cx.test.trace_event",
    ...
    assert!(
        body.contains("cx.test.trace_event"),
