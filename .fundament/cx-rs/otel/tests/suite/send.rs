...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "cx-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "cx.turns",
        "Total number of CX turns.",
        /*inc*/ 1,
    ...
    let counter = find_metric(&resource_metrics, "cx.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of CX turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "cx-cli".to_string()),
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "cx.tool_latency")
            ...
            .expect("cx.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "cx.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active CX operations.");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("cx.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
