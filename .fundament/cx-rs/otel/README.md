...
`cx-otel` is the OpenTelemetry integration crate for CX. It provides:

- Provider wiring for log/trace/metric exporters (`cx_otel::OtelProvider`
  and `cx_otel::provider`).
- Session-scoped business event emission via `cx_otel::SessionTelemetry`.
- Low-level metrics APIs via `cx_otel::metrics`.
- Trace-context helpers via `cx_otel::trace_context` and crate-root re-exports.
...
```rust
use cx_otel::config::OtelExporter;
use cx_otel::config::OtelHttpProtocol;
use cx_otel::config::OtelSettings;
...
let settings = OtelSettings {
    ...
    cx_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, CX
...
CX-specific session events. Rich session/business events should go through
...
`cx-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using CX-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use cx_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("cx.session_started", 1, &[("source", "tui")])?;
metrics.histogram("cx.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("cx.turns", 1, &[("model", "gpt-5.1")])?;
