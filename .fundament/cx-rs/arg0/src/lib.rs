...
use cx_windows_sandbox::CODEX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "cx-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current CX executable for child re-execs.
    ///
    ...
    /// instead of the real CX CLI.
    pub cx_self_exe: Option<PathBuf>,
    pub cx_linux_sandbox_exe: Option<PathBuf>,
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(cx_shell_escalation::run_shell_escalation_execve_wrapper(
            file, argv,
        ));
        match exit_code {
    ...
    if argv1 == CODEX_ARG0_EXEC_HELPER_ARG1 {
        cx_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == CODEX_FS_HELPER_ARG1 {
        cx_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == CODEX_WINDOWS_SANDBOX_ARG1 {
        cx_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                let cwd = match cx_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                let update_file_mode = cx_apply_patch::apply_patch_file_update_mode_from_env();
                match runtime.block_on(cx_apply_patch::apply_patch_with_options(
                    &patch_arg,
                    cx_apply_patch::ApplyPatchOptions {
                        update_file_mode,
                        ..Default::default()
                    },
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    cx_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_cx_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that CX will proceed successfully even if
...
/// While we want to deploy the CX CLI as a single executable for simplicity,
...
/// `cx-linux-sandbox` we *directly* execute
/// [`cx_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.cx/.env` before creating any threads.
...
///     `cx-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        cx_self_exe: current_exe.clone(),
        cx_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `cx-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().cx_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
/// Load env vars from ~/.cx/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `CODEX_`.
fn load_dotenv() {
    if let Ok(cx_home) = find_cx_home()
        && let Ok(iter) = dotenvy::from_path_iter(cx_home.join(".env"))
    {
        set_filtered(iter);
    }
}
...
///   with the hidden `--cx-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of CX
/// CLI.
...
fn prepare_path_entry_for_cx_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if cx_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (cx_home: {cx_home:?})"
                ),
            ));
        }
    }
...
"#,
    ...
    let paths = Arg0DispatchPaths {
        cx_self_exe: std::env::current_exe().ok(),
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("cx-path");
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_cx_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("cx-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                cx_self_exe: Some(PathBuf::from("/usr/bin/cx")),
                cx_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/cx"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("cx-helper-alias");
