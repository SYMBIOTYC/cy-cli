...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, cx_home)
        .import_plugins(
            /*cwd*/ None,
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".cx-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".cx-plugin"))
...
"#
    ...
    fs::write(
        source_plugin_root.join(".cx-plugin/plugin.json"),
...
async fn detect_home_supports_relative_external_agent_plugin_marketplace_path() {
    ...
    let items = service_for_paths(external_agent_home.clone(), cx_home)
...
async fn import_plugins_rejects_project_cwd_before_config_loading() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(&cx_home).expect("create cx home");
    fs::write(cx_home.join("config.toml"), "not valid toml").expect("write invalid config");

    let error = service_for_paths(external_agent_home, cx_home)
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, cx_home)
...
fn import_cursor_skills_reads_user_and_managed_directories() {
    ...
    let mut service = service_for_paths(external_agent_home, cx_home);
