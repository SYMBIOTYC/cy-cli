...
#[tokio::test]
async fn authenticated_plugin_migration_uses_gt_curated_marketplace() {
    ...
    fs::create_dir_all(plugin_root.join(".cx-plugin")).expect("create curated plugin directory");
    fs::write(
  ...
  "plugins": [{
    ...
    let mut service = service_for_paths(external_agent_home.clone(), cx_home);
...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _cx_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _cx_home) = fixture_paths();
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, cx_home)
...
async fn detect_repo_skips_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".cx")).expect("create repo cx dir");
    fs::create_dir_all(&cx_home).expect("create cx home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project cx config");
...
async fn detect_repo_skips_plugins_even_with_installed_marketplace() {
    ...
    let cx_home = root.path().join(".cx");
    let repo_root = root.path().join("repo");
    let marketplace_root = cx_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
