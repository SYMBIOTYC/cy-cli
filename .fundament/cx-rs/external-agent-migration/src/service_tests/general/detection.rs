...
async fn detect_home_lists_config_skills_and_agents_md() {
    ...
    let items = service_for_paths(external_agent_home.clone(), cx_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                cx_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                cx_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_cursor_repo_lists_skills_from_skills() {
    ...
    let mut service = service_for_paths(root.path().join(".cursor"), cx_home);
    service.source = ExternalAgentSource::Cur;
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, cx_home) = fixture_paths();
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::write(cx_home.join("config.toml"), "this is not valid = [toml")
        .expect("write invalid cx config");
    fs::write(
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), cx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
            cwds: Some(vec![repo_root.clone()]),
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".cx").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".cx").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        cx_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".cx").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: cx_config::HooksFile =
