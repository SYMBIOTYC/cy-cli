...
pub(crate) fn find_existing_session_import(
    ...
    let source_path = canonical_source_path(source_path)?;
    let ledger = load_import_ledger(cx_home)?;
...
pub(super) fn checkpoint_existing_session_import(
    ...
    let mut ledger = load_import_ledger(cx_home)?;
    ...
    save_import_ledger(cx_home, &ledger)?;
    Ok(true)
}
...
fn import_ledger_path(cx_home: &Path) -> PathBuf {
    cx_home.join(SESSION_IMPORT_LEDGER_FILE)
}
