...
pub fn prepare_validated_session_import_with_metadata_mode(
    ...
    pending.target = match ledger::find_existing_session_import(cx_home, &pending.source_path)? {
...
mod tests {
    ...
    fn prepares_changed_session_for_its_unique_import() {
        ...
        ledger::record_imported_session(&cx_home, &source_path, imported_thread_id)
            .expect("record import");
