...
async fn records_and_advances_checked_thread() {
    ...
    assert_eq!(
        cx_rollout::read_session_meta_line(&legacy_path)
            .await
...
async fn checks_rollouts_within_the_cursor_lookback() {
    ...
    assert_eq!(
        cx_rollout::read_session_meta_line(&older_path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
...
async fn recovers_pending_migrations_behind_the_checked_thread() {
    ...
    assert_eq!(
        cx_rollout::read_session_meta_line(&path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
...
async fn rechecks_changed_empty_rollouts() {
    ...
    let metadata = cx_rollout::builder_from_items(items.as_slice(), &empty_path)
    ...
    assert_eq!(
        cx_rollout::read_session_meta_line(&empty_path)
            .await
            .expect("read migrated metadata")
            .meta
            .history_mode,
