...
use cx_state::RolloutMigrationCursor;
use cx_state::RolloutMigrationSkippedRollout;
...
pub(super) async fn migrate_rollouts_on_startup(store: &LocalThreadStore) -> ThreadStoreResult<()> {
    ...
    if !pending_migration_thread_ids(&store.config.cx_home)
        .await?
        .is_empty()
    {
...
async fn inspect_rollout_path(
    ...
    match cx_rollout::read_session_meta_line(path).await {
...
async fn revalidate_skipped_rollouts(
    ...
    for skipped_rollout in skipped_rollouts {
        let path = store.config.cx_home.join(&skipped_rollout.rollout_path);
...
async fn find_all_rollout_paths(store: &LocalThreadStore) -> ThreadStoreResult<Vec<PathBuf>> {
    let mut paths =
        find_rollout_paths(&store.config.cx_home.join(cx_rollout::SESSIONS_SUBDIR)).await?;
    paths.extend(
        find_rollout_paths(
            &store
                .config
...
fn relative_rollout_path(store: &LocalThreadStore, path: &Path) -> String {
    path.strip_prefix(&store.config.cx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
