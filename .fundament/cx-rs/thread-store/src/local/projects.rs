...
fn state(store: &LocalThreadStore) -> ThreadStoreResult<&cx_rollout::StateDbHandle> {
    store
        .state_db
        .as_ref()
...
fn state_root(root: StoredProjectRoot) -> cx_state::ProjectRoot {
    cx_state::ProjectRoot { path: root.path }
}

fn stored_project(project: cx_state::Project) -> StoredProject {
    StoredProject {
        id: project.id,
        name: project.name,
        roots: project
