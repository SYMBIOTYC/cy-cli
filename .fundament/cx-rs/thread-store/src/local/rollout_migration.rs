...
use chrono::DateTime;
use cx_app_server_protocol::project_rollout_line;
...
impl LocalThreadStore {
    ...
    async fn migrate_rollouts_with_progress_inner(
        ...
        let _maintenance_guard = match options.mode {
            ...
            RolloutMigrationMode::Apply => Some(
                cx_rollout::try_acquire_rollout_maintenance_lock(&self.config.cx_home)
        ...
        paths.extend(
            find_rollout_paths(
                &self
                    .config
                    .cx_home
                    .join(cx_rollout::ARCHIVED_SESSIONS_SUBDIR),
            )
            .await?,
        );
        if options.mode == RolloutMigrationMode::Apply {
            let pending_thread_ids = pending_migration_thread_ids(&self.config.cx_home).await?;
        ...
        let legacy_names = if options.mode == RolloutMigrationMode::Apply {
            let thread_ids = paths
                ...
                .collect();
            cx_rollout::find_thread_names_by_ids(&self.config.cx_home, &thread_ids)
                .await
                .unwrap_or_default()
        } else {
            HashMap::new()
    ...
    async fn migrate_rollout_path(
        ...
        let metadata = match cx_rollout::read_session_meta_line(&path).await {
        ...
        let journal_path = migration_journal_path(&self.config.cx_home, thread_id);
    ...
    async fn migrate_one_rollout(
            ...
            || current_source_metadata.modified().ok() != source_modified
        {
            return Err(ThreadStoreError::Conflict {
                message: "rollout changed while migration was staging it; close older CX processes and retry".to_string(),
            });
        }
    ...
    async fn write_canonical_rollout(
        ...
        while let Some(record) = read_rollout_record(&mut source, &mut bytes).await? {
            ...
            if plan.is_none()
                && matches!(
                    &line.item,
                    RolloutItem::EventMsg(cx_protocol::protocol::EventMsg::ThreadRolledBack(_))
                )
            {
    ...
    async fn cleanup_failed_unpublished_migration(
        ...
        let Ok(metadata) = cx_rollout::read_session_meta_line(rollout_path).await else {
            return Ok(());
        };
        if metadata.meta.history_mode == ThreadHistoryMode::Paginated {
            return Ok(());
    ...
    async fn project_rollout_in_batches(
        ...
        let subagent_history_start_ordinal = cx_rollout::read_session_meta_line(rollout_path)
            .await
            .map_err(migration_error)?
            .meta
        ...
        while let Some(entry) = entries.next_entry().await.map_err(migration_error)? {
                ...
                && (name.ends_with(".jsonl") || name.ends_with(".jsonl.zst"))
            {
                ...
                if name.ends_with(".jsonl.zst")
                    && tokio::fs::try_exists(cx_rollout::plain_rollout_path(&path))
                        .await
                        .map_err(migration_error)?
                {
                    continue;
