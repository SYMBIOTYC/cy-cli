...
use chrono::DateTime;
use cx_app_server_protocol::ThreadHistoryChangeSet;
...
pub(super) async fn materialize_to_sqlite(
    ...
    let session_meta = cx_rollout::read_session_meta_line(rollout_path)
        .await
        .map_err(thread_store_io_error)?
        .meta;
...
async fn read_projection_steps(
    ...
    for line_bytes in bytes[..complete_byte_count].split_inclusive(|byte| *byte == b'\n') {
        ...
        let line = match cx_rollout::decode_rollout_line(value) {
            Ok(line) => Some(line),
            Err(err) => {
                warn!(
                    thread_id = %thread_id,
