...
pub(super) async fn revert(
    ...
    for segment in &mut lineage.segments {
        segment.rollout_path =
            cx_rollout::materialize_rollout_for_reference(segment.rollout_path.as_path())
                .await
                .map_err(|err| ThreadStoreError::Internal {
                    message: format!(
...
async fn create_replacement_recorder(
    store: &LocalThreadStore,
    source_meta: cx_rollout::SessionMeta,
    rollout_id: ThreadId,
    history_base: Option<cx_protocol::protocol::HistoryPosition>,
