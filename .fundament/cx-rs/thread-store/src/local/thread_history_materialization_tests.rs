...
use cx_rollout::RolloutRecorderParams;
...
/// Separate CX and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = cx_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from CX home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !cx_home.join(filename).exists(),
            "SQLite database should not be created under CX home"
        );
    }
}
...
async fn projection_store(cx_home: &Path) -> LocalThreadStore {
    let config = test_config(cx_home);
