...
impl LocalThreadStore {
    ...
    async fn resolve_rollout_lineage_with_representation(
        ...
        loop {
            ...
            let rollout_path = match representation {
                ...
                LineageRepresentation::PlainForReference => {
                    let rollout_path = super::helpers::scoped_rollout_path(
                        self.config.cx_home.clone(),
                        rollout_path.as_path(),
                        "CX home",
                    )?;
                    cx_rollout::materialize_rollout_for_reference(rollout_path.as_path())
                        .await
            ...
            let meta = cx_rollout::read_session_meta_line(rollout_path.as_path())
                .await
                .map_err(|err| ThreadStoreError::Internal {
                    message: format!(
...
async fn resolve_rollout_path_by_id(
    ...
    cx_rollout::find_rollout_path_by_rollout_id(store.config.cx_home.as_path(), rollout_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
