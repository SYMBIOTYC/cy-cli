...
use cx_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| cx_state::ThreadSection {
                name: if id == cx_state::PINNED_THREAD_SECTION_ID {
                    cx_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<cx_protocol::protocol::GitInfo> {
    ...
    Some(cx_protocol::protocol::GitInfo {
