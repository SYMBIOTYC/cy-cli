# cx-app-server-daemon

> `cx-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`cx-app-server-daemon` backs the machine-readable `cx app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for CX instances launched over SSH, including fresh developer
...
```sh
cx app-server daemon start
cx app-server daemon restart
cx app-server daemon enable-remote-control
cx app-server daemon disable-remote-control
cx app-server daemon stop
cx app-server daemon version
cx app-server daemon bootstrap --remote-control
```
...
$HOME/.cx/packages/standalone/current/cx app-server daemon bootstrap --remote-control
```
...
The daemon assumes CX is installed through `install.sh` and always launches
the standalone managed binary under `CODEX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `CODEX_HOME/packages/standalone/current/cx` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `CODEX_HOME/packages/standalone/current/cx` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `cx remote-control` bootstraps with `--remote-control` when the
