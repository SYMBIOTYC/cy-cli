...
use cx_app_server_transport::app_server_control_socket_path;
use cx_utils_home_dir::find_cx_home;
use managed_install::managed_cx_bin;
#[cfg(unix)]
use managed_install::managed_cx_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_cx_path: PathBuf,
    pub managed_cx_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "cx app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    fn from_environment() -> Result<Self> {
        ...
        let socket_path = app_server_control_socket_path(cx_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = cx_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_cx_bin: managed_cx_bin(cx_home.as_path()),
        })
    }
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by cx app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_cx_version(managed_cx_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_cx_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_cx_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_cx_version = self
            .managed_cx_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_cx_version}",
            self.managed_cx_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_cx_version = self.managed_cx_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_cx_path: self.managed_cx_bin.clone(),
            managed_cx_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_cx_bin)
            .await
    }
    ...
    fn ensure_managed_cx_bin(&self) -> Result<()> {
        if self.managed_cx_bin.is_file() {
            return Ok(());
        }

        let managed_cx_path = self.managed_cx_bin.display();
        Err(anyhow!(
            "managed standalone CX install not found at {managed_cx_path}\n\n\
             This command requires the standalone install managed by the CX installer, because \
    ...
    #[cfg(unix)]
    async fn managed_cx_version_best_effort(&self) -> Option<String> {
        managed_cx_version(&self.managed_cx_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_cx_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            cx_bin: managed_cx_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_cx_path: "cx".into(),
            managed_cx_version: Some("1.2.3".to_string()),
            socket_path: "cx.sock".into(),
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_cx_bin: temp_dir.path().join("missing-cx"),
        };
