...
pub(crate) fn managed_cx_bin(cx_home: &Path) -> PathBuf {
    cx_home
        .join("packages")
        .join("standalone")
        .join("current")
        .join(managed_cx_file_name())
}
...
#[cfg(unix)]
pub(crate) async fn managed_cx_version(cx_bin: &Path) -> Result<String> {
    let output = Command::new(cx_bin)
        .arg("--version")
        .output()
        .await
        .with_context(|| {
...
fn managed_cx_file_name() -> &'static str {
    if cfg!(windows) { "cx.exe" } else { "cx" }
}

#[cfg(unix)]
fn parse_cx_version(output: &str) -> Result<String> {
    let version = output
        .split_whitespace()
        .nth(1)
        .filter(|version| !version.is_empty())
        .ok_or_else(|| anyhow!("managed CX version output was malformed"))?;
