...
#[cfg(unix)]
use crate::managed_install::resolved_managed_cx_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/cx/install.sh";
...
async fn update_once(
    ...
    let managed_cx_bin = resolved_managed_cx_bin(&daemon.managed_cx_bin).await?;
    let managed_identity = executable_identity(&managed_cx_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_cx_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_cx_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_cx_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed CX binary {}",
            managed_cx_bin.display()
        )
    })
}
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone CX updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone CX updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone CX updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone CX updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone CX updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone CX updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone CX updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone CX updater")?
            .to_vec();
