...
fn cx_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_cx-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> cx_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<cx_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let cx_linux_sandbox_exe = Some(cx_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &cx_protocol::exec_output::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<cx_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> cx_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let cx_linux_sandbox_exe: Option<PathBuf> = Some(cx_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://cy.symbiotyc.workers.dev"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://cy.symbiotyc.workers.dev"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_cx_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_codex = tmpdir.path().join(".cx");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codex).expect("create .cx");

    let git_target = dot_git.join("config");
    let cx_target = dot_codex.join("config.toml");
    ...
    let cx_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", cx_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".cx write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(cx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_cx_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-cx");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_codex = tmpdir.path().join(".cx");
    symlink(&decoy, &dot_codex).expect("create .cx symlink");
    ...
    let cx_output = expect_denied(
        ...
        .await,
        ".cx symlink replacement should be denied",
    );
    assert_ne!(cx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_cx_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".cx symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".cx symlink build failure should deny: {output:?}"),
    };
...
registry="${{TMPDIR:-/tmp}}/cx-bwrap-synthetic-mount-targets-$(id -u)"
if touch "$registry/forged-marker" 2>/dev/null; then
  exit 22
fi
redirected_registry='{redirected_tmp}'/cx-bwrap-synthetic-mount-targets-$(id -u)
...
if grep -E '(^|[[:space:]])\.(git|cx|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_cx_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".cx"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .cx directory creation should be denied",
    );
    assert_ne!(mkdir_cx_output.exit_code, 0);
    assert!(!subdir.join(".cx").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = cx_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent ahosts cy.symbiotyc.workers.dev"]).await;
}
