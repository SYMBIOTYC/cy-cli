...
- a `cx-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the CX CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `cx-exec` CLI can check if its arg0 is `cx-linux-sandbox` and, if so, execute as if it were `cx-linux-sandbox`
  - this should also be true of the `cx` multitool CLI

On Linux, CX prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `cx-resources/bwrap` binary shipped
with CX.
CX also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. CX surfaces the same startup warning
...
it cannot create the required user namespaces, so CX rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `cx-resources/bwrap` path.
- If `bwrap` is missing, CX also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, CX surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; CX rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.cx`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `cx sandbox`; the host OS selects the sandbox backend.
