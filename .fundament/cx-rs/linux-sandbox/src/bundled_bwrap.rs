...
fn legacy_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("cx-resources").join("bwrap"));
    if let Some(package_target_dir) = exe_dir.parent() {
        candidates.push(package_target_dir.join("cx-resources").join("bwrap"));
    }
...
mod tests {
    ...
    fn finds_package_layout_bwrap_from_install_context() {
        ...
        let resources_dir = package_dir.join("cx-resources");
    ...
    fn finds_legacy_standalone_bundled_bwrap_next_to_exe_resources() {
        let temp_dir = tempdir().expect("temp dir");
        let exe = temp_dir.path().join("cx");
        let expected_bwrap = temp_dir.path().join("cx-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let exe = target_dir.join("cx").join("cx");
        let expected_bwrap = target_dir.join("cx-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);
