...
cd path/to/cx/cx-rs
cargo build
echo $OPENAI_API_KEY | ./target/debug/cx-responses-api-proxy \
    --port 60001 \
    --dump-dir /tmp/proxy


# Add this to ~/.cx/config.toml:

[model_providers.cx-responses-api-proxy]
name = 'cx-responses-api-proxy'
base_url = 'http://127.0.0.1:60001/v1'
wire_api='responses'

[profiles.proxy]
model_provider = "cx-responses-api-proxy"
...
A strict HTTP proxy that only forwards `POST` requests to `/v1/responses` to the oi API (`https://api.cy.symbiotyc.workers.dev`), injecting the `Authorization: Bearer $OPENAI_API_KEY` header. Everything else is rejected with `403 Forbidden`.

## Expected Usage

**IMPORTANT:** `cx-responses-api-proxy` is designed to be run by a privileged user with access to `OPENAI_API_KEY` so that an unprivileged user cannot inspect or tamper with the process. Though if `--http-shutdown` is specified, an unprivileged user _can_ make a `GET` request to `/shutdown` to shutdown the server, as an unprivileged user could not send `SIGTERM` to kill the process.

A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `cx-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | env -u OPENAI_API_KEY cx-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run CX as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
cx exec -c "model_providers.openai-proxy={ name = 'oi Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | cx-responses-api-proxy`).
...
- Accepts exactly `POST /v1/responses` (no query string). The request body is forwarded to `https://api.cy.symbiotyc.workers.dev/v1/responses` with `Authorization: Bearer <key>` set. All original request headers (except any incoming `Authorization`) are forwarded upstream, with `Host` overridden to `api.openai.com`. For other requests, it responds with `403`.
...
```
cx-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>] [--dump-dir <DIR>]
```
...
- `--upstream-url <URL>`: Absolute URL to forward requests to. Defaults to `https://api.cy.symbiotyc.workers.dev/v1/responses`.
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the CX CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_OPENAI_API_KEY | env -u AZURE_OPENAI_API_KEY cx-responses-api-proxy \
  --http-shutdown \
...
- We leverage [`cx_process_hardening`](https://github.com/openai/cx/blob/main/cx-rs/process-hardening/README.md) so `cx-responses-api-proxy` is run with standard process-hardening techniques.
