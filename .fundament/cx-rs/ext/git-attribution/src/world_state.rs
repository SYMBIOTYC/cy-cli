...
- Commit messages must end with `Co-authored-by: CX <noreply@openai.com>`. Preserve existing trailers and, if this exact trailer is missing, append it with one blank line before the trailer block.
...
- Pull request bodies must include the exact line `Generated with [CX](https://openai.com/cx/).`. Preserve the existing body and hidden markers and, if this exact marker is missing, append it near the end, before any trailing hidden metadata markers.
...
fn is_enabled_fragment(role: &str, text: &str) -> bool {
    role == "developer"
        ...
        && text.contains("Co-authored-by: CX <noreply@openai.com>")
        && (text.contains("Generated with [CX](https://openai.com/cx/).")
