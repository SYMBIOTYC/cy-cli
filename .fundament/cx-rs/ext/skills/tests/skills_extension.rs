...
use cx_exec_server::ExecutorCapabilityDiscoverySnapshot;
use cx_exec_server::LOCAL_FS;
use cx_extension_api::ConversationHistory;
use cx_extension_api::ExtensionData;
use cx_extension_api::ExtensionEventSink;
use cx_extension_api::ExtensionMetrics;
use cx_extension_api::ExtensionRegistryBuilder;
use cx_extension_api::ExtensionWarning;
use cx_extension_api::NoopTurnItemEmitter;
use cx_extension_api::PreviousWorldStateSection;
use cx_extension_api::RenderedWorldStateFragment;
use cx_extension_api::SkillInvocationInput;
use cx_extension_api::SkillInvocationKind;
use cx_extension_api::ThreadStartInput;
use cx_extension_api::ToolCall;
use cx_extension_api::ToolPayload;
use cx_extension_api::TurnInputContext;
use cx_extension_api::WorldStateContributionInput;
use cx_extension_api::WorldStateSectionContribution;
use cx_models_manager::model_info::model_info_from_slug;
use cx_otel::MetricsClient;
use cx_otel::MetricsConfig;
use cx_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use cx_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use cx_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use cx_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use cx_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use cx_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use cx_skills_extension::HostSkillsSnapshot;
use cx_skills_extension::InjectedHostSkillPrompts;
use cx_skills_extension::SkillLoadOutcome;
...
use cx_skills_extension::install_with_providers_and_metrics;
use cx_skills_extension::provider::SkillListQuery;
use cx_skills_extension::provider::SkillProvider;
...
async fn skill_world_state_fragments(
    registry: &cx_extension_api::ExtensionRegistry<TestConfig>,
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: cx_protocol::ThreadId::new(),
            turn_id,
            environments: &[],
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let cx_home = test_cx_home();
    let skill_path = cx_home.join("skills").join("demo").join("SKILL.md");
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_cx_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_cx_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "cx.skills.shadow_selection.catalog_entries")
...
async fn shadow_lru_selector_recovers_a_skill_invoked_on_an_earlier_turn() -> TestResult {
    ...
    let metric = snapshot
        .scope_metrics()
        .flat_map(opentelemetry_sdk::metrics::data::ScopeMetrics::metrics)
        .find(|metric| metric.name() == "cx.skills.shadow_selection.invocation")
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    ...
    let cx_home = AbsolutePathBuf::try_from(test_root.clone())?;
    ...
    let snapshot = service
        .snapshot_for_config(
            &HostSkillsLoadInput::new(cx_home, Vec::new(), config_layer_stack),
