...
use cx_exec_server::WalkOptions;
use cx_exec_server::WalkOutcome;
...
fn user_layer(cx_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: cx_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_cx_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/cx"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("cx");
    let project_folder = absolute(temp_dir.path().join("repo/.cx"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.cx"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_cx_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_codex = workspace.join(".cx");
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_codex = repository.join(".cx");
    let skill_root = dot_codex.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let cx_home = home_folder.join("cx");
    let system_folder = absolute(temp_dir.path().join("etc/cx"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_codex = repository.join(".cx");
    let nested_project_dot_codex = repository.join("nested/.cx");
    let user_skills = cx_home.join("skills");
    ...
    let system_skill = write_skill(&cx_home.join("skills/.system"), "system", "system-skill");
