...
use cx_exec_server::CapabilityRootDiscoverRequest;
use cx_exec_server::CapabilityRootsDiscoverParams;
use cx_exec_server::LOCAL_FS;
use cx_exec_server::discover_capability_roots;
use cx_protocol::protocol::Product;
use cx_skills::EnvironmentSkillMetadata;
use cx_skills::SkillDependencies;
use cx_skills::SkillPolicy;
...
async fn direct_environment_loader_preserves_plugin_dependencies_and_product_policy() {
    ...
    fs::create_dir_all(root.path().join(".cx-plugin")).expect("manifest dir");
    fs::create_dir_all(skill_dir.join("agents")).expect("metadata dir");
    fs::write(
        root.path().join(".cx-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )
    .expect("manifest");
    fs::write(
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(LOCAL_FS.as_ref(), &root_uri, Some(Product::CX)).await;
    ...
    assert_eq!(
        ...
        vec![EnvironmentSkillMetadata {
            ...
            policy: Some(SkillPolicy {
                ...
                products: vec![Product::CX, Product::Atlas],
            }),
        }]
    );
    let atlas =
...
async fn executor_bundle_parser_matches_direct_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".cx-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.cx-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "cx-name:search");
}
