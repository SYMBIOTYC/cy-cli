...
use cx_skills::ImplicitSkillLookup;
...
fn write_user_skill(cx_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = cx_home.path().join("skills").join(dir);
...
fn user_config_layer(cx_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(cx_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(cx_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(cx_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(cx_home, user_config_toml),
            ConfigLayerEntry::new(
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&cx_home, "");
    let skills_service =
        HostSkillsService::new(cx_home.path().abs(), /*bundled_skills_enabled*/ true);

    write_user_skill(&cx_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&cx_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn watchable_skill_root_paths_exclude_plugin_and_system_roots() {
    ...
    assert!(watchable_paths.contains(&cx_home.path().join("skills").abs()));
    assert!(!watchable_paths.contains(&plugin_skill_root.path));
    assert!(!watchable_paths.contains(&cx_home.path().join("skills/.system").abs()));
}
...
async fn snapshot_for_config_merges_extension_host_and_legacy_plugin_roots() {
    let cx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    write_user_skill(&cx_home, "user", "user-skill", "from the host loader");
    ...
    let config_layer_stack = config_stack(&cx_home, "[skills.bundled]\nenabled = false\n");
    let input = HostSkillsLoadInput::new(
        cwd.path().abs(),
    ...
    let skills_service =
        HostSkillsService::new(cx_home.path().abs(), /*bundled_skills_enabled*/ false);
...
async fn skills_list_snapshots_share_host_roots_only_within_one_request() {
    ...
    write_user_skill(&cx_home, "first", "first-skill", "first skill");
    let first = request
        .snapshot_for_cwd(
    ...
    write_user_skill(&cx_home, "second", "second-skill", "second skill");
    let second = request
        .snapshot_for_cwd(
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let cx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_codex = cwd.path().join(".cx");
    fs::create_dir_all(&repo_dot_codex).expect("create repo config dir");

    write_user_skill(&cx_home, "user", "user-skill", "from local user root");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&cx_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_cx_folder: repo_dot_codex.abs(),
                },
    ...
    let other_file_system: Arc<dyn cx_exec_server::ExecutorFileSystem> =
        Arc::new(cx_exec_server::LocalFileSystem::unsandboxed());
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let cx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = cx_home.path().join("skills/.system/bundled-skill");
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    write_user_skill(&cx_home, "late", "late-skill", "added after cache");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_stack = config_stack(&cx_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&cx_home, &disabled_skill_config, &enabled_skill_config);
    let skills_service =
        HostSkillsService::new(cx_home.path().abs(), /*bundled_skills_enabled*/ true);
