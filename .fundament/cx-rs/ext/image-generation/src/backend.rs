...
use cx_api::ImageResponse;
use cx_api::ImagesClient;
...
const X_CODEX_IMAGE_TURN_ID_HEADER: &str = "x-cx-image-turn-id";
...
impl ImageBackendError {
    fn from_api(error: cx_api::ApiError) -> Self {
        let message = error.to_string();
        Self {
            message,
            cx_error: map_api_error(error),
        }
    }
