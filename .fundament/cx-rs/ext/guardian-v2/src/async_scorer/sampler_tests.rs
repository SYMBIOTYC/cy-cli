...
fn assert_connection_metadata(server: &responses::WebSocketTestServer) -> Result<String> {
    ...
    assert_eq!(
        [
            ...
            handshake.header("x-cx-window-id"),
        ],
        [
    ...
    for request in server.single_connection() {
        ...
        metadata["x-cx-turn-metadata"] = serde_json::from_str(
            metadata["x-cx-turn-metadata"]
                .as_str()
        ...
        assert_eq!(
            ...
            json!({
                ...
                "x-cx-window-id": format!("{thread_id}:0"),
                "ws_request_header_x_openai_internal_cx_responses_lite": "true",
                "x-cx-turn-metadata": {
                    "session_id": "session-1",
                    "thread_id": thread_id,
...
async fn preconnected_sampler_reuses_authenticated_websocket_for_structured_requests() -> Result<()>
{
    ...
    assert_eq!(
        handshake.header("oi-Beta"),
        Some("responses_websockets=2026-02-06".to_owned())
    );
    assert_eq!(
        handshake.header("x-openai-internal-cx-responses-lite"),
        Some("true".to_owned())
    );
