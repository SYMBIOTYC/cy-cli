...
use cx_protocol::openai_models::GuardianV2ModelConfig;
use cx_protocol::openai_models::GuardianV2TranscriptModelConfig;
...
async fn installed_extension_reconnects_after_auth_refresh() -> Result<()> {
    ...
    let thread_store = test.cx.thread_extension_data();
...
async fn sandboxed_shell_classification_respects_review_scope() -> Result<()> {
    ...
    let thread_store = fixture.test.cx.thread_extension_data();
...
async fn sample_configured_conversation_history(
    ...
    let builder = test_codex()
        ...
        .with_model_info_override("cx-auto-review", |model_info| {
            model_info
                .model_messages
                .as_mut()
...
impl GuardianFailureFixture {
    ...
    async fn score_tool(&self, tool_name: ToolName) {
        let thread_store = self.test.cx.thread_extension_data();
...
async fn contributor_fails_closed_when_luna_classification_fails() -> Result<()> {
    ...
    fixture.registry.thread_lifecycle_contributors()[0]
        .on_thread_start(ThreadStartInput {
            ...
            extension_metrics: None,
            session_store: &fixture.session_store,
            thread_store: fixture.test.cx.thread_extension_data(),
        })
        .await;
...
async fn contributor_samples_tool_calls_with_the_existing_luna_pool() -> Result<()> {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        request["client_metadata"]["x-cx-turn-metadata"]
            .as_str()
    ...
    assert_eq!(
        request["client_metadata"]["x-cx-window-id"],
    ...
    assert_eq!(
        ...
        json!([
            ...
            {
                "type": "input_text",
                "text": "The CX agent has requested the following action:\n"
            },
