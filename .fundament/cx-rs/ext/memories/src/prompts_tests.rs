...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let cx_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = cx_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&cx_home)
        .await
        .unwrap();
