...
use cx_api::SearchClient;
use cx_api::SearchCommands;
use cx_api::SearchQuery;
use cx_api::SearchRequest;
use cx_api::SearchSettings;
use cx_core::X_CODEX_TURN_METADATA_HEADER;
use cx_core::web_search_action_detail;
use cx_extension_api::ExtensionTurnItem;
use cx_extension_api::FunctionCallError;
use cx_extension_api::ResponsesApiTool;
...
use cx_extension_api::parse_tool_input_schema_without_compaction;
use cx_extension_items::ExtensionItem;
use cx_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "cx.web_search.results.payload_bytes";
...
impl ToolExecutor<ToolCall> for WebSearchTool {
    ...
    fn handle(&self, call: ToolCall) -> cx_extension_api::ToolExecutorFuture<'_> {
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = cx_otel::global()
...
mod tests {
    ...
    fn search_request_headers_forward_thread_originator_and_turn_metadata() {
        let headers = search_request_headers(Some("gt_cca"), Some("turn-metadata"));
        assert_eq!(
            headers
                .get("originator")
