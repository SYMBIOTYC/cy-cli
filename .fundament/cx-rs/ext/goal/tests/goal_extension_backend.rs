...
use cx_extension_api::FunctionCallError;
use cx_extension_api::NoopTurnItemEmitter;
use cx_extension_api::ThreadResumeInput;
use cx_extension_api::ThreadStartInput;
use cx_extension_api::ThreadStopInput;
use cx_extension_api::ToolCall;
use cx_extension_api::ToolCallOutcome;
use cx_extension_api::ToolCallSource;
use cx_extension_api::ToolExecutor;
use cx_extension_api::ToolFinishInput;
use cx_extension_api::ToolPayload;
use cx_extension_api::TurnErrorInput;
use cx_extension_api::TurnStartInput;
use cx_extension_api::TurnStopInput;
use cx_goal_extension::GoalExtensionConfig;
use cx_goal_extension::GoalObjectiveUpdate;
use cx_goal_extension::GoalRuntimeHandle;
use cx_goal_extension::GoalService;
use cx_goal_extension::GoalSetRequest;
use cx_goal_extension::GoalTokenBudgetUpdate;
use cx_goal_extension::install_with_backend;
...
use cx_protocol::protocol::ThreadGoalStatus;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(cx_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(cx_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(cx_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(cx_state::ThreadGoalStatus::Active, goal.status);
...
async fn installed_tools(
    runtime: Arc<cx_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: cx_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = cx_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: cx_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        cx_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: cx_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<cx_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    cx_state::StateRuntime::init(
        cx_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &cx_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = cx_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: cx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        cx_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        cx_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        cx_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        cx_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        cx_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        cx_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
