...
use cx_extension_api::JsonToolOutput;
use cx_extension_api::ToolCall;
use cx_extension_api::ToolExecutor;
use cx_extension_api::ToolName;
...
use cx_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<cx_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> cx_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => cx_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => cx_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: cx_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            cx_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            cx_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<cx_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &cx_state::StateRuntime,
    thread_id: ThreadId,
    goal: &cx_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: cx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: cx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(status: ThreadGoalStatus) -> cx_state::ThreadGoalStatus {
    match status {
        ...
        ThreadGoalStatus::Blocked => cx_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => cx_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => cx_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => cx_state::ThreadGoalStatus::Complete,
    }
}
