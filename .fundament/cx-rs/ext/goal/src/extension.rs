...
use cx_extension_api::ConfigContributor;
...
use cx_extension_api::TokenUsageContributor;
...
use cx_extension_api::TurnAbortInput;
use cx_extension_api::TurnErrorInput;
use cx_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                cx_protocol::config_types::ModeKind::Plan
            ) {
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn cx_extension_api::ToolExecutor<cx_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
