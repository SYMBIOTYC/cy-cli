...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_cx_api_key_present = tracing::field::debug(auth_env.cx_api_key_env_present),
        auth_env_cx_api_key_enabled = tracing::field::debug(auth_env.cx_api_key_env_enabled),
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("cx_http_client::transport", LevelFilter::DEBUG)
                    .with_target("cx_api::sse", LevelFilter::DEBUG)
                    ...
                    .with_target("cx_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("cx_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `cx-logs.log` and before path-backed rollout
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("cx-logs.log"),
...
mod tests {
    ...
    fn logger_layer_filters_noisy_trace_payloads() {
        ...
        tracing::trace!(target: "cx_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "cx_http_client::transport", "transport-trace");
        tracing::trace!(target: "cx_api::sse", "sse-trace");
        tracing::trace!(target: "cx_api::sse::responses", "nested-sse-trace");
        tracing::debug!(target: "cx_http_client::transport", "transport-debug");
        tracing::debug!(target: "cx_api::sse::responses", "sse-debug");
        tracing::trace!(target: "cx_feedback_test", "unrelated-trace");
        log::trace!(target: "cx_feedback_test", "unrelated-log-trace");
        log::trace!(
    ...
    async fn feedback_upload_reports_transport_failures() {
        ...
        assert!(
            error
                .downcast_ref::<cx_http_client::RouteAwareRequestError>()
                .is_some_and(cx_http_client::RouteAwareRequestError::is_connect)
        );
    }

    #[test]
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("cx-feedback-extra-{}.jsonl", ThreadId::new());
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("cx-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("cx-feedback-extra-{suffix}.binunknown");
