...
pub use crate::cx_tool_config::CodexToolCallParam;
pub use crate::cx_tool_config::CodexToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "cx_mcp_server";
...
pub async fn run_main(
    ...
    reject_workload_identity(cx_login::is_workload_identity_selected())?;
...
fn reject_workload_identity(workload_identity_selected: bool) -> IoResult<()> {
    if workload_identity_selected {
        return Err(std::io::Error::new(
            ...
            "workload identity is not supported by `cx mcp-server`",
        ));
    }
    Ok(())
}
