//! Configuration object accepted by the `cx` MCP tool-call.
...
/// Client-supplied configuration for a `cx` tool-call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case", deny_unknown_fields)]
#[schemars(deny_unknown_fields)]
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the CX conversation.
    pub prompt: String,

    /// Optional override for the model name (e.g. 'gpt-5.2', 'gpt-5.2-cx').
...
/// Builds a `Tool` definition (JSON schema etc.) for the CX tool-call.
pub(crate) fn create_tool_for_cx_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "CX tool schema should serialize");

    Tool::new(
        "cx",
        "Run a CX session. Accepts configuration parameters matching the CX Config struct.",
        input_schema,
    )
    .with_title("CX")
    .with_raw_output_schema(cx_tool_output_schema())
}

fn cx_tool_output_schema() -> Arc<JsonObject> {
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the CX conversation and the
    ...
    pub async fn into_config(
        ...
        // Build the `ConfigOverrides` recognized by cx-core.
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this CX session.
    ...
    /// The *next user prompt* to continue the CX conversation.
    pub prompt: String,
}
...
/// Builds a `Tool` definition for the `cx-reply` tool-call.
pub(crate) fn create_tool_for_cx_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
    ...
    let input_schema = create_tool_input_schema(schema, "CX reply tool schema should serialize");

    Tool::new(
        "cx-reply",
        "Continue a CX conversation by providing the thread id and prompt.",
        input_schema,
    )
    .with_title("CX Reply")
    .with_raw_output_schema(cx_tool_output_schema())
...
mod tests {
    ...
    #[test]
    fn verify_cx_tool_json_schema() {
        let tool = create_tool_for_cx_tool_call_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Run a CX session. Accepts configuration parameters matching the CX Config struct.",
          "inputSchema": {
            ...
            "properties": {
              ...
              "config": {
                ...
                "description": "Individual config settings that will override what is in CX_HOME/config.toml.",
                "type": "object"
              },
              "cwd": {
              ...
              "model": {
                "description": "Optional override for the model name (e.g. 'gpt-5.2', 'gpt-5.2-cx').",
                "type": "string"
              },
              "prompt": {
                "description": "The *initial user prompt* to start the CX conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    #[test]
    fn cx_tool_call_param_rejects_removed_profile_field() {
    ...
    #[test]
    fn verify_cx_tool_reply_json_schema() {
        let tool = create_tool_for_cx_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a CX conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the CX conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this CX session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
