...
use cx_login::default_client::USER_AGENT_SUFFIX;
...
use crate::cx_tool_config::CodexToolCallParam;
use crate::cx_tool_config::CodexToolCallReplyParam;
use crate::cx_tool_config::create_tool_for_cx_tool_call_param;
use crate::cx_tool_config::create_tool_for_cx_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        cx_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.cx_home.clone()),
        );
        let skill_providers = cx_skills_extension::SkillProviders::new()
            .with_host_provider(Arc::new(cx_skills_extension::HostSkillProvider::new()));
        cx_skills_extension::install_with_providers_and_metrics(
            &mut extensions,
            skill_providers,
            cx_otel::global(),
            |config: &Config| cx_skills_extension::SkillsExtensionConfig {
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            cx_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            cx_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("cx-mcp-server", env!("CARGO_PKG_VERSION")).with_title("CX");

        // Preserve CX's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_cx_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult::with_all_items(vec![
            create_tool_for_cx_tool_call_param(),
            create_tool_for_cx_tool_call_reply_param(),
        ]);
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                            format!("Failed to load CX configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result);
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for CX tool: {e}"),
                    )]);
                    self.outgoing.send_response(id, result);
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for cx tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result);
                return;
            }
        ...
        // Spawn an async task to handle the CX session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the CX session and stream events back to the client.
            crate::cx_tool_runner::run_cx_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_cx_session_reply(
        &self,
        ...
        // parse arguments
        let cx_tool_call_reply_param: CodexToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse CX tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for CX tool: {e}"),
                    )]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for cx-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for cx-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match cx_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let cx = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::cx_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = cx_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let active_turns = Arc::clone(&active_turns);

            async move {
                crate::cx_tool_runner::run_cx_tool_session_reply(
                    thread_id,
                    cx,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the CX thread from the server.
        let cx_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
