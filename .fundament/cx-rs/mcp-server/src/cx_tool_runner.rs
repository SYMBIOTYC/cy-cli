//! Asynchronous worker that executes a **CX** tool-call inside a spawned
...
use cx_core::config::Config as CodexConfig;
...
/// To adhere to MCP `tools/call` response format, include the CX
...
/// Run a complete CX session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_cx_tool_session(
    id: RequestId,
    ...
    run_cx_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_cx_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_cx_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns).await;
}

async fn run_cx_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<CodexThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(cx_event_to_notification(&event)) above has
