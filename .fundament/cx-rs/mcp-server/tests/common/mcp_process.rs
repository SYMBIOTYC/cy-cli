...
impl McpProcess {
    pub async fn new(cx_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(cx_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = cx_utils_cargo_bin::cargo_bin("cx-mcp-server")
            .context("should find binary for cx-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("cx-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = cx_login::default_client::originator().value;
    ...
    /// Method "cx/event" with params.msg.type == "task_complete".
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_cx_event("task_complete").await
    }

    pub async fn read_stream_until_cx_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_cx_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_cx_event_matching(
        &mut self,
        event_type: &str,
        predicate: impl Fn(&serde_json::Value) -> bool,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        eprintln!("in read_stream_until_cx_event({event_type})");

        loop {
            let message = self.read_jsonrpc_message().await?;
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = notification.notification.method == "cx/event"
                        && notification
                            .notification
                            .params
                            .as_ref()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `cx-mcp-server` child process.
        //
