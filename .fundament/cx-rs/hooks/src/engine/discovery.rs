...
use cx_config::TomlValue;
use cx_config::version_for_toml;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_cx_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_cx_folder.clone())
...
fn append_matcher_groups(
    ...
    event_name: cx_protocol::protocol::HookEventName,
            ...
            let normalized = match handler {
                HookHandlerConfig::Command {
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        cx_protocol::protocol::HookEventName::PreToolUse
                            | cx_protocol::protocol::HookEventName::PostToolUse
                            | cx_protocol::protocol::HookEventName::SessionStart
                            | cx_protocol::protocol::HookEventName::UserPromptSubmit
                            | cx_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
                ...
                HookHandlerConfig::McpTool {
                    ...
                    if event_name == cx_protocol::protocol::HookEventName::SessionEnd {
                        source.record_load_failure(
                            format!(
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/cx/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.cx/config.toml").abs();
        let dot_cx_folder = test_path_buf("/tmp/worktree/.cx").abs();
