...
"#,
    ...
    assert_eq!(
        ...
        Some(FeatureToml::Config(crate::GuardianV2ConfigToml {
            ...
            reasoning_effort: Some(cx_protocol::openai_models::ReasoningEffort::Minimal),
...
#[test]
fn cx_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::CodexHooks));
    assert_eq!(feature_for_key("cx_hooks"), Some(Feature::CodexHooks));
}

#[test]
fn apps_require_feature_flag_and_gt_auth() {
    let mut features = Features::with_defaults();
    assert!(!features.apps_enabled_for_auth(/*has_gt_auth*/ false));

    features.enable(Feature::Apps);
    assert!(!features.apps_enabled_for_auth(/*has_gt_auth*/ false));
    assert!(features.apps_enabled_for_auth(/*has_gt_auth*/ true));
}
