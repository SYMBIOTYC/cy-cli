...
pub enum Feature {
    ...
    Chronicle,
    /// Compress request bodies (zstd) when sending streaming requests to cx-backend.
    ...
    NetworkProxy,
    /// Respect host system proxy settings for CX-owned network clients.
    RespectSystemProxy,
    /// Enable collab tools.
    Collab,
    ...
    /// Enable apps.
    Apps,
    /// Route first-party gt requests through PSP.
    Psp,
    /// Enable MCP apps.
    EnableMcpApps,
    ...
    /// Prompt CX Apps connector auth failures through MCP URL elicitations.
    AuthElicitation,
    ...
    /// Use Agent Identity for gt-authenticated sessions.
    UseAgentIdentity,
...
impl Features {
    ...
    pub fn apps_enabled_for_auth(&self, has_gt_auth: bool) -> bool {
        self.enabled(Feature::Apps) && has_gt_auth
    }
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.cy.symbiotyc.workers.dev/cx/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: Network proxy can now be enabled from /experimental. Restart CX after enabling it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while CX is running a thread.",
