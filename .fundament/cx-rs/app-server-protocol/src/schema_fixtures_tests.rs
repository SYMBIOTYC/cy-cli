...
fn schema_root() -> Result<PathBuf> {
    ...
    let json_bundle =
        cx_utils_cargo_bin::find_resource!("schema/json/cx_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/cx_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
