...
use cx_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use cx_protocol::config_types::MultiAgentMode;
use cx_protocol::items::AgentMessageContent;
use cx_protocol::items::AgentMessageItem;
use cx_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use cx_protocol::items::CollabAgentToolCallItem;
use cx_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use cx_protocol::items::CommandExecutionItem;
...
use cx_protocol::items::FileChangeItem;
use cx_protocol::items::ImageViewItem;
use cx_protocol::items::McpToolCallItem;
use cx_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use cx_protocol::items::ReasoningItem;
use cx_protocol::items::SubAgentActivityItem;
use cx_protocol::items::TurnItem;
use cx_protocol::items::UserMessageItem;
use cx_protocol::items::WebSearchItem as CoreWebSearchItem;
use cx_protocol::mcp::CallToolResult;
use cx_protocol::mcp::McpServerInfo;
use cx_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use cx_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use cx_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use cx_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use cx_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use cx_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use cx_protocol::protocol::AgentStatus as CoreAgentStatus;
use cx_protocol::protocol::AskForApproval as CoreAskForApproval;
use cx_protocol::protocol::ConversationTextRole;
use cx_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use cx_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use cx_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use cx_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use cx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use cx_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: cx_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        cx_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        cx_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.cx/state",
            "logDir": "file:///C:/Users/alice/.cx/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.cx/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.cx/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.cx/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.cx/models.json").expect("valid URI"))
    );
}
...
fn mcp_server_elicitation_request_from_core_openai_form_request() {
    ...
    .expect("oi form request should convert");
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        cx_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.cx/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![cx_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = cx_protocol::ThreadId::default();
    let receiver_thread_id = cx_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: cx_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            cx_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(cx_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@cx-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "cx-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@cx-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "cx-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn cx_error_info_serializes_http_status_code_in_camel_case() {
...
#[test]
fn cx_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
...
#[test]
fn cx_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(cx_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(cx_protocol::config_types::CollaborationMode {
            mode: cx_protocol::config_types::ModeKind::Plan,
            settings: cx_protocol::config_types::Settings {
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    ...
    assert_eq!(
        params.cx_response_handoff_channel_prefixes,
        Some(BTreeMap::from([
