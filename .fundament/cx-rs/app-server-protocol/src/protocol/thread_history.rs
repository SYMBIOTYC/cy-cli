...
use cx_protocol::items::parse_hook_prompt_message;
use cx_protocol::protocol::AgentMessageEvent;
use cx_protocol::protocol::AgentReasoningEvent;
use cx_protocol::protocol::AgentReasoningRawContentEvent;
use cx_protocol::protocol::AgentStatus;
use cx_protocol::protocol::ApplyPatchApprovalRequestEvent;
use cx_protocol::protocol::ContextCompactedEvent;
use cx_protocol::protocol::DynamicToolCallResponseEvent;
use cx_protocol::protocol::ErrorEvent;
use cx_protocol::protocol::EventMsg;
use cx_protocol::protocol::ExecCommandBeginEvent;
...
use cx_protocol::protocol::ImageGenerationBeginEvent;
use cx_protocol::protocol::ImageGenerationEndEvent;
use cx_protocol::protocol::ItemCompletedEvent;
use cx_protocol::protocol::ItemStartedEvent;
use cx_protocol::protocol::McpToolCallBeginEvent;
use cx_protocol::protocol::McpToolCallEndEvent;
use cx_protocol::protocol::PatchApplyBeginEvent;
use cx_protocol::protocol::PatchApplyEndEvent;
...
use cx_protocol::protocol::ViewImageToolCallEvent;
use cx_protocol::protocol::WebSearchBeginEvent;
use cx_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use cx_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use cx_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use cx_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `cx-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &cx_protocol::models::ResponseItem) {
        let cx_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &cx_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            cx_protocol::items::TurnItem::EnteredReviewMode(_)
                | cx_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            cx_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            cx_protocol::items::TurnItem::HookPrompt(_)
            | cx_protocol::items::TurnItem::CommandExecution(_)
            | cx_protocol::items::TurnItem::DynamicToolCall(_)
            | cx_protocol::items::TurnItem::CollabAgentToolCall(_)
            | cx_protocol::items::TurnItem::SubAgentActivity(_)
            | cx_protocol::items::TurnItem::Extension(_)
            | cx_protocol::items::TurnItem::EnteredReviewMode(_)
            | cx_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            cx_protocol::items::TurnItem::UserMessage(_)
            | cx_protocol::items::TurnItem::AgentMessage(_)
            | cx_protocol::items::TurnItem::Reasoning(_)
            | cx_protocol::items::TurnItem::WebSearch(_)
            | cx_protocol::items::TurnItem::ImageView(_)
            | cx_protocol::items::TurnItem::ImageGeneration(_)
            | cx_protocol::items::TurnItem::FileChange(_)
            | cx_protocol::items::TurnItem::McpToolCall(_)
            | cx_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &cx_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &cx_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &cx_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &cx_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &cx_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &cx_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &cx_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &cx_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &cx_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &cx_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &cx_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &cx_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &cx_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                cx_error_info: payload.cx_error_info.clone().map(Into::into),
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            cx_error_info: error.cx_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use cx_extension_items::ExtensionItem as CoreExtensionItem;
    use cx_extension_items::sleep::SleepItem as CoreSleepItem;
    use cx_protocol::ThreadId;
    use cx_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use cx_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use cx_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use cx_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use cx_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use cx_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use cx_protocol::items::TurnItem as CoreTurnItem;
    use cx_protocol::items::UserMessageItem as CoreUserMessageItem;
    use cx_protocol::items::build_hook_prompt_message;
    use cx_protocol::mcp::CallToolResult;
    use cx_protocol::models::ImageDetail;
    use cx_protocol::models::MessagePhase as CoreMessagePhase;
    use cx_protocol::models::WebSearchAction as CoreWebSearchAction;
    use cx_protocol::parse_command::ParsedCommand;
    use cx_protocol::protocol::AgentReasoningEvent;
    use cx_protocol::protocol::AgentReasoningRawContentEvent;
    use cx_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use cx_protocol::protocol::CodexErrorInfo;
    use cx_protocol::protocol::DynamicToolCallResponseEvent;
    use cx_protocol::protocol::EnteredReviewModeEvent;
    use cx_protocol::protocol::ExecCommandBeginEvent;
    use cx_protocol::protocol::ExecCommandEndEvent;
    use cx_protocol::protocol::ExecCommandSource;
    use cx_protocol::protocol::ExitedReviewModeEvent;
    use cx_protocol::protocol::ItemStartedEvent;
    use cx_protocol::protocol::McpInvocation;
    use cx_protocol::protocol::McpToolCallEndEvent;
    use cx_protocol::protocol::PatchApplyBeginEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![cx_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn replays_image_generation_end_events_into_turn_history() {
        let items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ImageGenerationEnd(ImageGenerationEndEvent {
                ...
                transparent_background: Some(true),
                failure: Some(
                    cx_extension_items::image_generation::ImageGenerationFailure::UsageLimitExceeded {
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    cx_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    cx_error_info: Some(CodexErrorInfo::ServerOverloaded),
                }),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(cx_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(cx_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: cx_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(cx_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                cx_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                cx_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                cx_error_info: Some(CodexErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn out_of_turn_error_does_not_create_or_fail_a_turn() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                cx_error_info: Some(CodexErrorInfo::BadRequest),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                cx_error_info: Some(CodexErrorInfo::ResponseStreamDisconnected {
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(cx_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(
                cx_protocol::models::ResponseItem::Message {
                    id: Some(cx_protocol::ResponseItemId::with_suffix("msg", "1")),
                    role: "user".into(),
                    content: vec![cx_protocol::models::ContentItem::InputText {
