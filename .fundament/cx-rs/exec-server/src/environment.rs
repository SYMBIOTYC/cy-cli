...
use crate::environment_toml::environment_provider_from_cx_home;
...
impl EnvironmentManager {
    ...
    pub async fn prepare_from_cx_home(
        ...
        let source = if let Some(config) = noise_environment_config_from_env()? {
            ...
            let provider = environment_provider_from_cx_home(cx_home.as_ref())?;
    ...
    pub async fn from_cx_home(
        ...
        Self::prepare_from_cx_home(cx_home)
            .await?
...
mod tests {
        ...
        let environment = Environment::remote_with_transport(
            ExecServerTransportParams::StdioCommand {
                command: StdioExecServerCommand {
                    program: "cx-missing-exec-server-for-test".to_string(),
                    args: Vec::new(),
                    env: HashMap::new(),
                    cwd: None,
                },
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            cx_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = cx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = cx_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            cx_protocol::models::PermissionProfile::from_runtime_permissions(
                &cx_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
