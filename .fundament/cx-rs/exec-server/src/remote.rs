...
use cx_websocket_client::WebSocketConnection;
use cx_websocket_client::WebSocketConnector;
use cx_websocket_client::WebSocketTlsMode;
...
impl EnvironmentRegistryClient {
    ...
    #[tracing::instrument(
        name = "cx.exec_server.remote.register",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "cx.exec_server.remote.register",
    ...
    #[tracing::instrument(
        name = "cx.exec_server.remote.environment_registry.connect",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "cx.exec_server.remote.environment_registry.connect",
...
impl HarnessKeyValidator for RegistryHarnessKeyValidator {
    ...
    #[tracing::instrument(
        name = "cx.exec_server.remote.environment_registry.validate_harness_key",
        skip_all,
        fields(
            otel.kind = "client",
            otel.name = "cx.exec_server.remote.environment_registry.validate_harness_key",
...
/// Noise connection configuration for a CX harness.
///
...
impl NoiseRendezvousEnvironmentConfig {
    pub(crate) fn new(
        ...
        let auth_provider = static_bearer_auth_provider(bearer_token, gt_account_id)?;
        Ok(Self {
            base_url,
            environment_id,
            auth_provider,
...
struct StaticBearerAuthProvider {
    ...
    gt_account_id: Option<HeaderValue>,
}
...
impl std::fmt::Debug for StaticBearerAuthProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StaticBearerAuthProvider")
            ...
            .field(
                "gt_account_id",
                &self.gt_account_id.as_ref().map(|_| "<redacted>"),
            )
            .finish()
    }
}
...
impl AuthProvider for StaticBearerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        ...
        if let Some(gt_account_id) = &self.gt_account_id {
            headers.insert(
                HeaderName::from_static("gt-account-id"),
                gt_account_id.clone(),
            );
        }
    }
...
fn static_bearer_auth_provider(
    ...
    let gt_account_id = gt_account_id
        .as_deref()
        .map(str::trim)
        ...
        .map(|account_id| {
            HeaderValue::try_from(account_id).map_err(|error| {
                ExecServerError::EnvironmentRegistryConfig(format!(
                    "gt account id is not a valid HTTP header: {error}"
                ))
            })
        })
        .transpose()?;
...
#[tracing::instrument(
    name = "cx.exec_server.remote.rendezvous.connect",
    skip_all,
    fields(
        otel.kind = "client",
        otel.name = "cx.exec_server.remote.rendezvous.connect",
        result = tracing::field::Empty,
    )
)]
...
mod tests {
    ...
    impl AuthProvider for StaticRegistryAuthProvider {
        ...
        fn resolve_auth_headers(&self) -> cx_api::AuthHeadersFuture<'_> {
            Box::pin(async {
