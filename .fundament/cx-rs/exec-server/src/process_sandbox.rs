...
use cx_network_proxy::NetworkPolicyAuditObserver;
use cx_network_proxy::NetworkPolicyDecider;
use cx_network_proxy::NetworkProxy;
use cx_network_proxy::NetworkProxyHandle;
use cx_network_proxy::NetworkProxyState;
use cx_network_proxy::RemoteNetworkProxyLaunchConfig;
use cx_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use cx_network_proxy::strip_managed_proxy_env;
use cx_protocol::models::PermissionProfile;
use cx_sandboxing::SandboxCommand;
use cx_sandboxing::SandboxDirectSpawnTransformRequest;
...
use cx_sandboxing::WindowsSandboxProxySettingsMode;
use cx_sandboxing::WindowsSandboxSpawnRequest;
...
pub(crate) async fn prepare_exec_request(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.cx_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.cx_self_exe) {
            sandbox_helper_paths.push(runtime_paths.cx_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.cx_linux_sandbox_exe.iter().cloned());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            cx_linux_sandbox_exe: runtime_paths.cx_linux_sandbox_exe.as_deref(),
