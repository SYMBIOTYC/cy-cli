...
use cx_sandboxing::SandboxExecRequest;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.cx_self_exe;
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    cx_linux_sandbox_exe: self.runtime_paths.cx_linux_sandbox_exe.as_deref(),
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    let mut roots = vec![runtime_paths.cx_self_exe.clone()];
    if let Some(path) = &runtime_paths.cx_linux_sandbox_exe
        && !roots.contains(path)
    {
...
pub(crate) fn spawn_command(
    ...
    env.retain(|name, _| !cx_protocol::shell_environment::is_non_inheritable_env_var(name));
    command.env_clear();
    command.envs(env);
    command.stdin(stdin);
    ...
    unsafe {
        command.pre_exec(|| {
            cx_utils_pty::pty::close_inherited_fds_except(&[]);
            Ok(())
        });
    }
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        ...
        let runtime_paths =
            ExecServerRuntimePaths::new(cx_self_exe, /*cx_linux_sandbox_exe*/ None)
        ...
        let readable = runtime_paths.cx_self_exe.clone();
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/cx".to_string()),
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = cx_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_only_linux_sandbox_alias_executable() {
        ...
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let cx_parent = runtime_paths.cx_self_exe.parent().expect("cx parent");
    ...
    fn sandbox_context_with_cwd(
        ...
        cx_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }
