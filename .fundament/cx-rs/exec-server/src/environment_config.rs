...
use cx_config::host_name;
use cx_config::loader::LocalTomlLayerStack;
use cx_config::loader::load_local_config_layers;
use cx_exec_server_protocol::EnvironmentConfigLayer;
use cx_exec_server_protocol::EnvironmentConfigLayerStack;
use cx_exec_server_protocol::EnvironmentConfigReadParams;
use cx_exec_server_protocol::EnvironmentConfigReadResponse;
...
pub(crate) async fn read_environment_config(
    ...
    let cx_home = find_cx_home().map_err(|error| {
        ReadEnvironmentConfigError::Internal(format!("failed to find CX home: {error}"))
    })?;
    let layers = load_local_config_layers(file_system, cx_home.as_path(), &cwd)
        .await
        .map_err(|error| {
    ...
    Ok(EnvironmentConfigReadResponse {
        ...
        cx_home_dir: PathUri::from_abs_path(&cx_home),
        hostname: host_name(),
