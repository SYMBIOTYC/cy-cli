...
//!
//! Linking only exec-server avoids depending on the full CX CLI binary
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    #[cfg(unix)]
    {
        ...
        if args.next().as_deref()
            == Some(OsStr::new(
                cx_exec_server::CODEX_ARG0_EXEC_HELPER_ARG1,
            ))
        {
    ...
    let runtime_paths =
        ExecServerRuntimePaths::new(current_exe, /*cx_linux_sandbox_exe*/ None)?;
    cx_exec_server::run_main(
        "ws://127.0.0.1:0",
        runtime_paths,
