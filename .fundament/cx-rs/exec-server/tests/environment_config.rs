...
use cx_exec_server::EnvironmentConfigLayer;
use cx_exec_server::EnvironmentConfigLayerStack;
use cx_exec_server::EnvironmentConfigReadParams;
use cx_exec_server::EnvironmentConfigReadResponse;
...
async fn environment_config_read_rejects_empty_selectors() -> anyhow::Result<()> {
    ...
    for (config_paths, expected_message) in [
        ...
        let error = environment
            .read_environment_config(EnvironmentConfigReadParams {
                cwd: PathUri::from_abs_path(&cx_home),
                config_paths,
                requirements_paths: Vec::new(),
            })
            .await
