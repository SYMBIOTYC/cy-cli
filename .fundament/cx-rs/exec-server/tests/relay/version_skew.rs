...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    ...
    let current = std::env::var_os(CURRENT_CODEX_ENV_VAR)
        .with_context(|| format!("{CURRENT_CODEX_ENV_VAR} must name the current CX binary"))?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current CX does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released CX does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(cx_home.path()).await?;
...
"#
    ...
    assert_eq!(
        std::fs::read_to_string(cx_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );
...
async fn mock_model(cx_home: &Path) -> Result<MockServer> {
