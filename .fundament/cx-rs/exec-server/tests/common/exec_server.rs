...
use cx_exec_server_protocol::JSONRPCRequest;
...
pub(crate) struct TestCodexHelperPaths {
    pub(crate) cx_exe: PathBuf,
    pub(crate) cx_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_cx_helper_paths() -> anyhow::Result<TestCodexHelperPaths> {
    let (helper_binary, cx_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestCodexHelperPaths {
        cx_exe: helper_binary,
...
{
    let helper_paths = test_cx_helper_paths()?;
    let cx_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.cx_exe);
    ...
    child.env("CODEX_HOME", cx_home.path());
    child.envs(env);
...
impl ExecServerHarness {
    pub(crate) fn cx_home(&self) -> &std::path::Path {
        self.cx_home.path()
    }
