...
use cx_http_client::cache_system_proxy_route_for_test;
...
async fn pending_noise_environment_connects_and_reconnects_after_ready_report() -> Result<()> {
    ...
    let (cx_exe, cx_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(cx_exe, cx_linux_sandbox_exe)?;
