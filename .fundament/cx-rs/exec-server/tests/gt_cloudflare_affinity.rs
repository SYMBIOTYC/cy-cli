...
async fn exec_server_replays_only_gt_cloudflare_cookies() -> anyhow::Result<()> {
...
impl TlsInterceptingProxy {
    fn start(expected_requests: usize) -> anyhow::Result<Self> {
        cx_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls_material() -> anyhow::Result<TlsMaterial> {
    ...
    ca_distinguished_name.push(DnType::CommonName, "CX affinity test CA");
    ...
    for request_index in 0..expected_requests {
        ...
        let response = match request_index {
            0 => concat!(
                ...
                "set-cookie: gt_session=secret; Path=/; Secure; HttpOnly\r\n",
                "\r\n",
                "ok",
            ),
            1 => concat!(
                ...
                "set-cookie: gt_session=still-secret; Path=/; Secure; HttpOnly\r\n",
                "\r\n",
                "ok",
            ),
            _ => concat!(
    ...
    T: DeserializeOwned,
{
    let message = server
        .wait_for_event(|event| match event {
            ...
            | JSONRPCMessage::Error(cx_exec_server_protocol::JSONRPCError { id, .. }) => {
                id == &request_id
            }
            _ => false,
        })
