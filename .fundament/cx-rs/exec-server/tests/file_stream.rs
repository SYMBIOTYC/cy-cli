...
use cx_exec_server::FsCloseParams;
use cx_exec_server::FsOpenParams;
use cx_exec_server::FsReadBlockParams;
use cx_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\cx-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\cx-fs-stream-{}", Uuid::new_v4());
...
#[cfg(unix)]
fn read_only_sandbox(path: std::path::PathBuf) -> cx_exec_server::FileSystemSandboxContext {
