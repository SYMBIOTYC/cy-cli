...
`cx-exec-server` is the library backing `cx exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`cx-utils-pty`.

It provides:

- a CLI entrypoint: `cx exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `cx` binary owns hidden helper dispatch for sandboxed
filesystem operations and `cx-linux-sandbox`.

## Transport

The server speaks the exec-specific `cx-exec-server-protocol` message
...
It uses the standard CX gt sign-in state; run `cx login` first when
...
`--use-agent-identity-auth`; CX then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `CODEX_API_KEY`;
CX sends it as a bearer token on the registration request. For example:
...
`cx.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/cx.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `cx-utils-pty`.
...
hidden helper process launched from the top-level `cx` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `cx exec-server` command
builds these paths from the `cx` arg0 dispatch state and resolves its HTTP
client factory from the effective CX configuration.
...
provider from CX auth state before starting remote mode.
