...
use crate::events::cx_artifact_operation_event_request;
use crate::events::cx_compaction_event_params;
use crate::events::cx_goal_event_params;
use crate::events::cx_hook_run_metadata;
use crate::events::cx_plugin_install_requested_metadata;
...
use cx_app_server_protocol::ClientResponse;
...
use cx_app_server_protocol::NetworkPolicyRuleAction;
...
use cx_app_server_protocol::WebSearchAction;
...
use cx_protocol::items::is_safe_plugin_relative_path;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn tool_item(
        notification: &'a cx_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
impl AnalyticsReducer {
    ...
    fn ingest_artifact_operation(
        ...
        out.push(TrackEventRequest::ArtifactOperation(
            cx_artifact_operation_event_request(input.tracking, input.operation),
        ));
    }
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(CodexDynamicToolCallEventRequest {
                    event_type: "cx_dynamic_tool_call_event",
    ...
    fn ingest_control_tool_call(
        ...
        let event = TrackEventRequest::ControlToolCall(CodexControlToolCallEventRequest {
            event_type: "cx_control_tool_call_event",
    ...
    fn ingest_turn_cx_error(&mut self, input: TurnCodexErrorFact) {
        ...
        turn_state.cx_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = cx_app_metadata(&tracking, mention);
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = cx_app_metadata(&tracking, app);
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(CodexHookRunEventRequest {
            ...
            event_params: cx_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
            ...
            event_params: cx_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            CodexPluginInstallRequestedEventRequest {
                ...
                event_params: cx_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
        ...
        out.push(TrackEventRequest::PluginInstallFailed(
            CodexPluginInstallFailedEventRequest {
                ...
                event_params: CodexPluginInstallFailedMetadata {
                    plugin: cx_plugin_metadata(plugin),
                    source,
                    error_type,
                    sub_error_type,
                },
        ...
        match notification {
            ServerNotification::ThreadArchived(notification) => {
                out.push(TrackEventRequest::ThreadArchive(ThreadArchiveEvent {
                    event_type: "cx_thread_archive_event",
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.cx_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: cx_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: CodexCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            CodexCompactionEventRequest {
                ...
                event_params: cx_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: CodexGoalEvent, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::Goal(Box::new(CodexGoalEventRequest {
            event_type: "cx_goal_event",
            event_params: cx_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: cx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(CodexTurnSteerEventRequest {
            event_type: "cx_turn_steer_event",
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(CodexTurnEventRequest {
            ...
            event_params: cx_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
    ...
    match item {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(CodexFileChangeEventRequest {
                event_type: "cx_file_change_event",
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                CodexMcpToolCallEventRequest {
                    event_type: "cx_mcp_tool_call_event",
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                CodexCollabAgentToolCallEventRequest {
                    event_type: "cx_collab_agent_tool_call_event",
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(CodexWebSearchEventRequest {
                event_type: "cx_web_search_event",
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                CodexImageGenerationEventRequest {
                    event_type: "cx_image_generation_event",
...
fn file_change_counts(changes: &[cx_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn cx_turn_event_params(
    ...
    let cx_error = turn_state.cx_error.as_ref();
    CodexTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        cx_error_kind: cx_error.map(|error| error.kind),
        cx_error_http_status_code: cx_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: cx_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        cx_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        cx_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        cx_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        cx_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.cx/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(
