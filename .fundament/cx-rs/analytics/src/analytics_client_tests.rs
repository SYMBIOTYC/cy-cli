...
use cx_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use cx_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use cx_app_server_protocol::CollabAgentToolCallStatus;
use cx_app_server_protocol::CommandAction;
...
use cx_app_server_protocol::DynamicToolCallStatus;
use cx_app_server_protocol::GuardianApprovalReview;
use cx_app_server_protocol::GuardianApprovalReviewAction;
use cx_app_server_protocol::GuardianApprovalReviewStatus;
use cx_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use cx_app_server_protocol::ImageGenerationItem;
use cx_app_server_protocol::InitializeCapabilities;
use cx_app_server_protocol::InitializeParams;
use cx_app_server_protocol::ItemCompletedNotification;
use cx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use cx_app_server_protocol::ItemStartedNotification;
use cx_app_server_protocol::JSONRPCErrorError;
use cx_app_server_protocol::McpToolCallAppContext;
use cx_app_server_protocol::McpToolCallStatus;
use cx_app_server_protocol::NonSteerableTurnKind;
use cx_app_server_protocol::PatchApplyStatus;
use cx_app_server_protocol::PermissionsRequestApprovalParams;
use cx_app_server_protocol::RequestId;
use cx_app_server_protocol::RequestPermissionProfile;
use cx_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use cx_app_server_protocol::ServerNotification;
use cx_app_server_protocol::ServerRequest;
use cx_app_server_protocol::ServerResponse;
use cx_app_server_protocol::SessionSource as AppServerSessionSource;
use cx_app_server_protocol::SubAgentActivityKind;
...
use cx_app_server_protocol::ThreadSource as AppServerThreadSource;
use cx_app_server_protocol::ThreadStartResponse;
use cx_app_server_protocol::ThreadStatus as AppServerThreadStatus;
...
use cx_app_server_protocol::TurnError as AppServerTurnError;
...
use cx_app_server_protocol::TurnStatus as AppServerTurnStatus;
...
use cx_app_server_protocol::WebSearchItem;
use cx_login::default_client::DEFAULT_ORIGINATOR;
use cx_login::default_client::originator;
use cx_plugin::AppConnectorId;
use cx_plugin::PluginCapabilitySummary;
use cx_plugin::PluginId;
use cx_plugin::PluginTelemetryMetadata;
...
use cx_protocol::protocol::HookExecutionMode;
use cx_protocol::protocol::HookHandlerType;
...
use cx_protocol::protocol::ThreadSource;
...
const TEST_PRODUCT_CLIENT_ID: &str = "cx_work_desktop";
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(cx_app_server_protocol::TurnStartResponse {
        turn: Turn {
            id: turn_id.to_string(),
            items_view: cx_app_server_protocol::TurnItemsView::Full,
            items: vec![],
...
fn sample_turn_completed_notification(
    ...
    cx_error_info: Option<cx_app_server_protocol::CodexErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: cx_error_info.map(|cx_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                cx_error_info: Some(cx_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                cx_error_info: Some(CodexErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "cx-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "cx-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "cx-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "cx-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(cx_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: cx_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.cx/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".cx/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.cx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/cx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.cx/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(CodexAppMentionedEventRequest {
        event_type: "cx_app_mentioned",
        event_params: cx_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        CodexAcceptedLineFingerprintsEventRequest {
            event_type: "cx_accepted_line_fingerprints",
            event_params: CodexAcceptedLineFingerprintsEventParams {
                event_type: "cx.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("cx".to_string()),
                model_slug: Some("gpt-5.1-cx".to_string()),
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "cx-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
...
"
    ...
    let event_types = contents
        ...
        .collect::<Vec<_>>();
    assert!(event_types.iter().any(|event| event == "cx_turn_event"));
    assert!(
        event_types
            .iter()
            .any(|event| event == "cx_accepted_line_fingerprints")
    );
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "status": "completed",
                "cx_error_kind": null,
                "cx_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "cx_thread_initialized",
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(CodexCommandExecutionEventRequest {
        event_type: "cx_command_execution_event",
        event_params: CodexCommandExecutionEventParams {
            base: CodexToolItemEventBase {
                ...
                app_server_client: CodexAppServerClientMetadata {
                    product_client_id: "cx_tui".to_string(),
                    client_name: Some("cx-tui".to_string()),
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "cx_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["cx_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        cx_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "cx_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["cx_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["cx_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("cx-auto-review".to_string()),
                    ...
                    guardian_model_provider_id: Some("cy".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "cx_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "cx_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "cx_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "cx_review_event");
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        cx_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        cx_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        cx_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        cx_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.cx_rs_version, "0.1.0");
...
async fn subagent_tool_items_inherit_parent_connection_metadata() {
    ...
    assert_eq!(payload[1]["event_type"], "cx_dynamic_tool_call_event");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
        event_type: "cx_plugin_used",
        event_params: cx_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(CodexPluginEventRequest {
        event_type: "cx_plugin_installed",
        event_params: cx_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(CodexPluginInstallFailedEventRequest {
        event_type: "cx_plugin_install_failed",
        event_params: CodexPluginInstallFailedMetadata {
            plugin: cx_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(CodexPluginEventRequest {
        event_type: "cx_plugin_installed",
        event_params: cx_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(CodexHookRunEventRequest {
        event_type: "cx_hook_run",
        event_params: cx_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(cx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(cx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(cx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(cx_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(cx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
...
async fn reducer_ingests_artifact_operation_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "cx_artifact_operation",
            "event_params": {
                "thread_id": "thread-1",
                "turn_id": "turn-1",
                "item_id": "call-1",
            ...
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    location: SkillInvocationLocation::Host {
                        path: skill_path,
                        scope: cx_protocol::protocol::SkillScope::User,
                    },
                    plugin_id: None,
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "cx_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "cx_app_mentioned");
    assert_eq!(payload[1]["event_type"], "cx_app_used");
    assert_eq!(payload[2]["event_type"], "cx_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        serde_json::to_value(&events).expect("serialize events"),
        json!([{
            "event_type": "cx_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "cx_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        CodexOnboardingExternalAgentImportFailureEventRequest {
            event_type: "cx_onboarding_external_agent_import_failure",
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("cx_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["cx_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("cx_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        vec![
            ("cx_command_execution_event", "session-thread-2"),
            ("cx_file_change_event", "session-thread-2"),
            ("cx_mcp_tool_call_event", "session-thread-2"),
            ("cx_dynamic_tool_call_event", "session-thread-2"),
            ("cx_collab_agent_tool_call_event", "session-thread-2"),
            ("cx_web_search_event", "session-thread-2"),
            ("cx_image_generation_event", "session-thread-2"),
        ]
    );
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(cx_app_server_protocol::CodexErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["cx_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["cx_error_http_status_code"],
        json!(null)
    );
}
...
async fn rejected_turn_interrupt_does_not_tag_interrupted_turn_event() {
    ...
    assert_eq!(payload["event_params"]["cx_error_kind"], json!(null));
}
