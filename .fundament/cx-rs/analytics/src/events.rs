...
use cx_protocol::protocol::GuardianAssessmentOutcome;
use cx_protocol::protocol::GuardianCommandSource;
...
pub(crate) struct CodexArtifactOperationEventParams {
    ...
    pub(crate) runtime: CxRuntimeMetadata,
...
pub(crate) fn cx_artifact_operation_event_request(
...
#[derive(Clone, Serialize)]
pub(crate) struct CxRuntimeMetadata {
    pub(crate) cx_rs_version: String,
...
pub enum GuardianApprovalRequestSource {
    /// Approval requested directly by the main CX turn.
    MainTurn,
...
pub(crate) struct CodexCompactionEventParams {
    ...
    pub(crate) cx_error_kind: Option<CxErrKind>,
    pub(crate) cx_error_http_status_code: Option<u16>,
...
pub(crate) struct CodexGoalEventParams {
    ...
    pub(crate) goal_status: cx_state::ThreadGoalStatus,
...
pub(crate) fn plugin_state_event_type(state: PluginState) -> &'static str {
    match state {
        PluginState::Installed => "cx_plugin_installed",
        PluginState::Uninstalled => "cx_plugin_uninstalled",
        PluginState::Enabled => "cx_plugin_enabled",
        PluginState::Disabled => "cx_plugin_disabled",
    }
}

pub(crate) fn cx_app_metadata(
    tracking: &TrackEventsContext,
    app: AppInvocation,
) -> CodexAppMetadata {
    CodexAppMetadata {
...
pub(crate) fn cx_plugin_metadata(plugin: PluginTelemetryMetadata) -> CodexPluginMetadata {
    cx_plugin_metadata_with_product_client_id(plugin, originator().value)
}

fn cx_plugin_metadata_with_product_client_id(
...
pub(crate) fn cx_plugin_install_requested_metadata(
    tracking: &TrackEventsContext,
...
pub(crate) fn cx_compaction_event_params(
    input: CxCompactionEvent,
    session_id: String,
    ...
    CodexCompactionEventParams {
        ...
        status: input.status,
        cx_error_kind: input.cx_error_kind,
        cx_error_http_status_code: input.cx_error_http_status_code,
...
pub(crate) fn cx_goal_event_params(
    input: CxGoalEvent,
    session_id: String,
    app_server_client: CodexAppServerClientMetadata,
    runtime: CxRuntimeMetadata,
...
pub(crate) fn cx_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
    ...
    CodexPluginUsedMetadata {
        plugin: cx_plugin_metadata_with_product_client_id(
            plugin,
...
pub(crate) fn cx_hook_run_metadata(
    tracking: &TrackEventsContext,
    hook: HookRunFact,
...
pub(crate) fn current_runtime_metadata() -> CxRuntimeMetadata {
    let os_info = os_info::get();
    CxRuntimeMetadata {
        cx_rs_version: env!("CARGO_PKG_VERSION").to_string(),
