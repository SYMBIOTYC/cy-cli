...
use cx_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_artifact_operation_event(thread_id: &str) -> TrackEventRequest {
    TrackEventRequest::ArtifactOperation(cx_artifact_operation_event_request(
        TrackEventsContext {
            model_slug: "gpt-5.1-cx".to_string(),
            thread_id: thread_id.to_string(),
            turn_id: "turn-1".to_string(),
            product_client_id: "cx_desktop".to_string(),
        },
        ArtifactOperation {
    ...
    TrackEventRequest::PluginUsed(CodexPluginUsedEventRequest {
        ...
        event_params: CodexPluginUsedMetadata {
            plugin: CodexPluginMetadata {
                ...
                product_client_id: Some("cx_desktop".to_string()),
            },
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://cy.symbiotyc.workers.dev/v1/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = cx_login::CodexAuth::create_dummy_gt_auth_for_testing();
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_cx_backend() {
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = cx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
