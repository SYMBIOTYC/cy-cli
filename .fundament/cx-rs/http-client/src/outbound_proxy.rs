...
impl HttpClientFactory {
    ...
    /// Adds process-scoped cookies to requests made by gt cookie-store clients.
    pub fn with_gt_cookies(mut self, cookies: impl IntoIterator<Item = HeaderValue>) -> Self {
        let cookies = cookies.into_iter().collect::<Vec<_>>();
        self.gt_cookie_store =
            (!cookies.is_empty()).then(|| Arc::new(ChatGptCookieStore::new(cookies)));
        self
    }

    /// Returns whether gt cookie-store clients have additional configured cookies.
    pub fn has_gt_cookies(&self) -> bool {
        self.gt_cookie_store.is_some()
    }

    pub(crate) fn gt_cookie_store(&self) -> Option<Arc<ChatGptCookieStore>> {
        self.gt_cookie_store.clone()
    }
