...
use crate::gt_hosts::is_allowed_gt_host;
...
// store to persist gt account, session, auth, or other user-specific cookie
// data.
...
/// Adds the process-local gt Cloudflare cookie jar used by CX HTTP clients.
///
...
fn is_gt_cookie_url(url: &reqwest::Url) -> bool {
    match url.scheme() {
        "https" => {}
        _ => return false,
    }
...
mod tests {
    ...
    #[test]
    fn additional_cookies_are_restricted_to_https_gt_hosts() {
        let store = ChatGptCookieStore {
    ...
    #[test]
    fn stores_and_returns_cloudflare_cookies_for_gt_hosts() {
        let store = ChatGptCloudflareCookieStore::default();
        let url = reqwest::Url::parse("https://cy.symbiotyc.workers.dev/v1/responses").unwrap();
    ...
    #[test]
    fn ignores_non_cloudflare_cookies_for_gt_hosts() {
        let store = ChatGptCloudflareCookieStore::default();
        let url = reqwest::Url::parse("https://cy.symbiotyc.workers.dev/v1/responses").unwrap();
    ...
    #[test]
    fn ignores_mixed_non_cloudflare_cookies_for_gt_hosts() {
        let store = ChatGptCloudflareCookieStore::default();
        let url = reqwest::Url::parse("https://cy.symbiotyc.workers.dev/v1/responses").unwrap();
        let cfuvid = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
        let account_cookie =
            HeaderValue::from_static("gt_session=secret; Path=/; Secure; HttpOnly");
    ...
    #[test]
    fn rejects_plain_http_gt_cookie_urls() {
        let store = ChatGptCloudflareCookieStore::default();
        let http_url = reqwest::Url::parse("http://cy.symbiotyc.workers.dev/v1/cx/responses")
            .expect("URL should parse");
        let https_url = reqwest::Url::parse("https://cy.symbiotyc.workers.dev/v1/responses")
            .expect("URL should parse");
        let set_cookie = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
    ...
    fn only_allows_https_urls() {
        let url = reqwest::Url::parse("http://cy.symbiotyc.workers.dev/v1/cx/responses").unwrap();

        assert!(!is_gt_cookie_url(&url));

        let url = reqwest::Url::parse("wss://cy.symbiotyc.workers.dev/v1/responses").unwrap();
