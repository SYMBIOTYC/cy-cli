...
impl RouteAwareClientPool {
    ...
    /// Creates a pool that retains the Cloudflare cookies required by gt endpoints.
    pub fn with_gt_cloudflare_cookies(
        http_client_factory: HttpClientFactory,
        route_class: ClientRouteClass,
    ) -> Self {
        Self::with_builder(
            http_client_factory,
            route_class,
            HttpClientBuilder::new().with_gt_cloudflare_cookie_store(),
        )
    }

    /// Creates a no-redirect pool that retains the Cloudflare cookies required by gt
    /// endpoints.
    pub fn with_gt_cloudflare_cookies_without_redirects(
        http_client_factory: HttpClientFactory,
        route_class: ClientRouteClass,
    ) -> Self {
        Self::with_builder(
    ...
    /// Creates a no-redirect gt Cloudflare-cookie pool without request diagnostics.
    pub fn with_gt_cloudflare_cookies_without_redirects_or_request_logging(
        http_client_factory: HttpClientFactory,
        route_class: ClientRouteClass,
    ) -> Self {
        Self::with_builder(
    ...
    /// Creates a gt Cloudflare-cookie pool without URL or response-header diagnostics.
    pub fn with_gt_cloudflare_cookies_without_request_logging(
        http_client_factory: HttpClientFactory,
        route_class: ClientRouteClass,
    ) -> Self {
        Self::with_builder(
    ...
    async fn retry_with_rustls(
        ...
        tracing::info!(
            event.name = "cx.http_client.tls_backend_fallback",
