...
`cx-http-client` is the low-level HTTP transport shared by CX crates. It is the intended
...
- the opt-in gt Cloudflare cookie store.

Another important motivation is consistent support for the `respect_system_proxy` feature. That
feature requires more than enabling reqwest's default proxy behavior: CX must resolve platform
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `cx-client`.
...
would skip CX's route selection for the redirect target. In `RespectSystemProxy` mode the pool
...
`RouteAwareClientPool::new_without_request_logging`. The corresponding gt cookie-pool
constructor is `with_gt_cloudflare_cookies_without_request_logging`.
...
```rust
use cx_http_client::ClientRouteClass;
use cx_http_client::ReqwestTransport;

let client = http_client_factory.build_client(api_url, ClientRouteClass::Api)?;
let transport = ReqwestTransport::from_http_client(client);
```

If the existing wrapper surface cannot support a use case, extend `cx-http-client` rather than
