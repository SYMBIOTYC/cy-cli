...
use anyhow::bail;
use clap::Parser;
use cx_core_api::AbsolutePathBuf;
use cx_core_api::AltScreenMode;
use cx_core_api::ApprovalsReviewer;
use cx_core_api::Arg0DispatchPaths;
use cx_core_api::AskForApproval;
use cx_core_api::AuthCredentialsStoreMode;
use cx_core_api::AuthManager;
use cx_core_api::AutoCompactTokenLimitScope;
use cx_core_api::CodexAppsToolsCache;
use cx_core_api::CodexHomeUserInstructionsProvider;
use cx_core_api::CodexThread;
use cx_core_api::Config;
use cx_core_api::ConfigLayerStack;
use cx_core_api::Constrained;
use cx_core_api::EnvironmentManager;
use cx_core_api::EventMsg;
use cx_core_api::ExecServerRuntimePaths;
use cx_core_api::ExtensionRegistryBuilder;
use cx_core_api::Features;
use cx_core_api::GhostSnapshotConfig;
use cx_core_api::History;
use cx_core_api::MemoriesConfig;
use cx_core_api::ModelAvailabilityNuxConfig;
use cx_core_api::MultiAgentV2Config;
...
use cx_core_api::ProjectConfig;
use cx_core_api::RealtimeAudioConfig;
use cx_core_api::RealtimeConfig;
use cx_core_api::SessionPickerViewMode;
use cx_core_api::SessionSource;
use cx_core_api::SqliteConfig;
use cx_core_api::StartIfIdleSubmission;
use cx_core_api::StartThreadOptions;
use cx_core_api::TerminalResizeReflowConfig;
use cx_core_api::ThreadManager;
use cx_core_api::ThreadStoreConfig;
use cx_core_api::ToolSuggestConfig;
use cx_core_api::TuiKeymap;
use cx_core_api::TuiNotificationSettings;
use cx_core_api::TuiPetAnchor;
use cx_core_api::TurnInputRequest;
use cx_core_api::UriBasedFileOpener;
use cx_core_api::UserInput;
use cx_core_api::WebSearchMode;
use cx_core_api::arg0_dispatch_or_else;
use cx_core_api::build_models_manager;
use cx_core_api::built_in_model_providers;
use cx_core_api::find_cx_home;
use cx_core_api::init_state_db;
use cx_core_api::install_image_generation_extension;
use cx_core_api::item_event_to_server_notification;
use cx_core_api::local_agent_graph_store_from_state_db;
use cx_core_api::resolve_installation_id;
use cx_core_api::set_default_originator;
use cx_core_api::thread_store_from_config;

#[derive(Debug, Parser)]
#[command(
    name = "cx-thread-manager-sample",
    about = "Run one CX turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("cx_thread_manager_sample".to_string()) {
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        ...
        config.cx_linux_sandbox_exe.clone(),
    )?;
    ...
    turn_output?;
    shutdown_result.context("shut down CX thread")?;
...
fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    ...
    let model_provider = model_providers
        ...
        .context("oi model provider should be available")?
        .clone();
    ...
    let mut config = Config {
        ...
        sqlite: SqliteConfig::from_sqlite_home(cx_home.clone()),
        log_dir: cx_home.join("log").to_path_buf(),
        cx_home,
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read CX event")?;
